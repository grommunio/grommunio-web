...
class IndexSqlite extends SQLite3
{
	...
	private function try_insert_content($search_entryid, $row, $folder_id, $recursive,
		$message_classes, $date_start, $date_end, $unread, $has_attachments) {
		// if match condition contains '@', $row['entryid'] will disappear. it seems a bug for php-sqlite
		if (0 == strlen($row['entryid'])) {
			...
			try {
				if ($folder_id != $row['folder_id']) {
					...
					while (true) {
						...
						$tmp_props = mapi_getprops($folder, array(PR_PARENT_ENTRYID, PR_FOLDER_ID));
						$folder_entryid = $tmp_props[PR_PARENT_ENTRYID];
						$tmp_fid = IndexSqlite::get_gc_value($tmp_props[PR_FOLDER_ID]);
						if ($tmp_fid == $folder_id) {
							break;
						} else if ($tmp_fid == PRIVATE_FID_ROOT) {
							return;
						}
					}
				}
		...
		if (isset($message_classes)) {
			...
			foreach ($message_classes as $message_class) {
				if (0 == strncasecmp($row['message_class'], $message_class, strlen($message_class))) {
					$found = true;
					break;
				}
			}
		...
		if (isset($unread) || isset($has_attachments)) {
			if (!isset($this->hide_attachments_proptag)) {
				$mapping = array("hide_attachments" => "PT_BOOLEAN:PSETID_Common:0x8514");
				$properties = getPropIdsFromStrings($this->store, $mapping);
				$this->hide_attachments_proptag = $properties['hide_attachments'];
			}
			$message = mapi_msgstore_openentry($this->store, $row['entryid']);
			$propvals = mapi_getprops($message, array(PR_MESSAGE_FLAGS, $this->hide_attachments_proptag));
			if (isset($unread)) {
	...
	public function search($search_entryid, $sender, $sending, $recipients, $subject, $content,
		$attachments, $others, $folder_entrid, $recursive, $message_classes, $date_start,
		$date_end, $unread, $has_attachments) {
		$search_folder = mapi_msgstore_openentry($this->store, $search_entryid);
		$tmp_props = mapi_getprops($search_folder, array(PR_FOLDER_ID));
		...
		if (isset($folder_entrid)) {
			try {
				...
				$tmp_props = mapi_getprops($folder, array(PR_FOLDER_ID));
		...
		while (($row = $results->fetchArray(SQLITE3_ASSOC)) && !$this->result_full()) {
			$this->try_insert_content($search_entryid, $row, $folder_id, $recursive,
				$message_classes, $date_start, $date_end, $unread, $has_attachments);
		}
		return true;
	}
	...
	private function refresh() {
		...
		foreach($items as $item) {
			...
			try {
				...
				$contents = mapi_table_queryallrows($table, array(PR_MID, PR_CHANGE_NUMBER, PR_ENTRYID));
				foreach ($contents as $content) {
					...
					if ($change_num > $last_cn) {
						...
						$messages[] = array(IndexSqlite::get_gc_value((int)$content[PR_MID]), $content[PR_ENTRYID]);
					}
				}
			} catch (Exception $e) {
			...
			$hierarchy[] = array($folder_id, $item[PR_LOCAL_COMMIT_TIME_MAX], $max_cn);
		}
		$stmt->close();
	...
	function insert_message($message_id, $message, $properties) {
		$others_tags = array(
			PR_DISPLAY_NAME,
		...
		$recips = mapi_table_queryallrows($table, array(PR_DISPLAY_NAME, PR_SMTP_ADDRESS));
		$recipients_string = '';
		$first = true;
		...
		$attachments = mapi_table_queryallrows($table, array(PR_ATTACH_LONG_FILENAME));
		$attachments_string = '';
		$first = true;
		...
		$sending = null;
		if (isset($propvals[PR_SENT_REPRESENTING_NAME]) && isset($propvals[PR_SENT_REPRESENTING_SMTP_ADDRESS])) {
			$sending = $propvals[PR_SENT_REPRESENTING_NAME] . "\n" . $propvals[PR_SENT_REPRESENTING_SMTP_ADDRESS];
		} else if (isset($propvals[PR_SENT_REPRESENTING_NAME])) {
			$sending = $propvals[PR_SENT_REPRESENTING_NAME];
		} else if (isset($propvals[PR_SENT_REPRESENTING_SMTP_ADDRESS])) {
			$sending = "\n" . $propvals[PR_SENT_REPRESENTING_SMTP_ADDRESS];
		}
		$sender = null;
		if (isset($propvals[PR_SENDER_NAME]) && isset($propvals[PR_SENDER_SMTP_ADDRESS])) {
			$sender = $propvals[PR_SENDER_NAME] . "\n" . $propvals[PR_SENDER_SMTP_ADDRESS];
		} else if (isset($propvals[PR_SENDER_NAME])) {
			$sender = $propvals[PR_SENDER_NAME];
		} else if (isset($propvals[PR_SENDER_SMTP_ADDRESS])) {
			$sender = "\n" . $propvals[PR_SENDER_SMTP_ADDRESS];
		}
		$subject = null;
		...
		$this->bind_insert($sender, $sending, $recipients_string, $subject,
			$body, $attachments_string, $others, $message_id, $entryid,
			IndexSqlite::get_gc_value((int)$propvals[PR_CHANGE_NUMBER]),
			IndexSqlite::get_gc_value((int)$propvals[PR_FOLDER_ID]),
			$propvals[PR_MESSAGE_CLASS], $last_time);
	}
	...
	private function bind_insert($sender, $sending, $recipients, $subject, $content, $attachments,
		$others, $message_id, $entryid, $change_num, $folder_id, $message_class, $date) {
		$this->stmt->clear();
		if (isset($sender)) {
