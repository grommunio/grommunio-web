...
// attribute value.
$aDangerousCharsReplacementTable = [
	['&#x029F;', '&#0671;', /* L UNICODE IPA Extension */
...
function sq_findnxreg($body, $offset, $reg) {
	...
	return [($offset + strlen($matches[1])), $matches[1], $matches[2]];
}
...
function sq_getnxtag($body, $offset) {
	...
	if ($pos >= $body_len) {
		return [false, false, false, $lt, $body_len];
	}
				...
				if ($char == "=") {
					...
					elseif ($quot == '"') {
						...
						if ($tagname === "img" && $regary === false) {
							...
							$regary = [($position + strlen($imageSrc)), $imageSrc];
						}
...
function sq_fixatts(
	...
	foreach ($attary as $attname => $attvalue) {
		 ...
		 */
		elseif ($attname == 'href' || $attname == 'src' || $attname == 'background') {
			$attary[$attname] = $attvalue;
		}
	}
	/*
...
function magicHTML($body) {
	...
	$add_attr_to_tag = [
		...
		"/^base$/i" => ['target' => '"_blank"',
			'title' => '"' . _("This external link will open in a new window") . '"',
			'rel' => '"noreferrer noopener"',
		],
		"/^area$/i" => ['target' => '"_blank"',
			'title' => '"' . _("This external link will open in a new window") . '"',
			'rel' => '"noreferrer noopener"',
		],
		"/^form$/i" => ['target' => '"_blank"',
			'title' => '"' . _("This external link will open in a new window") . '"',
			'rel' => '"noreferrer noopener"',
		],
	];
