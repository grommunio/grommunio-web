 ...
 */
		...
		*/
		function addEmailsToRecipientHistory($emailAddresses){
			...
			if (isset($storeProps[PR_EC_RECIPIENT_HISTORY_JSON])) {
				if (propIsError(PR_EC_RECIPIENT_HISTORY_JSON, $storeProps) == MAPI_E_NOT_ENOUGH_MEMORY) {
					$datastring = streamProperty($store, PR_EC_RECIPIENT_HISTORY_JSON);
				} else {
					$datastring = $storeProps[PR_EC_RECIPIENT_HISTORY_JSON];
				}
