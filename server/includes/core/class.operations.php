	...
	*/
	...
	class Operations
	{
		...
		*/
		...
		function getFavoritesFolders($commonViewFolder, &$storeData)
		{
			...
			foreach ($stores as $entryid => $store) {
				$props = mapi_getprops($store, array(PR_DEFAULT_STORE, PR_FINDER_ENTRYID));
			...
			if($message) {
				...
				if (isset($smtpprops[PR_SENT_REPRESENTING_ENTRYID])) {
					try {
						...
						if (isset($user)) {
							$user_props = mapi_getprops($user, array(PR_EMS_AB_THUMBNAIL_PHOTO));
				...
				if(!empty($attachments)) {
					...
					if (true == $cid_found && isset($htmlcontent)) {
						...
						if(count($matches) > 0) {
							...
							foreach($matches[1] as $match) {
								...
								foreach ($attachments as $key => $attachement) {
									if (isset( $attachement["props"]["cid"]) &&
										0 == strcasecmp($match, $attachement["props"]["cid"])) {
										$idx = $key;
								...
								$attachprop = mapi_getprops($attach, array(PR_ATTACH_DATA_BIN, PR_ATTACH_MIME_TAG));
		 ...
		 */
		function generateBodyHTML($body, $title = "grommunio-web"){
		...
		{
			$folderProps = mapi_getprops($folder, array(PR_MESSAGE_SIZE_EXTENDED));
		...
		*/
		...
		{
			$props = mapi_getprops($message, array(PR_NATIVE_BODY));
			if (isset($props[PR_NATIVE_BODY]) && 1 == $props[PR_NATIVE_BODY]) {
				return true;
			}
			return false;
		}
		...
		*/
		function getCompressedUserImage($userEntryId, $compressedQuality = 10)
		{
			try {
		 ...
		 * @return string A base64 encoded string (data url)
		 */
		function compressedImage($image, $compressedQuality = 10)
		{
