...
class IPMUser extends TestUser {
	...
	protected function saveItem($message, $open = true) {
		...
		$item = $this->execute($this->defaultItemModule, [
			'save' => $message,
		]);
	 ...
	 * @param bool  $open    true if the updated item should be opened, otherwise the
	 *                       saved properties will be returned
	 *
	 * @return MAPI_MESSAGE the saved message
	 */
	 ...
	 */
	protected function openItem($entryid, $extraProps = []) {
		$this->logon();
	 ...
	 * @return array The response from the PHP
	 */
	protected function deleteItem($entryid, $extraProps = [], $softDelete = false) {
		$this->logon();
	 ...
	 * @return array The response from the PHP
	 */
	protected function copyItem($entryid, $extraProps = [], $move = false) {
		$this->logon();
	 ...
	 */
	protected function loadItems($extraProps = [], $open = true) {
		$this->logon();

		$items = $this->execute($this->defaultListModule, [
			'list' => array_merge([
	...
	private function findItemsInFolder($folder, $restriction, $open = true) {
		...
		// the table.
		mapi_table_sort($table, [PR_LAST_MODIFICATION_TIME => TABLE_SORT_DESCEND]);
	...
	public function startSearch($proptag, $propvalue) {
		...
		$result = $this->execute($this->defaultListModule, [
			'search' => [
				'entryid' => bin2hex($this->defaultFolderEntryId),
				'store_entryid' => bin2hex($this->defaultStoreEntryId),
		...
		if (isset($result['search'], $result['search']['search_meta'], $result['search']['search_meta']['searchfolder_entryid'])
			) {
	 ...
	 */
	protected function getItemProps($items, $tags = []) {
		$this->logon();
	 ...
	 */
	public function addEmbeddedAttachment($message, $messageToEmbed) {
		$this->logon();
		...
		$props = $this->getItemProps($messageToEmbed, [PR_ENTRYID, PR_STORE_ENTRYID]);

		// open file
		$GLOBALS['attachment_state']->open();

		$attachid = $GLOBALS['attachment_state']->addEmbeddedAttachment($dialogAttachments, [
	...
	public function getAttachmentFromMessage($message, $attachNum = -1) {
		if (isset($message['attachments'], $message['attachments']['item'])) {
