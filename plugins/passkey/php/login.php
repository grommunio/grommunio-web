...
// The assertion challenge is SERVER-minted (see plugin.passkey.php) and stored in
// the EncryptionStore. We hand it to the browser as-is; the browser must NOT
// generate its own. If it is missing (e.g. expired), the ceremony cannot proceed
// and the user is bounced back to normal login.
$passkeyChallenge = $encryptionStore->get('passkeyAssertionChallenge');
if (empty($passkeyChallenge)) {
...
<head>
    ...
    <script>
        ...
        function authenticateWithPasskey() {
            ...
            // Use the SERVER-minted, single-use challenge. The browser never
            // generates its own challenge - that would defeat replay protection.
            const challenge = base64UrlToArrayBuffer('<?php echo $passkeyChallenge; ?>');
            ...
            const requestOptions = {
                publicKey: {
                    ...
                    rpId: '<?php echo htmlspecialchars(PLUGIN_PASSKEY_RP_ID, ENT_QUOTES); ?>'
                }
            };
