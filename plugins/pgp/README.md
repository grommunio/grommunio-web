...
Protection is applied at submission. Autosave pauses while encryption is selected, because **autosaved drafts and their attachments are stored in plaintext**; Settings → Mail → Compose can re-enable it. The draft saved at send time and the outbox copy are plaintext until the protection hook rewrites them, and the web server handles the unprotected draft while preparing canonical MIME for browser protection. Secure mailbox storage, backups and retention accordingly. Incoming decryption and decrypted attachment handling run in browser memory; no decrypted incoming body is imported back into a saved MAPI item. Replies/forwards that quote decrypted content can become new plaintext drafts and require their own protection choice.
...
Run the browser crypto/MIME tests, transport/session failure tests and mailbox store tests. The browser cryptography and mailbox storage suites need a GnuPG executable for their interoperability fixtures; the PHP suites replace php-mapi with doubles that return `false` on failure, so exception-mode failures of a live php-mapi are covered only by the live tests below:
...
The browser cryptography suite passed 100 assertions, including interoperability with real GnuPG, integrity and signature failures, protected-key import/export, expired-signer handling and revocation refresh. Browser transport/lifecycle/rendering tests passed 107 assertions and frontend tests passed 32. The PHP browser-transport suite passed 173 fault-injected assertions, the real concurrent session-receipt test passed 11, inline-attachment upload tests passed 15, threading-header tests passed 9, and keyserver SSRF policy passed 26. The mailbox storage suite passed 53 assertions using fault-injected MAPI and real protected-key packets. On the demo, a protected key generated by OpenPGP.js passed 24 actual MAPI persistence assertions, including exact binary armor preservation, public-only verification bundles, exclusion from ordinary folder tables, update revisions, private-key preservation and cleanup. Counts describe the executed suites, not an Outlook UI test.
...
GnuPG is an interoperability oracle only in `test/oracle/Gpg.php`; `testGpg.php` passed 60 assertions and `testMime.php` passed 42. These development tests, like `testBrowserCrypto.cjs` and `testMapiKeyStore.php`, need a GnuPG executable and isolated temporary keyrings, not production web keyring configuration. `server/test/openPgpAvailabilityTest.php enabled` covers the allow path of the core fail-closed check.
