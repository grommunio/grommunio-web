...
/**
 * @class Grommunio.settings.ui.QuotaBar
 * @extends Ext.BoxComponent
 * @xtype grommunio.quotabar
 *
 * Quotabar shows information about user's store size,
 * and denotes warning-quota, soft-quota and hard-quota information.
 *
 */
Grommunio.settings.ui.QuotaBar = Ext.extend(Ext.BoxComponent, {
	/**
	 * @cfg {Grommunio.hierarchy.data.MAPIStoreRecord} userStore
	...
	*/
	 ...
	 */
	quotaTemplate:'<div class="grommunio-quotabar">' +
						'<div class="grommunio-quotabar-normal"></div>' +
						'<div class="grommunio-quotabar-warn"></div>' +
						'<div class="grommunio-quotabar-soft"></div>' +
						'<div class="grommunio-quotabar-hard"></div>' +
					'</div>',
	...
	constructor: function(config)
	{
		...
		Grommunio.settings.ui.QuotaBar.superclass.constructor.call(this, config);
	...
	updateQuotaBar: function() {
		if(this.userStore) {
			if(this.el && this.el.child('div.grommunio-quotabar')) {
				...
				if(warnQuota && (!softQuota || warnQuota < softQuota) && (!hardQuota || warnQuota < hardQuota)) {
					quota.push({size: warnQuota, element: 'div.grommunio-quotabar-warn'});
				}

				if(softQuota && (!hardQuota || softQuota < hardQuota)) {
					quota.push({size: softQuota, element: 'div.grommunio-quotabar-soft'});
				}

				if(hardQuota) {
					quota.push({size: hardQuota, element: 'div.grommunio-quotabar-hard'});
				}
				...
				var maxAvailableWidth = this.el.child('div.grommunio-quotabar').getWidth(true);
				var factor = maxAvailableWidth/maxLimit;

				// Set blockSize
				var blockSize, totalSize = 0;
				var element = 'div.grommunio-quotabar-normal';
	...
	/**
	 * Function is called when data is update in {@link Grommunio.hierarchy.data.HierarchyStore}.
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} storeRecord, record of the hierarchy store
...
Ext.reg('grommunio.quotabar', Grommunio.settings.ui.QuotaBar);
