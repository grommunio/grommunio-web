...
/**
 * @class Grommunio.settings.ui.SettingsTreePanel
 * @extends Grommunio.common.ui.EditorTreeGrid
 * @xtype grommunio.settingstreepanel
 *
 ...
 */
Grommunio.settings.ui.SettingsTreePanel = Ext.extend(Grommunio.common.ui.EditorTreeGrid, {
	/**
	...
	/**
	 * @cfg {Grommunio.settings.SettingsModel} model The model which should be displayed
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			xtype: 'grommunio.editortreegrid',
			ref: 'treeGrid',
			border: false,
			rootVisible: true,
			enableHdMenu: false,
			enableSort: true,
			forceFit: true,
			loader: new Grommunio.settings.data.SettingsTreeLoader({
				model: config.model,
				autoExpandLevel: 4,
				autoExpandFilter: /^\/*(grommunio.*)?$/
			}),
			root: new Grommunio.settings.ui.SettingsTreeNode({
				text: _('Settings'),
				id: '/',
				expanded: false
			}),
			editors: {
				...
				'number': { xtype: 'grommunio.spinnerfield', plugins: [ 'grommunio.numberspinner' ] },
		...
		Grommunio.settings.ui.SettingsTreePanel.superclass.constructor.call(this, config);
		
		if (this.treeFilter && !(this.treeFilter instanceof Ext.tree.TreeFilter)) {
			this.treeFilter = new Grommunio.common.ui.TreeFilter(this, {autoClear: true});
		}
	},
	...
	initEvents: function()
	{
		Grommunio.settings.ui.SettingsTreePanel.superclass.initEvents.call(this);
	...
	/**
	 * Initialize a new {@link Grommunio.settings.SettingsModel Settings Model}.
	 * @param {Grommunio.settings.SettingsModel} model The model to load
	 ...
	 * has fired the {@link Grommunio.settings.SettingsModel#set} event to indicate that one or
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.settings.SettingsModel settingsModel}
	 * has fired the {@link Grommunio.settings.SettingsModel#remove} event to indicate that one or
	 ...
	 * This will {@link Grommunio.settings.SettingsModel#set update} the setting in the
	 * {@link Grommunio.settings.SettingsModel}.
	 * @param {Grommunio.common.ui.EditorTreeGrid} grid The grid which fired the event
	...
	{
		node.select();
		Grommunio.core.data.UIFactory.openDefaultContextMenu(node, { position: event.getXY(), settingsModel: this.model });
	},
	...
	onDestroy: function()
	{
		Grommunio.settings.ui.SettingsTreePanel.superclass.onDestroy.call(this);
...
Ext.reg('grommunio.settingstreepanel', Grommunio.settings.ui.SettingsTreePanel);
