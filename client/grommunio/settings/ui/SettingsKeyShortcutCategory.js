...
/**
 * @class Grommunio.settings.ui.SettingsKeyShortcutCategory
 ...
 * of grommunio Web.
 */
Grommunio.settings.ui.SettingsKeyShortcutCategory = Ext.extend(Grommunio.settings.ui.SettingsCategory, {
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			categoryIndex: 9998,
			iconCls: 'grommunio-settings-category-keyshortcut',
			items: this.generateWidgets(config)
		});

		Grommunio.settings.ui.SettingsKeyShortcutCategory.superclass.constructor.call(this, config);
	},

	/**
	 * Retrieve The list of {@link Grommunio.core.KeyMapMgr#keys key} configurations.
	 * Generate the key Combinations and Description based on the
	 * available {@link Grommunio.core.KeyMap KeyMap} configurations.
	...
	getShortcutDescription: function()
	{
		...
		Ext.iterate(Grommunio.core.KeyMapMgr.keys, function(key, bindings) {
	...
	/**
	 * Creates the {@link Grommunio.settings.ui.SettingsWidget widget} for 'Plugins'
	 ...
	 * It will also include {@link Grommunio.settings.ui.SettingsKeyShortcutWidget widget} to
	...
	generateWidgets: function(config)
	{
		...
		Ext.iterate(keyDescription, function(key, bindings) {
			widgets.push({
				...
				title: key,
				layout: 'form',
				itemCls: 'grommunio-settings-keyboard-shortcut-row',
	...
	/**
	 * Generate the key Combinations based on available {@link Grommunio.core.KeyMap KeyMap} configurations.
	 * It will also concatenate the combination for special keys like ENTER, F5, and DELETE accordingly.
	 * @param {Object|Array} keyConfig The {@link Grommunio.core.KeyMap KeyMap} configurations.
...
Ext.reg('grommunio.settingskeyshortcutcategory', Grommunio.settings.ui.SettingsKeyShortcutCategory);
