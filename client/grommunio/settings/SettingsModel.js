...
/**
 * @class Grommunio.settings.SettingsModel
 ...
 */
Grommunio.settings.SettingsModel = Ext.extend(Ext.util.Observable, {
	/**
	 ...
	 * Defaults to {@link Grommunio.settings.data.SettingsDefaultValue}.
	 */
	defaults: undefined,
	...
	constructor: function(config)
	{
		...
		if (!Ext.isObject(config.defaults)) {
			config.defaults = Grommunio.settings.data.SettingsDefaultValue.getDefaultValues();
		}

		Ext.apply(this, config);

		Grommunio.settings.SettingsModel.superclass.constructor.call(this, config);
	...
	{
		this.settings = Grommunio.core.Util.applyRecursive({}, obj, this.defaults);
	},
	...
	removeSettings: function(path)
	{
		...
		path = Grommunio.core.Util.uniqueArray(path);
	...
	restoreSettings: function(settings)
	{
		...
		// First remove all duplicates
		settings = Grommunio.core.Util.uniqueArray(settings, 'path');
	...
	save: function()
	{
		...
		var moduleName = Grommunio.core.ModuleNames.getListName('settings');
		var queued = false;
		...
		if (hasDeleted) {
			queued = this.addSaveAction(request, moduleName, Grommunio.core.Actions['delete'], this.deleted.slice()) || queued;
		}

		if (hasModified) {
			queued = this.addSaveAction(request, moduleName, Grommunio.core.Actions['set'], this.modified.slice()) || queued;
		}

		if (hasReset) {
			queued = this.addSaveAction(request, moduleName, Grommunio.core.Actions['reset'], this.resetSettings.slice()) || queued;
		}
	 ...
	 * @param {Grommunio.core.Request} request The request to add to
	 * @param {String} moduleName The settings module name
	 * @param {Grommunio.core.Actions} action The action to perform
	 ...
	 * @param {Grommunio.core.Actions} action The action which was sent
	...
	commitSaveAction: function(action, parameters)
	{
		...
		switch (action) {
			case Grommunio.core.Actions['delete']:
				list = this.deleted;
				break;
			case Grommunio.core.Actions['set']:
				list = this.modified;
				break;
			case Grommunio.core.Actions['reset']:
	...
	get: function(path, raw, returnDefaults)
	{
		...
		if (Ext.isObject(value)) {
			if (raw === true) {
				return Grommunio.core.Util.applyRecursive({}, value);
			}
