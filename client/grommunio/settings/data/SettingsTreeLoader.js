...
/**
 * @class Grommunio.settings.data.SettingsTreeLoader
 * @extends Ext.ux.tree.TreeGridLoader
 *
 * A Special treeloader to be used by the
 * {@link Grommunio.settings.ui.SettingsTreePanel SettingsTreePanel}.
 * This will dynamically load the child nodes for a given node by reading the
 * {@link Grommunio.core.data.SettingsModel Settings}.
 */
Grommunio.settings.data.SettingsTreeLoader = Ext.extend(Ext.ux.tree.TreeGridLoader, {
	/**
	 * @cfg {Number} autoExpandLevel The maximum level in which the TreeNodes will be
	 * expanded by default. e.g. When this is set to 1 then the node 'grommunio' will
	 * be expanded by default, when set to 2 the node 'grommunio/v1' will be expanded,
	 * etc.
	 */
	autoExpandLevel: 0,
	...
	/**
	 * @cfg {Grommunio.settings.SettingsModel} model The model which should be used for
	...
	constructor: function(config)
	{
		...
		Grommunio.settings.data.SettingsTreeLoader.superclass.constructor.call(this, config);
	},
	 ...
	 * the {@link Grommunio.core.data.SettingsModel Settings} to find the child nodes which
	...
	directFn: function(node, fn)
	{
		...
		for (var key in settings) {
			...
			data.push({
				...
				uiProvider: Grommunio.settings.ui.SettingsTreeNodeUI
			});
		}
