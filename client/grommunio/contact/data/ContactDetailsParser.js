...
/**
 * @class Grommunio.contact.data.ContactDetailsParser
 * @extends Object
 ...
 */
Grommunio.contact.data.ContactDetailsParser = Ext.extend(Object, {
	/**
	...
	{
		config = config || {};
		this.prefixOptions = Grommunio.contact.data.config.Prefix;
		this.suffixOptions = Grommunio.contact.data.config.Suffix;
		Ext.apply(this, config);
		Grommunio.contact.data.ContactDetailsParser.superclass.constructor.call(this, config);
	},
	...
	parseNameInfo: function(data)
	{
		...
		displayNameParts = Grommunio.core.Util.trimStringArray(displayNameParts);

		switch(displayNameParts.length) {
			case 4:
				// search for display name prefix in name
				var firstToken = displayNameParts.shift();
				result['display_name_prefix'] = Grommunio.core.Util.inArray(this.prefixOptions, firstToken, true) ? firstToken : '';

				// search for suffix in name
				var lastToken = displayNameParts.pop();
				result['generation'] = Grommunio.core.Util.inArray(this.suffixOptions, lastToken, true) ? lastToken : '';
	...
	parsePhoneInfo: function(data)
	{
		...
		phoneNumberParts = Grommunio.core.Util.trimStringArray(phoneNumberParts);
	...
	parseAddressInfo: function(data)
	{
		...
		addressParts = Grommunio.core.Util.trimStringArray(addressParts);
