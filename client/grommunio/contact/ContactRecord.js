...
/**
 * @class Grommunio.contact.ContactRecordFields
 ...
 */
Grommunio.contact.ContactRecordFields = [
	// name
	{name: 'fileas'},
...
/**
 * @class Grommunio.contact.ContactRecord
 ...
 * used as Contacts
 */
Grommunio.contact.ContactRecord = Ext.extend(Grommunio.core.data.IPMRecord, {
	...
	/**
	 * Convert this contact record into a {@link Grommunio.core.data.IPMRecipientRecord recipient}
	 * which can be used for composing news mails. If multiple email addresses are present in contact
	 * then it will create multiple {@link Grommunio.core.data.IPMRecipientRecord IPMRecipientRecord}.
	 *
	...
	convertToRecipient: function(recipientType, multiple)
	{
		...
		if(multiple === true) {
			for(var index = 1; index < 4; index++) {
				if(!Ext.isEmpty(this.get('email_address_' + index))) {
					var props = {
						entryid: Grommunio.core.EntryId.wrapContactProviderEntryId(this.get('entryid'), Grommunio.core.mapi.ObjectType.MAPI_MAILUSER),
						object_type: Grommunio.core.mapi.ObjectType.MAPI_MAILUSER,
						display_type: Grommunio.core.mapi.DisplayType.DT_MAILUSER,
						display_type_ex: Grommunio.core.mapi.DisplayType.DT_MAILUSER,
					...
					recipientRecords.push(Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, props));
				}
			}
		...
		if(!Ext.isEmpty(this.get('email_address'))) {
			...
			return Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, props);
		}
	},

	/**
	 * Convert this {@link Grommunio.contact.ContactRecord ContactRecord} into a {@link Grommunio.contact.DistlistMemberRecord DistlistMemberRecord}
	...
	convertToDistlistMember: function()
	{
		...
		return Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_DISTLIST_MEMBER, {
			...
			distlist_type: Grommunio.core.mapi.DistlistType.DL_USER,
	...
	/**
	 * Function is used to update subject property on {@link Grommunio.core.data.IPMRecord IPMRecord}
	...
	updateSubject: function()
	{
		...
		if (Ext.isEmpty(fileas) && !Ext.isEmpty(subject)) {
			...
			var fileasArray = display.trim().split(new RegExp(Grommunio.contact.data.config.SP + '|' + Grommunio.contact.data.config.NBSP, 'g'));

			if (fileasArray.length === 1) {
				fileas = fileasArray[0];
			} else {
				fileas = fileasArray.join(Grommunio.contact.data.config.NBSP);
			}
		}
	...
	/**
	 * Function is used to update address_book_long and address_book_mv properties on {@link Grommunio.core.data.IPMRecord IPMRecord}.
	 * These properties are used by {@link Grommunio.addressbook.dialogs.AddressBookContentPanel AddressBookContentPanel} to show contact items
	 * in the addressbook.
	 *
	...
	afterOpen: function()
	{
		if (this.isSubMessage()) {
			this.beginEdit();
			this.set('attach_num', []);
			this.set('message_flags', Grommunio.core.mapi.MessageFlags.MSGFLAG_READ);
			this.set('entryid', '');
			this.endEdit();
			this.phantom = true;
		...
		return Grommunio.contact.ContactRecord.superclass.afterOpen.call(this, arguments);
	},
...
Grommunio.core.data.RecordFactory.addFieldToMessageClass('IPM.Contact', Grommunio.contact.ContactRecordFields);
Grommunio.core.data.RecordFactory.setSubStoreToMessageClass('IPM.Contact', 'attachments', Grommunio.core.data.IPMAttachmentStore);
Grommunio.core.data.RecordFactory.setBaseClassToMessageClass('IPM.Contact', Grommunio.contact.ContactRecord);
