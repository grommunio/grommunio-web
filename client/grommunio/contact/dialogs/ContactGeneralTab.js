...
/**
 * @class Grommunio.contact.dialogs.ContactGeneralTab
 * @extends Ext.form.FormPanel
 * @xtype grommunio.contactgeneraltab
 *
 * This class is used to create layout of general tab panel.
 */
Grommunio.contact.dialogs.ContactGeneralTab = Ext.extend(Ext.form.FormPanel, {
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			xtype: 'grommunio.contactgeneraltab',
			cls: 'grommunio-contactgeneraltab',
			title: _('General'),
			autoScroll: true,
			border: false,
			layoutConfig: {
		...
		Grommunio.contact.dialogs.ContactGeneralTab.superclass.constructor.call(this, config);
	},
	 ...
	 * @param {Object} config config object of {@Grommunio.contact.dialogs.ContactGeneralTab ContactGeneralTab}.
	...
	createNameFieldset: function(config)
	{
		return {
			title: _('Name'),
			defaultType: 'grommunio.compositefield',
			defaults: {
				anchor: '99%'
			},
			items: [{
	...
	createAttachmentFieldset: function(config)
	{
		return {
			...
			columnWidth: 1,
			border: false,
			defaultType: 'grommunio.resizablecompositefield',
			defaults: {
				hideLabel: true,
				anchor: '99%'
			},
			items: [{
				cls: 'grommunio-contactcreatepanel-field-attachments',
				items: [{
	 ...
	 * and it will call function that will open the {@link Grommunio.contact.dialogs.ContactPhoneContent ContactPhoneContent}.
	 ...
	 * this function is used as wrapper to discard arguments passed with the handler
	 * and it will call function that will open the {@link Grommunio.contact.dialogs.ContactAddressContent ContactAddressContent}.
	 * @param {Ext.SplitButton} buttonEl split button element which was clicked.
	 * @param {Ext.EventObject} eventObj event object for the click event.
	 */
	...
	/**
	 * Callback function for {@link Grommunio.addressbook.dialogs.ABUserSelectionContentPanel AddressBook}
	 ...
	 * according to updated {@link Grommunio.core.data.IPMRecord record} data.
	 */
	...
	onContextMenuClick: function(eventObj, target, object)
	{
		if(this.hasContactPhoto) {
			...
			Grommunio.core.data.UIFactory.openDefaultContextMenu(attachmentRecord, { position: eventObj.getXY(), parent: this});
		}
	},

	/**
	 * Function will get attached contact picture record
	 * from {@link Grommunio.core.data.IPMAttachmentStore IPMAttachmentStore}.
	 * @return {Grommunio.core.data.IPMAttachmentRecord} returns the contact picture attachment record.
	 */
	 ...
	 * {@link Grommunio.core.data.IPMAttachmentStore IPMAttachmentStore}. Also it will
	 ...
	 * @param {Grommunio.core.data.IPMAttachmentRecord} attachmentRecord Attachment record.
	 */
	 ...
	 * which is going to add the contact picture(Attachment record) in {@link Grommunio.core.data.IPMAttachmentStore store}.
	 *
	...
	onMenuItemSelection: function(menu, menuItem, eventObj)
	{
		if(!Ext.isEmpty(menuItem)) {
			var compositeField = menu.findParentByType('grommunio.compositefield');
	 ...
	 * property in {@link Grommunio.core.data.IPMRecord IPMRecord} is changed. This function is also called when
	...
	generateFileAsItems: function()
	{
		...
		if(!Ext.isEmpty(displayName)) {
			...
			var displayNameArray = displayName.trim().split(new RegExp(Grommunio.contact.data.config.SP + '|' + Grommunio.contact.data.config.NBSP, 'g'));

			// remove whitespaces from array
			displayNameArray = Grommunio.core.Util.trimStringArray(displayNameArray);
			...
			if(displayNameArray.length > 0) {
				if(displayNameArray.length === 1) {
					...
					var surname = displayNameArray[displayNameArray.length - 1] + ',' + Grommunio.contact.data.config.NBSP;
					var remainder = displayNameArray.slice(0, displayNameArray.length - 1);

					contactName = surname + remainder.join(Grommunio.contact.data.config.NBSP);
					fileasOptions.push([contactName]);

					// givenname middlename surname
					fileasOptions.push([displayNameArray.join(Grommunio.contact.data.config.NBSP)]);
				}
			}
			...
			if(!Ext.isEmpty(companyName)) {
				...
				if(!Ext.isEmpty(contactName)) {
					...
					fileasOptions.push([contactName + Grommunio.contact.data.config.NBSP + '(' + companyName + ')']);
					// companyname (givenname | givenname middlename surname | surname, givenname middlename)
					fileasOptions.push([companyName + Grommunio.contact.data.config.NBSP + '(' + contactName + ')']);
				}
			}
	...
	/**
	 * Function will update fileas field in {@link Grommunio.core.data.IPMRecord IPMRecord} based on
	...
	onDisplayNameChange: function(field, newValue, oldValue)
	{
		if (field.validateValue(field.processValue(newValue))) {
			...
			var showDetailedContent = function() {
				...
				var settingValue = container.getSettingsModel().get('grommunio/v1/contexts/contact/show_name_dialog');
	...
	onAddressChange: function(field, newValue, oldValue)
	{
		if (field.validateValue(field.processValue(newValue))) {
			...
			var showDetailedContent = function() {
				...
				var settingValue = container.getSettingsModel().get('grommunio/v1/contexts/contact/show_address_dialog');
				if (settingValue === true && !Ext.isEmpty(newValue)) {
					if (parsedData['incomplete_info'] === true) {
	...
	{
		Grommunio.contact.Actions.openDetailedNameContent(this.record, { parser: this.getContactParser(), parsedData: parsedData });
	},
	...
	{
		Grommunio.contact.Actions.openDetailedAddressContent(this.record, { parser: this.getContactParser(), parsedData: parsedData, property: property });
	},
	...
	{
		Grommunio.contact.Actions.openDetailedPhoneContent(this.record, { parser: this.getContactParser(), property: property });
	},

	/**
	 * @return {Grommunio.contact.data.ContactDetailsParser} contact details parser
	 */
   ...
   * for this {@link Grommunio.contact.dialogs.ContactGeneralTab ContactGeneralTabPanel}.
   * @private
   */
  initEvents: function ()
  {
...
Ext.reg('grommunio.contactgeneraltab', Grommunio.contact.dialogs.ContactGeneralTab);
