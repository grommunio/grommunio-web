...
/*
 * #dependsFile client/grommunio/mail/data/ConversationManagers.js
 * #dependsFile client/grommunio/mail/data/ConversationItemsResponseHandler.js
 */
Ext.namespace('Grommunio.mail');

/**
 * @class Grommunio.mail.MailStore
 * @extends Grommunio.core.data.ListModuleStore
 * @xtype grommunio.mailstore
 *
 ...
 */
Grommunio.mail.MailStore = Ext.extend(Grommunio.core.data.ListModuleStore, {
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			conversationManager: new Grommunio.mail.data.ConversationManagers()
		});

		Grommunio.mail.MailStore.superclass.constructor.call(this, config);
		...
		if ( !this.standalone ){
			Grommunio.core.PresenceManager.registerStore(this, ['sender', 'received_by', 'sent_representing']);
		}
	},
	...
	getRecordsForUpdateData: function(records, action)
	{
		if (!Ext.isDefined(records) || action !== Ext.data.Api.actions.open) {
			return Grommunio.mail.MailStore.superclass.getRecordsForUpdateData.apply(this, arguments);
		}
	...
	reload: function(options)
	{
		...
		Grommunio.mail.MailStore.superclass.reload.call(this, options);
	},
	...
	containsConversations: function()
	{
		...
		var infiniteScrollEnabled = container.getSettingsModel().get('grommunio/v1/contexts/mail/enable_live_scroll');
		...
		return (Grommunio.core.EntryId.compareEntryIds(this.entryId, inbox.get('entryid')));
	},
	 ...
	 * twice.
	 *
	 * @param {Grommunio.mail.MailStore} store This store.
	 * @param {Grommunio.mail.MailRecord[]} records The loaded records.
	 ...
	 * @param {Grommunio.mail.MailRecord[]} members The items of the conversation, newest first.
	 ...
	 * @return {Grommunio.mail.MailRecord} The header record.
	 * @private
	 */
	getConversationHeaderRecord: function(conversationId, members, totalCount)
	{
		...
		var values = {
			...
			message_flags: read ? Grommunio.core.mapi.MessageFlags.MSGFLAG_READ : 0,
		...
		header = Grommunio.core.data.RecordFactory.createRecordObjectByMessageClass('IPM.Note', values);
	 ...
	 *
	 * @param {Grommunio.mail.MailRecord} record The record that should be checked
	 ...
	 *
	 * @param {Grommunio.mail.MailRecord} headerRecord The header record of the conversation.
	 * @private
	 */
	 ...
	 *
	 * @param {Grommunio.mail.MailRecord} headerRecord
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.MapiRecord} headerRecord The header record of the conversation
	 * @return {Grommunio.core.data.MapiRecord[]} The records that belong to the requested conversation
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.MapiRecord} record The conversation item record whose header record needs to be found.
	 * @return {Grommunio.core.data.MapiRecord} returns header record for the given conversation item
	...
	getStoreLength: function()
	{
		...
		return Grommunio.mail.MailStore.superclass.getStoreLength.apply(this, arguments);
	},
	...
	{
		if (filterType === Grommunio.common.data.Filters.UNREAD) {
			var unreadFilterRestriction = Grommunio.core.data.RestrictionFactory.dataResBitmask(
				'PR_MESSAGE_FLAGS',
			...
			if (model) {
				...
				if(!Ext.isEmpty(previewedRecord) && this.hasFilterApplied) {
					return Grommunio.core.data.RestrictionFactory.createResOr([
						Grommunio.core.data.RestrictionFactory.dataResProperty(
							'entryid',
							Grommunio.core.mapi.Restrictions.RELOP_EQ,
...
Ext.reg('grommunio.mailstore', Grommunio.mail.MailStore);
