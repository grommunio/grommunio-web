...
/**
 * @class Grommunio.mail.ui.MailPreviewPanel
 * @extends Grommunio.core.ui.PreviewPanel
 * @xtype grommunio.mailpreviewpanel
 */
Grommunio.mail.ui.MailPreviewPanel = Ext.extend(Grommunio.core.ui.PreviewPanel, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.mail.ui.MailPreviewPanel.superclass.constructor.call(this, config);
	},
	 ...
	 *
	 * @param {Grommunio.core.data.MAPIRecord} record The record to be previewed.
	...
	getConversationForRecord: function(record)
	{
		...
		// Grommunio.advancesearch.AdvanceSearchStore#dedupeConversations).
	 ...
	 *
	 * @param {Grommunio.core.data.MAPIRecord} record The record to preview.
	 * @private
	 */
	showRecordInPanel: function(record)
	{
		...
		if (conversation === false) {
			...
			Grommunio.mail.ui.MailPreviewPanel.superclass.showRecordInPanel.apply(this, arguments);
			return;
		}
	...
	update: function(record, contentReset)
	{
		if ( this.record && this.record !== record && this.record.store && this.record.store !== record.store ){
			Grommunio.core.PresenceManager.unregisterStore(this.record.store);
		}
		if ( record && record.store ){
			Grommunio.core.PresenceManager.registerStore(record.store, 'sender');
		}

		if ( this.record && this.record.subStores && this.record.subStores.recipients ){
			Grommunio.core.PresenceManager.unregisterStore(this.record.subStores.recipients);
		}
		if ( record && record.subStores && record.subStores.recipients ){
			Grommunio.core.PresenceManager.registerStore(record.subStores.recipients);
		}

		Grommunio.mail.ui.MailPreviewPanel.superclass.update.apply(this, arguments);
	...
	onResponse: function(button)
	{
		...
		Grommunio.mail.Actions.openCreateMailResponseContent(this.record, this.model, button.responseMode);
	}
});

Ext.reg('grommunio.mailpreviewpanel', Grommunio.mail.ui.MailPreviewPanel);
