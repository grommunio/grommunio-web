...
/**
 * @class Grommunio.mail.ui.ConversationViewPanel
 * @extends Ext.Panel
 * @xtype grommunio.conversationviewpanel
 *
 ...
 */
Grommunio.mail.ui.ConversationViewPanel = Ext.extend(Ext.Panel, {
	/**
	 ...
	 * @property
	 * @type Grommunio.mail.MailRecord
	 */
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			xtype: 'grommunio.conversationviewpanel',
			cls: 'grommunio-mailconversationpanel',
			border: false,
			autoScroll: true,
			bodyStyle: 'padding: 9px;'
		});

		Grommunio.mail.ui.ConversationViewPanel.superclass.constructor.call(this, config);
	},
	 ...
	 *
	 * @param {Grommunio.mail.MailRecord[]} records The items of the conversation, newest first.
	 * @param {Grommunio.mail.MailRecord} selected The record that is selected in the mail grid.
	 */
	 ...
	 *
	 * @param {Grommunio.mail.MailStore} store The store to monitor.
	 * @private
	 */
	 ...
	 * its own {@link Grommunio.core.plugins.RecordComponentPlugin} so the message
	 * view components inside bind against the card, and thus against this
	 * card's message.
	 *
	 * @param {Grommunio.mail.MailRecord} record The message.
	...
	adjustCardBody: function(card)
	{
		...
		if (!card.messageBodyCmp) {
			var found = card.findByType('grommunio.messagebody');
	 ...
	 *
	 * @param {Grommunio.mail.MailRecord} record The record to focus.
	 * @private
	 */
	...
	onDestroy: function()
	{
		...
		Grommunio.mail.ui.ConversationViewPanel.superclass.onDestroy.apply(this, arguments);
	}
});

Ext.reg('grommunio.conversationviewpanel', Grommunio.mail.ui.ConversationViewPanel);
