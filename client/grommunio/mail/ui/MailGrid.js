...
/**
 * @class Grommunio.mail.ui.MailGrid
 * @extends Grommunio.common.ui.grid.MapiMessageGrid
 * @xtype grommunio.mailgrid
 */
Grommunio.mail.ui.MailGrid = Ext.extend(Grommunio.common.ui.grid.MapiMessageGrid, {
	/**
	 ...
	 * {@link Grommunio.hierarchy.data.MAPIFolderRecord#getDefaultFolderKey folder key}
	 * for the {@link Grommunio.hierarchy.data.MAPIFolderRecord folders} which contain
	...
	/**
	 * @cfg {Boolean} expandSingleConversation True if {@link Grommunio.mail.settings.SettingsConversationWidget#enableConversations enabled conversation view }
	 * and {@link Grommunio.mail.settings.SettingsConversationWidget#singleExpand single expand conversation} user settings enabled False otherwise
	 *
	...
	constructor: function(config)
	{
		...
		config.plugins.push('grommunio.mailgridrowactions');
		...
		Ext.applyIf(config, {
			...
			view: new Grommunio.mail.ui.MailGridView(this.initViewConfig()),
		...
		Grommunio.mail.ui.MailGrid.superclass.constructor.call(this, config);
	},
	...
	initEvents: function()
	{
		Grommunio.mail.ui.MailGrid.superclass.initEvents.call(this);
		...
		// (see Grommunio.mail.MailStore#regroupConversations).
	...
	viewConfigGetRowClass : function(record, rowIndex, rowParams, store)
	{
		...
		if (this.enableRowBody) {
			var meta = {}; // Metadata object for Grommunio.common.ui.grid.Renderers.
			var value = ''; // The value which must be rendered
			rowParams.body = '<div class="grommunio-grid-body-container">';
			...
			if (!record.isConversationHeaderRecord()) {
				...
				var categoriesHtml = Grommunio.common.categories.Util.getCategoriesHtml(categories, record.get('store_entryid'));
			...
			meta = {};
			value = Grommunio.common.ui.grid.Renderers.subject(record.get('subject'), meta, record);
	...
	{
		var flagged = Grommunio.core.mapi.FlagStatus.flagged;
	...
	{
		return new Grommunio.mail.ui.MailRowSelectionModel({
			singleSelect: false
		});
	},
	...
	{
		return new Grommunio.mail.ui.MailGridColumnModel();
	},
	...
	{
		if(oldView === Grommunio.mail.data.Views.LIVESCROLL) {
	 ...
	 * {@link Grommunio.core.Context#viewmodechange viewmodechange} event. This will check
	 ...
	 * of the {@link Grommunio.mail.ui.MailGrid MailGrid}. Either use the extended (and more flexible)
	...
	onContextViewModeChange: function(context, newViewMode, oldViewMode)
	{
		switch(newViewMode){
			...
			case Grommunio.mail.data.ViewModes.BOTTOM_PREVIEW:
				var compact = newViewMode === Grommunio.mail.data.ViewModes.RIGHT_PREVIEW;
	 ...
	 * {@link Grommunio.common.ui.grid.ColumnModel#setConfig}. At this time we can
	...
	onBeforeConfigChange: function(columnModel, config)
	{
		...
		Grommunio.mail.ui.MailGrid.superclass.onBeforeConfigChange.apply(this, arguments);
	},
	 ...
	 *
	 * @param {Grommunio.mail.ui.MailGrid} grid The mail grid
	 ...
	 * add category icon.
	 *
	 * @param {Grommunio.mail.ui.MailGrid} grid The grid which was right clicked
	...
	onRowClick : function(grid, rowIndex, event)
	{
		...
		if ( Ext.get(event.target).hasClass('k-category-add') ){
			Grommunio.common.Actions.openCategoriesMenu([record], event.getXY());
		}
	},
	...
	onCellContextMenu: function(grid, rowIndex, cellIndex, event)
	{
		...
		if (record && record.isConversationHeaderRecord()) {
			...
			if (!Ext.isEmpty(items)) {
				Grommunio.core.data.UIFactory.openDefaultContextMenu(items, {
		...
		return Grommunio.mail.ui.MailGrid.superclass.onCellContextMenu.call(this, grid, rowIndex, cellIndex, event);
	},
	 ...
	 * Open a {@link Grommunio.mail.dialogs.ShowMailContentPanel contentpanel} which contains the selected item.
	 *
	...
	onRowDblClick: function(grid, rowIndex, e)
	{
		...
		var browserWindow = Grommunio.core.BrowserWindowMgr.getOpenedWindow(record);
		if (browserWindow) {
			browserWindow.focus();
		} else {
			Grommunio.common.Actions.openMessageContent(record, undefined, true);
		}
	},
	 ...
	 *
	 * @param {Grommunio.mail.ui.MailRowSelectionModel} selectionModel The selectionModel
	 * @param {Number} rowIndex Then index to be selected.
	 * @param {Boolean} keepExisting False if other selections will be cleared
	 * @param {Grommunio.core.data.IPMRecord} record The record to be selected
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The header record of the conversation.
	 * @param {Grommunio.mail.ui.MailRowSelectionModel} selectionModel The selectionModel
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The record needs to expand.
	 * @param {Grommunio.mail.ui.MailRowSelectionModel} selectionModel The selectionModel
	 * @param {Number} rowIndex Then index to be selected.
	 * @param {Boolean} keepExisting False if other selections will be cleared
	 */
	...
	/**
	 * Helper function used to check {@link Grommunio.mail.settings.SettingsConversationWidget#enableConversations enabled conversation view }
	 * and {@link Grommunio.mail.settings.SettingsConversationWidget#singleExpand single expand conversation} user setting enabled.
	 *
	 * @return {Boolean} True if {@link Grommunio.mail.settings.SettingsConversationWidget#enableConversations enabled conversation view } and
	 * {@link Grommunio.mail.settings.SettingsConversationWidget#singleExpand expand single conversation} is enabled else false.
	 */
	allowExpandOneConversation: function()
	{
		var settingsModel = container.getSettingsModel();
		return settingsModel.get("grommunio/v1/contexts/mail/enable_conversation_view") && settingsModel.get("grommunio/v1/contexts/mail/expand_single_conversation");
	},

	/**
	 * Event handler which is triggered when the user selects a row from the {@link Ext.grid.GridPanel}.
	 * This will updates the {@link Grommunio.mail.MailContextModel MailContextModel} with the record which
	...
	{
		this.cancelPreviewRecordTask();
		Grommunio.mail.ui.MailGrid.superclass.beforeDestroy.apply(this, arguments);
	},
	 ...
	 * on {@link Grommunio.mail.ui.MailGrid mailgrid}. it will first stop the
	...
	/**
	 * Event handler triggers when the configuration of {@link Grommunio.mail.ui.MailGridColumnModel MailGridColumnModel}
	 * is changed.
	 */
	...
	{
		return container.getSettingsModel().get('grommunio/v1/contexts/mail/enable_grouping');
	},

	/**
	 * Called after a row has been removed for the GridView.
	 * This will be called from {@link Grommunio.common.ui.grid.GridPanel#onWriteRecord onWriteRecord}.
	 * Function will check whether the {@link Grommunio.mail.ui.MailRowSelectionModel#updatedLast updatedLast} config is available
	...
	onRowRemoved: function(rowIndex)
	{
		...
		Grommunio.mail.ui.MailGrid.superclass.onRowRemoved.call(this, rowIndex);
	},
	...
	{
		if (activeTab.id === "grommunio-mainpanel-content") {
			this.getView().setFocus();
		}
	},

	/**
	 * Event handler triggers when tab was close. Which call {@link Grommunio.mail.ui.MailGridView#setFocus setFocus} function
	 * to set the focus on mail grid. 
	 */
	onTabClose: function()
	{
		if (container.getTabPanel().getActiveTab().id === "grommunio-mainpanel-content") {
			this.getView().setFocus();
		}
	}
});

Ext.reg('grommunio.mailgrid', Grommunio.mail.ui.MailGrid);
