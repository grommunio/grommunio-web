...
/**
 * @class Grommunio.mail.dialogs.MailCreatePanel
 * @extends Ext.form.FormPanel
 * @xtype grommunio.mailcreatepanel
 *
 * Panel that is used to compose a mail messages.
 */
Grommunio.mail.dialogs.MailCreatePanel = Ext.extend(Ext.form.FormPanel, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.mail.dialogs.MailCreatePanel.superclass.constructor.call(this, config);
	},
	...
	onRender: function()
	{
		Grommunio.mail.dialogs.MailCreatePanel.superclass.onRender.apply(this, arguments);
	...
	{
		var recipients = container.getSettingsModel().get('grommunio/v1/contexts/mail/sendas', true);
		var items = [];
		...
		Ext.each(recipients, function(recipient) {
			var record = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, recipient);
	...
	initMessageFormPanel: function(config)
	{
		...
		return [{
			xtype: 'container',
			cls: 'grommunio-mailcreatepanel-extrainfo',
			ref: 'extraInfoPanel',
			autoHeight: true,
			hidden: true
		},{
			...
			hideLabel: true,
			ref: 'fromField',
			cls: hasEmptySendAsList ? 'grommunio-mailcreatepanel-field-from-large' : 'grommunio-mailcreatepanel-field-from',
			anchor: '100%',
			autoHeight: true,
			items: [{
				...
				boxStore: new Grommunio.core.data.IPMRecipientStore({
					listeners: {
			...
			hideLabel: true,
			cls: 'grommunio-mailcreatepanel-field-to',
			anchor: '100%',
			autoHeight: false,
			items: [{
				xtype: 'button',
		...
		},
		{
			xtype: 'grommunio.resizablecompositefield',
			hideLabel: true,
			cls: 'grommunio-mailcreatepanel-field-cc',
			anchor: '100%',
			autoHeight: false,
			items: [{
				xtype: 'button',
		...
		},
		{
			...
			ref: 'bccField',
			hideLabel: true,
			cls: 'grommunio-mailcreatepanel-field-bcc',
			anchor: '100%',
			autoHeight: false,
			items: [{
				xtype: 'button',
		...
		{
			xtype: 'textfield',
			cls: 'grommunio-mailcreatepanel-field-subject',
			name: 'subject',
			...
			xtype: 'grommunio.resizablecompositefield',
			hideLabel: true,
			cls: 'grommunio-mailcreatepanel-field-attachments',
			anchor: '100%',
			autoHeight: true,
			items: [{
			...
			cls: 'grommunio-mailcreatepanel-field-editor',
			enableKeyEvents: true,
			ref: 'editorField',
			hideLabel: true,
	...
	update: function(record, contentReset)
	{
		...
		if(contentReset) {
			...
			if (this.useHtml) {
				...
				if (!Ext.isEmpty(body) && Ext.isFunction(record.inlineImgOutlookToGrommunio)) {
					body = record.inlineImgOutlookToGrommunio(body);
				} else if (Ext.isEmpty(body) && Ext.isFunction(record.getBody)) {
					body = record.getBody(true);
				}
				if (Ext.isEmpty(body)) {
					body = Grommunio.core.HTMLParser.convertPlainToHTML(record.get('body') || '');
				}
	...
	getExtraInfoMessage: function()
	{
		...
		if (flagStatus !== Grommunio.core.mapi.FlagStatus.flagged) {
			return false;
		}
	...
	/**
	 * Event handler for the {@link Grommunio.common.ui.HtmlEditor#valuecorrection valuecorrection}
	 * event handler which is fired when the Browser has changed the HTML data after it has been set
	 * into the {@link Grommunio.common.ui.HtmlEditor}. This will apply the corrected value
	 ...
	 * @param {Grommunio.common.ui.HtmlEditor} field The field which fired the event
	...
	onBodyValueCorrection: function(field, value, oldValue)
	{
		...
		if (this.editorField.isHtmlEditor()) {
			record.data.html_body = record.inlineImgGrommunioToOutlook(value);
		} else {
	...
	/**
	 * Event Handler for {@link Grommunio.core.data.IPMStore store} open event. Handling a
	...
	/**
	 * Event handler which is triggered when {@link Grommunio.common.ui.HtmlEditor editor}
	...
	onEditorInitialized: function(htmlEditor, tinymceEditor)
	{
		...
		Grommunio.core.KeyMapMgr.activate(this.dialog, 'global', element);
		Grommunio.core.KeyMapMgr.activate(this.dialog, 'contentpanel.record.message', element);
	},
...
Ext.reg('grommunio.mailcreatepanel', Grommunio.mail.dialogs.MailCreatePanel);
