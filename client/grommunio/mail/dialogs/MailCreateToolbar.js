...
/**
 * @class Grommunio.mail.dialogs.MailCreateToolbar
 * @extends Grommunio.core.ui.ContentPanelToolbar
 * @xtype grommunio.mailcreatetoolbar
 */
Grommunio.mail.dialogs.MailCreateToolbar = Ext.extend(Grommunio.core.ui.ContentPanelToolbar, {
	 ...
	 * @param {Grommunio.mail.dialogs.MailCreateToolbar} toolbar This toolbar
	 */
	/**
	 ...
	 * @param {Grommunio.mail.dialogs.ShowMailToolbar} toolbar This toolbar
	 */
	...
	constructor: function (config)
	{
		...
		Grommunio.mail.dialogs.MailCreateToolbar.superclass.constructor.call(this, config);
	},

	/**
	 * Called automatically when the {@link Grommunio.mail.dialogs.MailCreateContentPanel content panel}
	 * is being rendered. This will add a listener to the {@link Grommunio.mail.dialogs.MailCreateContentPanel#bcctoggle} button.
	 * @private
	 */
	onRender: function ()
	{
		Grommunio.mail.dialogs.MailCreateToolbar.superclass.onRender.apply(this, arguments);
	...
	createActionButtons: function ()
	{
		return [{
			...
			overflowText: _('Send'),
			ref: 'sendButton',
			tooltip: _('Send') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Enter', true),
			menu: {
				defaults: {
			...
			handler: function() {
					Grommunio.mail.Actions.openRecipientSelectionContent(this.record, {
	...
	createOptionButtons: function ()
	{
		return [{
			...
			importance: Grommunio.core.mapi.Importance.URGENT,
			enableToggle: true,
			...
			ref: 'lowPriority',
			toggleGroup: 'priorityGroup',
			importance: Grommunio.core.mapi.Importance.NONURGENT,
			enableToggle: true,
			toggleHandler: this.onPriorityGroupToggle,
			scope: this
		},{
			...
			scope: this
		}].concat(Grommunio.common.ui.SecurityButtons.createButtons());
	},
	...
	createRightAlignedOptionButtons: function()
	{
		...
		if (Grommunio.supportsPopOut() && Grommunio.core.BrowserWindowMgr.isMainWindowActive()) {
			return [{
	 ...
	 * @param {Grommunio.settings.SettingsModel} settingsModel The settings which contains signature information
	...
	onSettingsChange: function (settingsModel, changedSettings)
	{
		...
		if (!Ext.isEmpty(changedSettings)) {
			...
			for (var i = 0; i < changedSettings.length; i++) {
				if (changedSettings[i].path && changedSettings[i].path.indexOf('grommunio/v1/contexts/mail/signatures/all') >= 0) {
		...
		var signatures = settingsModel.get('grommunio/v1/contexts/mail/signatures/all', true);
	 ...
	 * This will call the {@link Grommunio.mail.Actions#redirectToSignatureWidget}.
	 */
	onClickAddSignatureHandler: function()
	{
		Grommunio.mail.Actions.redirectToSignatureWidget();
	},
	 ...
	 * This will call the {@link Grommunio.mail.Actions#openDelayedDeliveryContent}.
	 ...
	 * This will {@link Grommunio.core.ui.RecordContentPanel#saveRecord save} the given record.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	 ...
	 * This will {@link Grommunio.core.ui.RecordContentPanel#deleteRecord delete} the given record.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	 ...
	 * This will call the {@link Grommunio.mail.Actions#openMailOptionsContent}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	...
	{
		Grommunio.mail.Actions.openMailOptionsContent(this.record, {
			autoSave: false
		});
	},
	 ...
	 * This will call the {@link Grommunio.common.Actions#openFlagsMenu}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	...
	{
		Grommunio.common.Actions.openFlagsMenu(this.record, eventObject.getXY(), false);
	},
	...
	onPriorityGroupToggle: function (button,pressed)
	{
		if (pressed) {
			...
			this.record.set('importance', Grommunio.core.mapi.Importance.NORMAL);
		}
	},
	 ...
	 * This will update UI of the {@link Grommunio.mail.dialogs.MailCreateContentPanel}
		...
		if (contentReset === true || record.isModifiedSinceLastUpdate('importance')) {
			switch (record.get('importance')) {
				...
				default:
					...
					// from Grommunio.mail.dialogs.MailViewOptionsContentPanel dialog.
	 ...
	 * being uploaded.
	 *
	 * @param {Grommunio.core.data.IPMAttachmentStore} attachmentStore The store which fired the event
	...
	onAttachmentChange: function(attachmentStore, record)
	{
		...
		if (isAllAttachUploaded) {
			this.saveBtn.setTooltip(_('Save') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + S', true));
		} else {
	 ...
	 * This will call the {@link Grommunio.mail.Actions#openMailContent}
	 * with record and its containing {@link Grommunio.core.ui.MessageContentPanel dialog}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	...
	{
		Grommunio.mail.Actions.popoutMailContent(this.record, this.dialog);
	}
});
Ext.reg('grommunio.mailcreatetoolbar', Grommunio.mail.dialogs.MailCreateToolbar);
