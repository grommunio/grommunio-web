...
/**
 * @class Grommunio.mail.dialogs.ShowMailToolbar
 * @extends Grommunio.core.ui.ContentPanelToolbar
 * @xtype mail.showmailtoolbar
 */
Grommunio.mail.dialogs.ShowMailToolbar = Ext.extend(Grommunio.core.ui.ContentPanelToolbar, {
	...
	constructor: function(config)
	{
		...
		Grommunio.mail.dialogs.ShowMailToolbar.superclass.constructor.call(this, config);
	},
	...
	createActionButtons: function(insertionPointBase)
	{
		...
		return preActionButtonsInsert.concat([{
			...
			iconCls: 'icon_reply',
			ref: 'replyBtn',
			actionType: Grommunio.mail.data.ActionTypes.REPLY,
			...
			ref: 'replyAllBtn',
			actionType: Grommunio.mail.data.ActionTypes.REPLYALL,
			handler: this.onMailResponseButton,
			scope: this
		},{
			xtype: 'button',
			...
			ref: 'forwardBtn',
			actionType: Grommunio.mail.data.ActionTypes.FORWARD,
			handler: this.onMailResponseButton,
			scope: this
		},{
			xtype: 'button',
	...
	onBeforeShowReplyButton: function(btn)
	{
		...
		if ( messageClass.substring(0, 20) === 'IPM.Schedule.Meeting' ){
			btn.getEl().removeClass('grommunio-action');
		} else {
			btn.getEl().addClass('grommunio-action');
		}
	},
	...
	createOptionButtons: function()
	{
		return [{
			...
			xtype: 'splitbutton',
			cls: 'grommunio-more-options-btn',
	 ...
	 *
	 * @param {Grommunio.mail.dialogs.ShowMailToolbar} scope The scope for the menu items
	 * @return {Grommunio.core.ui.menu.ConditionalMenu} the dropdown menu for the more button
	 */
	moreMenuButtons: function(scope)
	{
		return {
			...
			items: [{
				...
				actionType: Grommunio.mail.data.ActionTypes.EDIT_AS_NEW,
				handler: this.onMailResponseButton,
				scope: scope
			}, {
				xtype: 'grommunio.conditionalitem',
	...
	/**
	 * Handler for the beforeshow event of the {@link Grommunio.core.ui.menu.ConditionalItem menu item}. Will
	...
	{
		Grommunio.common.Actions.markAsRead(this.record, item.readState);
	},
	 ...
	 * This will call {@link Grommunio.common.Actions#openCategoriesContent}.
	 * @private
	 */
	onOpenCategories: function()
	{
		Grommunio.common.Actions.openCategoriesContent(this.record);
	},

	/**
	 * Open the {@link Grommunio.common.dialogs.CopyMoveContent CopyMoveContent} for copying
	 * or moving the given record.
	 * @private
	 */
	onCopyMove: function()
	{
		Grommunio.common.Actions.openCopyMoveContent(this.record);
	},

	/**
	 * Event handler when the "Print" button has been pressed.
	 * This will call the {@link Grommunio.common.Actions#openPrintDialog}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	 ...
	 * This will call the {@link Grommunio.mail.Actions#openCreateMailResponseContent}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	...
	{
		if (button.actionType === Grommunio.mail.data.ActionTypes.FORWARD) {
			var messageClass = this.record.get('message_class') || '';
		...
		var isOwnedByMainWindow = Grommunio.core.BrowserWindowMgr.isOwnedByMainWindow(button);
		var configObject = !isOwnedByMainWindow ? {layerType: 'separateWindows'} : undefined;

		Grommunio.mail.Actions.openCreateMailResponseContent(this.record, model, button.actionType, configObject);
		if (container.getSettingsModel().get('grommunio/v1/contexts/mail/close_on_respond') === true) {
			this.dialog.close();
		}
	},
	 ...
	 * This will call the {@link Grommunio.common.Actions#openSaveEmlDialog} function.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	...
	{
		Grommunio.common.Actions.openSaveEmlDialog(this.record);
	},

	/**
	 * Handler for the beforeshow event of the {@link Grommunio.core.ui.menu.ConditionalItem menuitem}. Will
...
Ext.reg('grommunio.showmailtoolbar', Grommunio.mail.dialogs.ShowMailToolbar);
