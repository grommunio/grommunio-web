...
/**
 * @class Grommunio.mail.URLAction
 * @extends Object
 *
 * The URL Actions for Mail Context. Currently only mailto action is handled.
 * @singleton
 */
Grommunio.mail.URLAction = Ext.extend(Object, {
	/**
	 * @constructor
	 */
	constructor: function()
	{
		...
		Grommunio.core.URLActionMgr.register('mailto', handler);
	},

	/**
	 * Function reads data passed as href from {@link Grommunio.core.URLActionMgr URLActionMgr} and
	...
	handleMailTo: function(action, href, handler)
	{
		...
		urlData = Grommunio.core.Util.objectKeysToLowerCase(urlData);
		...
		var recipientTypes = {
			to: Grommunio.core.mapi.RecipientType.MAPI_TO,
			cc: Grommunio.core.mapi.RecipientType.MAPI_CC,
			bcc: Grommunio.core.mapi.RecipientType.MAPI_BCC
		};
		...
		for (var key in recipientTypes) {
			if(!Ext.isEmpty(urlData[key])){
				...
				{
					recipientRecords.push(Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, {
		...
		Grommunio.common.Actions.openMessageContent(record);
	}
});

Grommunio.mail.URLAction = new Grommunio.mail.URLAction();
