Ext.namespace('Grommunio.mail.widget');

/**
 * @class Grommunio.mail.widget.MailWidget
 * @extends Grommunio.core.ui.widget.AbstractFolderWidget
 *
 * Widget that shows the unread mail.
 */
Grommunio.mail.widget.MailWidget = Ext.extend(Grommunio.core.ui.widget.AbstractFolderWidget, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		var store = new Grommunio.mail.MailStore();
		...
		store.setRestriction({
			'search': Grommunio.core.data.RestrictionFactory.dataResBitmask(
		...
		Grommunio.mail.widget.MailWidget.superclass.constructor.call(this, config);
	},

	/**
	 * Update the filter. This will filter the records using
	 * {@link Grommunio.core.data.IPMRecord#isRead} and {@link Grommunio.core.data.IPMRecord#isConversationHeaderRecord}.
	 * @private
	 */
	...
	viewConfigGetRowClass: function(record, rowIndex, rowParams, store)
	{
		...
		meta = {};
		value = Grommunio.common.ui.grid.Renderers.attachment(record.get('hasattach'), meta, record);
		...
		meta = {};
		value = Grommunio.common.ui.grid.Renderers.importance(record.get('importance'), meta, record);
		rowParams.body += String.format('<td style="width: 24px"><div class="grid_compact {0}" style="height: 24px; width: 24px;">{1}</div></td>', meta.css, value);
	...
	onRowContextMenu: function(grid, rowIndex, event)
	{
		...
		Grommunio.core.data.UIFactory.openDefaultContextMenu(selectionModel.getSelections(), { position: event.getXY(), model: model });
	},
...
Grommunio.onReady(function() {
	container.registerWidget(new Grommunio.core.ui.widget.WidgetMetaData({
		...
		widgetConstructor: Grommunio.mail.widget.MailWidget
	}));
});
