...
/*
 * #dependsFile client/grommunio/common/ui/htmleditor/Fonts.js
 */
Ext.namespace('Grommunio.mail.settings');

/**
 * @class Grommunio.mail.settings.SettingsSignaturesWidget
 ...
 * the Signatures settings in the {@link Grommunio.mail.settings.SettingsMailCategory mail category}.
 */
Grommunio.mail.settings.SettingsSignaturesWidget = Ext.extend(Grommunio.settings.ui.SettingsWidget, {
	/**
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			items: [{
				...
				items: [{
					xtype: 'grid',
					name: 'grommunio/v1/contexts/mail/signatures/all',
					ref: '../signaturesGrid',
					cls:'grommunio-settings-signatures',
					columnWidth: 0.5,
					height: 200,
					store: store,
					hideHeaders: true,
					...
					items: [{
						xtype: 'combo',
						ref: '../../newMessageCombo',
						name: 'grommunio/v1/contexts/mail/signatures/new_message',
						...
						name: 'grommunio/v1/contexts/mail/signatures/replyforward_message',
		...
		Grommunio.mail.settings.SettingsSignaturesWidget.superclass.constructor.call(this, config);
	},
	 ...
	 * If {@link Grommunio.mail.settings.SettingsMailCategory SettingsMailCategory} has
	...
	update: function(settingsModel)
	{
		...
		var useHtml = settingsModel.get('grommunio/v1/contexts/mail/dialogs/mailcreate/use_html_editor');
	 ...
	 * {@link Grommunio.settings.SettingsModel#set set} event. This will
	 * check if the 'use_html_editor' setting was changed by the user,
	 * and will {@link Grommunio.common.ui.EditorField#setHtmlEditor toggle} the
	 * {@link Grommunio.common.ui.EditorField EditorField} accordingly.
	 * @param {Grommunio.settings.SettingsModel} settingsModel The model which fired the event
	...
	onSettingsSet: function(settingsModel, settings)
	{
		...
		for (var i = 0, len = settings.length; i < len; i++) {
			...
			if (setting.path === 'grommunio/v1/contexts/mail/dialogs/mailcreate/use_html_editor') {
	 ...
	 * {@link Grommunio.common.ui.EditorField Content Field} to ensure the user can directly
	 * start typing
	 *
	 ...
	 * after it has been set into the {@link Grommunio.common.ui.HtmlEditor}.
...
Ext.reg('grommunio.settingssignatureswidget', Grommunio.mail.settings.SettingsSignaturesWidget);
