...
/**
 * @class Grommunio.mail.settings.SettingsOofWidget
 ...
 * the Out of Office settings in the {@link Grommunio.mail.settings.SettingsMailCategory mail category}.
 */
Grommunio.mail.settings.SettingsOofWidget = Ext.extend(Grommunio.settings.ui.SettingsWidget, {
	/**
	 * @cfg {Grommunio.common.outofoffice.data.OofStore} store store to use for loading out of office settings
	 */
	store: undefined,

	/**
	 * @cfg {Grommunio.common.outofoffice.data.OofRecord} record selected record to populate data into view by {@link #updateView}
	 * and save data.
	 */
	record: undefined,
	...
	constructor: function(config)
	{
		...
		if(Ext.isEmpty(config.store)) {
			config.store = new Grommunio.common.outofoffice.data.OofStore();
		}
		...
		Grommunio.mail.settings.SettingsOofWidget.superclass.constructor.call(this, config);
	...
	/**
	 * initialize events for the {@link Grommunio.mail.settings.SettingsOofWidget SettingsOofWidget}.
	 * @private
	 */
	initEvents: function()
	{
		Grommunio.mail.settings.SettingsOofWidget.superclass.initEvents.call(this);
	...
	/**
	 * @return {Grommunio.common.outofoffice.data.OofStore} The store which holds record of users' out of office settings.
	 */
	...
	getNewConfig: function()
	{
		...
		backDate = backDate.clearTime().add(Date.MINUTE, container.getSettingsModel().get('grommunio/v1/main/start_working_hour'));
		...
		items.push({
			...
			defaults:{
				xtype: 'container',
				layout: 'form',
				cellCls: 'grommunio-settings-oof-table-cellpadding'
			},
			items: [{
				items:[{
					xtype: 'radio',
				...
				items:[{
					...
					dateFieldConfig: {
						showNow: true,
						invalidClass: 'grommunio-settings-oof-invalid'
					},
			...
			xtype: 'displayfield',
			anchor: '100%',
			cls: 'grommunio-settings-oof-autoreplay',
			hideLabel: true,
			...
			items: [{
				...
				{
					xtype: 'container',
					cls: 'grommunio-settings-oof-subject',
					anchor: '100%',
					layout: 'hbox',
					items: [{
						xtype: 'displayfield',
	...
	createComboboxStore: function()
	{
		...
		hierarchyStore.getStores().forEach(function(store) {
			...
			var hasSufficientPermission = (subtree.get('rights') & Grommunio.core.mapi.Rights.RIGHTS_SECRETARY) === Grommunio.core.mapi.Rights.RIGHTS_SECRETARY;
			...
			if (storeEntryId != myStoreEntryId && hasSufficientPermission) {
				...
				processedStores.push(Grommunio.core.Util.bin2hex(store.get('user_name')));
			}
		});

		var sharedStores = container.getSettingsModel().get('grommunio/v1/contexts/hierarchy/shared_stores', true);
	...
	{
		checkbox.wrap.addClass('grommunio-settings-oof-table-cellalign');
	},
	 ...
	 * {@link Grommunio.settings.SettingsModel}.
	 * @param {Grommunio.settings.SettingsModel} settingsModel latest settings
	 */
	...
	/**
	 * Called by the {@link #update} or when first time load of {@link Grommunio.mail.settings.SettingsOofWidget}
	 ...
	 * This will set current time to the respective sibling {@link Grommunio.common.ui.DateTimeField#timeField time field}.
	 ...
	 * @param {Grommunio.settings.SettingsContextModel} model the settings model
	 ...
	 * by the {@link Grommunio.common.outofoffice.data.OofStore} inside the {@link Grommunio.mail.settings.SettingsOofWidget}.
	...
	/**
	 * Function will be used to reload data in the {@link Grommunio.common.outofoffice.data.OofStore OofStore}.
	 */
...
Ext.reg('grommunio.settingsoofwidget', Grommunio.mail.settings.SettingsOofWidget);
