...
/**
 * @class Grommunio.mail.KeyMapping
 * @extends Object
 *
 * The map of keys used in the Mail Context.
 * @singleton
 */
Grommunio.mail.KeyMapping = Ext.extend(Object, {
	/**
	 * @constructor
	 */
	constructor: function()
	{
		...
		Grommunio.core.KeyMapMgr.register('global', newMailKeys);

		// These keys might need to be in mail-global handler
		Grommunio.core.KeyMapMgr.register('grid.mapimessage.mail', mailGridKeys);

		// Reply/Reply All/Forward/Edit as New Key handlers
		Grommunio.core.KeyMapMgr.register('grid.mapimessage.mail', itemActionKeys);
		Grommunio.core.KeyMapMgr.register('previewpanel.mail', itemActionKeys);
		Grommunio.core.KeyMapMgr.register('contentpanel.record.message.showmail', itemActionKeys);
	},
	...
	{
		Grommunio.mail.Actions.openCreateMailContent(container.getContextByName('mail').getModel());
	},
	 ...
	 * @param {Ext.Component} component The component on which key event is fired.
	 * @return {Grommunio.core.ContextModel} The model of component..
	 * @private
	 */
	...
	onReplyMail: function(key, event, component)
	{
		...
		if(record){
			Grommunio.mail.Actions.openCreateMailResponseContent(record, model, Grommunio.mail.data.ActionTypes.REPLY);
		}
	},
	...
	onReplyAllMail: function(key, event, component)
	{
		...
		var model = this.getModel(component);
		if(record){
			Grommunio.mail.Actions.openCreateMailResponseContent(record, model, Grommunio.mail.data.ActionTypes.REPLYALL);
		}
	},
	...
	onForwardMail: function(key, event, component)
	{
		...
		var model = this.getModel(component);
		if(record){
			Grommunio.mail.Actions.openCreateMailResponseContent(record, model, Grommunio.mail.data.ActionTypes.FORWARD);
		}
	},
	...
	onEditAsNewMail: function(key, event, component)
	{
		...
		var model = this.getModel(component);
		if(record){
			Grommunio.mail.Actions.openCreateMailResponseContent(record, model, Grommunio.mail.data.ActionTypes.EDIT_AS_NEW);
		}
	},
	...
	onReadUnreadToggle: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			if (records[0].isRead()){
				Grommunio.common.Actions.markAsRead(records, false);
			} else {
				Grommunio.common.Actions.markAsRead(records, true);
			}
		}
	},
	...
	onFlagToggle: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			...
			for(var i = 0, len = records.length; i < len; i++){
				...
				if (currentFlagStatus == Grommunio.core.mapi.FlagStatus.cleared) {
					record.set('flag_status', Grommunio.core.mapi.FlagStatus.flagged);
					record.set('flag_icon', Grommunio.core.mapi.FlagIcon.red);
				} else if (currentFlagStatus == Grommunio.core.mapi.FlagStatus.completed) {
					record.set('flag_status', Grommunio.core.mapi.FlagStatus.flagged);
					record.set('flag_icon', Grommunio.core.mapi.FlagIcon.red);
				} else {
					record.set('flag_status', Grommunio.core.mapi.FlagStatus.completed);
					record.set('flag_icon', Grommunio.core.mapi.FlagIcon.clear);
				}
			}
...
Grommunio.mail.KeyMapping = new Grommunio.mail.KeyMapping();
