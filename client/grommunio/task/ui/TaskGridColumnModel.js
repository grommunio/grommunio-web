...
/**
 * @class Grommunio.task.ui.TaskGridColumnModel
 * @extends Grommunio.common.ui.grid.ColumnModel
 */
Grommunio.task.ui.TaskGridColumnModel = Ext.extend(Grommunio.common.ui.grid.ColumnModel, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.task.ui.TaskGridColumnModel.superclass.constructor.call(this, config);
	},
	...
	createSimpleColumns: function()
	{
		return[{
			...
			width: 24,
			renderer: Grommunio.common.ui.grid.Renderers.reminder,
			fixed: true
		}, {
			xtype: 'checkcolumn',
			dataIndex: 'complete',
			...
			headerCls: 'grommunio-icon-column flag k-unsortable',
			dataIndex: 'flag_due_by',
			width: 24,
			renderer: Grommunio.common.ui.grid.Renderers.flag,
			fixed: true,
			sortable: false
		}];
	},
	...
	columnRenderer: function (value, p, record)
	{
		switch (this.dataIndex) {
			case 'subject':
				value = Grommunio.common.ui.grid.Renderers.subject(value, p, record);
				break;
			case 'display_to':
				value = Grommunio.common.ui.grid.Renderers.name(value, p, record);
				break;
			case 'startdate':
			case 'duedate':
				value = Grommunio.common.ui.grid.Renderers.utcdate(value, p, record);
				break;
			case 'percent_complete':
				value = Grommunio.common.ui.grid.Renderers.percentage(value, p, record);
				break;
			case 'owner':
				value = Grommunio.common.ui.grid.Renderers.text(value, p, record);
				break;
			case 'reminder_time':
				value = Grommunio.common.ui.grid.Renderers.dateWithTime(value, p);
				break;
		}

		if((record.get('complete') ||
				record.get('flag_status') === Grommunio.core.mapi.FlagStatus.completed) &&
			!Ext.isEmpty(value)) {
	 ...
	 * to add 'grommunio-grid-empty-cell' css class to grid cell. This will hide strikethrough line that will appear
	...
	completeColumnRenderer: function(value, p, record)
	{
		...
		if (!Ext.isDefined(value) && record.get('flag_status') === Grommunio.core.mapi.FlagStatus.completed ) {
			value = true;
		}
	...
	onCompleteColumnProcessEvent: function(name, e, grid, rowIndex, colIndex)
	{
		...
		if ( !(record instanceof Grommunio.task.TaskRecord) ) {
			Grommunio.common.flags.Util.updateCategories(record);

			// we need different set of flag properties as this is
			// the mail record we are dealing with.
			var mailFlagProps = {
				flag_icon: Grommunio.core.mapi.FlagIcon.clear,
				flag_complete_time: new Date(),
				flag_request: '',
				flag_status: Grommunio.core.mapi.FlagStatus.completed,
				reminder: false
			};
		...
		if (!record.isNormalTask()) {
			if (!record.isTaskOwner() && !record.isTaskRequest()) {
				...
				record.respondToTaskRequest(Grommunio.core.mapi.TaskMode.UPDATE);
			}
		} else {
			// save changes
			record.save();
	...
	/**
	 * This will switch the {@link Grommunio.task.ui.TaskGridColumnModel columnmodel}
	...
	setConfig: function(config, initial)
	{
		...
		Grommunio.task.ui.TaskGridColumnModel.superclass.setConfig.call(this, config, initial);
	}
});
