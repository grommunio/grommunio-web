...
/**
 * @class Grommunio.task.ui.TaskFlagsMenu
 * @extends Grommunio.common.flags.ui.FlagsMenu
 * @xtype grommunio.taskflagsmenu
 *
 * The TaskFlagsMenu is the menu that is shown for flags.
 */
Grommunio.task.ui.TaskFlagsMenu = Ext.extend(Grommunio.common.flags.ui.FlagsMenu, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.task.ui.TaskFlagsMenu.superclass.constructor.call(this, config);
	},
	/**
	...
	setFlag: function(menuItem)
	{
		...
		Grommunio.task.ui.TaskFlagsMenu.superclass.setFlag.apply(this, arguments);
	},

	/**
	 * Set necessary flag related properties into given record(s).
	 *
	 * @param {Grommunio.core.data.IPMRecord} records The record(s) for which configured flag needs to be set.
	...
	setFlagProperties: function (records, flagProperties)
	{
		records.forEach(function(record){
			...
			if (this.action === 'complete') {
				...
				record.set('status', Grommunio.core.mapi.TaskStatus.COMPLETE);
			} else if (record.get('complete')) {
				record.set('complete', false);
				record.set('percent_complete', 0);
				record.set('status', Grommunio.core.mapi.TaskStatus.NOT_STARTED);
			}
...
Ext.reg('grommunio.taskflagsmenu', Grommunio.task.ui.TaskFlagsMenu);
