...
/**
 * @class Grommunio.task.TaskRecordFields
 ...
 */
Grommunio.task.TaskRecordFields = [
	...
	{name: 'taskstate', type: 'int', defaultValue: Grommunio.core.mapi.TaskState.NORMAL},
	{name: 'taskmode', type: 'int', defaultValue: Grommunio.core.mapi.TaskMode.NOTHING},
	...
	{name: 'owner'},
	{name: 'status', type: 'int', defaultValue: Grommunio.core.mapi.TaskStatus.NOT_STARTED},
	...
	{name: 'ownership', type: 'int', defaultValue:Grommunio.core.mapi.TaskOwnership.NEWTASK},
	{name: 'conversation_topic'},
	{name: 'task_assigner'},
	{name: 'task_assigned_time', type: 'date', dateFormat: 'timestamp', defaultValue: null},
	{name: 'taskhistory', type: 'int', defaultValue:Grommunio.core.mapi.TaskHistory.NONE},
	{name: 'task_goid'},
	{name: 'tasklastuser'},
	{name: 'flag_status'},
	{name: 'flag_icon'},
	...
	{name: 'task_acceptance_state', type: 'int', defaultValue:Grommunio.core.mapi.TaskAcceptanceState.NOT_DELEGATED},
	{name: 'updatecount', type: 'int'},
	{name: 'task_not_found', type: 'boolean'}
];

Grommunio.core.data.RecordFactory.addFieldToMessageClass('IPM.Task', Grommunio.task.TaskRecordFields);
Grommunio.core.data.RecordFactory.setSubStoreToMessageClass('IPM.Task', 'reply-to', Grommunio.core.data.IPMRecipientStore);
Grommunio.core.data.RecordFactory.addListenerToMessageClass('IPM.Task', 'createphantom', function(record, data) {
	record.beginEdit();
	...
	if (!data || !Ext.isDefined(data.reminder)) {
		...
		if (!store || !store.isPublicStore()) {
			reminder = container.getSettingsModel().get('grommunio/v1/contexts/task/default_reminder');
		}
		record.set('reminder', reminder);
	}

	if (record.get('reminder')) {
		var time = new Date().clearTime().add(Date.MINUTE, container.getSettingsModel().get('grommunio/v1/contexts/task/default_reminder_time'));
...
/**
 * @class Grommunio.task.TaskRecord
 * @extends Grommunio.core.data.MessageRecord
 *
 * An extension to the {@link Grommunio.core.data.MessageRecord MessageRecord} specific to Task Request/Response Messages.
 */
Grommunio.task.TaskRecord = Ext.extend(Grommunio.core.data.MessageRecord, {
	/**
	 * @return {Boolean} true if the {@link Grommunio.core.data.TaskRecord TaskRecord} is task request or
	 * task is not normal task else false.
	 */
	isTaskRequest: function()
	{
		return (this.get('taskmode') !== Grommunio.core.mapi.TaskMode.NOTHING) || this.isMessageClass('IPM.TaskRequest');
	},
	...
	{
		return this.get('taskstate') === Grommunio.core.mapi.TaskState.ACCEPT ||
			this.get('taskstate') === Grommunio.core.mapi.TaskState.DECLINE;
	},
	...
	{
		return this.get('ownership') === Grommunio.core.mapi.TaskOwnership.DELEGATEDTASK;
	},
	...
	{
		return this.get('ownership') === Grommunio.core.mapi.TaskOwnership.OWNTASK;
	},
	...
	{
		return this.get('taskstate') === Grommunio.core.mapi.TaskState.OWNER;
	},
	...
	{
		return this.get('taskhistory') === Grommunio.core.mapi.TaskHistory.ASSIGNED;
	},
	...
	{
		return this.get('taskhistory') === Grommunio.core.mapi.TaskHistory.ACCEPTED;
	},
	...
	{
		return this.get('taskhistory') === Grommunio.core.mapi.TaskHistory.UPDATED;
	},
	...
	isNormalTask: function ()
	{
		...
		return (taskState === Grommunio.core.mapi.TaskState.NORMAL ||
		taskState === Grommunio.core.mapi.TaskState.OWNER_NEW);
	},
	...
	{
		return (this.get('taskstate') === Grommunio.core.mapi.TaskState.DECLINE &&
			this.get('taskhistory') === Grommunio.core.mapi.TaskHistory.DECLINED);
	},
	...
	{
		return (this.get('taskstate') === Grommunio.core.mapi.TaskState.OWNER_NEW &&
	...
	/**
	 * Respond to a task request with the correct {@link Grommunio.core.mapi.TaskMode}.
	 *
	 * @param {Grommunio.core.mapi.ResponseStatus} responseType accept/decline
	 */
		...
		if (Ext.isDefined(responseType)) {
			switch(responseType)
			{
				case Grommunio.core.mapi.TaskMode.ACCEPT:
					...
					this.set('taskhistory', Grommunio.core.mapi.TaskHistory.ACCEPTED, true);
					break;
				case Grommunio.core.mapi.TaskMode.DECLINE:
					this.addMessageAction('action_type', 'declineTaskRequest');
					this.set('taskhistory', Grommunio.core.mapi.TaskHistory.DECLINED, true);
					break;
			}
	...
	{
		this.beginEdit();
		this.set('taskstate', Grommunio.core.mapi.TaskState.OWNER_NEW);
		this.set('taskmode', Grommunio.core.mapi.TaskMode.REQUEST);
		this.endEdit();
	}
});
Grommunio.core.data.RecordFactory.setBaseClassToMessageClass('IPM.Task', Grommunio.task.TaskRecord);
