...
/**
 * @class Grommunio.task.dialogs.TaskEditContentPanel
 * @extends Grommunio.core.ui.MessageContentPanel
 * @xtype grommunio.taskeditcontentpanel
 *
 * this class will be used to create a task edit content panel
 */
Grommunio.task.dialogs.TaskEditContentPanel = Ext.extend(Grommunio.core.ui.MessageContentPanel, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.task.dialogs.TaskEditContentPanel.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.task.dialogs.TaskEditContentPanel.superclass.createSendValidationQueue.apply(this, arguments);
	...
	validateSenderIsNotRecipient: function (callback)
	{
		...
		Ext.each(recipients, function (recipient) {
			if (Grommunio.core.EntryId.compareEntryIds(container.getUser().getEntryId(), recipient.get('entryid')) && !isSenderIsRecipient) {
				isSenderIsRecipient = true;
			}

			if (recipient.get('recipient_type') === Grommunio.core.mapi.RecipientType.MAPI_TO) {
				recipientCount++;
			}

			if (recipient.get('object_type') === Grommunio.core.mapi.ObjectType.MAPI_DISTLIST) {
				isDistlist = true;
				return false;
			}
		},this);
	...
	sendRecord: function()
	{
		...
		return Grommunio.task.dialogs.TaskEditContentPanel.superclass.sendRecord.apply(this, arguments);
	}
});

// register dialog
Ext.reg('grommunio.taskeditcontentpanel', Grommunio.task.dialogs.TaskEditContentPanel);
