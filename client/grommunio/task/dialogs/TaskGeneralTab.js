...
/**
 * @class Grommunio.task.dialogs.TaskGeneralTab
 * @extends Ext.form.FormPanel
 * @xtype grommunio.taskgeneraltab
 *
 * Main tab in the {@link Grommunio.task.dialogs.TaskPanel}
 * that is used to create Tasks and also Assign Task
 */
Grommunio.task.dialogs.TaskGeneralTab = Ext.extend(Ext.form.FormPanel, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.task.dialogs.TaskGeneralTab.superclass.constructor.call(this, config);
	},
	 ...
	 * which contains by the {@link Grommunio.task.TaskRecord record}.
	 *
	 * @return {Object} Configuration object for the panel containing the fields
	 * @private
	 */
	...
	createStatusPanel: function()
	{
		var statusStore = {
			xtype: 'jsonstore',
			fields: ['name', 'value'],
			data: Grommunio.task.data.TaskStatus.status
		};
	...
	createPriorityCompletePanel: function()
	{
		...
		return {
			...
			items: [{
				xtype: 'combo',
				plugins: [ 'grommunio.fieldlabeler' ],
				...
				plugins: [ 'grommunio.fieldlabeler', 'grommunio.percentspinner' ],
	...
	createTaskRequestSettingPanel: function()
	{
		return{
			...
			items: [{
				...
				plugins: 'grommunio.formfieldtooltipplugin',
				name: 'taskupdates',
				width: 200,
				listeners:{
					'change': this.onPropertyChange,
	...
	updateUI: function(record, contentReset)
	{
		...
		this.taskExtraInfo.setVisible(taskHistory !== Grommunio.core.mapi.TaskHistory.NONE);

		if ((taskHistory === Grommunio.core.mapi.TaskHistory.ASSIGNED ||
			taskHistory === Grommunio.core.mapi.TaskHistory.DECLINED ||
			taskMode === Grommunio.core.mapi.TaskMode.DECLINE ||
			...
			this.editorField.setReadOnly(taskMode !== Grommunio.core.mapi.TaskMode.DECLINE);
			...
			if (contentReset && taskState === Grommunio.core.mapi.TaskState.OWNER_NEW && !record.isTaskRequest()) {
	...
	onStatusSelect: function(field, record, number)
	{
		...
		var complete = newValue === Grommunio.core.mapi.TaskStatus.COMPLETE;
		if(complete){
			this.record.set('complete', true);
			this.record.set('percent_complete', 1);
			this.record.set('date_completed', new Date());
		} else if (newValue === Grommunio.core.mapi.TaskStatus.NOT_STARTED) {
	...
	onCompleteSpin: function(field)
	{
		...
		if(newValue >= 0 && newValue < 0.1){
			this.record.set('status', Grommunio.core.mapi.TaskStatus.NOT_STARTED);
			this.record.set('complete', false);
			this.record.set('percent_complete', newValue);
			this.record.set('date_completed', null);
		} else if(newValue >= 0.1 && newValue < 1){
			this.record.set('status', Grommunio.core.mapi.TaskStatus.IN_PROGRESS);
			this.record.set('complete', false);
			this.record.set('percent_complete', newValue);
			this.record.set('date_completed', null);
	...
	{
		this.record.set('flag_icon', complete ? Grommunio.core.mapi.FlagIcon.clear : Grommunio.core.mapi.FlagIcon.red);
		this.record.set('flag_complete_time', complete ? new Date() : null);
		this.record.set('flag_request', complete ? '' : 'Follow up');
		this.record.set('flag_status', complete ? Grommunio.core.mapi.FlagStatus.completed : Grommunio.core.mapi.FlagStatus.flagged);
	},
...
Ext.reg('grommunio.taskgeneraltab', Grommunio.task.dialogs.TaskGeneralTab);
