...
/**
 * @class Grommunio.task.dialogs.TaskDetailTab
 * @extends Ext.form.FormPanel
 * @xtype grommunio.taskdetailtab
 *
 * details tab in the {@link Grommunio.task.dialogs.TaskEditPanel}
 * that is used to create Tasks.
 */
Grommunio.task.dialogs.TaskDetailTab = Ext.extend(Ext.form.FormPanel, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.task.dialogs.TaskDetailTab.superclass.constructor.call(this, config);
	...
	{
		var isTaskAssignerCopy = (record.get('taskhistory') === Grommunio.core.mapi.TaskHistory.DECLINED || record.isTaskAssigned() || record.isTaskOrganized());
		...
		if (contentReset === true || record.isModified('taskstate')) {
			switch (record.get('taskstate')) {
				case Grommunio.core.mapi.TaskState.ACCEPT:
	...
	onCreateUnassignedCopy: function (button)
	{
		...
		record.set('task_acceptance_state', Grommunio.core.mapi.TaskAcceptanceState.NOT_DELEGATED);
	 ...
	 * as {@link Grommunio.core.mapi.TaskStatus#COMPLETE complete} or
	 * {@link Grommunio.core.mapi.TaskStatus#NOT_STARTED not started}.
...
Ext.reg('grommunio.taskdetailtab', Grommunio.task.dialogs.TaskDetailTab);
