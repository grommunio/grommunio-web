...
/**
 * @class Grommunio.task.dialogs.TaskToolbar
 * @extends Grommunio.core.ui.ContentPanelToolbar
 * @xtype grommunio.tasktoolbar
 */
Grommunio.task.dialogs.TaskToolbar = Ext.extend(Grommunio.core.ui.ContentPanelToolbar, {
	 ...
	 * @param {Grommunio.task.dialogs.TaskToolbar} toolbar This toolbar
	 */
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.task.dialogs.TaskToolbar.superclass.constructor.call(this, config);
	},
	...
	createActionButtons: function()
	{
		return [{
			...
			text: _('Send'),
			overflowText: _('Send'),
			tooltip: _('Send task to assignees') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Enter', true),
			...
			xtype: 'button',
			overflowText: _('Save'),
			tooltip: _('Save') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + S', true),
			iconCls: 'icon_floppy',
			ref: 'saveTaskRequest',
			handler: this.onSave,
			scope: this
			...
			tooltip: _('Save')+' & '+_('Close') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + S', true),
			cls: 'grommunio-action',
	...
	/**
	 * Event listener for the added event of the {@link Grommunio.common.attachment.ui.AttachmentButton attachmentButton}
	 ...
	 *
	 * @param {Ext.Component} item The item that was added. This can be a {@link Grommunio.common.attachment.ui.AttachmentButton}
	...
	onAttachmentButtonAdded: function(item)
	{
		if ( item.isXType('menuitem') ){
			...
			item.update = Grommunio.common.attachment.ui.AttachmentButton.prototype.update.createDelegate(this.attachmentButton);
		}
	},
	 ...
	 * This will {@link Grommunio.core.data.RecordContentPanel#saveRecord save} the given record.
	 ...
	 * This will {@link Grommunio.core.data.IPMRecipient#resolve resolve} all recipients.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	 ...
	 * This will open the {@link Grommunio.common.recurrence.dialogs.RecurrenceContentPanel RecurrenceContentPanel}.
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	onRecurrence: function(button)
	{
		Grommunio.common.Actions.openRecurrenceContent(this.record, false);
	},

	/**
	 * Event handler when the "Set Flag" button has been pressed.
	 * This will call the {@link Grommunio.task.Actions#openFlagsMenu}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	...
	onSetFlagButton: function (button, eventObject)
	{
		...
		Grommunio.task.Actions.openFlagsMenu(this.record, options);
	},
	...
	onAssignment: function(button)
	{
		...
		this.record.set('taskstate', Grommunio.core.mapi.TaskState.OWNER_NEW);
		this.record.set('taskmode', Grommunio.core.mapi.TaskMode.REQUEST);
		this.record.endEdit();
	},
	...
	{
		this.record.beginEdit();
		this.record.set('taskstate', Grommunio.core.mapi.TaskState.NORMAL);
		this.record.set('taskmode', Grommunio.core.mapi.TaskMode.NOTHING);
		this.record.endEdit();
	},
	...
	onComplete: function(button)
	{
		...
		this.record.set('status', Grommunio.core.mapi.TaskStatus.COMPLETE);
		this.record.set('date_completed', new Date());
		this.record.set('flag_icon', Grommunio.core.mapi.FlagIcon.red);
		this.record.set('flag_complete_time', new Date());
		this.record.set('flag_request', '');
		this.record.set('flag_status', Grommunio.core.mapi.FlagStatus.completed);
		this.record.endEdit();
	 ...
	 * this will convert the assigned task to {@link Grommunio.core.mapi.TaskState#OWNER_NEW Normal task}.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	onRestoreToTaskList: function (button)
	{
		...
		record.set('taskstate', Grommunio.core.mapi.TaskState.OWNER_NEW);
		record.set('taskmode', Grommunio.core.mapi.TaskMode.NOTHING);
		record.set('taskhistory', Grommunio.core.mapi.TaskHistory.NONE);
		record.set('ownership', Grommunio.core.mapi.TaskOwnership.NEWTASK);
		record.set('icon_index', Grommunio.core.mapi.IconIndex['task']);
	 ...
	 * This will open the {@link Grommunio.common.categories.dialogs.CategoriesContentPanel CategoriesContentPanel}.
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	onCategories: function(button)
	{
		Grommunio.common.Actions.openCategoriesContent(this.record, {autoSave: false});
	},

	/**
	 * Event handler which is called when the user pressed the 'Delete' button.
	 * This will call {@link Grommunio.core.ui.RecordContentPanel#deleteRecord} to delete the record
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	 ...
	 * This will call {@link Grommunio.common.Actions.openPrintDialog} to print the record
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	onPrint: function()
	{
		Grommunio.common.Actions.openPrintDialog(this.record);
	},
	/**
	...
	onPrivateGroupToggle: function(button)
	{
		...
		if (button.pressed) {
			this.record.set('sensitivity', Grommunio.core.mapi.Sensitivity['PRIVATE']);
		} else {
			this.record.set('sensitivity', Grommunio.core.mapi.Sensitivity['NONE']);
		}
		this.record.endEdit();
	},
	 ...
	 * from the {@link Grommunio.core.data.IPMRecord record}.
	 *
	...
	update: function(record, contentReset)
	{
		...
		if(record.isSubMessage()) {
			...
			// isTaskDecline gets true if task state is Grommunio.core.mapi.TaskState.DECLINE;
			var isTaskDecline = record.get('taskstate') === Grommunio.core.mapi.TaskState.DECLINE;
			...
			if ( this.restoreToTaskList.isVisible() ) {
				this.restoreToTaskList.getEl().addClass('grommunio-action');
			}
			if (isReceivedTask) {
			...
			if (contentReset === true || isModifiedTaskMode) {
				...
				case Grommunio.core.mapi.TaskMode.REQUEST:
					var isTaskRequestInstance = record instanceof Grommunio.task.TaskRequestRecord;
					var isVisible = (!record.isTaskDelegated() && !isTaskRequestInstance) || record.get('taskstate') === Grommunio.core.mapi.TaskState.OWNER_NEW;
					...
					break;
				case Grommunio.core.mapi.TaskMode.NOTHING:
				/* falls through */
				default:
					this.sendBtn.setVisible(false);
					this.saveTaskRequest.setVisible(false);
			...
			if(!contentReset &&
				...
				record.get('taskhistory') === Grommunio.core.mapi.TaskHistory.NONE) {
				this.assignTask.setVisible(record.isNormalTask());
			}
		}
	...
	/**
	 * Send the {@link Grommunio.core.data.IPMRecord record} to the recipient
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
...
Ext.reg('grommunio.tasktoolbar', Grommunio.task.dialogs.TaskToolbar);
