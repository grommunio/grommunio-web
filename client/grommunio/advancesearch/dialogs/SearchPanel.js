...
/**
 * @class Grommunio.advancesearch.dialogs.SearchPanel
 * @extends Ext.Panel
 * @xtype grommunio.searchpanel
 *
 * #dependsFile client/grommunio/advancesearch/KQLParser.js
 */
Grommunio.advancesearch.dialogs.SearchPanel = Ext.extend(Ext.Panel, {
	...
	constructor: function(config)
	{
		...
		Grommunio.advancesearch.dialogs.SearchPanel.superclass.constructor.call(this, config);
	},
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore mapi store in which new folders are added.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord/Grommunio.hierarchy.data.MAPIFolderRecord[]} record folder record(s) which are added in hierarchy.
	 * @private
	 */
	onHierarchyAddFolder: function(store, mapiStore, record)
	{
		...
		if (record.isSearchFolder() && searchStoreUniqueId === searchToolBox.searchTabId) {
			...
			if(folder.get('flag') === Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.ALL_FOLDERS) {
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The store which fired the event
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore mapi store in which new folders was removed.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord/Grommunio.hierarchy.data.MAPIFolderRecord[]} record folder record(s) which was removed in hierarchy.
	 * @private
	 */
	onHierarchyRemoveFolder: function (store, mapiStore, record)
	{
		...
		if (searchStoreRecord && Grommunio.core.EntryId.compareEntryIds(searchStoreRecord.get('entryid'), record.get('entryid'))) {
	...
	/**
	 * Event handler triggered before {@link Grommunio.advancesearch.dialogs.SearchContentPanel searchcontentpanel}
	 * gets close. it will call {@link #onSearchStop}, which stops the search.
	 * @param {Grommunio.advancesearch.dialogs.SearchContentPanel} contentPanel which gets the close
	 */
	onBeforeCloseContentPanel: function(contentPanel)
	{
		...
		/**
		 * Model has hold the selected record in Grommunio.core.ContextModel.selectedRecords config
	 ...
	 * {@link Grommunio.core.ContextModel#searchfinished} event. This will update
	...
	/**
	 * Function is reset the {@link Grommunio.common.searchfield.ui.SearchTextField SearchTextField}
	...
	/**
	 * Event handler which will be called when the {@link #model} fires the
	 * {@link Grommunio.core.ContextModel#searchexception} event. This will
	...
	/**
	 * Function will be used to start actual search on {@link Grommunio.core.data.ListModuleStore ListModuleStore},
	 * and also it will register event on {@link Grommunio.core.data.ListModuleStore ListModuleStore} to get
	 * updated status of search.
	 * @param {Grommunio.common.searchfield.ui.SearchTextField} advanceSearchField the advance search field which
	 * performs the search.
	 * @private
	 */
	onSearchStart: function(advanceSearchField)
	{
		...
		var displayTitle = Grommunio.advancesearch.KQLParser.stripKeywords(searchText)
			.replace(/["']/g, '')
	...
	/**
	 * Function will be used to execute stop search request on {@link Grommunio.core.data.ListModuleStore ListModuleStore},
	...
	/**
	 * Event handler triggers when {@link Grommunio.advancesearch.dialogs.SearchToolBoxPanel search tool box}
	 * layout was rendered also it will start the search.
	 * @param {Grommunio.advancesearch.dialogs.SearchToolBoxPanel} searchToolBox The search tool box panel.
	 */
	...
	/**
	 * Event handler triggers when {@link Grommunio.advancesearch.dialogs.SearchToolBoxPanel#searchCriteria}
	...
	/**
	 * Event handler triggers when {@link Grommunio.common.searchfield.ui.SearchTextField SearchTextField}
	 * gets render and also it will set the search text in search field.
	 *
	 * @param {Grommunio.common.searchfield.ui.SearchTextField} searchTextField the search text field which
	...
	/**
	 * Event handler triggered when value of {@link Grommunio.common.searchfield.ui.SearchTextField Search text field}
	 * was validated.
	 *
	 * @param {Grommunio.common.searchfield.ui.SearchTextField} searchTextField search text field which contains
	 * search text.
	 */
	...
	/**
	 * Event handler triggers when {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	 * is render. it will update the {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	...
	onRenderSearchFolderCombo: function(searchFolderCombo)
	{
		...
		if (searchFolderSettingObj) {
			...
			if (searchComboSettingObj['folder_type'] === Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.ALL_FOLDERS) {
				var folder = store.getAt(store.find('flag', searchComboSettingObj['folder_type']));
				value = folder.get('value');
			} else {
				var index = 0;
				if (searchComboSettingObj['folder_type'] === Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.CURRENT_SELECTED_FOLDER) {
					index = 1;
					store.removeAt(index);
				} else if (store.getAt(0).get('flag') === Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.IMPORTED_FOLDER) {
					store.removeAt(index);
				}
	 ...
	 *
	 * @param {Grommunio.common.searchfield.ui.SearchFolderCombo} combo The combo box which fired the event.
	 ...
	 * {@link Grommunio.advancesearch.dialogs.SearchToolBoxPanel SearchToolBox} was rendered
	 * also it will call when search folder change in {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}.
	 * Also function is responsible to disable "Include sub folder" check box
	 * and apply the tooltip when "All folders" option in {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	...
	onRenderSubFolderCheckbox: function (checkBox)
	{
		var searchFolderCombo = this.searchToolbar.getSearchFolderCombo();
		var record = searchFolderCombo.findRecord('value', searchFolderCombo.getValue());
		var isAllFolderSelected = record.get('flag') === Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.ALL_FOLDERS;
	...
	/**
	 * Event handler triggered before selection performs in {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	 * Will open {@link Grommunio.advancesearch.dialogs.SelectFolderContentPanel SelectFolderContentPanel}, if
	 * "Other…" option was selected. also it will check select folder from {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	 * is supports search folder if not we hide the {@link Ext.form.FieldSet IncludeSubFolderFieldSet}.
	 *
	 * @param {Grommunio.common.searchfield.ui.SearchFolderCombo} combo The combo which fired the event.
	 * @param {Ext.data.Record} record The data record returned from the underlying store
	...
	onBeforeSelectSearchFolder: function (combo, record, index)
	{
		if(record.get('value') === 'other') {
			combo.collapse();
			Grommunio.advancesearch.Actions.openSelectSearchFolderDialog({
	 ...
	 * {@link Grommunio.advancesearch.dialogs.SearchToolbarPanel search toolbar panel}
	 *
	...
	/**
	 * Event handler which triggered when {@link Grommunio.advancesearch.ui.SearchResultPreviewPanel SearchResultPreviewPanel}
	 * was updated.
	 *
...
Ext.reg('grommunio.searchpanel', Grommunio.advancesearch.dialogs.SearchPanel);
