...
/**
 * @class Grommunio.advancesearch.ui.SearchGrid
 * @extends Grommunio.common.ui.grid.MapiMessageGrid
 * @xtype grommunio.searchgrid
 */
Grommunio.advancesearch.ui.SearchGrid = Ext.extend(Grommunio.common.ui.grid.MapiMessageGrid, {
	/**
	 * @cfg {Grommunio.advancesearch.AdvanceSearchContext} searchContext The searchContext to which this panel belongs
	 */
	searchContext: undefined,

	/**
	 * The {@link Grommunio.advancesearch.AdvanceSearchContextModel} which is obtained from the {@link #context}.
	 * @property
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			cm: new Grommunio.advancesearch.ui.SearchGridColumnModel({
				grid: this,
		...
		Grommunio.advancesearch.ui.SearchGrid.superclass.constructor.call(this, config);
	},
	...
	initEvents: function()
	{
		Grommunio.advancesearch.ui.SearchGrid.superclass.initEvents.call(this);
	...
	{
		return new Grommunio.advancesearch.ui.AdvanceSearchRowSelectionModel({
			singleSelect: false
		});
	},
	...
	viewConfigGetRowClass: function (record, rowIndex, rowParams, store)
	{
		...
		if (this.enableRowBody) {
			...
			var categoriesHtml = Grommunio.common.categories.Util.getCategoriesHtml(categories);
	...
	getBodyCell: function (record)
	{
		...
		switch (messageClass) {
			...
			case 'IPM.TaskRequest':
				body = Grommunio.common.ui.grid.Renderers.percentage(record.get('percent_complete'), meta, record);
				break;
			default:
	...
	getSubjectCell: function (record)
	{
		...
		if (record.isMessageClass('IPM.Contact')) {
			subject = Grommunio.advancesearch.ui.SearchGridRenderers.phoneNumber(record.get('home_telephone_number'), meta, record);
		} else if (!record.isMessageClass('IPM.StickyNote')) {
			subject = Grommunio.common.ui.grid.Renderers.subject(record.get('subject'), meta, record);
		}
	...
	{
		if(oldView === Grommunio.common.data.Views.LIVESCROLL) {
	 ...
	 * @param {Grommunio.advancesearch.ui.SearchGrid} grid The search grid
	...
	onHeaderClick: function(grid, columnIndex, event)
	{
		...
		if ( Ext.isEmpty(suggestion) || event.target.className!=='grommunio-search-suggestion' ){
			return;
		}
	 ...
	 *
	 * @param {Grommunio.advancesearch.ui.SearchGrid} grid the grid
	...
	onCellClick: function(grid, rowIndex, columnIndex, e)
	{
		...
		if (dataIndex === 'icon_index') {
			...
			Grommunio.common.Actions.openFlagsMenu(record, e.getXY(), false);
		}
	},
	 ...
	 *
	 * @param {Grommunio.advancesearch.ui.SearchGrid} grid The grid which was right clicked
	 * @param {Number} rowIndex The index number of the row which was right clicked
	 * @param {Ext.EventObject} event The event structure
	...
	onRowContextMenu: function(grid, rowIndex, event)
	{
		...
		return Grommunio.core.data.UIFactory.openDefaultContextMenu(records, { position: event.getXY(), context: this.searchContext, shadowEdit: false });
	},
	 ...
	 *
	 * @param {Grommunio.advancesearch.ui.SearchGrid} grid The Grid on which the user double-clicked
	 ...
	 * This will updates the {@link Grommunio.advancesearch.AdvanceSearchContextModel AdvanceSearchContextModel} with the record which
	...
	/**
	 * Event handler which is triggered when the {@link Grommunio.advancesearch.ui.SearchGrid grid}
	 * {@link Grommunio.core.data.IPMRecord record} selection is changed. This will inform
	 * the {@link Grommunio.advancesearch.AdvanceSearchContextModel contextmodel} about the change.
	 *
	 * @param {Ext.grid.RowSelectionModel} selectionModel The selection model used by the grid.
	 * @private
	 */
	 ...
	 * on {@link Grommunio.advancesearch.ui.SearchGrid search grid}. it will first stop the
	...
	/**
	 * Event handler for the {@link Grommunio.core.ContextModel#searchstop searchstop} event.
	 * This will {@link Grommunio.common.ui.LoadMask#hide hide} the {@link Grommunio.common.ui.LoadMask loadmask}, if any.
	 ...
	 * @param {Grommunio.advancesearch.AdvanceSearchStore} store The store that holds the data
	 * of this grid.
	...
	onBeforeUpdateSearch: function(store, searchMeta)
	{
		...
		// with the response
		if ( Ext.isDefined(store.lastOptions) && store.lastOptions.originalActionType!==Grommunio.core.Actions.updatesearch ){
			var cm = this.getColumnModel();
			if ( Ext.isEmpty(store.suggestion) ){
				...
				cm.setColumnHeader(0, _('Did you mean') +' <span class="grommunio-search-suggestion">' + encodedSuggestion + '</span> ?');
	...
	{
		this.cancelPreviewRecordTask();
		Grommunio.advancesearch.ui.SearchGrid.superclass.beforeDestroy.apply(this, arguments);
	}
});

Ext.reg('grommunio.searchgrid', Grommunio.advancesearch.ui.SearchGrid);
