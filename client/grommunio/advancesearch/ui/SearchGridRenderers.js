...
/**
 * @class Grommunio.advancesearch.ui.SearchGridRenderers
 ...
 * @singleton
 */
Grommunio.advancesearch.ui.SearchGridRenderers = {
	...
	subjectWithBodyColumn: function (value, p, record)
	{
		...
		switch (messageClass) {
			case 'IPM.Contact':
				subject = Grommunio.advancesearch.ui.SearchGridRenderers.phoneNumber(record.get('home_telephone_number'), {}, record);
		...
		if (record.searchConversationCount > 1) {
			counter = Grommunio.common.ui.grid.Renderers.conversationCount(record.searchConversationCount);
		}
	...
	nameColumn: function (value, p, record)
	{
		...
		return Grommunio.common.ui.grid.Renderers.presenceStatus(userName, p, userRecord);
	},
