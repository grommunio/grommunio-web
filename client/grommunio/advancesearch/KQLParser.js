...
/**
 * @class Grommunio.advancesearch.KQLParser
 * @singleton
 * @extends Object
 */
Grommunio.advancesearch.KQLParser = Ext.extend(Object, {
	lexer: null,
	...
	createTokenRestriction: function(tokens, anyFieldPreference) {
		...
		var propMap = {
			...
			to: [{type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_TO}],
			cc: [{type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_CC}],
			bcc: [{type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_BCC}],
			category: ['categories'],
			...
			any: [
				...
				{type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_TO},
			{type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_CC},
			{type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_BCC}
			]
		};
		if (Ext.isArray(anyFieldPreference) && anyFieldPreference.length) {
			...
			Ext.each(anyFieldPreference, function(field) {
				...
				switch (field) {
					case 'display_to':
						entry = {type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_TO};
						break;
					case 'display_cc':
						entry = {type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_CC};
						break;
					case 'display_bcc':
						entry = {type:'recipient', recipientType: Grommunio.core.mapi.RecipientType.MAPI_BCC};
						break;
					default:
						entry = field;
				}
		...
		tokens.forEach(function(token) {
			...
			if ( curRes.length > 1 ) {
				curRes = Grommunio.core.data.RestrictionFactory.createResOr(curRes);
			} else {
				curRes = curRes[0];
			}

			if ( token.negate ) {
				curRes = Grommunio.core.data.RestrictionFactory.createResNot(curRes);
			}
		...
		if ( op === 'OR' ) {
			restrictions = Grommunio.core.data.RestrictionFactory.createResOr(restrictions);
		} else {
			restrictions = Grommunio.core.data.RestrictionFactory.createResAnd(restrictions);
		}
	...
	createStringRestriction: function(prop, value) {
		return Grommunio.core.data.RestrictionFactory.dataResContent(
			prop,
	 ...
	 *
	 * @param {Number} recipientType The {@link Grommunio.core.mapi.RecipientType RecipientType}
	 * to put the restriction on
	 * @param {String} value The value the restriction will search for
	 * @return {Array} A Restriction array
	 */
	createRecipientRestriction: function(recipientType, value) {
		return Grommunio.core.data.RestrictionFactory.createResSubRestriction('PR_MESSAGE_RECIPIENTS',
			Grommunio.core.data.RestrictionFactory.createResAnd([
				Grommunio.core.data.RestrictionFactory.dataResProperty('PR_RECIPIENT_TYPE',
					Grommunio.core.mapi.Restrictions.RELOP_EQ,
					recipientType
				),
				Grommunio.core.data.RestrictionFactory.createResOr([
	...
	createAttachmentRestriction: function(value) {
		return Grommunio.core.data.RestrictionFactory.createResSubRestriction('PR_MESSAGE_ATTACHMENTS',
...
Grommunio.advancesearch.KQLParser = new Grommunio.advancesearch.KQLParser();
