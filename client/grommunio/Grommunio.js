...
Ext.apply(Grommunio, {
	 ...
	 * grommunio Web has initialized the bare essentials. When the event is fired, the {@link Grommunio.core.Container} will
	 ...
	 *
	 * This will instantiate {@link Grommunio.core.Container container}.
	 * @private
	 */
	initializeGlobals: function()
	{
		...
		Grommunio.core.Util.enableLeaveRequester();
		...
		const handlePageHide = function () {
			if (Grommunio.core.BrowserWindowMgr.browserWindows.length > 1) {
				Grommunio.core.BrowserWindowMgr.closeAllBrowserWindow();
			}
			container.getRequest().paralyze(Grommunio.core.data.ParalyzeReason.BROWSER_RELOADING);
		};
		...
		this.reMultipleEmailAddresses = new RegExp('([^,;\\n\\r]*?<{0,1}' + Grommunio.emailAddressRegExpString + ')>{0,1}(?=(?:$|[,;\\n\\r\\s]))', 'g');

		// Create global container object
		container = new Grommunio.core.Container();
		...
		Grommunio.core.DarkMode.init();
		Grommunio.core.Density.init();
		Grommunio.core.FolderType.init();
		...
		DOMPurify.setConfig(Grommunio.sanitizerConfig);
	 ...
	 * as well as the exception events on the {@link Grommunio.core.data.IPMStoreMgr} and
	 * {@link Grommunio.core.ResponseRouter}.
	 * @private
	 */
	initializeEnvironment: function()
	{
		// Register the State provider which uses the SettingsModel.
		Ext.state.Manager.setProvider(new Grommunio.core.data.SettingsStateProvider());
		...
		// Enable tooltips
		Ext.QuickTips.init();
		Grommunio.core.KeyboardFocus.init(document);
	},
	 ...
	 * 'grommunio-contextmenu-enabled' class and isn't a regular text input then the
	...
	onBodyContextMenu: function(event, el)
	{
		...
		// 'grommunio-contextmenu-enabled' CSS class is applied
		// on the element.
		if ( el.hasClass('grommunio-contextmenu-enabled') || el.up('div.grommunio-contextmenu-enabled')){
			return;
		}
	...
	onException: function(proxy, type, action, options, response, args)
	{
		...
		if (type === 'response') {
			...
			switch (errorObj.type) {
				case Grommunio.core.ErrorType['MAPI']:
				case Grommunio.core.ErrorType['GROMMUNIO']:
				case Grommunio.core.ErrorType['GENERAL']:
		...
		// See web issue #113
		if (args && Ext.isArray(args.sendRecords)) {
	 ...
	 * @param {Grommunio.core.Request} request The request object
	 * @param {Grommunio.core.data.ParalyzeReason} reason The reason to paralyze grommunio Web
	 * @private
	 */
	onConnectionParalyze: function(request, reason)
	{
		...
		switch (reason) {
			case Grommunio.core.data.ParalyzeReason.BROWSER_RELOADING:
			/* falls through */
			default:
				// No message for the user needed.
				return;
			case Grommunio.core.data.ParalyzeReason.SESSION_EXPIRED:
				...
				break;
			case Grommunio.core.data.ParalyzeReason.SESSION_INVALID:
	 ...
	 * This determines what the user has pressed, and if the user wishes to {@link Grommunio.core.Container#logout}
	 * immediately or not. If not, then a {@link Grommunio.core.ui.notifier.Notifier notification} will be shown
	...
	/**
	 * Periodic function which is used to update the {@link Grommunio.core.ui.notification.Notifier notification}
	 ...
	 * @param Grommunio.core.PingService} service The ping service handling the connection loss
	...
	/**
	 * Event handler for the {@link Grommunio.core.PingService#retry} event. This will
	...
	/**
	 * Event handler for the {@link Grommunio.core.Request#connectionloss} event. This will register the
	 * event handlers to the provided {@link Grommunio.core.PingService} which will give us the information
	 * about the next reconnect attempt.
	 * @param {Grommunio.core.Request} request The request object
	 * @param {Grommunio.core.PingService} service The ping service which is going to handle the connection loss
	 * @private
	 */
	...
	/**
	 * Event handler for the {@link Grommunio.core.Request#connectionrestore} event. This will remove
	 * the error.connection {@link Grommunio.core.ui.notification.Notifier notification}
	 * and will show a info.connection.restore {@link Grommunio.core.ui.notification.Notifier notification}.
	 * @param {Grommunio.core.Request} request The request object
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.Request} request
	...
	onVersionChanged: function(request, version)
	{
		container.getNotifier().notify('info.version', _('grommunio Web was updated'), _('A new version is available, click here to reload.'), {
			persistent: true,
			listeners: {
				click: Grommunio.core.Util.reloadWebapp,
				scope: Grommunio.core.Util
			}
		});
	},
	...
	/**
	 * Validate the {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore} to determine
	 * if the {@link Grommunio.hierarchy.data.HierarchyStore#getDefaultStore Default Store} is present
	 * along with all the {@link Grommunio.hierarchy.data.HierarchyStore#getDefaultFolder Default Folders}.
	 * If there is a problem, this will show a {@link Grommunio.core.ui.notifier.Notifier Notification}
	 ...
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The Hierarchy Store to validate
	 * @private
	 */
	...
	/**
	 * Event handler which is fired when the user clicked on the {@link Grommunio.core.ui.notifier.Notifier notification}.
	 ...
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The store which was loaded
	...
	onHierarchyLoad: function(store, records, options)
	{
		if (!Ext.isEmpty(records)) {
			...
			Grommunio.core.URLActionMgr.execute(urlActionData);
			delete urlActionData;

			// Start the keepalive to make sure we stay logged into the grommunio-server,
	...
	/**
	 * Preload the per-mailbox category list ({@link Grommunio.common.categories.CategoryListManager})
	...
	{
		var manager = Grommunio.common.categories.CategoryListManager;
		if ( !manager ) {
			return;
		}
	...
	/**
	 * Handler for {@link Grommunio.common.categories.CategoryListManager#load}.
	 ...
	 * load the {@link Grommunio.hierarchy.data.HierarchyStore} and open the
	 * {@link Grommunio.core.ui.MainViewport MainViewport}.
	 */
	loadWebclient: function()
	{
		// Initialize globals & environment
		Grommunio.initializeGlobals();
		Grommunio.initializeEnvironment();
	...
	{
		var defaultContext = container.getSettingsModel().get('grommunio/v1/main/default_context');
	 ...
	 *
	 * @param {Grommunio.common.outofoffice.data.OofStore} store The store which was loaded
	 * @param {Grommunio.common.outofoffice.data.OofRecord} records The records which were loaded by the store
	 * @param {Object} options The options which were originally passed to {@link Ext.data.Store#load}.
	 * @private
	 */
	onOofStoreLoad: function(store, records, options)
	{
		...
		for (var i=0; i< records.length; i++) {
			if (Grommunio.core.EntryId.compareEntryIds(records[i].get('entryid'), loginUserEntryId)) {
	 ...
	 * the environment and open the {@link Grommunio.core.ui.WelcomeViewport WelcomeViewport}.
	 */
