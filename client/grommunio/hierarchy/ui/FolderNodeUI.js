...
/**
 * @class Grommunio.hierarchy.ui.FolderNodeUI
 ...
 */
Grommunio.hierarchy.ui.FolderNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {

	/**
	 * The currently active countertype for this folder node
	 * @property
	 * @type Grommunio.hierarchy.data.CounterTypes
	 */
	...
	renderElements: function(n, a, targetNode, bulkRender)
	{
		...
		buf = '<li class="x-tree-node">' +
				'<div ext:tree-node-id="' + n.id + '" class="x-tree-node-el x-tree-node-leaf x-unselectable grommunio-hierarchy-node" unselectable="on">' +
					// indent space
					...
					'<a hidefocus="on" class="x-tree-node-anchor grommunio-hierarchy-node-anchor" ' +
						...
						(a.hrefTarget ? ' target="' + a.hrefTarget + '"' : "") + ">" +
							...
							'<span class="grommunio-hierarchy-node-foldername" unselectable="on">' + (n.tpl ? n.tpl.apply(a) : n.text) + '</span>' +
	...
	/**
	 * Function is used to show folder owner name along with {@link Grommunio.hierarchy.data.FavoritesFolderRecord favorites} folder name.
	 * @param {Grommunio.hierarchy.ui.FolderNode} node which has to show folder owner name.
	 */
	 ...
	 * @param {Grommunio.hierarchy.ui.FolderNode} node The node which is being updated
	 */
	updateCounter: function(node)
	{
		...
		switch (newCounterType) {
			case Grommunio.hierarchy.data.CounterTypes.TOTAL:
				...
				if (!Ext.isDefined(currentCounterType)) {
					elNode.addClass('grommunio-hierarchy-node-total-count');
				} else if (currentCounterType !== newCounterType) {
					elNode.replaceClass('grommunio-hierarchy-node-unread-count', 'grommunio-hierarchy-node-total-count');
				}
				counterNode.removeClass('grommunio-hierarchy-node-nopadding');
			...
			case Grommunio.hierarchy.data.CounterTypes.UNREAD:
				...
				if (!Ext.isDefined(currentCounterType)) {
					elNode.addClass('grommunio-hierarchy-node-unread-count');
				} else if (currentCounterType !== newCounterType) {
					elNode.replaceClass('grommunio-hierarchy-node-total-count', 'grommunio-hierarchy-node-unread-count');
				}
				counterNode.removeClass('grommunio-hierarchy-node-nopadding');
				counterNode.update(folder.getCounterValue());
				counterNode.repaint();
			...
			default:
				...
				if (Ext.isDefined(currentCounterType) && currentCounterType !== newCounterType) {
					elNode.removeClass(['grommunio-hierarchy-node-total-count', 'grommunio-hierarchy-node-unread-count']);
				}
				counterNode.addClass('grommunio-hierarchy-node-nopadding');
				counterNode.update('');
				counterNode.repaint();
				break;
		}
		...
		if (hasUnread) {
			elNode.addClass('grommunio-hierarchy-node-withcounter');
		} else {
			elNode.removeClass('grommunio-hierarchy-node-withcounter');
		}
	},
	 ...
	 * drag and drop manager for {@link Grommunio.hierarchy.ui.Tree Tree}.
	...
	onOver: function(e)
	{
		...
		Grommunio.core.Util.setOverflowTooltip(this.folderOwnerNode, name);

		Grommunio.hierarchy.ui.FolderNodeUI.superclass.onOver.apply(this, arguments);
	},
	...
	onClick : function(e)
	{
		...
		if (!container.getServerConfig().isWidgetEnabled() && folder.isOwnRoot()){
			...
			if (ownerTree && inboxFolder) {
				...
				Grommunio.hierarchy.Actions.openFolder(inboxFolder);
				return;
			}
		}

		Grommunio.hierarchy.ui.FolderNodeUI.superclass.onClick.apply(this, arguments);
	}
});
