...
/**
 * @class Grommunio.hierarchy.ui.ContextMenu
 * @extends Grommunio.core.ui.menu.ConditionalMenu
 * @xtype grommunio.hierarchycontextmenu
 */
Grommunio.hierarchy.ui.ContextMenu = Ext.extend(Grommunio.core.ui.menu.ConditionalMenu, {

	/**
	 * @cfg contextNode Holds {@link Grommunio.hierarchy.ui.FolderNode foldernode} on which 'contextmenu' event has occurred
	 */
	 ...
	 * @property
	 * @type Grommunio.hierarchy.ui.Tree
	 */
	...
	constructor: function(config)
	{
		...
		Grommunio.hierarchy.ui.ContextMenu.superclass.constructor.call(this, config);
	},

	/**
	 * Create the Action context menu items.
	 * @param {Object} config Configuration object for the {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * @return {Grommunio.core.ui.menu.ConditionalItem[]} The list of Action context menu items
	 * @private
	 *
	 * Note: All handlers are called within the scope of {@link Grommunio.hierarchy.ui.ContextMenu HierarchyContextMenu}
	 */
	createContextMenuItems: function(config)
	{
		...
		return [{
			...
			beforeShow: function(item, record) {
				var access = record.get('access') & Grommunio.core.mapi.Access.ACCESS_READ;
          ...
          record.isDefaultFolder()
			...
			beforeShow: function(item, record) {
				var access = record.get('access') & Grommunio.core.mapi.Access.ACCESS_MODIFY;
				if (
          !access ||
          !this.contextTree ||
          !this.contextNode
			...
			beforeShow: function(item, record) {
				var access = record.get('access') & Grommunio.core.mapi.Access.ACCESS_CREATE_HIERARCHY;
			...
			beforeShow: function(item, record) {
				var access = record.get('access') & Grommunio.core.mapi.Access.ACCESS_DELETE;
				if (
          !access ||
          record.isIPMSubTree() ||
          record.isDefaultFolder() ||
		...
		},
		{
			...
			beforeShow: function(item, record) {
				...
				if (container.getSettingsModel().get('grommunio/v1/contexts/hierarchy/hide_favorites')) {
	...
	/**
	 * Gets all recursive subfolders of {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}
	 * in a flat list, not a tree
	 *
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder which children are returned
	 ...
	 * {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu} Opens
	 * {@link Grommunio.hierarchy.dialogs.FolderPropertiesContent FolderPropertiesContent}
	 *
	...
	onContextItemSharedFolderAndProperties: function(btn)
	{
		...
		Grommunio.hierarchy.Actions.openFolderPropertiesContent(folder, config);
	},
	 ...
	 * @param {Grommunio.core.data.IPFRecord} record The folder record on which
	...
	{
		var access = record.get('access') & Grommunio.core.mapi.Access.ACCESS_CREATE_CONTENTS;
		...
		switch (item.name) {
			...
			break;
			default:
				hasImportSupport = Grommunio.core.ContainerClass.isClass(record.get('container_class'), 'IPF.Note', true);
		}
	 ...
	 * {@link Grommunio.core.ui.menu.ConditionalItem menu items}
	 * @private
	 */
	...
	/**
	 * Fires on selecting 'Open' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * @private
	 */
	onContextItemOpen: function()
	{
		...
		Grommunio.hierarchy.Actions.openFolder(this.records);
	},

	/**
	 * Fires on selecting 'Copy/Move Folder' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Opens {@link Grommunio.common.dialogs.CopyMoveContent CopyMoveContent} which copies or moves the folder in the Hierarchy
	 * @private
	 */
	...
	/**
	 * Fires on selecting 'Rename' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	...
	/**
	 * Fires on selecting 'New Folder' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Opens {@link Grommunio.common.dialogs.CreateFolderContent CreateFolderContent} which adds new folder to Hierarchy
	 * @private
	 */
	onContextItemNewFolder: function()
	{
		Grommunio.hierarchy.Actions.openCreateFolderContent(this.records);
	},

	/**
	 * Fires on selecting 'Delete Folder' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Asks user to confirm deletion of folder and if true then calls {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore}
	 * to delete folder.
	 * @private
	 */
			...
			fn: function (buttonClicked) {
				if (buttonClicked === 'yes') {
					...
					if (isFolderDeleted) {
						...
						if (container.getSettingsModel().get('grommunio/v1/contexts/mail/delegate_wastebasket_style') === 4) {
	...
	/**
	 * Fires on selecting the 'Empty Folder' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Asks user to confirm deletion of all contents of selected folder and if true then calls {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore}
			...
			fn: function(buttonClicked) {
				if (buttonClicked == 'yes') {
					if (this.records.get('content_count') >= 500) {
						Grommunio.hierarchy.Actions.emptyFolderBatched(this.records);
					} else {
	...
	/**
	 * Fires on selecting 'Mark All msgs as read' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Asks {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore} to mark all messages on the selected folder as read.
	 * @private
	 */
	...
	/**
	 * Fires when a color is selected from the {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Saves the new color to the settings and calls doLayout on the context to re-render.
	 *
	 * @param {Grommunio.core.ui.menu.ConditionalItem} item The menu item that was clicked on
	 */
	onContextItemSelectColor: function(item)
	{
		...
		var colorScheme = Grommunio.core.ColorSchemes.getColorScheme(colorSchemeName);
	...
	/**
	 * Fires on selecting 'Reload' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Reloads {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore} and populates new data in {@link Grommunio.hierarchy.ui.Tree Tree}.
	 * @private
	 */
	...
	{
		Grommunio.common.Actions.openRestoreContent(this.records);
	},

	/**
	 * Open upload files dialog to import into folder.
	 *
	 * @param {Grommunio.core.ui.menu.ConditionalItem} button The menu item that was clicked on
	 * @private
	 */
	onContextImportItem: function(button)
	{
		...
		var config = Ext.apply({}, {
			accept: accept,
			callback: Grommunio.common.Actions.importItemCallback.createDelegate(Grommunio.common.Actions, [ button.getRecords() ], 1)
		});
		Grommunio.common.Actions.openImportContent(this.records, config);
	},

	/**
	 * Event handler triggers when 'Add to Favorites' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Add to Favorites mark as Favorites to selected {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
	 */
	...
	/**
	 * Event handler triggers when 'Remove From Favorites' menu option from {@link Grommunio.hierarchy.ui.ContextMenu ContextMenu}
	 * Remove From Favorites menu option unmark selected {@link Grommunio.hierarchy.data.MAPIFolderRecord folder} from Favorite list .
	 */
	...
	/**
	 * Helper function to get all the available {@link Grommunio.hierarchy.ui.Tree tree} and update
	...
	/**
	 * Helper function to find node based on supplied entryid from {@link Grommunio.hierarchy.ui.Tree tree}
	 * and set the style of particular node with supplied color.
	 * @param {Grommunio.hierarchy.ui.Tree} hierarchyTree The tree in which node needs to be searched
	 * @param {String} folderEntryid The entryid of the selected folder
	 * @param {String} colorSchemeBase The chosen color
	 * @private
	 */
...
Ext.reg('grommunio.hierarchycontextmenu', Grommunio.hierarchy.ui.ContextMenu);
