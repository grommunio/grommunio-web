...
/*
 * #dependsFile client/grommunio/hierarchy/data/SharedFolderTypes.js
 */
Ext.namespace('Grommunio.hierarchy.dialogs');

/**
 * @class Grommunio.hierarchy.dialogs.SharedFolderPanel
 * @extends Ext.Panel
 * @xtype grommunio.sharedfolderpanel
 */
Grommunio.hierarchy.dialogs.SharedFolderPanel = Ext.extend(Ext.Panel, {
	/**
	 * @cfg {Grommunio.hierarchy.data.SharedFolderTypes} defaultSelectedFolderType Default selected folder in combolist.
	 */
	defaultSelectedFolderType: Grommunio.hierarchy.data.SharedFolderTypes['ALL'],

	/**
	 * @cfg {Grommunio.core.data.IPMRecipientStore} store The store in which the user is stored which
	...
	constructor: function(config)
	{
		...
		Grommunio.hierarchy.dialogs.SharedFolderPanel.superclass.constructor.call(this, config);
	},

	/**
	 * Creates recipient selection field.
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store to hook into the RecipientField
	 ...
	 * @param {Grommunio.hierarchy.data.SharedFolderTypes} defaultSelectedFolderType The folder type which will be
	 * selected by default
	...
	createFolderTypeCombo: function(defaultSelectedFolderType)
	{
		return {
			...
			store: new Ext.data.JsonStore({
				fields: ['value', 'name'],
				data: Grommunio.hierarchy.data.SharedFolderTypes.folders
			}),
			lazyInit: false,
			editable: false,
			displayField: 'name',
	...
	onRender: function()
	{
		Grommunio.hierarchy.dialogs.SharedFolderPanel.superclass.onRender.apply(this, arguments);
	 ...
	 * Folder Type selection is for the {@link Grommunio.common.data.FolderContentTypes#ipmsubtree} or not.
	 * @private
	 */
	updateSubFolderCheckBox: function()
	{
		var enabled = this.folderTypeCombo.getValue() !== Grommunio.hierarchy.data.SharedFolderTypes['ALL'];
...
Ext.reg('grommunio.sharedfolderpanel', Grommunio.hierarchy.dialogs.SharedFolderPanel);
