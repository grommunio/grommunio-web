...
/**
 * @class Grommunio.hierarchy.dialogs.FolderPropertiesPermissionTab
 * @extends Ext.Panel
 * @xtype grommunio.folderpropertiespermissiontab
 *
 * Permissions tab in the {@link Grommunio.hierarchy.dialogs.FolderPropertiesContentPanel}
 * that is used to set permissions on folder for other users.
 */
Grommunio.hierarchy.dialogs.FolderPropertiesPermissionTab = Ext.extend(Ext.Panel, {
	/**
	 * @constructor
	 * @param {Object} config
	 */
	...
	constructor: function(config)
	{
		...
		Grommunio.hierarchy.dialogs.FolderPropertiesPermissionTab.superclass.constructor.call(this, config);
	},
	...
	createUserListPanel: function(emptyText)
	{
		return {
			...
			items: [{
				xtype: 'grid',
				ref: '../userView',
				store: new Grommunio.hierarchy.data.MAPIFolderPermissionsSubStore(),
				hideHeaders: true,
				enableHdMenu: false,
				border: true,
				viewConfig: {
	...
	createProfilePanel: function(isAppointmentDialog)
	{
		var profileStore = {
			xtype: 'jsonstore',
			fields: ['name', 'value'],
			data: isAppointmentDialog ? Grommunio.hierarchy.data.PermissionProfilesCalendar : Grommunio.hierarchy.data.PermissionProfiles
		};
			...
			items: {
				...
				items: [{
					...
					items: [{
						...
						forceSelection: true,
						editable: false,
						defaultValue: Grommunio.core.mapi.Rights.RIGHTS_NO_RIGHTS,
						listeners: {
	...
	createPermissionPanel: function(isAppointmentDialog)
	{
		const readRadioGroupItems = isAppointmentDialog ? [{
			...
			boxLabel: _('None'),
			hideLabel: true,
			rightsValue: Grommunio.core.mapi.Rights.RIGHTS_NO_RIGHTS
		},{
			name: 'allowread',
			boxLabel: _('Free/Busy time'),
			hideLabel: true,
			rightsValue: Grommunio.core.mapi.Rights.RIGHTS_CAL_FBSIMPLE
		},{
			name: 'allowread',
			boxLabel: _('Free/Busy time, subject, location'),
			hideLabel: true,
			rightsValue: Grommunio.core.mapi.Rights.RIGHTS_CAL_FBSIMPLE | Grommunio.core.mapi.Rights.RIGHTS_CAL_FBDETAILED
		},{
			name: 'allowread',
			boxLabel: _('Full Details'),
			hideLabel: true,
			rightsValue: Grommunio.core.mapi.Rights.RIGHTS_READ_ANY | Grommunio.core.mapi.Rights.RIGHTS_CAL_FBSIMPLE | Grommunio.core.mapi.Rights.RIGHTS_CAL_FBDETAILED | Grommunio.core.mapi.Rights.RIGHTS_FOLDER_VISIBLE
		}] : [{
				name: 'allowread',
				boxLabel: _('None'),
				hideLabel: true,
				...
				boxLabel: _('Full Details'),
				hideLabel: true,
				rightsValue: Grommunio.core.mapi.Rights.RIGHTS_READ_ANY | Grommunio.core.mapi.Rights.RIGHTS_FOLDER_VISIBLE
			}
		];
		return {
			xtype: 'form',
					...
					items: [{
						...
						items: [{
							...
							items: [{
								...
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_CREATE
							},{
								xtype: 'checkbox',
								boxLabel: _('Create subfolders'),
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_CREATE_SUBFOLDER
							},{
								xtype: 'checkbox',
								boxLabel: _('Edit own'),
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_EDIT_OWNED
							},{
								xtype: 'checkbox',
								boxLabel: _('Edit all'),
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_EDIT_OWNED | Grommunio.core.mapi.Rights.RIGHTS_EDIT_ANY
							}],
							listeners: {
								change: this.onPermissionChecked,
								scope: this
					...
					items: [{
						...
						items: [{
							...
							items: [{
								...
								boxLabel: _('None'),
								hideLabel: true,
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_NONE
							},{
								name: 'allowdelete',
								boxLabel: _('Own'),
								hideLabel: true,
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_DELETE_OWNED
							},{
								name: 'allowdelete',
								boxLabel: _('All'),
								hideLabel: true,
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_DELETE_ANY | Grommunio.core.mapi.Rights.RIGHTS_DELETE_OWNED
							}],
							listeners: {
								change: this.onPermissionChecked,
								scope: this
						...
						items: [{
							...
							items: [{
								...
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_CREATE_FOLDER | Grommunio.core.mapi.Rights.RIGHTS_FOLDER_CONTACT | Grommunio.core.mapi.Rights.RIGHTS_FOLDER_VISIBLE
							},{
								xtype: 'checkbox',
								boxLabel: _('Folder contact'),
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_FOLDER_CONTACT
							},{
								xtype: 'checkbox',
								boxLabel: _('Folder visible'),
								rightsValue: Grommunio.core.mapi.Rights.RIGHTS_FOLDER_VISIBLE
							}],
							listeners: {
								change: this.onPermissionChecked,
								scope: this
	 ...
	 * user to the permission store. This will call {@link Grommunio.common.Actions#openABUserSelectionContent}
	...
	/**
	 * Event handler for the {@link Grommunio.common.Actions#openABUserSelectionContent} function,
	 * which is used to select a user to be added to the permissions store.
	 * @param {Grommunio.addressbook.AddressBookRecord} record The selected user/group
	 * @private
	 */
	onUserSelected: function(record)
	{
		...
		if (store.findExact('entryid', permission.get('entryid')) < 0) {
			...
			var msg;
			if (permission.get('object_type') === Grommunio.core.mapi.ObjectType.MAPI_MAILUSER) {
	 ...
	 * {@link Grommunio.hierarchy.data.PermissionProfiles profile}. If it doesn't
	...
	updatePermissionFromGroup: function(group, permissions)
	{
		...
		items.each(function(item) {
			if (Ext.isDefined(item.rightsValue)) {
				if (item.rightsValue != Grommunio.core.mapi.Rights.RIGHTS_READONLY &&
					item.rightsValue != Grommunio.core.mapi.Rights.RIGHTS_CAL_READ_FULL) {
				mask |= item.rightsValue;
				}
				if (item.checked) {
					flag |= item.rightsValue;
					// If "Edit all" is checked, check "Edit own"
					if (item.rightsValue === Grommunio.core.mapi.Rights.RIGHTS_CAL_EDIT_ALL) {
						var option = items.get(2);
						option.setValue(true);
					}
					// If "Folder owner" is checked, check other folder permissions
					if (item.rightsValue === Grommunio.core.mapi.Rights.RIGHTS_FOLDER_OWNER) {
	...
	/**
	 * Load the given {@link Grommunio.core.mapi.Rights permissions mask} into the UI.
	 ...
	 * @param {Number} permissions Mask of {@link Grommunio.core.mapi.Rights rights values}.
	 * @private
	 */
	...
	/**
	 * Read the UI to determine the {@link Grommunio.core.mapi.Rights permissions mask}
	 ...
	 * @returns {Number} Mask of {@link Grommunio.core.mapi.Rights rights values}.
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.UserPermissionRecord} user The user to load into the form
	 * @private
	 */
...
Ext.reg('grommunio.folderpropertiespermissiontab', Grommunio.hierarchy.dialogs.FolderPropertiesPermissionTab);
