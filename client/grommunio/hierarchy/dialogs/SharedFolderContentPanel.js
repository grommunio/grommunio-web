...
/**
 * @class Grommunio.hierarchy.dialogs.SharedFolderContentPanel
 * @extends Grommunio.core.ui.ContentPanel
 * @xtype grommunio.sharedfoldercontentpanel
 */
Grommunio.hierarchy.dialogs.SharedFolderContentPanel = Ext.extend(Grommunio.core.ui.ContentPanel, {
	/**
	...
	constructor: function(config)
	{
		...
		if (!config.store) {
			config.store = new Grommunio.core.data.IPMRecipientStore({
		...
		Grommunio.hierarchy.dialogs.SharedFolderContentPanel.superclass.constructor.call(this, config);
	},
	...
	onOpen: function()
	{
		if (this.store && this.store.getCount() > 0) {
			...
			if (!recipient.isResolved()) {
				...
				return;
			} else if (recipient.get('object_type') !== Grommunio.core.mapi.ObjectType.MAPI_MAILUSER) {
				Ext.MessageBox.show({
...
Ext.reg('grommunio.sharedfoldercontentpanel', Grommunio.hierarchy.dialogs.SharedFolderContentPanel);
