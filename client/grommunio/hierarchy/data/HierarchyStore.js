...
/*
 * #dependsFile client/grommunio/hierarchy/data/HierarchyProxy.js
 */
Ext.namespace('Grommunio.hierarchy.data');

/**
 * @class Grommunio.hierarchy.data.HierarchyStore
 * @extends Grommunio.core.data.IPFStore
 * @xtype grommunio.hierarchystore
 *
 * {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore} holds
 * {@link Grommunio.hierarchy.data.MAPIStoreRecord MAPIStoreRecord} as records, which defines store information
 * of all opened stores(own, shared, public).
 */
Grommunio.hierarchy.data.HierarchyStore = Ext.extend(Grommunio.core.data.IPFStore, {
	/**
	 * @cfg {Boolean} stateful A flag which causes the store to create a {@link #state} object in which
	 * the state for the various {@link Grommunio.hierarchy.data.MAPIFolderRecord folders} can be saved.
	...
	constructor: function (config)
	{
		...
		Ext.applyIf(config, {
			...
			standalone: true,
			proxy: new Grommunio.hierarchy.data.HierarchyProxy(),
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.hierarchy.data.HierarchyStore} store The hierarchy store that fired the event
			 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} storeRecord The MAPI store that contains the folder that was added
			 * @param {Grommunio.hierarchy.data.IPFRecord/Grommunio.hierarchy.data.IPFRecord[]} folder folder record that is added to the hierarchy store.
			 */
			'addFolder',
			/**
			 * @event removeFolder
			 * Fires when a folder has been removed.
			 * @param {Grommunio.hierarchy.data.HierarchyStore} store This hierarchy store that fired the event
			 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} storeRecord The MAPI store that contained the folder that was removed
			 * @param {Grommunio.hierarchy.data.IPFRecord} folder folder which is removed from the hierarchy store.
			 */
			'removeFolder',
			/**
			 * @event updateFolder
			 * Fires when a folder has been updated
			 * @param {Grommunio.hierarchy.data.HierarchyStore} store
			 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} storeRecord
		...
		Grommunio.hierarchy.data.HierarchyStore.superclass.constructor.call(this, config);
	 ...
	 * This will apply the {@link Grommunio.core.EntryId#compareStoreEntryIds compareStoreEntryIds} function
	...
	{
		return Grommunio.core.EntryId.compareStoreEntryIds(a, b);
	},
	...
	load: function(options)
	{
		...
		return Grommunio.hierarchy.data.HierarchyStore.superclass.load.call(this, options);
	},
	 ...
	 * from the Hierarchy.
	 * @param {Grommunio.hierarchy.data.MAPIStore} record The MAPIStore which must be
	...
	remove: function(record, removeFavorites)
	{
		...
		if (record.isSharedStore()) {
			...
			Grommunio.hierarchy.data.HierarchyStore.superclass.remove.call(this, record);
		}
	},
	 ...
	 * @param {Grommunio.hierarchy.data.SharedFolderTypes} foldertype The foldertype which will be opened
	...
	open: function(username, foldertype, subfolders)
	{
		...
		var settings = container.getSettingsModel().get(
			'grommunio/v1/contexts/hierarchy/shared_stores/' + Grommunio.core.Util.bin2hex(username), true);
	 ...
	 * the received {@link Grommunio.hierarchy.data.MAPIStoreRecord store} is
	 * {@link Grommunio.core.data.MAPIRecord#applyData merged} into the already existing
	 * {@link Grommunio.hierarchy.data.MAPIStoreRecord store}.
	...
	loadRecords: function(o, options, success)
	{
		...
		Grommunio.hierarchy.data.HierarchyStore.superclass.loadRecords.call(this, o, options, success);
	},
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord[]} records The records which are hooked
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} record The record to unhook
	 * @private
	 */
	...
	onAfterLoad: function(store, records, options)
	{
		if (options.actionType === 'opensharedfolder') {
			...
			var settingsBase = 'grommunio/v1/contexts/hierarchy/shared_stores/' + Grommunio.core.Util.bin2hex(user_name);
        ...
        if (folder_type === "calendar") {
          ...
          Grommunio.hierarchy.Actions.openFolder(foldersToOpen);
        } else {
		...
		Grommunio.hierarchy.Actions.setTitleCounter(this);
	},
	 ...
	 * 
	 * @param {Grommunio.hierarchy.data.MAPIStore} record The MAPIStore which is going save.
	...
	save: function(record, removeFavorites)
	{
		...
		Grommunio.hierarchy.data.HierarchyStore.superclass.save.apply(this, arguments);
	},
	...
	onBeforeSave: function(store, data)
	{
		...
		if (!Ext.isEmpty(data['close'])) {
			...
			for (var i = 0, len = stores.length; i < len; i++) {
				...
				settings.remove('grommunio/v1/contexts/hierarchy/shared_stores/' + Grommunio.core.Util.bin2hex(username));
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord[]} records The record which is added
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.IPFSubStore} substore
	 * @param {Grommunio.hierarchy.data.IPFRecord} folder which is added to the hierarchy
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.IPFSubStore} substore
	 * @param {Grommunio.hierarchy.data.IPFRecord} folder which is removed from the hierarchy
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.IPFSubStore} store
	 * @param {Grommunio.hierarchy.data.IPFRecord} record which is updated in the hierarchy.
	 * @param {String} operation The update operation being performed. Value may be one of
	 * {@link Ext.data.Record#EDIT}, {@link Ext.data.Record#REJECT}, {@link Ext.data.Record#COMMIT}.
	 * @private
	 */
	...
	/**
	 * @return {Array} list of all {@link Grommunio.hierarchy.data.MAPIStoreRecord Store}
	 */
	getStores: function()
	{
	...
	/**
	 * @return {Grommunio.hierarchy.data.MAPIStoreRecord} default store
	 */
	getDefaultStore: function()
	{
		var index = this.findExact('mdb_provider', Grommunio.core.mapi.MDBProvider.GROMMUNIO_SERVICE_GUID);
	...
	/**
	 * @return {Grommunio.hierarchy.data.MAPIStoreRecord} public store
	 */
	getPublicStore: function()
	{
		var index = this.findExact('mdb_provider', Grommunio.core.mapi.MDBProvider.GROMMUNIO_STORE_PUBLIC_GUID);
	...
	/**
	 * Function is used get default {@link Grommunio.hierarchy.data.MAPIFolderRecord MAPIFolderRecord}
	 * from {@link Grommunio.hierarchy.data.MAPIStoreRecord MAPIStoreRecord},
	 * if {@link Grommunio.hierarchy.data.MAPIStoreRecord MAPIStoreRecord} is not passed then
	 * it will by default take default store.
	 *
	 * @param {String} name name of the default folder (i.e. 'inbox' or 'contacts')
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStoreRecord (optional) {@link Grommunio.hierarchy.data.MAPIStoreRecord MAPIStoreRecord}
	 * whose default folder we should return, if not passed then it will use default store.
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord} a folder if a default folder with the given name was found, or undefined otherwise.
	 */
	...
	/**
	 * Retrieves a {@link Grommunio.hierarchy.data.MAPIStoreRecord MAPIStoreRecord} by owner entryid.
	 * @param {String} ownerEntryId entry id of the owner of the store.
	 * @return {Grommunio.hierarchy.data.MAPIStoreRecord} store owned by user.
	 */
	getStoreByOwnerEntryId: function(ownerEntryId)
	{
		for(var i = 0; i < this.getCount(); i++) {
			...
			if(Grommunio.core.EntryId.compareABEntryIds(folderStore.get('mailbox_owner_entryid'), ownerEntryId)) {
				return folderStore;
			}
		}
	},
	 ...
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord} a folder matching the given ID, or undefined if not found.
	 */
	 ...
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord} a folder
	 * default folder for the supplied message_class
	 */
	getDefaultFolderFromMessageClass: function(messageClass)
	{
		var folderType = Grommunio.core.MessageClass.getDefaultFolderTypeFromMessageClass(messageClass);
	...
	{
		var folderType = Grommunio.core.ContainerClass.getDefaultFolderTypeFromContainerClass(containerClass);
		if (!Ext.isEmpty(folderType)) {
			return this.getDefaultStore().getDefaultFolder(folderType);
		}
	},
	 ...
	 * a {@link Grommunio.core.data.Notifications#newMail newMail}
	...
	onNotifyNewmail: function(action, records, data, timestamp, success)
	{
		...
		for (var i = 0, len = records.length; i < len; i++) {
			...
			if (!timestamp || records[i].lastExecutionTime(Grommunio.core.Actions['list']) < timestamp) {
				/*
				 ...
				 */
				if(records[i].lastOptions.actionType === Grommunio.core.Actions['updatelist']) {
		...
		if(!Ext.isEmpty(data)){
			Ext.each(data.item, function(folder){
				...
				if (newmail && folder_keys.indexOf(folderKey) === -1 && folderStore.isContainerClass('IPF.Note') &&
					Grommunio.hierarchy.data.HierarchyStore.notifiesNewMail(folderStore)) {
					...
					if (!Grommunio.core.BrowserWindowMgr.isMainWindowActive()) {
						mainBrowserWindowBody = Grommunio.core.BrowserWindowMgr.getMainBrowserWindowBody();
					}
	 ...
	 * {@link Grommunio.core.Request Request} object to reset the counter every time the clients sends a
	 * request to the server.
	 */
	startKeepAlive: function()
	{
		var interval = container.getSettingsModel().get('grommunio/v1/contexts/hierarchy/polling_interval');
	 ...
	 * @param {Grommunio.core.Actions} action The action type that will be send to the server
	 ...
	 * for {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore}.
	 * Reason to create this function was we don't have any operation type for keepalive requests
	 * other than CRUD so what we do is call {@link Grommunio.hierarchy.data.HierarchyProxy#request}
	 * method of {@link Grommunio.hierarchy.data.HierarchyProxy HierarchyProxy} with 'read' operation
	...
	{
		this.sendSimpleActionToServer(Grommunio.core.Actions['keepalive']);
	},
	...
	{
		this.sendSimpleActionToServer(Grommunio.core.Actions['sharedstoreupdate']);
	},

	/**
	 * Function will be used to send {Grommunio.core.Actions#destroysession destroysession} requests to the server. This will be used when
	 ...
	 * other than CRUD so what we do is call {@link Grommunio.hierarchy.data.HierarchyProxy#request}
	 * method of {@link Grommunio.hierarchy.data.HierarchyProxy HierarchyProxy} with 'read' operation
	 * and {Grommunio.core.Actions#destroysession destroysession} as {@link #actionType} in options,
	...
	{
		this.sendSimpleActionToServer(Grommunio.core.Actions['destroysession']);
	},
	...
	{
		if (!this.state) {
			this.state = new Grommunio.hierarchy.data.HierarchyState();
		}
	},
	 ...
	 * This calls {@link Grommunio.hierarchy.data.HierarchyState#getStateNameForFolder getStateNameForFolder} on the {@link #state}.
	 *
	 ...
	 * This calls {@link Grommunio.hierarchy.data.HierarchyState#applyStateForFolder applyStateForFolder} on the {@link #state}.
	 ...
	 * @return {Grommunio.core.data.IPFRecord[]} An array of folders
	 */
...
Ext.reg('grommunio.hierarchystore', Grommunio.hierarchy.data.HierarchyStore);
 ...
 *
 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder in which the mail arrived
 * @return {Boolean} True when a notification should be shown for this folder
 * @static
 */
Grommunio.hierarchy.data.HierarchyStore.notifiesNewMail = function(folder)
{
	var settings = container.getSettingsModel();
	var scope = settings.get('grommunio/v1/main/notifier/info/newmail/folders/scope');
	...
	if (scope === 'selected') {
		var selected = settings.get('grommunio/v1/main/notifier/info/newmail/folders/selected');
		...
		for (var i = 0, len = folders.length; i < len; i++) {
			if (!Ext.isEmpty(folders[i]) && Grommunio.core.EntryId.compareEntryIds(folders[i], entryid)) {
				return true;
			}
		}
