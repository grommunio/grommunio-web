...
/**
 * @class Grommunio.hierarchy.data.IPFSubStore
 * @extends Grommunio.core.data.IPFStore
 * @xtype grommunio.ipfsubstore
 *
 * {@link Grommunio.hierarchy.data.IPFSubStore} holds {@link Grommunio.hierarchy.data.MAPIFolderRecords} as records, which defines store information
 * of all opened stores(own, shared, public)
 */
Grommunio.hierarchy.data.IPFSubStore = Ext.extend(Grommunio.core.data.IPFStore, {
	/**
	...
	constructor: function (config)
	{
		...
		Grommunio.hierarchy.data.IPFSubStore.superclass.constructor.call(this, config);
	...
	createRecords: function(store, record, index)
	{
		...
		Grommunio.hierarchy.data.IPFSubStore.superclass.createRecords.call(this, store, record, index);
	},
	...
	destroyRecord: function(store, record, index)
	{
		...
		Grommunio.hierarchy.data.IPFSubStore.superclass.destroyRecord.call(this, store, record, index);
	},
	...
	save: function()
	{
		...
		Grommunio.hierarchy.data.IPFSubStore.superclass.save.apply(this, arguments);
	},
	...
	onBeforeSave: function(store, data)
	{
		...
		if (!Ext.isEmpty(data['close'])) {
			...
			for (var i = 0, len = folders.length; i < len; i++) {
				...
				// reload.
				settings.remove('grommunio/v1/contexts/hierarchy/shared_stores/' + Grommunio.core.Util.bin2hex(user) + '/' + type);
...
Ext.reg('grommunio.ipfsubstore', Grommunio.hierarchy.data.IPFSubStore);
