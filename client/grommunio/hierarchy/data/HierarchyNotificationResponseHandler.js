...
/**
 * @class Grommunio.hierarchy.data.HierarchyNotificationResponseHandler
 ...
 */
Grommunio.hierarchy.data.HierarchyNotificationResponseHandler = Ext.extend(Grommunio.core.data.AbstractNotificationResponseHandler, {
	/**
	 * Handle the {@link Grommunio.core.Actions#folder 'folder'} action.
		...
		for (var i = 0, len = responseObj.data.length; i < len; i++) {
			...
			if (mapiFolderRecord) {
				...
				if (rawFolder.folderdelete) {
					this.addNotification(Grommunio.core.data.Notifications.objectDeleted, mapiFolderRecord, folder);
				} else {
					this.addNotification(Grommunio.core.data.Notifications.objectModified, mapiFolderRecord, folder);
				}
			} else if (!rawFolder.folderdelete) {
				this.addNotification(Grommunio.core.data.Notifications.objectCreated, null, folder);
			}
		}
	},

	/**
	 * Handle the {@link Grommunio.core.Actions#newmail 'newmail'} action.
	...
	doNewmail: function(data)
	{
		...
		this.addNotification(Grommunio.core.data.Notifications.newMail, folderStores, data);
	},

	/**
	 * Handle the stores action we receive as notification.
	 * This will obtain the list of {@link Grommunio.hierarchy.data.MAPIStoreRecord MAPIStoreRecord}
	 * which must be updated, and generate the Notification.
	 * @param {Object} data The entire response object which will be processed during this transaction.
	 * @private
	 */
	doStores: function(data)
	{
		...
		this.addNotification(Grommunio.core.data.Notifications.objectModified, mapiStoreRecord, stores);
	}
});
