...
/**
 * @class Grommunio.hierarchy.data.StoreOrder
 ...
 * {@link Grommunio.hierarchy.ui.TreeSorter}). This manager holds an explicit order for
 ...
 *
 * Stores are identified by their {@link Grommunio.hierarchy.data.MAPIStoreRecord#user_name
 ...
 * @singleton
 */
Grommunio.hierarchy.data.StoreOrder = Ext.extend(Ext.util.Observable, {
	/**
	 ...
	 * @type String
	 */
	settingsPath: 'grommunio/v1/contexts/hierarchy/store_order',
	...
	constructor: function()
	{
		this.addEvents(
			 ...
			 * @param {Grommunio.hierarchy.data.StoreOrder} storeOrder This object
			 * @param {String[]} order The new order as a list of {@link #getStoreKey store keys}
			 */
			'change'
		);

		Grommunio.hierarchy.data.StoreOrder.superclass.constructor.call(this);
	},

	/**
	 * Obtain the identifier under which the given store is recorded in the order.
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore The store to identify
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore The store to check
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} store1 The first store to compare
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} store2 The second store to compare
	 ...
	 * {@link Grommunio.hierarchy.ui.TreeSorter}, which pin them in an unfiltered tree
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore The store to rank
...
Grommunio.hierarchy.data.StoreOrder = new Grommunio.hierarchy.data.StoreOrder();
