...
/**
 * @class Grommunio.hierarchy.data.HierarchyTreeLoader
 * @extends Ext.tree.TreeLoader
 *
 * A Special treeloader to be used by the {@link Grommunio.hierarchy.ui.HierarchyTreePanel HierarchyTree}.
 ...
 */
Grommunio.hierarchy.data.HierarchyTreeLoader = Ext.extend(Ext.tree.TreeLoader, {
	/**
	 * @cfg {Grommunio.hierarchy.ui.Tree} tree The tree on which this treeloader
	 * is applied.
	 */
	tree: undefined,
	...
	constructor: function(config)
	{
		...
		Grommunio.hierarchy.data.HierarchyTreeLoader.superclass.constructor.call(this, config);
	 ...
	 * @param {Grommunio.core.data.HierarchyStore} store The store to bind
	...
	/**
	 * Event handler for the {@link Grommunio.hierarchy.data.HierarchyStore#load load} event.
	...
	/**
	 * Handles the 'remove' even from the {@link Grommunio.hierarchy.data.HierarchyStore hierarchyStore}.
	 * This will lookup any folder inside the tree which belongs to the given {@link Grommunio.hierarchy.data.MAPIStoreRecord},
	 * and removes it from the tree.
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The store which fired the event
	 * @param {Grommunio.core.data.IPFRecord} record The MAPIStoreRecord which was removed from the hierarchy
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord/Grommunio.hierarchy.data.MAPIFolderRecord[]} folder folders which are added in hierarchy.
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The store which fired the event
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore mapi store in which folders are updated.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} record folders which are updated in hierarchy.
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The store which fired the event
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} mapiStore mapi store in which folders are removed.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folders which are removed from hierarchy.
	 * @private
	 */
	 ...
	 * @param {Grommunio.common.favorites.data.FavoritesFolderRecord} record The favorite which was added
	 ...
	 * the {@link Grommunio.hierarchy.data.MAPIFolderRecord} to find the child nodes which
	...
	/**
	 * Obtain the list of nodes which are positioned below the given {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
	 * If a subfolder is {@link Grommunio.hierarchy.ui.Tree#nodeFilter filtered} the subfolder will not be added itself,
	 * but subfolder will be used for a recursive call to {@link #getFilteredChildNodes} to see if the sub-subfolders
	 * do match the filter.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder which is checked
	...
	/**
	 * Obtain the parent node for the given {@link Grommunio.hierarchy.data.MAPIFolderRecord folder},
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder which is being checked
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} base The root folder for the search, if
	...
	createNode: function(attr)
	{
		...
		if (folder) {
			...
			attr.uiProvider = Grommunio.hierarchy.ui.FolderNodeUI;
	...
	{
		this.bindStore(null);
		Grommunio.hierarchy.data.HierarchyTreeLoader.superclass.destroy.apply(this, arguments);
	}
});
