...
/**
 * @class Grommunio.core.ui.NavigationRail
 * @extends Object
 *
 ...
 */
Grommunio.core.ui.NavigationRail = Ext.extend(Object, {
	/**
	 * @cfg {Grommunio.core.ui.NavigationPanel} panel The navigation panel.
	 */
	panel: undefined,
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord[]} folders The folders to add
	 * @private
	 */
	addButtons: function(html, folders)
	{
		Ext.each(folders, function(folder) {
			...
			html.push('<button type="button" class="' + cls + '" data-index="' + this.folders.length + '"' + tip + ' aria-label="' + name + '">',
				'<span class="k-nav-rail-icon ' + Grommunio.common.ui.IconClass.getIconClass(folder) + '"' + tip + '></span>',
	...
	/**
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord[]} The favorite folders
	 * @private
	 */
	...
	/**
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord[]} The default folders of the current context
	 * @private
	 */
	...
	/**
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord[]} folders The folders to search
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder to find
	...
	contains: function(folders, folder)
	{
		return folders.some(function(other) {
			return Grommunio.core.EntryId.compareEntryIds(other.get('entryid'), folder.get('entryid'));
		});
	},

	/**
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord|Array} folders The selected folders
	 * @private
	 */
