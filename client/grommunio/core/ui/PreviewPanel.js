...
/**
 * @class Grommunio.core.ui.PreviewPanel
 * @extends Ext.Panel
 * @xtype grommunio.previewpanel
 *
 * Panel that shows a preview of a {@link Grommunio.core.data.IPMRecord message}.
 */
Grommunio.core.ui.PreviewPanel = Ext.extend(Ext.Panel, {
	// Insertion points for this class
	/**
	 * @insert previewpanel.toolbar.left
	 * Insertions point for inserting buttons in previewpane's toolbar on left side.
	 * @param {Grommunio.core.ui.PreviewPanel} panel This panel
	 * @param {Grommunio.core.ContextModel} model The contextmodel ({@link #model}).
	 */
	...
	/**
	 * @cfg {Grommunio.core.ContextModel} model The model for which this
	...
	/**
	 * @cfg {Grommunio.core.data.IPMRecord} record (See {@link Grommunio.core.plugins.RecordComponentPlugin#record}).
	 */
	record: undefined,
	...
	constructor: function(config)
	{
		...
		config.tbar = Ext.applyIf(config.tbar || {}, {
			cls: 'grommunio-previewpanel-toolbar',
		...
		config.plugins.push({
			...
			ptype: 'grommunio.enablefocusplugin'
		});

		if (container.getSettingsModel().get('grommunio/v1/contexts/mail/readflag_time_enable') === true) {
			config.plugins.push({
				ptype: 'grommunio.markasreadplugin'
			});
		}

		Grommunio.core.ui.PreviewPanel.superclass.constructor.call(this, config);
	 ...
	 * {@link Grommunio.core.KeyMapMgr KeyMapMgr}.
	 * @private
	 */
	afterRender: function()
	{
		Grommunio.core.ui.PreviewPanel.superclass.afterRender.apply(this, arguments);
		var xtypes = this.getXTypes();

		// The first part leading up to grommunio.previewpanel will be stripped
		xtypes = xtypes.replace('component/box/container/panel/grommunio.previewpanel','');

		// Then the "grommunio." will be stripped off from start, the xtype like "grommunio.previewpanel".
		xtypes = xtypes.replace(/\/grommunio\./g,'.');
		...
		Grommunio.core.KeyMapMgr.activate(this, xtypes);
	},
	...
	removeAll: function(autoDestroy)
	{
		...
		destroyedItems.concat(Grommunio.core.ui.PreviewPanel.superclass.removeAll.apply(this, arguments));

		return destroyedItems;
	},

	/**
	 * Event handler which is trigggerd when the {@link Grommunio.core.ContextModel ContextModel} receives a new
	 * record for previewing (it fired the {@link Grommunio.core.ContextModel#previewrecordchange previewrecordchange} event).
	 * This updates the {@link Ext.Container previewpanel} with the selected record to be previewed.
	 *
	 * @param {Grommunio.core.ContextModel} model The context model.
	...
	/**
	 * Event handler for the {@link Grommunio.core.plugins.RecordComponentPlugin#beforeloadrecord beforeloadrecord} event.
	...
	/**
	 * Event handler for the {@link Grommunio.core.plugins.RecordComponentPlugin#loadrecord loadrecord} event.
	...
	/**
	 * Event handler for the {@link Grommunio.core.plugins.RecordComponentPlugin#exceptionrecord} event.
	 ...
	 * the {@link Grommunio.core.ContextModel ContextModel} and displays into panel.
	 */
	...
	{
		this.cancelLoadMaskTask();
		Grommunio.core.ui.PreviewPanel.superclass.beforeDestroy.apply(this, arguments);
	},
	...
	{
		return 'preview/' + Grommunio.core.ui.PreviewPanel.superclass.getStateName.call(this);
	}
});
Ext.reg('grommunio.previewpanel', Grommunio.core.ui.PreviewPanel);
