...
/**
 * @class Grommunio.core.ui.RecordContentPanel
 * @extends Grommunio.core.ui.ContentPanel
 * @xtype grommunio.recordcontentpanel
 *
 * The {@link Grommunio.core.ui.RecordContentPanel RecordContentPanel} extends the normal {@link Grommunio.core.ui.ContentPanel ContentPanel},
 * and should be used for any content panels which contain an {@link Grommunio.core.data.MAPIRecord MAPIRecord}.
 *
 * FIXME: Provide default buttons which are common for all content panels containing a record
 */
Grommunio.core.ui.RecordContentPanel = Ext.extend(Grommunio.core.ui.ContentPanel, {
	/**
	 ...
	 * to instantiate the {@link Grommunio.core.plugins.RecordComponentPlugin RecordComponent}.
	 * See the plugin for the available configuration options.
	 */
	recordComponentPluginConfig: undefined,

	/**
	 * @cfg {Grommunio.core.data.MAPIRecord} record (See {@link Grommunio.core.plugins.RecordComponentPlugin#record}).
	 */
	record: undefined,

	/**
	 * When this panel is {@link Grommunio.core.ui.ContentPanel#isModal modal} then this property will contain the
	 * original {@link Grommunio.core.data.MAPIRecord record} which is displayed in this content panel. During
	...
	/**
	 * Reference to the {@link Grommunio.core.plugins.RecordComponentPlugin RecordComponent} plugin
	 ...
	 * {@link Grommunio.core.plugins.RecordComponentPlugin RecordComponent} itself.
	 *
	 * @property
	 * @type Grommunio.core.plugins.RecordComponentPlugin
	 */
	 ...
	 * maintaining an array of {@link Grommunio.core.data.MAPIRecord#actions message actions} that will be matched with message action
	 ...
	 * This option is only used when the {@link Grommunio.settings.SettingsModel setting} 'grommunio/v1/main/confirm_close_dialog'
	 * is enabled.
	 */
	confirmClose: false,
	 ...
	 * {@link Grommunio.core.data.MAPIStore MAPIStore} when user has not changed anything and closed
	 * the {@link Grommunio.core.data.UIFactoryLayer UIFactoryLayer}.
	 */
	...
	constructor: function(config)
	{
		...
		if (config.confirmClose && !container.getSettingsModel().get('grommunio/v1/main/confirm_close_dialog')) {
		...
		this.addEvents(
			 ...
			 * Fires when the record of this {@link Grommunio.core.ui.RecordContentPanel contentpanel} is about to
			 ...
			 * can return false to prevent the {@link Grommunio.core.data.MAPIRecord MAPIRecord} to be saved.
			 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The contentpanel to which the record belongs
			 * @param {Grommunio.core.data.MAPIRecord} record The record which is being saved
			 * @return {Boolean} false to cancel the save action
			 */
			'beforesaverecord',
			/**
			 * @event saverecord
			 * Fires when the record of this {@link Grommunio.core.ui.RecordContentPanel contentpanel} is being
			 ...
			 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The contentpanel from where the record is saved
			 * @param {Grommunio.core.data.IPMRecord} record The record which has been saved
			 */
			'aftersaverecord'
		);
		...
		Grommunio.core.ui.RecordContentPanel.superclass.constructor.call(this, config);
	...
	/**
	 * Initialize all {@link Grommunio.core.data.MAPIRecord record} related events
	 * for this {@link Grommunio.core.ui.RecordContentPanel contentpanel}.
	 * @private
	 */
	initEvents: function()
	{
	...
	/**
	 * See {@link Grommunio.core.plugins.RecordComponentPlugin#setRecord}.
	 *
	 ...
	 * @param {Grommunio.core.data.MAPIRecord} record The record that is being saved
	 */
	...
	saveRecord: function(storeSave)
	{
		...
		if (storeSave !== false) {
			...
			if (Grommunio.core.MessageClass.isClass(record.get('message_class'), 'IPM.Appointment', true)) {
				var subStoresChanged = false;
				if (record.subStores) {
					Ext.iterate(record.subStores, function(name, store) {
						if (store && (!Ext.isEmpty(store.modified) || !Ext.isEmpty(store.removed))) {
							subStoresChanged = true;
						}
					});
				}
				if (record.phantom) {
					container.getUndoManager().captureCreateGesture(record);
				} else if (!subStoresChanged) {
					// recipient/attachment changes live in substores which a
					// property command cannot restore
	...
	deleteRecord: function()
	{
		...
		if (this.recordComponentPlugin.isChangedByUser === true) {
			...
			Grommunio.common.Actions.deleteRecords(this.modalRecord || this.record);
		}
		return;
	},
	 ...
	 * If the user pressed 'yes' the record will be deleted by {@link Grommunio.common.Actions#deleteRecords}.
	 ...
	 * @param {Grommunio.core.data.MAPIRecord} record The record which was set
	 * @param {Grommunio.core.data.MAPIRecord} oldrecord The oldrecord which was previously set
	 */
	 ...
	 * for description.
	 * @param {Grommunio.core.data.MAPIRecord} record The record which was subject of the request
	...
	closeWrap: function()
	{
		...
		Grommunio.core.ui.RecordContentPanel.superclass.closeWrap.apply(this, arguments);
	},

	/**
	 * Function is used to remove {@link Grommunio.core.data.MAPIRecord MAPIRecord} from
	 * {@link Grommunio.core.data.MAPIStore MAPIStore} when {@link #removeRecordOnCancel}
	...
	/**
	 * Event handler will be called when the {@link Grommunio.core.plugins.RecordComponentPlugin RecordComponentPlugin}
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record Store's record, on which write event is fired
	 * @protected
	 */
	 ...
	 * {@link Grommunio.core.data.MAPIRecord#set} treats these as different
	...
	close: function()
	{
		...
		Grommunio.core.ui.RecordContentPanel.superclass.close.call(this);
	},
	...
	doClose: function()
	{
		...
		Grommunio.core.ui.RecordContentPanel.superclass.doClose.call(this);
	},
	 ...
	 *
	 * @return {Grommunio.common.dialogs.CustomMessageBox}
	 * @private
	 */
	showCloseConfirmationDialog: function()
	{
		if (!(Grommunio.common && Grommunio.common.dialogs && Grommunio.common.dialogs.CustomMessageBox)) {
		...
		this.closeConfirmDialog = new Grommunio.common.dialogs.CustomMessageBox({
			title: _('Unsaved changes'),
			msg: confirmMsg,
			icon: '',
			contentPanel: this,
...
Ext.reg('grommunio.recordcontentpanel', Grommunio.core.ui.RecordContentPanel);
