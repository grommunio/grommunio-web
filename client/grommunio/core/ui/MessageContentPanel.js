...
/**
 * @class Grommunio.core.ui.MessageContentPanel
 * @extends Grommunio.core.ui.RecordContentPanel
 * @xtype grommunio.messagecontentpanel
 *
 * This extends the normal {@link Grommunio.core.ui.RecordContentPanel RecordContentPanel} which is
 * used for displaying and editing {@link Grommunio.core.data.IPMRecord records}. This
 ...
 */
Grommunio.core.ui.MessageContentPanel = Ext.extend(Grommunio.core.ui.RecordContentPanel, {

	/**
	 * The queue containing callback functions which can be used to validate the
	 * {@link Grommunio.core.data.IPMRecord} which is about to be send. If the queue
	...
	constructor: function(config)
	{
		...
		config.plugins.push({
			ptype: 'grommunio.markasreadplugin',
		...
		this.addEvents(
			 ...
			 * This will allow Subclasses modify the {@link Grommunio.core.data.IPMRecord record} before
			 ...
			 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The contentpanel from where the record is send
			 ...
			 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The contentpanel from where the record is send
			 * @param {Grommunio.core.data.IPMRecord} record The record which has been send
			 */
			'sendrecord',
			/**
		...
		Grommunio.core.ui.MessageContentPanel.superclass.constructor.call(this, config);
	 ...
	 * When any record which belongs to {@link Grommunio.core.data.IPMStore} gets updated, respective
	 * {@link Grommunio.core.data.ShadowStore ShadowStore} record needs to be updated, if any.
	 *
	 * @param {Grommunio.core.data.IPMStore} store The store which performs the update
	...
	{
		if (Grommunio.core.EntryId.compareEntryIds(record.get('entryid'), this.record.get('entryid'))) {
	...
	createSendValidationQueue: function()
	{
		...
		this.sendValidationQueue = new Grommunio.core.data.CallbackQueue();
	...
	displayInfoMask: function()
	{
		...
		if (this.record.hasMessageAction('send') || this.record.getMessageAction('sendResponse')) {
			...
			Grommunio.core.ui.MessageContentPanel.superclass.displayInfoMask.apply(this, arguments);
		}
	...
	hideInfoMask: function(success)
	{
		...
		if (this.record.hasMessageAction('send') || this.record.getMessageAction('sendResponse')) {
			...
			Grommunio.core.ui.MessageContentPanel.superclass.hideInfoMask.apply(this, arguments);
		}
	},
	...
	onBeforeSaveRecord: function(store, data)
	{
		...
		Grommunio.core.ui.MessageContentPanel.superclass.onBeforeSaveRecord.apply(this, arguments);
	},
	 ...
	 *
	 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The contentpanel which fired the event
	...
	{
		Grommunio.core.ui.MessageContentPanel.superclass.onUpdateRecord.apply(this, arguments);
	...
	{
		Grommunio.core.ui.MessageContentPanel.superclass.onExceptionRecord.apply(this, arguments);
	 ...
	 * immediately one after another. Function registers {@link Grommunio.core.data.MAPIStore#write}
	 * event which will call {@link #sendRecord} function when saving has been completed.
	 *
	 * This function is use to solve below problem.
	 *
	 * If we call {@link #sendRecord} straight away after {@link Grommunio.core.ui.RecordContentPanel #aftersaverecord} event then
	 * {@link Ext.data.Store #write} event is triggered on {@link Grommunio.core.data.ShadowStore ShadowStore} for this record,
	 * It will call {@link Grommunio.core.data.MAPIRecord.clearMessageActions clearMessageActions}(i.e. send=true),
	...
	validateEmptyRecipients: function(callback)
	{
		...
		if (!Ext.isDefined(recipientStore) || recipientStore.getCount() === 0) {
			...
			recipientStore.each(function(recip) {
				if (recip.get('recipient_type') !== Grommunio.core.mapi.RecipientType.MAPI_ORIG) {
					invalid = false;
					return false;
				}
			}, this);
	...
	validateMissingAttachment: function(callback)
	{
		...
		if (container.getSettingsModel().get('grommunio/v1/contexts/mail/attachment_reminder_enable') !== true) {
			callback(true);
			return;
		}
	 ...
	 * {@link Grommunio.core.data.IPMRecord#addMessageAction 'send' message action} and will then
...
Ext.reg('grommunio.messagecontentpanel', Grommunio.core.ui.MessageContentPanel);
