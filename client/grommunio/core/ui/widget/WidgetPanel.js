...
/**
 * @class Grommunio.core.ui.widget.WidgetPanel
 * @extends Grommunio.core.ui.MainViewSidebar
 * @xtype grommunio.widgetpanel
 *
 * The main panel which contains {@link Grommunio.core.ui.widget.Widget widgets}.
 */
Grommunio.core.ui.widget.WidgetPanel = Ext.extend(Grommunio.core.ui.MainViewSidebar, {
	/**
	...
	constructor: function(config)
	{
		...
		var tools = [{
			...
			scope: this,
			handler: function() {
				Grommunio.common.Actions.openWidgetsContent({
					widgetPanel: this,
		...
		Grommunio.core.ui.widget.WidgetPanel.superclass.constructor.call(this, config);
	...
	/**
	 * Event handler which is fired when a {@link Grommunio.core.ui.widget.Widget widget} is
	 * being dropped on this panel. This will {@link #removeGuid unregister} the widget
	 * from the previous {@link Grommunio.core.ui.widget.WidgetPanel owner} and
	...
	loadWidgets: function()
	{
		...
		if(!Ext.isDefined(guids)) {
			...
			Ext.each(guids.split(/\W+/), function(guid) {
				...
				var widgetName = settings.get(Grommunio.core.ui.widget.Widget.settingsPath(guid, '__name'));
				...
				if (widgetMetaData) {
					...
					settings.remove(Grommunio.core.ui.widget.Widget.settingsPath(guid, ''));
				}
			}, this);
	...
	createWidget: function(name, column)
	{
		...
		if (widgetMetaData) {
			var guid = Grommunio.generateId(32);
	 ...
	 * @param {Grommunio.core.ui.widget.Widget} widget The widget to destroy
	 * @private
	 */
	destroyWidget: function(widget)
	{
		...
		// Remove settings.
		container.getSettingsModel().remove(Grommunio.core.ui.widget.Widget.settingsPath(widget.guid, ''));

		widget.ownerCt.remove(widget, true);
	}
});

Ext.reg('grommunio.widgetpanel', Grommunio.core.ui.widget.WidgetPanel);
