...
/**
 * @class Grommunio.core.ui.CommandPalette
 * @extends Ext.Window
 * @xtype grommunio.commandpalette
 *
 * A search box over the views, folders, settings pages and the "new item"
 * actions of the main toolbar, opened with Ctrl+K.
 */
Grommunio.core.ui.CommandPalette = Ext.extend(Ext.Window, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.core.ui.CommandPalette.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.core.ui.CommandPalette.superclass.afterRender.apply(this, arguments);
	...
	onDestroy: function()
	{
		...
		Grommunio.core.ui.CommandPalette.superclass.onDestroy.apply(this, arguments);
	},
	...
	afterShow: function()
	{
		Grommunio.core.ui.CommandPalette.superclass.afterShow.apply(this, arguments);
	...
	{
		var tabbar = Ext.getCmp('grommunio-mainmenu');
		if (!tabbar) {
			return;
		}

		tabbar.items.each(function(tab) {
			if (!(tab instanceof Grommunio.core.ui.MainTab) || tab.hidden || !tab.context) {
				return;
			}
			...
			var iconCls = this.viewIcons[tab.context] || (folder ? Grommunio.common.ui.IconClass.getIconClass(folder) : '');
			commands.push({
				group: _('Views'),
				text: tab.text,
				iconCls: iconCls,
	...
	{
		var items = container.populateInsertionPoint('main.maintoolbar.new.item', Ext.getCmp('grommunio-maintoolbar')) || [];
		items = Grommunio.core.Util.sortArray(items, 'ASC', 'newMenuIndex');
		items = Grommunio.core.Util.uniqueArray(items, 'text');
	...
	addSettings: function(commands)
	{
		...
		var tabs = Ext.ComponentMgr.all.filterBy(function(cmp) {
			return cmp instanceof Grommunio.settings.ui.SettingsCategoryTab;
		});
		var panel = Ext.ComponentMgr.all.filterBy(function(cmp) {
			return cmp instanceof Grommunio.settings.ui.SettingsCategoryWidgetPanel;
		}).first();
	...
	addTools: function(commands)
	{
		...
		var dark = Grommunio.core.DarkMode.isDark();
		commands.push({
			group: _('Appearance'),
			text: dark ? _('Switch to light mode') : _('Switch to dark mode'),
			run: function() {
				Grommunio.core.DarkMode.setMode(dark ? 'light' : 'dark');
			}
		});
		...
		commands.push({
			...
			run: function() {
				container.getSettingsModel().set('grommunio/v1/main/density', compact ? 'comfortable' : 'compact');
			}
		});
 ...
 * @static
 */
Grommunio.core.ui.CommandPalette.toggle = function()
{
	var current = Grommunio.core.ui.CommandPalette.current;
	if (current) {
		current.close();
		return;
	}

	current = Grommunio.core.ui.CommandPalette.current = new Grommunio.core.ui.CommandPalette();
	current.on('destroy', function() {
		Grommunio.core.ui.CommandPalette.current = undefined;
	});
	current.show();
};

Ext.reg('grommunio.commandpalette', Grommunio.core.ui.CommandPalette);
