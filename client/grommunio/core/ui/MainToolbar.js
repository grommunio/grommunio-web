...
/**
 * @class Grommunio.core.ui.MainToolbar
 * @extends Grommunio.core.ui.Toolbar
 * @xtype grommunio.maintoolbar
 */
Grommunio.core.ui.MainToolbar = Ext.extend(Grommunio.core.ui.Toolbar, {
	 ...
	 * @param {Grommunio.core.ui.MainToolbar} toolbar This toolbar
	 */
	/**
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			xtype: 'grommunio.maintoolbar',
			id: 'grommunio-maintoolbar',
			cls: 'grommunio-maintoolbar',
			ariaLabel: _('Main toolbar')
		});

		Grommunio.core.ui.MainToolbar.superclass.constructor.call(this, config);
	 ...
	 * done when the contexts are switched by the {@link Grommunio.core.Container Container}.
	 * @private
	 */
	addNewItems: function()
	{
		...
		itemMenu = Grommunio.core.Util.sortArray(itemMenu, 'ASC', 'newMenuIndex');

		// Remove doubles
		itemMenu = Grommunio.core.Util.uniqueArray(itemMenu, 'text');
		...
		if (itemMenu.length !== 0) {
			...
			menu.push({
				xtype: 'menuitem',
				id: 'grommunio-maintoolbar-newitem-defaultbutton',
		...
		this.addItems({
			xtype: 'splitbutton',
			id: 'grommunio-maintoolbar-newitem',
	 ...
	 * {@link Grommunio.core.Container Container}. This event handler will set the correct icon and
	...
	/**
	 * Fired when the {@link Grommunio.core.Container container} fires the {@link Grommunio.core.Container#contextswitch} event.
		...
		for(var i = 0,len = contexts.length; i < len; i++){
			...
			if (Array.isArray(menu) && menu.length) {
				if (context.groupViewBtns) {
					menuItems.push({
						xtype: 'splitbutton',
						id: 'grommunio-maintoolbar-view-'+context.getName(),
						scale: 'large',
						iconCls: 'icon_large_view',
						tooltip: _('Switch view') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Alt + 1..9', false),
						hidden: true,
						contextName: context.getName(),
						menu: Grommunio.core.Util.uniqueArray(menu, 'text'),
						listeners: {
					...
					for (var j = 0, jlen = menu.length; j < jlen; j++) {
						...
						var viewCounter = j + 1;
						item.tooltip = item.text + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Alt + '+ viewCounter, false);
						delete item.text;

						item.listeners = Grommunio.core.Util.mergeListeners(item.listeners, {
							render: this.onRenderViewButton
						});
	 ...
	 * view menu by checking {@link Grommunio.core.Context#current_view_mode view mode}
	 * of menu item and for sticky note switch view menu we use
	 * {@link Grommunio.core.Context#current_view view} to highlight the
	 * menu item.
	 *
	 ...
	 * @param {Grommunio.core.Context} context which contains the switch view split button.
	 */
	 ...
	 * {@link Grommunio.core.Container Container}. This event handler will enable or disable the
	 ...
	 addPrintButton: function()
	 {
		 ...
		 for(var i=0,len=contexts.length; i<len; i++){
			...
			if(Array.isArray(menu) && menu.length){
				menuItems.push({
					xtype: 'splitbutton',
					id: 'grommunio-maintoolbar-print-'+context.getName(),
					name: 'defaultPrintBtn',
					scale: 'large',
					iconCls: 'icon_large_print',
					tooltip: _('Print') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + P', true),
					...
					menu: {
						...
						items: Grommunio.core.Util.uniqueArray(menu, 'text'),
	...
	addActionItems: function()
	{
		...
		// The undo/redo feature is opt-in, see Grommunio.core.data.UndoManager#enabled.
		if (container.getSettingsModel().get('grommunio/v1/main/undo_redo/enable') === true) {
			menuItems.push({
				xtype: 'splitbutton',
				id: 'grommunio-maintoolbar-undo',
				scale: 'large',
				overflowText: _('Undo'),
				tooltip: _('Undo') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Z', true),
				...
				xtype: 'button',
				id: 'grommunio-maintoolbar-redo',
				scale: 'large',
				overflowText: _('Redo'),
				tooltip: _('Redo') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Y', true),
		...
		menuItems.push({
			xtype: 'button',
			id: 'grommunio-maintoolbar-addressbook',
			scale: 'large',
			...
			xtype: 'button',
			id: 'grommunio-maintoolbar-refresh',
			scale: 'large',
			title: _('Refresh'),
	...
	/**
	 * Open the {@link Grommunio.addressbook.dialogs.AddressBookContentPanel AddressBookContentPanel}
	 * @private
	 */
	onAddressBook: function()
	{
		Grommunio.addressbook.Actions.openAddressBook();
	},

	/**
	 * Undo the most recent action via the {@link Grommunio.core.data.UndoManager UndoManager}.
	 * @private
	 */
	onUndo: function()
	{
	...
	/**
	 * Redo the most recently undone action via the {@link Grommunio.core.data.UndoManager UndoManager}.
	 * @private
	 */
	onRedo: function()
	{
	 ...
	 * (once) for the {@link Grommunio.core.data.UndoManager#stackchange} event
	...
	/**
	 * Event handler for the {@link Grommunio.core.data.UndoManager#stackchange}
	 ...
	 * @param {Grommunio.core.data.UndoManager} undoManager The undo manager
	 * @private
	 */
	onUndoStackChange: function(undoManager)
	{
		var undoHint = Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Z', true);
		var redoHint = Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + Y', true);
	...
	onUndoMenuItemDeactivate: function(item)
	{
		...
		item.parentMenu.items.each(function(other) {
			if (other.container) {
				other.container.removeClass('grommunio-undo-item-included');
			}
		});
	},
	...
	/**
	 * This will Refresh the view and fire {@link Grommunio.core.data.ListModuleStore#reload}
	 * @private
	 */
	onRefresh: function()
	{
	...
	onPrint: function()
	{
		...
		if (context) {
			Grommunio.common.Actions.openPrintDialog(context);
		}
	},

	/**
	 * Adds an event handler to the Refresh button that will register the contextswitch event of the
	 * {@link Grommunio.core.Container Container}. This event handler will hide or unhide the
...
Ext.reg('grommunio.maintoolbar', Grommunio.core.ui.MainToolbar);
