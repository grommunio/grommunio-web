...
/**
 * @class Grommunio.core.ui.NavigationPanel
 * @extends Grommunio.core.ui.MainViewSidebar
 * @xtype grommunio.navigationpanel
 *
 * NavigationPanel provides basic navigation through {@link Grommunio.hierarchy.ui.Tree} and
 * {@link Grommunio.core.ui.NavigationButtonPanel} and other components which are provided by different
 * contexts. E.g. DatePicker provided by CalendarContext
 */
Grommunio.core.ui.NavigationPanel = Ext.extend(Grommunio.core.ui.MainViewSidebar, {
	 ...
	 * @param {Grommunio.core.ui.NavigationPanel} panel This panel
	 */
	/**
	 * @insert navigation.center
	 * Insertion point for adding a {@link Grommunio.core.ui.ContextNavigationPanel ContextNavigationPanel}
	 * to the {@link #centerPanel centerPanel}. This hierarchy panel will be shown when the Context it
	 * is related to is active. For that to work the {@link Grommunio.core.ui.ContextNavigationPanel#context context}
	 ...
	 * the Context-related {@link Grommunio.core.ui.ContextNavigationPanel ContextNavigationPanel} will
	 * be shown.
	 * @property
	 * @type Boolean
	 */
	...
	constructor: function (config)
	{
		...
		items.push({
				xtype: 'container',
				ref: 'centerPanel',
				cls: 'grommunio-navigationpanel-centerpanel',
				flex: 1,
				layout: {
					type: 'card',
					deferredRender: true
		...
		for(var i=0,len=items.length; i<len; i++){
			...
			if(Ext.isEmpty(items[i].cls)){
				items[i].cls = 'grommunio-navigationpanel-item';
			} else {
				items[i].cls += ' grommunio-navigationpanel-item';
			}
		}
		...
		Ext.applyIf(config, {
			...
			id: 'grommunio-navigationpanel',
			cls: 'grommunio-navigation grommunio-panel grommunio-context-mainpanel',
			...
			animCollapse: false,
			headerCfg: { cls: 'grommunio-main-header x-panel-header' }
		});

		// parent constructor
		Grommunio.core.ui.NavigationPanel.superclass.constructor.call(this, config);

		// If previous value is there in state to "show all the folders" then simply assign it to respective config
		// TODO: This is achieved by overriding the applyState in Grommunio.hierarchy.ui.HierarchyTreePanel class.
		// But, the instance of Grommunio.core.ui.NavigationPanel is not created when applyState gets executed.
		var checkBoxState = container.getSettingsModel().get('grommunio/v1/state/sidebars/hierarchytree/showallcheckbox');
		this.showFolderList = checkBoxState || false;
	},

	/**
	 * @return {Ext.Container} Container which contains {@link Grommunio.hierarchy.ui.FolderSelectTree HierarchyTree}
	 * component and {@link Grommunio.hierarchy.ui.OpenSharedStorePanel OpenSharedStorePanel} component.
	 */
	getAllFoldersPanel: function()
	{
		return {
			...
			cls: 'grommunio-navigationpanel-centerpanel-allfolders',
			ref: 'allFoldersPanel',
			items: [{
				layout: 'fit',
	...
	/**
	 * @return {Grommunio.hierarchy.ui.Tree} Tree component showing folder list
	 */
	 ...
	 * in {@link Grommunio.hierarchy.ui.HierarchyTreePanel HierarchyTreePanel} of currently active panel in {@link #centerPanel centerPanel}.
	 * 
	 * @param {Boolean} isContextSwitched true if the context is switched else false.
	 ...
	 * {@link Grommunio.core.ui.ContextNavigationPanel ContextNavigationPanel} will be shown. If the
	...
	initEvents: function()
	{
		...
		Grommunio.core.ui.NavigationPanel.superclass.initEvents.apply(this, arguments);
	},
	...
	saveState: function()
	{
		if (container.getCurrentContext().getName() !== "settings") {
			Grommunio.core.ui.NavigationPanel.superclass.saveState.apply(this);
		}
	}
});
Ext.reg('grommunio.navigationpanel', Grommunio.core.ui.NavigationPanel);
