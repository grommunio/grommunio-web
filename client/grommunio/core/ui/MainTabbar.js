...
/**
 * @class Grommunio.core.ui.MainTabBar
 * @extends Ext.Toolbar
 * @xtype grommunio.maintabbar
 *
 ...
 * {@link Grommunio.core.ui.MainTab MainTab} to represent the tabs.
 */
Grommunio.core.ui.MainTabBar = Ext.extend(Ext.Toolbar, {
	 ...
	 * added.
	 * @param {Grommunio.core.ui.MainTabBar} toolbar This toolbar
	 */
	/**
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			xtype: 'grommunio.maintabbar',
			id: 'grommunio-mainmenu',
			cls: 'grommunio-maintabbar',
			defaultType: 'grommunio.maintab',
			ariaLabel: _('Main navigation')
		});

		Grommunio.core.ui.MainTabBar.superclass.constructor.call(this, config);
	...
	initBar: function()
	{
		...
		leftItems = Grommunio.core.Util.sortArray(leftItems, 'ASC', 'tabOrderIndex');
		// The right items are sorted so that the first item appears to the most right
		rightItems = Grommunio.core.Util.sortArray(rightItems, 'DESC', 'tabOrderIndex');
		...
		var loginText = {
				xtype: 'tbtext',
				width: 'auto',
				cls: 'grommunio-maintabbar-logintext',
		...
		var reminder = {
			...
			handler: function() {
				...
				Grommunio.common.Actions.openReminderContent(store.getRange());
			},
			listeners: {
				afterRender: function(reminderBtn) {
					var store = container.getReminderStore();
					var recordLength = store.getRange().length;
					reminderBtn.getEl().setStyle('backgroundImage', 'url(\'' + Grommunio.common.ui.IconClass.getReminderSvgIcon( recordLength ) + '\')');
			...
			style: {
				backgroundImage: 'url(\'' + Grommunio.common.ui.IconClass.getReminderSvgIcon() + '\')',
		...
		if ( !(container.getServerConfig().usingSSO() || !container.getServerConfig().showLogoutButton()) || Grommunio.isDeskApp ){
			var logoutButton = {
				text: _('Logout'),
	...
	{
		var currentMode = Grommunio.core.DarkMode.getMode();
		return {
			width: 30,
			id: 'mainmenu-button-darkmode',
			tooltip: this.getDarkModeTooltip(currentMode),
			handler: function() {
				var mode = Grommunio.core.DarkMode.getMode();
				...
				Grommunio.core.DarkMode.setMode(mode);
			},
			style: {
	...
	addTooltip: function(leftItems, rightItems)
	{
		...
		Ext.each(contextItems, function(context, index) {
			context.tooltip = context.text + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + '+index, false);
		});
	},
...
Ext.reg('grommunio.maintabbar', Grommunio.core.ui.MainTabBar);
