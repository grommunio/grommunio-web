...
/**
 * @class Grommunio.core.KeyMapMgr
 * @extends Object
 *
 * The {@link Grommunio.core.KeyMap KeyMap} manager.
 * @singleton
 */
Grommunio.core.KeyMapMgr = Ext.extend(Object, {
	/**
	 * The list of registered {@link Grommunio.core.KeyMap keymaps}. It contains the list with the element the
	...
	/**
	 * The list of key configurations (see {@link Grommunio.core.KeyMap#addBinding Grommunio.core.KeyMap.addBinding})
	 ...
	 * @param {Object|Array} keyConfig The config (see {@link Grommunio.core.KeyMap#addBinding addBinding}).
	 */
	...
	/**
	 * Function will register all key bindings that are registered in {@link Grommunio.core.KeyMapMgr this}
	 ...
	 * to the same {@link Grommunio.core.KeyMap KeyMap}.
	 *
	 ...
	 * will be registered (eg Grommunio.common.ui.HTMLEditor).
	 */
	activate: function(component, mapId, element)
	{
		...
		var setting = container.getSettingsModel().get('grommunio/v1/main/keycontrols');

		// Filter basic shortcuts
		if (setting === Grommunio.settings.data.KeyboardSettings.BASIC_KEYBOARD_SHORTCUTS) {
			bindings = bindings.filter(function(binding) {
				return Ext.isDefined(binding.basic);
			});
		} else if (setting === Grommunio.settings.data.KeyboardSettings.NO_KEYBOARD_SHORTCUTS) {
			bindings = [];
		}
		...
		if(keymap) {
			...
			this.keyMaps.add(elementId, new Grommunio.core.KeyMap(component, bindings, element));
		}
	},
	...
	/**
	 * Handler function that will be called when any component registered with {@link Grommunio.core.KeyMapMgr KeyMapMgr}
	...
	/**
	 * Disables the {@link Grommunio.core.KeyMap keymap} and removes it from list of {@link Grommunio.core.KeyMapMgr#keyMaps}.
	 * {@link Grommunio.core.KeyMap#disable} removes event listener which is used to check and fire handler events for bindings
	 * but it doesn't remove bindings registered with the {@link Grommunio.core.KeyMap keymap} that will be done only when
	...
	/**
	 * Function returns {@link Grommunio.core.KeyMap keymap} bound to component or element.
	 * @param {Ext.Element} element The element which should be used to check for bound keymaps.
	 * @return {Grommunio.core.KeyMap} keymap object registered on component/element
	 */
	...
	/**
	 * Will enable the {@link Grommunio.core.KeyMap keymap} that is registered on the specified element.
	 * @param {Ext.Element} element The element on which keymap is bound.
	 */
	...
	/**
	 * Will disable the {@link Grommunio.core.KeyMap keymap} that is registered on the specified element.
	 * @param {Ext.Element} element The element on which keymap is bound.
	 */
	...
	/**
	 * Checks if the {@link Grommunio.core.KeyMap keymap} is enabled on the specified element.
	 * @param {Ext.Element} element The element on which keymap is bound.
	...
	{
		return container.getSettingsModel().get('grommunio/v1/main/keycontrols') !== Grommunio.settings.data.KeyboardSettings.NO_KEYBOARD_SHORTCUTS;
	},
...
Grommunio.core.KeyMapMgr = new Grommunio.core.KeyMapMgr();
