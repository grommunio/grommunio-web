...
/**
 * @class Grommunio.core.ResponseRouter
 * @extends Ext.util.Observable
 *
 * The router for Responses to requests made by the {@link Grommunio.core.Request Request} object.
 * Each response is delivered to its destination {@link Ext.data.Store store} through the
 * {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler}.
 * Upon receiving a response, the ResponseRouter will determine if it is a response to
 * a direct request from the {@link Grommunio.core.Request Request Object} or a notification
 * generated by the PHP-side.
 * If the response came from a request from the {@link Grommunio.core.Request Request Object} a
 * {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler} will have been registered
 * by the {@link Ext.data.DataProxy Proxy} which made the request. If the
 * {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler} is not available the
 * response is considered a Notification, in which case the
 * {@link Grommunio.core.data.NotificationResolver NotificationResolver} is used to generate
 * a special {@link Grommunio.core.data.AbstractNotificationResponseHandler ResponseHandler} which can
 * update the {@link Ext.data.Store stores} which contain the updated {@link Ext.data.Record records}.
 */
Grommunio.core.ResponseRouter = Ext.extend(Ext.util.Observable, {
	/**
	 * The collection of {@link Grommunio.core.data.AbstractResponseHandler ResponseHandlers}
	...
	constructor: function(config)
	{
		...
		Grommunio.core.ResponseRouter.superclass.constructor.call(this, config);
	},

	/**
	 * Register a {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler} to the
	 ...
	 * must be registered.
	 * @param {Grommunio.core.data.AbstractResponseHandler} handler The ResponseHandler
	...
	/**
	 * Get a {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler} from
	 ...
	 * @param {String} id The unique request identifier on which the ResponseHandler
	 * could be registered.
	 * @return {Grommunio.core.data.AbstractResponseHandler} The registered response handler.
	 * @private
	 */
	...
	/**
	 * Removes a {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler} from the
	 * {@link Grommunio.core.ResponsRouter ResponsRouter} to prevent it being called twice.
	 * @param {String} id The unique request identifier of the ResponseHandler which will be
	 * deregistered from {@link Grommunio.core.ResponsRouter ResponsRouter}.
	 */
	...
	/**
	 * Get a {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler} from
	 * the {@link Grommunio.core.data.NotificationResolver NotificationResolver}. This
	 * will construct a special {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler}
	 ...
	 * {@link Grommunio.core.data.AbstractResponseHandlers ResponseHandlers} which
	 * are affected by this error to report the error using:
	 * {@link Grommunio.core.data.AbstractResponseHandlers#responseFailure responseFailure}.
	 * @param {Object} requestdata The request data which was send to the server.
	 * @param {Object} xmlHttpRequest The raw browser response object.
	 */
	receiveFailure: function(requestData, xmlHttpRequest)
	{
		...
		Ext.iterate(requestData.grommunio, function(moduleName, modules) {
	...
	/**
	 * Resolve all response data into a collection of {@link Grommunio.core.data.AbstractResponseHandlers}
	...
	/**
	 * Perform a transaction through the {@link Grommunio.core.data.AbstractResponseHandler ResponseHandler}.
	 *
	 * @param {Grommunio.core.data.AbstractResponseHandler} handler The handler which should be used
	...
	processResponse: function(jsonData)
	{
		...
		if (!Ext.isEmpty(jsonData.grommunio.error)) {
			// Fire the exception event on the DataProxy like this, as the response cannot be matched to a specific proxy.
			Ext.data.DataProxy.fireEvent('exception', Ext.data.DataProxy, 'remote', null, null, jsonData.grommunio, null);
			return;
		}
		...
		var handlers = this.resolveResponseHandlers(jsonData.grommunio);
