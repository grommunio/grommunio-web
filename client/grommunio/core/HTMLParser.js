...
/**
 * @class Grommunio.core.HTMLParser
 *
 * Class for performing operations on HTML content.
 *
 * @singleton
 */
Grommunio.core.HTMLParser = (function() {
	...
	return {
		...
		convertPlainToHTML: function(content)
		{
			...
			content = Grommunio.core.HTMLParser.entityEncode(content, 'ENT_NOQUOTES');
			...
			content = Grommunio.core.HTMLParser.nl2br(content);
		...
		convertHTMLToPlain: function(content)
		{
			...
			// remove style tags
			content = Grommunio.core.HTMLParser.stripStyles(content);

			// remove script tags
			content = Grommunio.core.HTMLParser.stripScripts(content);
			...
			content = Grommunio.core.HTMLParser.stripUnwantedTags(content, ['br']);

			// decode html entities
			content = Grommunio.core.HTMLParser.entityDecode(content);
			...
			// convert all breaklines
			content = Grommunio.core.HTMLParser.br2nl(content);
		...
		*/
		...
		{
			var hashMap = Grommunio.core.HTMLParser.getHTMLTranslationTable('HTML_ENTITIES', quoteStyle);
			var symbol = '';
			var entity = '';
		 ...
		 */
		inlineImgOutlookToGrommunio: function(body, storeEntryId, entryId, attachNum)
		{
		...
		/**
		 * Function to convert from Grommunio inline img format to Outlook format (src="cid:...")
		...
		inlineImgGrommunioToOutlook: function(body)
		{
			...
			var urlToCid = function(match, srcStart, imgCid, srcEnd, offset, str) {
				if(imgCid) {
					// undo the encoding of inlineImgOutlookToGrommunio, but never let the value end the attribute
					var cid = imgCid;
					try {
