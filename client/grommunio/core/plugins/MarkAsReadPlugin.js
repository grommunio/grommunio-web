...
/**
 * @class Grommunio.core.plugins.MarkAsReadPlugin
 * @extends Object
 * @ptype grommunio.markasreadplugin
 *
 * Special plugin which must be used in combination with the
 * {@link Grommunio.core.plugins.RecordComponentPlugin}. This will
 * mark the message as read after a certain time.
 */
Grommunio.core.plugins.MarkAsReadPlugin = Ext.extend(Object, {
	/**
	...
	/**
	 * Event handler for the {@link Grommunio.core.plugins.RecordComponentPlugin#setrecord setrecord} event
	 ...
	 * @param {Grommunio.core.data.MAPIRecord} record The new record which is being set
	 * @param {Grommunio.core.data.MAPIRecord} oldrecord The old record which was previously set
	 * @private
	 */
	...
	/**
	 * Event handler for the {@link Grommunio.core.plugins.RecordComponentPlugin#beforesetrecord beforesetrecord} event
	 * on the {@link #field}. This will get the original record because it is {@link Ext.data.Record#copy copied} within
	 * {@link Grommunio.core.plugins.RecordComponentPlugin#setRecord setRecord} method to start shadow editing.
	 * @param {Ext.Component} field The component which fired the event
	 * @param {Grommunio.core.data.MAPIRecord} record The new record which is being set
	 * @param {Grommunio.core.data.MAPIRecord} oldrecord The old record which was previously set
	 * @private
	...
	/**
	 * Called when {@link Grommunio.mail.ui.MailGrid#store store} is loading new data.
	 * This will {@link Grommunio.core.plugins.MarkAsReadPlugin#resetReadFlagTimer reset}
	 * the {@link Grommunio.core.plugins.MarkAsReadPlugin#readFlagTimer}.
	 *
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.mail.ui.MailGrid#store store} fires the
	 * remove event. This will {@link Grommunio.core.plugins.MarkAsReadPlugin#resetReadFlagTimer reset}
	 * the {@link Grommunio.core.plugins.MarkAsReadPlugin#readFlagTimer} if
	 * {@link Grommunio.core.plugins.MarkAsReadPlugin#record record} is removed
	 * from {@link Grommunio.mail.ui.MailGrid#store store}
	 *
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The store which fired the event
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} record The record which was deleted
	 * @private
	 */
	...
	/**
	 * Event handler for the {@link Grommunio.core.plugins.RecordComponentPlugin#beforeloadrecord beforeloadrecord}
	 ...
	 * @param {Ext.Component} field The component which fired the event
	 * @param {Grommunio.core.data.MAPIRecord} record The (shadow) record about to be loaded
	 * @private
	 */
	onBeforeLoadRecord: function(field, record)
	{
		...
		if (!this.ignoreReadFlagTimer) {
			var delay = container.getSettingsModel().get('grommunio/v1/contexts/mail/readflag_time');
			if (delay > 0) {
				return;
			}
		}
	...
	onLoadRecord: function(field, record)
	{
		if (record && !record.phantom && !record.isRead()) {
			...
			var flags = this.record.get('message_flags') | Grommunio.core.mapi.MessageFlags.MSGFLAG_READ;
	...
	startReadFlagTimer: function()
	{
		if (!this.readFlagTimer) {
			var timeout = container.getSettingsModel().get('grommunio/v1/contexts/mail/readflag_time') * 1000;
	 ...
	 * as {@link Grommunio.common.Actions.markAsRead read}.
	 * @private
	 */
	markAsRead: function()
	{
		if (this.record && !Ext.isEmpty(this.record.getStore())) {
			Grommunio.common.Actions.markAsRead(this.record);
		}
		delete this.readFlagTimer;
	},
...
Ext.preg('grommunio.markasreadplugin', Grommunio.core.plugins.MarkAsReadPlugin);
