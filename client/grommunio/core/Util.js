...
Grommunio.core.Util =
{
	...
	sortArray: function(list, order, sort)
	{
		...
		if (Ext.isFunction(sort)) {
			...
			fn = function(obj1, obj2) {
				return Grommunio.core.Util.numericComparison(obj1[sort], obj2[sort]);
			};
		} else {
	...
	caseInsensitiveComparison: function(string1, string2)
	{
		...
		return Grommunio.core.Util.caseSensitiveComparison(v1, v2);
	},
	...
	trimStringArray: function(arrayToTrim)
	{
		...
		for(var index = 0, len = tmpArray.length; index < len; index++) {
			if(Array.isArray(tmpArray[index])) {
				...
				arrayToTrim.push(Grommunio.core.Util.trimStringArray(tmpArray[index]));
			} else {
	...
	inArray: function(multiDimArray, tokenToSearch, caseInSensitive, matchPartial)
	{
		for(var index = 0, len = multiDimArray.length; index < len; index++) {
			if(Array.isArray(multiDimArray[index])) {
				// recursively call the same function
				if(Grommunio.core.Util.inArray(multiDimArray[index], tokenToSearch, caseInSensitive, matchPartial)) {
					return true;
				}
			} else {
				if(tokenToSearch) {
	...
	{
		if(defaults) {
			obj = Grommunio.core.Util.applyRecursive(obj, defaults);
		}
		...
		if(obj && config && (Ext.isObject(config) || Array.isArray(config))) {
			for(var key in config) {
				if (Ext.isDefined(obj[key]) && Ext.isObject(obj[key])) {
					...
					obj[key] = Grommunio.core.Util.applyRecursive(obj[key], config[key]);
				} else if (Ext.isObject(config[key])) {
					obj[key] = Grommunio.core.Util.applyRecursive({}, config[key]);
				} else {
					// normal copy
					obj[key] = config[key];
				}
	...
	{
		if(defaults) {
			obj = Grommunio.core.Util.applyIfRecursive(obj, defaults);
		}
		...
		if(obj && config && (Ext.isObject(config) || Array.isArray(config))) {
			for(var key in config) {
				if(Ext.isDefined(obj[key]) && Ext.isObject(obj[key])) {
					// object with child elements, so call this function recursively
					obj[key] = Grommunio.core.Util.applyIfRecursive(obj[key], config[key]);
				} else if (Ext.isObject(config[key])) {
					obj[key] = Grommunio.core.Util.applyIfRecursive({}, config[key]);
	 ...
	 *	{
	 * 		'grommunio/v1/main/settingA': 'value1',
	 * 		'grommunio/v1/main/settingB': 'value2'
	 *	}
	 *
	...
	flattenObject: function(obj, sep, path)
	{
		...
		if (Ext.isObject(obj)) {
			for (var key in obj) {
				Ext.apply(ret, Grommunio.core.Util.flattenObject(obj[key], sep, path + separator + key));
			}
		} else {
			ret[path] = obj;
		}
	...
	{
		Grommunio.core.Util.setSelectionRange(obj, position, position);
	},
	...
	{
		var contentPanels = Grommunio.core.data.ContentPanelMgr && Grommunio.core.data.ContentPanelMgr.contentPanels;
		...
		return contentPanels.findIndexBy(function(contentPanel) {
			return (
				...
				contentPanel.isXType('grommunio.recordcontentpanel') &&
	 ...
	 * It will also check the value of {@link Grommunio.core.Util#skipRequester skipRequester}
	...
	copyToClipboard: function (text)
	{
		...
		if (Grommunio.core.BrowserWindowMgr.isMainWindowActive() === false) {
			doc = Grommunio.core.BrowserWindowMgr.getActive().document;
		}
	...
	getDateByLanguageFormat: function(value, language, format)
	{
		if (!Ext.isDefined(language)) {
			language = container.getSettingsModel().get('grommunio/v1/main/language');
		}
	...
	/**
	 * Returns the {@link Grommunio.core.data.IPMRecord} a file dropped onto a message
	 ...
	 * by both body editors ({@link Grommunio.common.form.TextArea} and
	 * {@link Grommunio.common.ui.HtmlEditor}) so their drop-to-attach behaviour cannot
	 * drift apart.
	 * @param {Ext.Component} field The body editor the file was dropped on
	 * @return {Grommunio.core.data.IPMRecord} The owning record, or undefined
	 */
	getDropTargetRecord : function(field)
	{
		...
		var panel = field.findParentByType('grommunio.mailcreatepanel') ||
			field.findParentByType('grommunio.recordcontentpanel');
