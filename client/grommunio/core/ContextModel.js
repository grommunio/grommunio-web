...
/**
 * @class Grommunio.core.ContextModel
 ...
 * {@link Grommunio.core.Context context}
 */
Grommunio.core.ContextModel = Ext.extend(Grommunio.core.data.StatefulObservable, {
	/**
	...
	/**
	 * @cfg {Grommunio.core.data.IPMStore} store The store
	 * which contains all {@link Grommunio.core.data.IPMRecord records}
	 * which must be shown within this {@link Grommunio.core.Context context},
	 */
	store: undefined,

	/**
	 * @cfg {Grommunio.hierarchy.data.MAPIFolderRecord[]} folders The folders
	...
	/**
	 * @cfg {Grommunio.core.data.IPMRecord[]} selectedRecords The records
	 * which are currently selected within this {@link Grommunio.core.Context context}.
	 */
	selectedRecords: undefined,

	/**
	 * @cfg {Grommunio.hierarchy.data.MAPIFolderRecord[]} defaultFolder default folder for the contextModel.
	 */
	...
	/**
	 * A key-value array of {@link Grommunio.hierarchy.data.MAPIFolderRecord Folder} and {@link Grommunio.core.data.IPMRecord Record} entryids.
	...
	/**
	 * A key-value array of {@link Grommunio.hierarchy.data.MAPIFolderRecord Folder} and {{@link Grommunio.core.data.IPMRecord Records} entryids.
	...
	constructor: function(config)
	{
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.core.data.IPMRecord} record the record.
			 */
			'previewrecordchange',
			/**
			 ...
			 * @param {Grommunio.core.data.IPMRecord[]} records The selected records
			 */
			 ...
			 * @param {Array} folders Selected folders as an array of {Grommunio.hierarchy.data.MAPIFolderRecord Folder} objects.
			 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder Folder to remove.
			 */
			'beforefolderchange',
			/**
			 * @event modechange
			 ...
			 * @param {Grommunio.core.ContextModel} model The model which fired the event
			 * @param {Grommunio.core.data.IPMProxy} proxy object that received the error.
		...
		Grommunio.core.ContextModel.superclass.constructor.call(this, config);
		...
		Grommunio.core.data.IPFStoreMgr.on('afterrecordupdate', this.afterRecordUpdate, this);
	},
	...
	{
		if(options && options.actionType === Grommunio.core.Actions['updatesearch']) {
	 ...
	 *
	 * @param {Grommunio.core.hierarchyStore} store that holds hierarchy data.
	 * @private
	 */
	onHierarchyLoad: function(hierarchyStore)
	{
		...
		var folderType = Grommunio.core.MessageClass.getDefaultFolderTypeFromMessageClass(this.store.preferredMessageClass);
	...
	/**
	 * Event handler which is triggered when a {@link Grommunio.core.data.IPFRecord record} has been
	 * updated from the server. This function will check whether action on the
	 * {@link Grommunio.core.data.IPFRecord record} does affect on the current context model i.e.
	 * {@link Grommunio.core.Actions.emptyFolder}, {@link Grommunio.core.Actions.readAllMsgs}, if it
	 * affects then we should reload current context model.
	 *
	 * @param {Grommunio.core.data.IPFStore} IPFStore
	 * @param {Grommunio.core.data.IPFRecord} record The Record which has been updated
	...
	/**
	 * Gets the store.
	 * @return {Grommunio.core.data.IPMStore} store object.
	 */
	getStore: function()
	{
		return this.store;
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder to add.
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord} the folder if it was added,
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder to remove.
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord} the folder if it was removed,
	 ...
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord[]} selected folders as an array of
	 * {@link Grommunio.hierarchy.data.MAPIFolderRecord MAPIFolder} objects.
	 */
	getFolders: function()
	{
	 ...
	 * used within the current selection of folders. If this {@link Grommunio.core.ContextModel ContextModel} is not enabled
	 ...
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord} a folder object if found, undefined otherwise.
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord/String} folder Either the MAPIFolder, or
	 * the id from the MAPIFolder to be used as id.
	 * @return {Boolean} true iff the given folder is selected.
	 */
	hasFolder: function(folder)
	{
		if (folder instanceof Grommunio.core.data.MAPIRecord) {
	...
	/*
	 * Create a new {@link Grommunio.core.data.IPMRecord IPMRecord} record which is associated to this context.
	 ...
	 * @return {Grommunio.core.data.IPMRecord} IPMRecord which is associated to this context.
	 */
	 ...
	 * @param {Grommunio.core.IPMFolder} folder The target folder in which the new record will be created.
	 * @param {Grommunio.core.DateRange} dateRange The {@link Grommunio.core.DateRange DateRange} object
	 * @return {Boolean} true if user has enough permissions, false otherwise.
	 */
	checkCreateRights: Ext.emptyFn,

	/**
	 * Set the {@link Grommunio.core.data.IPMRecord records} which have been
	 * selected within the {@link Grommunio.core.Context context}. Raise the
	...
	/**
	 * Get the currently selected {@link Grommunio.core.data.IPMRecord records}
	 * from this {@link Grommunio.core.Context context}.
	 *
	 * @return {Grommunio.core.data.IPMRecord[]} The selected records
	 */
	getSelectedRecords: function()
	{
		...
		return Grommunio.common.Actions.resolveRecords(records, store);
	},
	 ...
	 * but it check that action type is {@link Grommunio.core.Actions#updatelist 'updatelist'} then
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The record which is set as preview
	 ...
	 *
	 * @return {Grommunio.core.data.IPMRecord}
	 */
	...
	/**
	 * Check if the {@link Grommunio.hierarchy.data.MAPIStoreRecord store}
	 * of the {@link #getDefaultFolder folder} in which we intend to search has support for
	 * {@link Grommunio.hierarchy.data.MAPIStoreRecord#hasSearchSupport Search folders}.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder (optional) If passed, this folder's store
	...
	startLiveScroll: function(cursor, store)
	{
		 ...
		 */
		var isEnableLiveScroll = container.getSettingsModel().get('grommunio/v1/contexts/mail/enable_live_scroll');
		...
		if (this.fireEvent('beforelivescrollstart', this, cursor) !== false) {
			...
			if(this.fireEvent('livescrollstart', this, cursor) !== false) {
				var options = {
					restriction: {
						start: cursor,
						limit: container.getSettingsModel().get('grommunio/v1/main/page_size')
					}
				};
				...
				if (store.hasFilterApplied) {
					var filter = store.getFilterRestriction(Grommunio.common.data.Filters.UNREAD);
					if (store.hasFilterApplied) {
	...
	/**
	 * Stop {@link Grommunio.core.data.ListModuleStore#stopLiveScroll stopLiveScroll} in the {@link #store}
	...
	/**
	 * Event handler for the {@link Grommunio.core.data.ListModuleStore#exception} event
	 ...
	 * {@link Grommunio.core.data.ListModuleStore#search searching} in the {@link #store}.
	 * @param {Object} restriction The restriction which should be applied for the search
	 * @param {Boolean} subfolders True if searching should also include the subfolders
	...
	startSearch: function(restriction, subfolders, options)
	{
		...
		if (this.fireEvent('beforesearchstart', this, restriction, subfolders) !== false) {
			...
			if (Ext.isDefined(store.searchFolder[store.searchStoreUniqueId])) {
				...
				forceCreateSearchFolder = Grommunio.core.EntryId.compareEntryIds(searchFolderEntryid, store.searchFolderEntryId);
			}
	...
	/**
	 * Stop {@link Grommunio.core.data.ListModuleStore#stopSearch searching} in the {@link #store}
	...
	/**
	 * Event handler for the {@link Grommunio.core.data.ListModuleStore#beforeupdatesearch} event
	 * of the {@link #store}. This will fire the {@link #searchupdate} event and will
	 * check if the {@link Grommunio.core.mapi.Search#isSearchRunning search is still running},
	...
	onSearchUpdate: function(store, searchInfo)
	{
		...
		if (!Grommunio.core.mapi.Search.isSearchRunning(searchInfo['searchState'])) {
	...
	{
		Grommunio.core.ContextModel.superclass.initStateEvents.call(this);
	...
	getState: function()
	{
		var state = Grommunio.core.ContextModel.superclass.getState.call(this) || {};
	 ...
	 * is not a {@link Grommunio.core.MultiFolderContextModel MultiFolderContextModel} it can never
	...
	{
		if ( !(this instanceof Grommunio.core.MultiFolderContextModel) ){
		...
		Grommunio.core.ContextModel.superclass.applyState.call(this, state);
	}
});
