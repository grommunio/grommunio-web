...
/**
 * @class Grommunio.core.data.MAPIStore
 * @extends Ext.data.GroupingStore
 * @xtype grommunio.mapistore
 *
 ...
 */
Grommunio.core.data.MAPIStore = Ext.extend(Ext.data.GroupingStore, {
	/**
	...
	/**
	 * Set to true when the {Grommunio.core.data.MAPIStore} starts saving, set to false when done.
	 * @property
	 * @type Boolean
	 * @private
	 */
	...
	constructor: function(config)
	{
		...
		this.addEvents(
			/**
			 * @event open
			 * Fires when the {@link Grommunio.core.data.MAPIStore MAPIStore} gets extra data for a specific
			 * {@link Grommunio.core.data.MAPIRecord MAPIRecord}.
			 * @param {Grommunio.core.data.MAPIStore} store The {@link Grommunio.core.data.MAPIStore MAPIStore} which issues
			 * open request to get extra data for specific record.
			 * @param {Grommunio.core.data.MAPIRecord} record Record which is being opened to get extra information.
			 */
			'open'
		);

		Grommunio.core.data.MAPIStore.superclass.constructor.call(this, config);
	...
	/**
	 * Initialize all events which Grommunio.core.data.MAPIStore MAPIStore} will listen to.
	 * @protected
	 */
	initEvents: function()
	{
	...
	/**
	 * Check if a particular {@link Grommunio.core.Action action} is being executed
	 * by the {@link #proxy} of this store. When no action is given, this function
	 * will check if the proxy is busy with any action.
	 *
	 * @param {Grommunio.core.Action} action The action which is being checked
	 ...
	 * {@link Grommunio.core.data.MAPIProxy#cancelRequest cancel} those requests.
	 */
	...
	reload: function(options)
	{
		...
		Grommunio.core.data.MAPIStore.superclass.reload.call(this, options);
	},
	 ...
	 *
	 * @param {Grommunio.core.data.MAPIRecord/Grommunio.core.data.MAPIRecord[]} records The records which
	...
	save: function(records) {
		...
		if (!Ext.isDefined(records)) {
			return Grommunio.core.data.MAPIStore.superclass.save.call(this);
		}
	...
	/**
	 * Event handler which is raised when a {@link Grommunio.core.data.MAPIRecord MAPIRecord} has been added
	 * to this {@link Grommunio.core.data.MAPIStore MAPIStore}.
	 *
	 * @param {Grommunio.core.data.MAPIStore} store The {@link Grommunio.core.data.MAPIStore MAPIStore} to which the store was added.
	 * @param {Grommunio.core.data.MAPIRecord[]} records The array of {@link Grommunio.core.data.MAPIRecord records} which have been added.
	...
	/**
	 * Event handler which is raised when a {@link Grommunio.core.data.MAPIRecord MAPIRecord} has been removed
	 * from this {@link Grommunio.core.data.MAPIStore MAPIStore}.
	 *
	 * @param {Grommunio.core.data.MAPIStore} store The {@link Grommunio.core.data.MAPIStore MAPIStore} from which the records were removed.
	 * @param {Grommunio.core.data.MAPIRecord[]} records The array of {@link Grommunio.core.data.MAPIRecord records} which have been removed.
	 ...
	 * all {@link Grommunio.core.data.MAPIRecord#actions Message Actions} from the given records.
	 *
	 * @param {Grommunio.core.data.MAPIStore} store The store which fired the event
	...
	/**
	 * Iterates through all {@link Grommunio.core.data.MAPIRecord records} and sets the
	 * reference to the {@link Grommunio.core.data.MAPIStore MAPIStore} to which it belongs.
	 *
	 * @param {Grommunio.core.data.MAPIStore} store The {@link Grommunio.core.data.MAPIStore MAPIStore} to which the
	 * {@link Grommunio.core.data.MAPIRecord records} must be assigned.
	 * @param {Grommunio.core.data.MAPIRecord[]} records The array of
	 * {@link Grommunio.core.data.MAPIRecord records} which must be updated.
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.MAPIRecord[]} records records for which we need extra properties.
	 ...
	 * @param {Grommunio.core.data.MAPIRecord|Grommunio.core.dataMAPIRecord[]} records updated records.
	...
	onOpenRecords: function(success, records, data)
	{
		...
		records.forEach(function(record) {
			...
			if (records.length !== 1 && Array.isArray(data) && data.length !== 0 &&
				!Grommunio.core.EntryId.compareEntryIds(record.get('entryid'), data[0].entryid)) {
				return;
			}
	 ...
	 * then this function will also use {@link Grommunio.core.EntryId#compareEntryIds}. For
	 * 'store_entryid' then {@link Grommunio.core.EntryId#compareStoreEntryIds} is used.
	...
	getById: function(id)
	{
		...
		var item = Grommunio.core.data.MAPIStore.superclass.getById.call(this, id);
	...
	filterBy: function(fn, scope)
	{
		...
		Grommunio.core.data.MAPIStore.superclass.filterBy.apply(this, arguments);
	},
	...
	clearFilter: function()
	{
		...
		Grommunio.core.data.MAPIStore.superclass.clearFilter.apply(this, arguments);
	},

	/**
	 * Callback function which will be called when 'read' action is executed
	 * and {@link Grommunio.core.data.JsonReader JsonReader} has deserialized data
	 * into {@link Grommunio.core.data.MAPIRecord MAPIRecord},
	 * so the records can be added to the {@link Grommunio.core.data.NoSyncStore NoSyncStore}.
	 * @param {Object} o response object containing array of {@link Grommunio.core.data.MAPIRecord MAPIRecord}
	 * and optionally a property indicating total number of records.
	 * @param {Object} options optionally can contain 'add' which will append {@link Grommunio.core.data.MAPIRecord MAPIRecord}
	 * to the existing set of cached {@link Grommunio.core.data.MAPIRecord MAPIRecord}.
	 * @private
	 */
	loadRecords: function(o, options, success)
	{
		Grommunio.core.data.MAPIStore.superclass.loadRecords.apply(this, arguments);
	...
	clearGrouping: function()
	{
		...
		if (this.groupField) {
			Grommunio.core.data.MAPIStore.superclass.clearGrouping.apply(this, arguments);
		}
	},
	...
	destroy: function()
	{
		...
		Grommunio.core.data.MAPIStore.superclass.destroy.apply(this, arguments);
	}
});

Ext.reg('grommunio.mapistore', Grommunio.core.data.MAPIStore);
