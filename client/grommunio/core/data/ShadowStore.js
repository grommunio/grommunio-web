...
/**
 * @class Grommunio.core.data.ShadowStore
 ...
 */
Grommunio.core.data.ShadowStore = Ext.extend(Grommunio.core.data.MAPIStore, {
	/**
	 * @cfg {Boolean} standalone If true, the {@link Grommunio.core.data.ShadowStore ShadowStore}
	 * will not be hooked into the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr} and
	 * {@link Grommunio.core.data.IPFStoreMgr IPFStoreMgr}. This will prevent listening to events
	 * coming from the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr} or
	 * {@link Grommunio.core.data.IPFStoreMgr IPFStoreMgr}.
	 * Defaults to false.
	 */
	standalone: false,

	/**
	 * @cfg {Boolean} serveronly If true, the {@link Grommunio.core.data.ShadowStore ShadowStore}
	 * will be hooked into the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr} and
	 * {@link Grommunio.core.data.IPFStoreMgr IPFStoreMgr}, but will only do that for events which
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			batch: false,
			proxy: new Grommunio.core.data.ShadowProxy(),
			writer: new Grommunio.core.data.JsonWriter(),
			reader: new Grommunio.core.data.JsonReader()
		});

		Grommunio.core.data.ShadowStore.superclass.constructor.call(this, config);
	 ...
	 *
	 * Implemented for the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr} which calls this function
	 * on {@link Grommunio.core.data.IPMStore IPMStores} to determine if a given folder has been loaded.
	 ...
	 * matches the supplied entryid argument.
	 *
	 * Implemented for the {@link Grommunio.core.data.IPFStoreMgr IPFStoreMgr} which calls this function
	 * on {@link Grommunio.core.data.IPFStore IPFStores} to determine if a given store has been loaded.
	 * Since the ShadowStore will never load data from the server, this function always returns false.
	 *
	 * @param {String|Array} entryidList Entryid of the folder
	 * @return {Boolean} Returns true when entryid matches, false when it does not.
...
Ext.reg('grommunio.shadowstore', Grommunio.core.data.ShadowStore);
