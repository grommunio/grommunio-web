...
/**
 * @class Grommunio.core.data.UserIdObject
 * @extends Object
 *
 * An object that identifies a user and has some functions to create and
 * compare.
 */
Grommunio.core.data.UserIdObject = Ext.extend(Object, {
	/**
	 * The type of this user. Can be GROMMUNIO or SMTP
	 * @property
	 */
	type: '',
	...
	/**
	 * The username of this user. Only available for GROMMUNIO type users.
	 * @property
	 */
	username: '',

	/**
	 * The entryid of this user. Only available for GROMMUNIO type users.
	 * @property
	 */
	entryid: '',
	...
	/**
	 * Compares the {@link Grommunio.core.data.UserIdObject UserIdObject} to the passed
	 * {@link Grommunio.core.data.UserIdObject}. Returns true if both objects
	 * identify the same user.
	 * @param {Grommunio.core.data.UserIdObject} userIdObject A user of whom the id data
	...
	equals: function(userIdObject)
	{
		...
		if ( !Ext.isEmpty(this.entryid) && !Ext.isEmpty(userIdObject.entryid) ) {
			return Grommunio.core.EntryId.compareEntryIds(this.entryid, userIdObject.entryid);
		}
	 ...
	 * @param {Grommunio.core.data.UserIdObject} user A user whose id data will be used
	...
	syncWithUser: function(user)
	{
		if ( user.type === 'EX' ){
			...
			// email address for GROMMUNIO users, so we shouldn't overwrite if we have one)
