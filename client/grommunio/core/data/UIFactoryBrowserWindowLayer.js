...
/**
 * @class Grommunio.core.data.UIFactoryBrowserWindowLayer
 ...
 */
Grommunio.core.data.UIFactoryBrowserWindowLayer = Ext.extend(Grommunio.core.data.UIFactoryLayer, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.core.data.UIFactoryBrowserWindowLayer.superclass.constructor.call(this, config);
	},
	...
	create: function(component, config)
	{
		...
		if ( Ext.isDefined(window.deskappOpenWindow) ){
			...
			var browserWindow = Grommunio.core.BrowserWindowMgr.getOpenedWindow(config.record);
			if (browserWindow) {
				browserWindow.focus();
			} else {
	...
	{
		if (separateWindowInstance) {
			Grommunio.core.BrowserWindowMgr.register(separateWindowInstance, component, config);
		} else {
			// Seems popup is blocked, just remember that it is blocked
			Grommunio.core.BrowserWindowMgr.isPopupsBlocked = true;
			...
			Grommunio.core.BrowserWindowMgr.blockedPopupsContent.push({
				component: component,
				config: config
			});
		}
	...
	{
		var availableBrowserWindows = Grommunio.core.BrowserWindowMgr.browserWindows;
...
Grommunio.core.data.UIFactory.registerLayer(new Grommunio.core.data.UIFactoryBrowserWindowLayer());
