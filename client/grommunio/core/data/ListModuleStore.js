...
/**
 * @class Grommunio.core.data.ListModuleStore
 * @extends Grommunio.core.data.IPMStore
 * @xtype grommunio.listmodulestore
 *
 ...
 */
Grommunio.core.data.ListModuleStore = Ext.extend(Grommunio.core.data.IPMStore, {
	/**
	...
	/**
	 * @cfg {Grommunio.hierarchy.data.MAPIFolderRecord} folder instead of passing entryId and storeEntryId,
	...
	/**
	 * used in synchronizing {@link Grommunio.core.data.ListModuleStore store} which indicate true after deleting {@link Grommunio.core.data.IPMRecords[] records}
	 * from {@link Grommunio.core.data.ListModuleStore store}.
	 * @property
	 * @type Boolean
	 */
	syncStore: false,
	...
	/**
	 * @cfg {Number} which is hold number of records loaded in {@link Grommunio.core.data.ListModuleStore store}
	 */
	totalLoadedRecord: undefined,

	/**
	 * The LoadMask object which will be shown when the {@link Grommunio.core.data.ListModuleStore store}
	 ...
	 * that should be used when requesting data using {@link Grommunio.core.data.IPMProxy IPMProxy).
	 */
	...
	constructor: function(config)
	{
		...
		var module = Grommunio.core.ModuleNames.getModule(config.preferredMessageClass, true);
		if (Ext.isEmpty(module) && (config.preferredMessageClass != this.preferredMessageClass)) {
			module = Grommunio.core.ModuleNames.getModule(this.preferredMessageClass, true);
		}
		...
		var recordType = Grommunio.core.data.RecordFactory.getRecordClassByMessageClass(config.preferredMessageClass);
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.core.data.ListModuleStore} store store that started search.
			 ...
			 * be fired when {@link Grommunio.core.data.ListModuleStore store} needs to send an 'updatesearch'
		...
		this.totalLoadedRecord = container.getSettingsModel().get('grommunio/v1/main/page_size');

		Grommunio.core.data.ListModuleStore.superclass.constructor.call(this, config);
	 ...
	 * This will apply the {@link Grommunio.core.EntryId#compareEntryIds compareEntryIds} function
	...
	{
		return Grommunio.core.EntryId.compareEntryIds(a, b);
	},

	/**
	 * Function will set action type.
	 * @param {String} actionType action type that will be used for sending request to server,
	 * valid values are defined in {@link Grommunio.core.Actions Actions}.
	 */
	...
	load: function(options)
	{
		...
		if (options.actionType == Grommunio.core.Actions['search']) {
			/*
		 ...
		 */
		if(options.actionType == Grommunio.core.Actions['list'] && Array.isArray(options.params.restriction.search) && Ext.isDefined(options.params.search_folder_entryid)) {
			delete options.params.restriction.search;
		}

		// remove options that are not needed, although sending it doesn't hurt
		if (options.actionType == Grommunio.core.Actions['updatesearch'] ||
			options.actionType == Grommunio.core.Actions['stopsearch']) {
		...
		if (options.actionType == Grommunio.core.Actions['list'] && Array.isArray(options.params.restriction.search) && Ext.isDefined(this.searchFolderEntryId) && this.useSearchFolder) {
			delete options.params.restriction.search;
		}

		options.params.timezone_iana = Intl.DateTimeFormat().resolvedOptions().timeZone;

		return Grommunio.core.data.ListModuleStore.superclass.load.call(this, options);
	},
...
myStore.reload(lastOptions);
	...
	reload: function(options)
	{
		if (this.hasSearchResults ) {
			...
			Grommunio.core.data.ListModuleStore.superclass.reload.apply(this, arguments);
		}
	},

	/**
	 * Helper function to detect if {@link #this} is the {@link Grommunio.advancesearch.AdvanceSearchStore AdvanceSearchStore} or not.
	 * @return {Boolean} True if this is referred to an instance of {@link Grommunio.advancesearch.AdvanceSearchStore AdvanceSearchStore}, false otherwise.
	 */
	...
	search: function(options)
	{
		...
		if(this.useSearchFolder) {
			Ext.apply(options, {
				actionType: Grommunio.core.Actions['search']
			});
		}
	...
	updateSearch: function(options)
	{
		...
		Ext.apply(options, {
			actionType: Grommunio.core.Actions['updatesearch']
		});
		...
		Ext.apply(this.lastOptions, {
			...
			originalActionType: Grommunio.core.Actions.updatesearch
		});
	},
	...
	stopSearch: function(options)
	{
		...
		Ext.apply(options, {
			actionType: Grommunio.core.Actions['stopsearch']
		});
	 ...
	 * add {@link Grommunio.core.data.IPMRecords[] records} to {@link Grommunio.core.data.ListModuleStore store}
	 ...
	 * {@link Grommunio.core.data.IPMRecords records}.
	...
	loadRecords: function(data, options, success, metaData)
	{
		...
		Grommunio.core.data.ListModuleStore.superclass.loadRecords.apply(this, arguments);
	},

	/**
	 * Function will be called when search response is received by {@link Grommunio.core.data.IPMProxy proxy},
	...
	updateSearchInfo: function(searchResponse, page)
	{
		...
		if (searchData.searchState & Grommunio.core.mapi.Search.TWIR_TOTALLY &&
		...
		if(Grommunio.core.mapi.Search.isSearchRunning(searchData.searchState)) {
			this.searchUpdateTimer = this.updateSearch.defer(
				container.getSettingsModel().get('grommunio/v1/contexts/search/updatesearch_timeout'),
				this
			);
		} else {
			delete this.searchUpdateTimer;
	...
	/**
	 * This will provide the restriction based on the given {@link Grommunio.common.data.Filters.UNREAD Filter} or
	 * {@link Grommunio.task.data.DataModes datemode}.
	 */
...
Ext.reg('grommunio.listmodulestore', Grommunio.core.data.ListModuleStore);
