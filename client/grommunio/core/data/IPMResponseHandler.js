...
/**
 * @class Grommunio.core.data.IPMResponseHandler
 * @extends Grommunio.core.data.ProxyResponseHandler
 */
Grommunio.core.data.IPMResponseHandler = Ext.extend(Grommunio.core.data.ProxyResponseHandler, {
	/**
	...
	compareResponseDataToRecord: function(data, record)
	{
		...
		if(!record.phantom) {
			if(!Ext.isEmpty(record.get('attach_num'))) {
				if(Grommunio.core.EntryId.compareEntryIds(data.entryid, record.get('entryid'))) {
		...
		return Grommunio.core.data.IPMResponseHandler.superclass.compareResponseDataToRecord.apply(this, arguments);
	}
});
