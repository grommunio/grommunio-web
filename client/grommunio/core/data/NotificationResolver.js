...
/**
 * @class Grommunio.core.data.NotificationResolver
 * @extends Ext.util.Observable
 *
 * The NotificationResolver is used when the {@link Grommunio.core.ResponseRouter ResponseRouter}
 ...
 * {@link Grommunio.core.data.AbstractNotificationResponseHandler ResponseHandler} will
 * be needed to handle the response correctly.
 */
Grommunio.core.data.NotificationResolver = Ext.extend(Ext.util.Observable, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.core.data.NotificationResolver.superclass.constructor.call(this, config);
	},
	...
	/**
	 * Obtain the {@link Grommunio.core.data.AbstractNotificationResponseHandler ResponseHandler}
	 ...
	 * up the most suitable {@link Grommunio.core.data.AbstractNotificationResponseHandler ResponseHandler}.
	 *
	 ...
	 * @return {Grommunio.core.data.AbstractResponseHandler} The response handler
	...
	getHandlerForResponse: function(moduleName, response)
	{
		...
		if (Array.isArray(handlers)) {
			if (handlers.length > 1) {
				return new Grommunio.core.data.CompositeResponseHandler({
					handlers: handlers
				});
			} else {
				return handlers[0];
	 ...
	 * the {@link Grommunio.hierarchy.data.HierarchyNotificationResponseHandler Response Handlers}
	 * for the {@link Grommunio.core.data.IPFStore IPFStores}.
	 *
	 * @param {Object} response The Json response data for which the responseHandler is needed
	 * @return {Grommunio.core.data.AbstractResponseHandler[]} The response handlers
	...
	getHandlersForIPFResponse: function(response)
	{
		...
		if(response['folders'] || response['newmail']) {
			...
			folderStores = Grommunio.core.data.IPFStoreMgr.getStoresForStores(folderParents);
			...
			if (Array.isArray(folderStores)) {
				...
				for (var i = 0, len = folderStores.length; i < len; i++) {
					responseHandlers.push(new Grommunio.hierarchy.data.HierarchyNotificationResponseHandler({
			...
			return new Grommunio.hierarchy.data.HierarchyNotificationResponseHandler({
				store: hierarchyStore,
				reader: hierarchyStore.reader,
				notifyObject: hierarchyStore
			});
		} else if(response['addressbook']) {
			var addressBookStore = Grommunio.addressbook.AddressBookHierarchyStore;
			return new Grommunio.addressbook.AddressBookHierarchyNotificationResponseHandler({
			...
			return new Grommunio.task.data.TodoTaskListNotificationResponseHandler({
				store: taskStore,
	...
	/**
	 * Helper function for {@link #getHandlerForResponse}, this will construct
	 * the {@link Grommunio.core.data.IPMNotificationResponseHandler Response Handlers}
	 * for the {@link Grommunio.core.data.IPMStore IPMStores}.
	 *
	 * @param {Object} response The Json response data for which the responseHandler is needed
	 * @return {Grommunio.core.data.AbstractResponseHandler[]} The response handlers
	 * which are suitable for handling the given response object.
	...
	getHandlersForIPMResponse: function(response)
	{
		...
		messageStores = Grommunio.core.data.IPMStoreMgr.getStoresForFolders(messageParents);

		if (Array.isArray(messageStores)) {
			var responseHandlers = [];
			for (var i = 0, len = messageStores.length; i < len; i++) {
				responseHandlers.push(new Grommunio.core.data.IPMNotificationResponseHandler({
			...
			return new Grommunio.core.data.IPMNotificationResponseHandler({
				store: messageStores,
