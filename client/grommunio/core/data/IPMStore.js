...
/**
 * @class Grommunio.core.data.IPMStore
 * @extends Grommunio.core.data.MAPIStore
 * @xtype grommunio.ipmstore
 *
 * The {@link Grommunio.core.data.IPMStore IPMStore} represents the collection of messages.
 */
Grommunio.core.data.IPMStore = Ext.extend(Grommunio.core.data.MAPIStore, {
	/**
	 * @cfg {Boolean} standalone If true, the {@link Grommunio.core.data.IPMStore IPMStore}
	 * will not be hooked into the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr}. This will prevent
	 * listening to events coming from the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr}.
	 * Defaults to false.
	 */
	standalone: false,

	/**
	 * @cfg {Boolean} serveronly If true, the {@link Grommunio.core.data.IPMStore IPMStore}
	 * will be hooked into the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr}, but will
	...
	constructor: function(config)
	{
		...
		Grommunio.core.data.IPMStore.superclass.constructor.call(this, config);

		// Register store with the store manager
		if (!this.standalone) {
			Grommunio.core.data.IPMStoreMgr.register(this, this.serveronly);
		}
	},

	/**
	 * Initialize all events which Grommunio.core.data.IPMStore IPMStore} will listen to.
	 * @private
	 */
	initEvents: function()
	{
		Grommunio.core.data.IPMStore.superclass.initEvents.call(this);

		if (!this.standalone) {
			if (!this.serveronly) {
				Grommunio.core.data.IPMStoreMgr.on('beforerecordsave', this.onExternalSave, this);
			}
			Grommunio.core.data.IPMStoreMgr.on('afterrecordwrite', this.onExternalWrite, this);
		}
	},

	/**
	 * Filter a list of {@link Grommunio.core.data.IPMRecord records} by checking if the record
	...
	/**
	 * Event handler which is raised when a {@link Grommunio.core.data.IPMRecord IPMRecord} is about
	 * to be saved by a different {@link Grommunio.core.data.IPMStore IPMStore}. However the same
	 * {@link Grommunio.core.data.IPMRecord IPMRecord} may have been listed in multiple
	 * {@link Grommunio.core.data.IPMStore IPMStores} at the same time. Thus check if the saved
	 * {@link Grommunio.core.data.IPMRecord IPMRecord} is also listed in this store, and if that is the
	 * case, update it here as well. This way we don't have to wait until a reload from the
	 * server occurs, and the {@link Grommunio.core.data.IPMRecord IPMRecord} is updated on all stores
	 * instantly.
	 *
	...
	/**
	 * Event handler which is raised when a {@link Grommunio.core.data.IPMRecord IPMRecord} has been
	 * updated from the server. This {@link Grommunio.core.data.IPMRecord IPMRecord} may have been listed in
	 * multiple {@link Grommunio.core.data.IPMStore IPMStores} at the same time. Thus we need to
	 * check if this {@link Grommunio.core.data.IPMRecord IPMRecord} is also listed in this store, and if that
	 ...
	 * @param {Grommunio.core.data.IPMrecord} record The most recent version of the record
		...
		if (Ext.isDefined(folderList)) {
			...
			for (var i = 0, len = folderList.length; i < len; i++) {
				for (var j = 0, jlen = entryidList.length; j < jlen; j++) {
					if (Grommunio.core.EntryId.compareEntryIds(folderList[i].get('entryid'), entryidList[j])) {
						return true;
					}
				}
			}
	 ...
	 * {@link Grommunio.core.data.IPMNotificationResponseHandler NotificationResponseHandler}
	 * when a notification has been received for a {@link Grommunio.core.data.IPMRecord record} which
	...
	onNotifyObjectcreated: function(action, records, data, timestamp, success)
	{
		...
		if (!timestamp || this.lastExecutionTime(Grommunio.core.Actions['list']) < timestamp) {
			this.reload();
		}
	},
	...
	{
		if (!this.standalone) {
			Grommunio.core.data.IPMStoreMgr.un('afterrecordwrite', this.onExternalWrite, this);
			if (!this.serveronly) {
				Grommunio.core.data.IPMStoreMgr.un('beforerecordsave', this.onExternalSave, this);
			}
			Grommunio.core.data.IPMStoreMgr.unregister(this, this.serveronly);
		}
		Grommunio.core.data.IPMStore.superclass.destroy.call(this);
	}
});

Ext.reg('grommunio.ipmstore', Grommunio.core.data.IPMStore);
