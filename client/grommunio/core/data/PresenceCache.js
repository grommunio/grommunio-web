...
/**
 * @class Grommunio.core.data.PresenceCache
 * @extends Object
 *
 ...
 * @singleton
 */
Grommunio.core.data.PresenceCache = Ext.extend(Object, {
	/**
	 ...
	 * Grommunio.core.data.UserIdObject user}, the statusByPlugin for the
	 * user, and a squashed {@link Grommunio.core.data.PresenceStatus status}.
	 * @property
	 * @private
	 */
	cache: {},
	 ...
	 * @param {Grommunio.core.data.UserIdObject} user The user for which the entry in the
	...
	getUser: function(user, noSync) {
		...
		if (user.entryid in this.cache) {
			...
			// GROMMUNIO after the names have been resolved)
			if ( noSync !== true ){
				this.syncUsers([user]);
			}
	...
	/**
	 * Synchronizes the given {@link Grommunio.core.data.UserIdObject users} with the cache.
	 * @param {Grommunio.core.data.UserIdObject[]} users The users that need to be synced
	 * with the cached users.
	 * @return {Grommunio.core.data.UserIdObject[]}
	 */
		...
		Ext.each( users, function(user, index){
			...
			if (Ext.isDefined(syncedUser)) {
				...
				this.cache[user.entryid] = {
					user: user,
					statusByPlugin: {},
					status: Grommunio.core.data.PresenceStatus.UNKNOWN
				};
				syncedUser = this.cache[user.entryid].user;
			}
	 ...
	 * @param {Grommunio.core.data.UserIdObject} userInfo The user for which
	 * a {@link Grommunio.core.data.PresenceStatus presence status} is
	 * requested.
	 * @return {Grommunio.core.data.PresenceStatus|undefined}
	 */
	 ...
	 * @param {Grommunio.core.data.UserIdObject} user The user for which the status is added
	 * @param {Grommunio.core.data.PresenceStatus} status The status for the given user according to the
	 * given plugin
	 */
	addStatusForUser: function(pluginName, user, status){
		...
		presenceUser.status = Grommunio.core.data.PresenceStatus.UNKNOWN;
	...
	/**
	 * Returns a list of {Grommunio.core.data.UserIdObject} objects in the cache.
	 * @return {Array} of {@link Grommunio.core.data.UserIdObject UserIdObjects}
	 */
...
Grommunio.core.data.PresenceCache = new Grommunio.core.data.PresenceCache();
