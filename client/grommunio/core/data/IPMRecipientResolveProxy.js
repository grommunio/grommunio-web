...
/**
 * @class Grommunio.core.data.IPMRecipientResolveProxy
 ...
 */
Grommunio.core.data.IPMRecipientResolveProxy = Ext.extend(Grommunio.core.data.MAPIProxy, {
	/**
	...
	getResponseHandlerForRequest: function(modulename, serverAction, action, records, parameters, reader, callback, scope, args)
	{
		...
		return new Grommunio.core.data.IPMRecipientResolveResponseHandler({
			proxy: this,
	...
	request: function(action, records, parameters, reader, callback, scope, args)
	{
		switch (action)
		{
			case Grommunio.core.Actions['checknames']:
