...
/**
 * @class Grommunio.core.data.IPMRecipientStore
 * @extends Grommunio.core.data.MAPISubStore
 */
Grommunio.core.data.IPMRecipientStore = Ext.extend(Grommunio.core.data.MAPISubStore, {
	/**
	 * The proxy that handles the resolving requests.
	 * @property
	 * @type Grommunio.core.data.IPMRecipientResolveProxy
	 */
	...
	/**
	 * The {@link Grommunio.core.data.JsonReader} used for handling expand responses
	 * @property
	 * @type Ext.data.DataReader
	 */
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			writer: new Grommunio.core.data.JsonRecipientWriter(),
			// provide a default reader
			reader: new Grommunio.core.data.JsonRecipientReader({
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.core.data.IPMRecipientStore} store The store which fired the event
			 * @param {Grommunio.core.data.IPMRecipientRecord[]} records The records that have been send for resolving
			 */
			'beforeresolve',
			/**
			 * @event resolved
		...
		Grommunio.core.data.IPMRecipientStore.superclass.constructor.call(this, config);

		this.resolveProxy = new Grommunio.core.data.IPMRecipientResolveProxy();
		this.expandProxy = new Grommunio.core.data.IPMExpandDistlistProxy();

		this.relayEvents(this.resolveProxy, ['exception']);

		this.resolveReader = new Ext.data.JsonReader({
			root: 'result'
		}, Grommunio.core.data.IPMRecipientResolveRecord);

		this.expandReader = new Grommunio.core.data.JsonReader({
			root: 'result',
			id: 'entryid',
			dynamicRecord: false
		}, Grommunio.core.data.IPMExpandDistlistRecord);
	...
	/**
	 * Parse a String into a {@link Grommunio.core.data.IPMRecipientRecord}.
	 *
	 * @param {String} str The string to parse
	 * @param {Grommunio.core.mapi.RecipientType} type The recipientType which must be applied to the recipient
	 * @return {Grommunio.core.data.IPMRecipientRecord} The created recipient
	 */
	parseRecipient: function(str, type)
	{
		var recipient = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(this.customObjectType);
		...
		if (mailStart != -1) {
			...
			if (!Grommunio.core.Util.validateEmailAddress(str)) {
				...
				recipient.set('object_type',Grommunio.core.mapi.ObjectType.MAPI_MAILUSER);
			}
		}

		recipient.set('recipient_type', type || Grommunio.core.mapi.RecipientType.MAPI_TO);
		recipient.endEdit();
		return recipient;
	},

	/**
	 * Obtain the list of currently {@link Grommunio.core.data.IPMRecipientRecord#isResolved resolved}
	 * {@link Grommunio.core.data.IPMRecipientRecord recipients}.
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records (optional) The list of records which
	 * must be filtered. If not provided, {@link #getRange} is assumed.
	 * @return {Grommunio.core.data.IPMRecipientRecord[]} The list of resolved recipients
	 */
	...
	/**
	 * Obtain the list of currently {@link Grommunio.core.data.IPMRecipientRecord#isResolved non-resolved}
	 * {@link Grommunio.core.data.IPMRecipientRecord recipients}. We also filter the list with recipients
	 * which have not yet been {@link Grommunio.core.data.IPMRecipientRecord#attemptedToResolve attempted}
	 * to resolve to prevent too much traffic to the server.
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records (optional) The list of records which
	 * must be filtered. If not provided, {@link #getRange} is assumed.
	 * @return {Grommunio.core.data.IPMRecipientRecord[]} The list of non-resolved recipients
	 */
	...
	/**
	 * Obtain the list of currently {@link Grommunio.core.data.IPMRecipientRecord which are invalid}
	 * {@link Grommunio.core.data.IPMRecipientRecord recipients}.reciepitns which do not have valid
	 * SMTP address neither have a entryid and have been attempted to be resolved
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records (optional) The list of records which
	 * must be filtered. If not provided, {@link #getRange} is assumed.
	 * @return {Grommunio.core.data.IPMRecipientRecord[]} The list of invalid recipients
	 */
	...
	/**
	 * Resolve the given {@link Grommunio.core.data.IPMRecipientRecord IPMRecipientRecord IPMRecipientRecords}
	 * @param {Grommunio.core.data.IPMRecipientRecord/Array} records (optional) The record or records to resolve,
	...
	resolve: function(records, options)
	{
		...
		for (var i = 0; i < records.length; i++) {
			...
			var preferDisplayNameLookup = this.preferDisplayNameForResolve &&
				...
				!Grommunio.core.Util.validateEmailAddress(displayName);
			...
			if (Ext.isEmpty(emailAddress)) {
				if (!preferDisplayNameLookup && Grommunio.core.Util.validateEmailAddress(smtpAddress)) {
					emailAddress = smtpAddress;
				} else {
					emailAddress = '';
				}
			} else if (!Grommunio.core.Util.validateEmailAddress(emailAddress) &&
			           emailAddress.toLowerCase().indexOf('/o=') !== 0 &&
			           Grommunio.core.Util.validateEmailAddress(smtpAddress)) {
		...
		var args = Ext.apply({}, options, {
			actionType: Grommunio.core.Actions['checknames'],
			listRequest: true,
		...
		this.resolveProxy.request(Grommunio.core.Actions['checknames'], records, parameters, this.resolveReader, this.onCheckNamesResult, this, args);
	},
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The recipient to resolve.
	...
	resolveRecipientByEmailAddress: function(record, callback, scope)
	{
		...
		if (Ext.isEmpty(smtpAddress) || !Grommunio.core.Util.validateEmailAddress(smtpAddress)) {
		...
		this.resolveProxy.request(
			Grommunio.core.Actions['checknames'],
			[ record ],
			parameters,
			this.resolveReader,
	...
	/**
	 * Callback function from the {@link Grommunio.core.data.IPMRecipientResolveResponseHandler CheckNamesResponseHandler},
	 ...
	 * {@link Grommunio.common.Actions.openCheckNamesContent openCheckNamesContent} will be called to
		...
		for (var i = 0; i < records.length; i++) {
			...
			if (!Ext.isEmpty(resolveResult.id)) {
				...
				// Display the checknames dialog to let the user choose what of the multiple matches he wants
					...
					Grommunio.common.Actions.openCheckNamesContent(resolveResult.result.records, recipientRecord);
				}
	...
	/**
	 * Expand the given {@link Grommunio.core.data.IPMRecipientRecord IPMRecipientRecord}
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The record to expand.
	...
	expand: function(record, recurse, options)
	{
		...
		// The arguments that can be used in the callback function
		var args = Ext.apply({}, options, {
			actionType: Grommunio.core.Actions['expand'],
			listRequest: true,
			recipientType: record.get('recipient_type'),
			params: parameters
		});

		this.expandProxy.request(Grommunio.core.Actions['expand'], record, parameters, this.expandReader, this.onExpandResult, this, args);
	},

	/**
	 * Callback function from the {@link Grommunio.core.data.IPMExpandDistlistResponseHandler ExpandDistlistResponseHandler},
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store which fired the event
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records The records which were added
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store which fired the event
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records The records which were updated
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecipientResolveProxy} proxy The proxy which fired the event.
	...
	/**
	 * Function which is use to verified that any recipient from the list of {@link Grommunio.core.data.IPMRecipientRecord}
	 * has recipient type {@link Grommunio.core.mapi.RecipientType.MAPI_BCC}.
	 * @param {Array} recipients (optional) The list of {@link Grommunio.core.data.IPMRecipientRecord}.
	 ...
	 * {@link Grommunio.core.mapi.RecipientType.MAPI_BCC}.
	 */
	hasBccRecipients: function(recipients)
	{
		...
		for (var i = 0, len = recipients.length; i < len; i++) {
			...
			if (recipient.get('recipient_type') === Grommunio.core.mapi.RecipientType.MAPI_BCC) {
				hasBcc = true;
				break;
			}
		}
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecipientRecord/Object} record The record or object to inspect.
	 * @return {String} The normalized email address.
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The recipient record which need to check.
	...
	isRecipientExists: function(record, callbackFn, scope)
	{
		...
		return records.some(function(recipient) {
			...
			return Grommunio.core.EntryId.compareEntryIds(recipient.get('entryid'), entryid) && matchingEmails;
		}, scope || this);
	}
});
