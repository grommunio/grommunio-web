...
/**
 * @class Grommunio.core.data.IPMAttachmentStore
 * @extends Grommunio.core.data.MAPISubStore
 */
Grommunio.core.data.IPMAttachmentStore = Ext.extend(Grommunio.core.data.MAPISubStore, {
	/**
	...
	/**
	 * @cfg {Grommunio.core.ObjectType} attachmentRecordType to create a custom {Grommunio.core.data.IPMAttachmentRecord}
	 */
	attachmentRecordType: Grommunio.core.mapi.ObjectType.MAPI_ATTACH,
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			id: Grommunio.generateId(32),
			// provide a default proxy
			proxy: new Grommunio.core.data.IPMAttachmentProxy({
				listModuleName: 'attachments',
				itemModuleName: 'attachments'
			}),
			// provide a default writer
			writer: new Grommunio.core.data.JsonAttachmentWriter(),
			// provide a default reader
			reader: new Grommunio.core.data.JsonAttachmentReader({}, Grommunio.core.data.RecordFactory.getRecordClassByCustomType(this.attachmentRecordType))
		});

		Grommunio.core.data.IPMAttachmentStore.superclass.constructor.call(this, config);
		...
		Grommunio.core.data.GrommunioCustomEventDispatcher.addEvents(
	 ...
	 *
	 * @param {Grommunio.core.data.IPMAttachmentRecord[]} attachmentRecords The attachments to archive
	...
	getDownloadAttachmentUrl: function(attachmentRecord, allAsZip)
	{
		...
		Grommunio.core.data.GrommunioCustomEventDispatcher.fireEvent('attachmentstorebeforegetbaseurl', this, attachmentRecord, allAsZip, eventData);
	 ...
	 * @param {Grommunio.core.data.IPMAttachmentRecord} attachmentRecord The attachment to import into given folder
	 ...
	 * only possible when none of the limits from the {@link Grommunio.core.data.ServerConfig ServerConfig}
	 ...
	 * {@link Grommunio.core.ui.notifier.Notifier#notify} if one of the limits was
	 * exceeded.
	...
	canUploadFiles: function(files, options)
	{
		...
		Grommunio.core.data.GrommunioCustomEventDispatcher.fireEvent('attachmentstorebeforecheck', this, files, options, eventData);
	 ...
	 * {@link Grommunio.core.data.IPMAttachmentRecord attachment records} and
	...
	uploadFiles: function(files, form, isHidden, params)
	{
		...
		var activeBrowserWindow = Grommunio.core.BrowserWindowMgr.getActive() || window;
		...
		Grommunio.core.data.GrommunioCustomEventDispatcher.fireEvent('attachmentstorebeforeupload', this, files, form, isHidden, params, eventData);
		...
		for (var i = 0; i < files.length; i++) {
			...
			if (isFileList) {
				...
				attach = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(this.attachmentRecordType, {
					'name': originalName,
					'size': file['size'],
					'filetype': file['type'],
					'hidden': Ext.isDefined(isHidden) ? isHidden : false,
					'attach_method': Grommunio.core.mapi.AttachMethod.ATTACH_BY_VALUE,
	...
	{
		var postfixId = Grommunio.generateId(8);
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The record which needs to be added as an attachment into store.
	...
	addAsAttachment: function(record, config)
	{
		...
		// Create an attachment record, with all the passed data, use record factory so default values will be applied
		var attach = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.mapi.ObjectType.MAPI_ATTACH, data);
	...
	/**
	 * Add given {@link Grommunio.core.data.IPMRecord IPMRecord} as embedded attachment to
	 * {@link Grommunio.core.data.IPMAttachmentStore IPMAttachmentStore}, this will generate new
	 * {@link Grommunio.core.data.IPMAttachmentRecord attachment record} and add those to the store and
	 * will send request to server to save ics attachment info to state file.
	 * @param {Grommunio.core.data.IPMRecord} record The record which needs to be added as ics attachment into store.
	 *
	...
	addIcsAsAttachment: function(record)
	{
		var data = {
			...
			'attach_method': Grommunio.core.mapi.AttachMethod.ATTACH_BY_VALUE,
			'attach_id': Grommunio.generateId(8)
		};
	...
	/**
	 * Add given {@link Grommunio.core.data.IPMRecord IPMRecord} as embedded attachment to {@link Grommunio.core.data.IPMAttachmentStore IPMAttachmentStore}, this will generate new
	 * {@link Grommunio.core.data.IPMAttachmentRecord attachment record} and
	 * add those to the store and will send request to server to save embedded attachment info to state file.
	 * @param {Grommunio.core.data.IPMRecord} record The record which needs to be added as embedded attachment into store.
	 *
	 * @return data object which is used to create attachment record.
	 */
	addEmbeddedAttachment: function(record)
	{
		var data = {
			...
			'store_entryid': record.get('store_entryid'),
			// attach method to indicate this is an embedded attachment
			'attach_method': Grommunio.core.mapi.AttachMethod.ATTACH_EMBEDDED_MSG,
			'attach_id': Grommunio.generateId(8)
		};
	...
	{
		Grommunio.core.data.IPMAttachmentStore.superclass.destroyRecord.apply(this, arguments);
		...
		Grommunio.core.data.GrommunioCustomEventDispatcher.fireEvent('attachmentstorebeforedestroyrecord', this, record, eventData);
	 ...
	 * @param {Grommunio.core.data.IPMAttachmentRecord} attachmentRecord The attachment to import into given folder
	 * @param {Grommunio.core.data.IPMRecord IPMRecord} message The record to which given attachment belongs
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The selected folder
	 * @private
	 */
