...
/**
 * @class Grommunio.core.data.IPMRecipientRecordFields
 *
 * Array of default fields for the {@link Grommunio.core.data.IPMRecipientRecord} object.
 * These fields will always be added, regardless of the exact type of
 * {@link Grommunio.core.data.IPMRecipientRecord record}.
 */
Grommunio.core.data.IPMRecipientRecordFields = [
	{name: 'entryid'},
	{name: 'search_key'},
	{name: 'rowid', type: 'int'},
...
/**
 * @class Grommunio.core.data.IPMRecipientRecord
 * @extends Ext.data.Record
 */
Grommunio.core.data.IPMRecipientRecord = Ext.extend(Ext.data.Record, {
	/**
	...
	/**
	 * Copy the {@link Grommunio.core.data.IPMRecipientRecord Record} to a new instance
	 * @param {String} newId (optional) A new Record id, defaults to the id of the record being copied. See id.
	 * @return {Grommunio.core.data.IPMRecipientRecord} The copy of the record.
	 */
	copy: function(newId)
	{
		var copy = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, this.data, newId || this.id);
	...
	/**
	 * Applies all data from an {@link Grommunio.core.data.IPMRecipientRecord IPMRecipientRecord}
	 * to this instance. This will update all data.
	 *
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The record to apply to this
	 * @return {Grommunio.core.data.IPMRecipientRecord} this
	 */
	...
	isValid: function()
	{
		if (this.isMeetingOrganizer()) {
			...
			return Grommunio.core.data.IPMRecipientRecord.superclass.isValid.call(this);
		}
	},
	...
	isResolved: function()
	{
		...
		return Grommunio.reSingleEmailAddress.test(smtp);
	},
	...
	isOneOff: function()
	{
		return Grommunio.core.EntryId.isOneOffEntryId(this.get('entryid'));
	},
	...
	{
		return Grommunio.core.Util.validateEmailAddress(this.get('smtp_address'));
	},
	...
	{
		if (this.get('object_type') == Grommunio.core.mapi.ObjectType.MAPI_MAILUSER && Grommunio.core.EntryId.hasContactProviderGUID(this.get('entryid'))) {
			return true;
		}
		return false;
	},
	...
	{
		if (this.get('object_type') == Grommunio.core.mapi.ObjectType.MAPI_DISTLIST && Grommunio.core.EntryId.hasContactProviderGUID(this.get('entryid'))) {
			return true;
		}
		return false;
	},
	...
	isMeetingOrganizer: function()
	{
		...
		return (recipientFlags & Grommunio.core.mapi.RecipientFlags.recipOrganizer) === Grommunio.core.mapi.RecipientFlags.recipOrganizer;
	},

	/**
	 * Compare this {@link Grommunio.core.data.IPMRecipientRecord record} instance with another one to see
	 * if they are the same IPM recipient.
	 *
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The IPMRecipientRecord to compare with
	...
	/**
	 * Convert this recipient into a {@link Grommunio.core.data.MAPIRecord record}
	 * which can be used in the addressbook. This can only work if this
	 * recipient is {@link #isResolved resolved}.
	 *
	 * @return {Grommunio.core.data.MAPIRecord} The addressbook record which
	...
	convertToABRecord: function()
	{
		...
		switch (displayType) {
			...
			case Grommunio.core.mapi.DisplayType.DT_REMOTE_MAILUSER:
				objectType = Grommunio.core.mapi.ObjectType.MAPI_MAILUSER;
				break;
			case Grommunio.core.mapi.DisplayType.DT_DISTLIST:
			case Grommunio.core.mapi.DisplayType.DT_ORGANIZATION:
			case Grommunio.core.mapi.DisplayType.DT_PRIVATE_DISTLIST:
	...
	/**
	 * Applies the data from an {@link Grommunio.core.data.IPMRecipientResolveRecord IPMRecipientResolveRecord}
	 * to this IPMRecipient.
	 * @param {Grommunio.core.data.IPMRecipientResolveRecord} record The record with resolve data
	 ...
	 * property value from {@link Grommunio.core.data.IPMRecipientRecord}.
	 *
	...
	/**
	 * Format a {@link Grommunio.core.data.IPMRecipientRecord} to a String.
	...
	/**
	 * This will call {@link Grommunio.core.EntryId#createOneOffEntryId} with required parameters
	...
	generateOneOffEntryId: function()
	{
		...
		this.set('entryid', Grommunio.core.EntryId.createOneOffEntryId(displayName, addrType, emailAddress));
	},
	 ...
	 */
	isSharedRecord: function(type = Grommunio.core.mapi.ObjectType.MAPI_MAILUSER)
	{
		return this.get('object_type') == type &&
			!Grommunio.core.EntryId.hasContactProviderGUID(this.get('entryid')) &&
			!Grommunio.core.EntryId.hasABProviderGUID(this.get('entryid')) &&
			!Grommunio.core.EntryId.isOneOffEntryId(this.get('entryid'));
	}
});

// Register a custom type to be used by the Record Factory
Grommunio.core.data.RecordCustomObjectType.addProperty('GROMMUNIO_RECIPIENT');

Grommunio.core.data.RecordFactory.setBaseClassToCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, Grommunio.core.data.IPMRecipientRecord);
Grommunio.core.data.RecordFactory.addFieldToCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, Grommunio.core.data.IPMRecipientRecordFields);
