...
/**
 * @class Grommunio.core.data.IPMExpandDistlistProxy
 ...
 * with a {@link Grommunio.core.data.IPMRecipientStore IPMRecipientStore} and JSON reader.
 */
Grommunio.core.data.IPMExpandDistlistProxy = Ext.extend(Grommunio.core.data.MAPIProxy, {
	/**
	...
	getResponseHandlerForRequest: function(modulename, serverAction, action, records, parameters, reader, callback, scope, args)
	{
		...
		return new Grommunio.core.data.IPMExpandDistlistResponseHandler({
			proxy: this,
	...
	request: function(action, records, parameters, reader, callback, scope, args)
	{
		switch (action)
		{
			case Grommunio.core.Actions['expand']:
