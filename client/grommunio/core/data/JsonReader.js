...
/**
 * @class Grommunio.core.data.JsonReader
 * @extends Ext.data.JsonReader
 *
 * This extension of the {@link Ext.data.JsonReader} supports {@link Grommunio.core.data.IPMStore stores}
 * which can hold different type of {@link Grommunio.core.data.IPMRecord records}.
 *
 * If in the constructor no recordType is provided, dynamic {@link Grommunio.core.data.IPMRecord record} type
 * support is assumed. With dynamic types, the incoming response data is used to determine which
 * {@link Grommunio.core.data.MAPIRecord MAPIRecord} must be constructed for each individual root element.
 */
Grommunio.core.data.JsonReader = Ext.extend(Ext.data.JsonReader, {
	/**
	 ...
	 * the {@link Grommunio.core.data.RecordFactory} to detect the recordType rather
	 ...
	 * which must be read from response. If no type is given, {@link Grommunio.core.data.JsonReader}
	...
	constructor: function(meta, recordType)
	{
		...
		if (!Ext.isDefined(recordType)) {
			...
			recordType = Grommunio.core.data.RecordFactory.getRecordClassByMessageClass('IPM');
		}
		...
		Grommunio.core.data.JsonReader.superclass.constructor.call(this, meta, recordType);
	...
	buildExtractors: function()
	{
		...
		Grommunio.core.data.JsonReader.superclass.buildExtractors.call(this);
	...
	/**
	 * Obtain the mapping between response objects and {@link Grommunio.core.data.IPMRecord record} fields for
	 * the given recordType. If no mapping yet exist for this recordType one will be constructed
	 * and added to the {@link Grommunio.core.data.IPMRecord.efCache cache}.
	 *
	 * @param {String} key The unique key for this record type (used for caching purposes).
	 * @param {Array} items The array of {@link Grommunio.core.data.IPMRecord record} items.
	 * @param {Number} len The length of the items array.
	 * @return {Array} The name/value list of response to {@link Grommunio.core.data.IPMRecord record} fields.
	 * @private
	 */
	...
	extractValues: function(data, items, len)
	{
		...
		{
			var recordType = Grommunio.core.data.RecordFactory.getRecordClassByRecordData(data);
			if (!Ext.isDefined(recordType)) {
	 ...
	 * @param {Boolean} returnRecords [false] Set true to return instances of {@link Grommunio.core.data.MAPIRecord MAPIRecord}.
	 * @private
	 */
	extractData: function(root, returnRecords)
	{
		...
		if (returnRecords === true) {
			for (var i = 0; i < root.length; i++) {
				...
				if (this.dynamicRecord === true) {
					record = Grommunio.core.data.RecordFactory.createRecordObjectByRecordData(data, id);
				}
			...
			for (var i = 0; i < root.length; i++) {
				...
				var Record = undefined;
				if (this.dynamicRecord === true) {
					Record = Grommunio.core.data.RecordFactory.getRecordClassByRecordData(n.props || n);
				}
	 ...
	 * to the props field and return the merged data. so {@link Grommunio.core.JsonReader JsonReader}
