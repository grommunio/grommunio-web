 ...
 * always be send to the server when {@link Grommunio.core.data.MAPIRecord#set record.set}
...
/**
 * @class Grommunio.core.data.MAPIRecord
 ...
 *
 */
Grommunio.core.data.MAPIRecord = Ext.extend(Ext.data.Record, {
	/**
	...
	/**
	 * The key-value object of {@link Grommunio.core.data.MAPISubStore MAPISubStore} which stores
	 * data of the complex properties like recipients, attachments etc. This container is mainly
	 * created to hold all {@link Grommunio.core.data.MAPISubStore MAPISubStore} at one place so
	 * {@link Grommunio.core.data.JsonWriter JsonWriter} can determine which {@link Grommunio.core.data.MAPISubStore MAPISubStore}
	 * it needs to serialize when serializing an {@link Grommunio.core.data.MAPIRecord MAPIRecord}.
	 * @property
	 * @type Object
	 * @private
	 */
	 ...
	 * can be configured to prevent any events from the {@link Grommunio.core.data.IPMStore store} to
	 * be propagated through the {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr}
	 * @property
	 * @type Boolean
	 * @private
	 */
	...
	{
		Grommunio.core.data.MAPIRecord.superclass.constructor.call(this, data, id);
	...
	copy: function(newId)
	{
		var copy = Grommunio.core.data.RecordFactory.createRecordObjectByRecordData(this.data, newId || this.id);
	...
	/**
	 * Applies all data from an {@link Grommunio.core.data.MAPIRecord Record}
	 * to this instance. This will update all data, attachments, recipients, etc..
	 *
	 * @param {Grommunio.core.data.MAPIRecord} record The record to apply to this
	 * @param {Boolean} cheapCopy True to allow cheap assignment rather then the more
	 * expensive copying of all data.
	 * @return {Grommunio.core.data.MAPIRecord} this
	 */
	 ...
	 * This will call {@link Grommunio.core.data.MAPIStore#save} with itself
	 * as argument.
	 */
	save: function()
	{
	 ...
	 *
	 * @param {Grommunio.core.data.MAPIRecord} record The MAPIRecord to compare with
	 ...
	 * {@link Grommunio.core.data.IPMStore store} regarding this record by the
	 * {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr}.
	...
	/**
	 * Checks if events can from the {@link Grommunio.core.data.IPMStore store} regarding
	...
	set: function(name, value, force)
	{
		...
		Grommunio.core.data.MAPIRecord.superclass.set.call(this, name, value);
	...
	afterEdit: function()
	{
		Grommunio.core.data.MAPIRecord.superclass.afterEdit.call(this);
		delete this.updateModifications;
		delete this.updateSubStoreModifications;
	},
	...
	{
		Grommunio.core.data.MAPIRecord.superclass.afterReject.call(this);
		delete this.updateModifications;
		delete this.updateSubStoreModifications;
	},
	...
	beginEdit: function()
	{
		...
		if (this.editingCount === 1) {
			Grommunio.core.data.MAPIRecord.superclass.beginEdit.call(this);
	...
	cancelEdit: function()
	{
		...
		// If this is not a nested call, we can direct the call to the superclass.
		if (this.editingCount === 0) {
			Grommunio.core.data.MAPIRecord.superclass.cancelEdit.call(this);
			delete this.updateModifications;
			delete this.updateSubStoreModifications;
		}
	},
	...
	endEdit: function()
	{
		...
		// If this is not a nested call, we can direct the call to the superclass.
		if (this.editingCount === 0) {
			Grommunio.core.data.MAPIRecord.superclass.endEdit.call(this);
		}
	},
	...
	commit: function(silent)
	{
		...
		Grommunio.core.data.MAPIRecord.superclass.commit.call(this, silent);
	},
	...
	/**
	 * Get the Message Action list for the {@link Grommunio.core.data.MAPIRecord record}.
	...
	/**
	 * Get Message Action for the {@link Grommunio.core.data.MAPIRecord record}.
	...
	/**
	 * Unset private the {@link Grommunio.core.data.MAPIRecord record} to a different
	 * {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder to copy/move the record to
	 */
	...
	/**
	 * Copy the {@link Grommunio.core.data.MAPIRecord record} to a different
	 * {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder to copy the record to
	 */
	...
	/**
	 * Move the {@link Grommunio.core.data.MAPIRecord record} to a different
	 * {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder to copy the record to
	 */
	 ...
	 * @return {Grommunio.core.data.MAPISubStore} The new substore.
	 */
	 ...
	 * @return {Grommunio.core.data.MAPISubStore} The substore.
	 */
	 ...
	 * @param {Grommunio.core.data.MAPISubStore} store the subStore.
	 * @return {Grommunio.core.data.MAPISubStore} The substore.
	 */
	 ...
	 * @param {Grommunio.core.data.MAPISubStore} remoteSubStore The store to merge
	...
	idComparison: function(entryIdOne, entryIdTwo)
	{
		...
		entryIdOne = Ext.isString(entryIdOne) && Grommunio.core.EntryId.hasContactProviderGUID(entryIdOne)
			? Grommunio.core.EntryId.unwrapContactProviderEntryId(entryIdOne): entryIdOne;

		entryIdTwo = Ext.isString(entryIdTwo) && Grommunio.core.EntryId.hasContactProviderGUID(entryIdTwo)
			? Grommunio.core.EntryId.unwrapContactProviderEntryId(entryIdTwo): entryIdTwo;

		return Grommunio.core.EntryId.compareEntryIds(entryIdOne, entryIdTwo);
	},
	 ...
	 * @param {Grommunio.core.data.MAPISubStore} store The store which was changed
	 * @param {Grommunio.core.data.MAPIRecord[]} records The records which were added/modified/removed
	 * @private
	 */
	 ...
	 * contents.
	 * @param {Grommunio.core.data.MAPISubStore} store The store which fired the event
	 * @param {Grommunio.core.data.MAPIRecord[]} records The records which were modified
	...
	/**
	 * Convenience method to get {@link Grommunio.core.mapi.DisplayType} or {@link Grommunio.core.mapi.DisplayTypeEx}
	 * property value from {@link Grommunio.core.data.IPMRecord}.
	 *
	 * @return {Grommunio.core.mapi.DisplayType|Grommunio.core.mapi.DisplayTypeEx} The display type value.
	 */
	getDisplayType: function()
	{
		...
		switch(displayType) {
			case Grommunio.core.mapi.DisplayType.DT_MAILUSER:
			case Grommunio.core.mapi.DisplayType.DT_DISTLIST:
				returnValue = displayTypeEx & ~Grommunio.core.mapi.DisplayTypeEx.DTE_FLAG_ACL_CAPABLE;
				break;
			default:
	...
	{
		return (this.get('access') & Grommunio.core.mapi.Access.ACCESS_DELETE) > 0;
	},
	...
	{
		return (this.get('access') & Grommunio.core.mapi.Access.ACCESS_MODIFY) > 0;
	},
	 ...
	 *
	 * @param {Grommunio.core.data.ListModuleStore} store This store
	 * @private
	 */
