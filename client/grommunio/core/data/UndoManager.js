...
/**
 * @class Grommunio.core.data.UndoManager
 ...
 * {@link Grommunio.core.data.IPMStoreMgr IPMStoreMgr} save events. Because every
 ...
 * 'undoResponse' by the {@link Grommunio.core.data.ProxyResponseHandler}.
 *
 ...
 *
 * The whole feature is opt-in through the 'grommunio/v1/main/undo_redo/enable'
 * setting, see {@link #enabled}.
 */
Grommunio.core.data.UndoManager = Ext.extend(Ext.util.Observable, {
	/**
	 ...
	 * the 'grommunio/v1/main/undo_redo/enable' setting. The feature is opt-in:
	...
	constructor: function(config)
	{
		...
		this.enabled = container.getSettingsModel().get('grommunio/v1/main/undo_redo/enable') === true;
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.core.data.UndoManager} undoManager This undo manager
			 */
			'stackchange'
		);

		Grommunio.core.data.UndoManager.superclass.constructor.call(this, config);

		if (this.enabled) {
			Grommunio.core.data.IPMStoreMgr.on('beforerecordsave', this.onBeforeRecordSave, this);
			Grommunio.core.data.IPMStoreMgr.on('afterrecordwrite', this.onAfterRecordWrite, this);
			Grommunio.core.data.IPMStoreMgr.on('storeexception', this.onStoreException, this);
		}
	},
	...
	/**
	 * @return {Grommunio.core.data.UndoCommand} The command which will be undone
	...
	/**
	 * @return {Grommunio.core.data.UndoCommand} The command which will be redone
	 ...
	 * @param {Grommunio.core.data.UndoCommand} command The command to record
	 */
	 ...
	 * @param {String} mode Either 'undo' or 'redo'
	 * @param {Grommunio.core.data.UndoCommand} command The command which failed
	 * @private
	 */
	...
	/**
	 * Event handler for the {@link Grommunio.core.data.IPMStoreMgr#beforerecordsave}
	 ...
	 *
	 * @param {Grommunio.core.data.IPMStore} store The store which is being saved
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The records being deleted
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The records being updated
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The records being created
	 * @private
	 */
	 ...
	 * saved through the {@link Grommunio.core.data.ShadowStore ShadowStore}
	 ...
	 * @param {Grommunio.core.data.IPMRecord|Array} records The records to capture
	 */
	 ...
	 * through the {@link Grommunio.core.data.ShadowStore ShadowStore}.
	...
	{
		if (!record || !(record instanceof Grommunio.core.data.IPMRecord)) {
			return false;
		}
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record The record being captured
	...
	/**
	 * Event handler for the {@link Grommunio.core.data.IPMStoreMgr#afterrecordwrite}
	 ...
	 * on the undo stack.
	 *
	 * @param {Grommunio.core.data.IPMStore} store The store which was written
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The records which were written
	 * @private
	 */
	...
	/**
	 * Event handler for the {@link Grommunio.core.data.IPMStoreMgr#storeexception}
	 ...
	 * @return {Grommunio.core.data.UndoCommand} the command, or false when none
	...
	/**
	 * Build a {@link Grommunio.core.data.UndoLocationCommand} for a confirmed
	 ...
	 * @return {Grommunio.core.data.UndoLocationCommand} the command or false
	 * @private
	 */
	buildLocationCommand: function(gesture, items)
	{
		...
		return new Grommunio.core.data.UndoLocationCommand({
	...
	/**
	 * Build a {@link Grommunio.core.data.UndoPropsCommand} for a confirmed
	 * property-change gesture.
	 * @param {Object} gesture The gesture description
	 * @param {Array} items The confirmed pending items
	 * @return {Grommunio.core.data.UndoPropsCommand} the command or false
	 * @private
	 */
	buildPropsCommand: function(gesture, items)
	{
		...
		return new Grommunio.core.data.UndoPropsCommand({
	...
	describePropsGesture: function(items)
	{
		...
		if (keyList.length === 1 && keyList[0] === 'message_flags') {
			var wasRead = (items[0].oldValues.message_flags & Grommunio.core.mapi.MessageFlags.MSGFLAG_READ) > 0;
		...
		var isAppointment = Grommunio.core.MessageClass.isClass(items[0].ids.message_class, 'IPM.Appointment', true);
...
/**
 * @class Grommunio.core.data.UndoCommand
 * @extends Object
 *
 * Base class for entries in the {@link Grommunio.core.data.UndoManager} history.
 ...
 * {@link Grommunio.core.data.ShadowStore ShadowStore}.
 */
Grommunio.core.data.UndoCommand = Ext.extend(Object, {
	/**
	 ...
	 * @return {Grommunio.core.data.IPMRecord} the record
	 * @protected
	 */
	createDetachedRecord: function(ids)
	{
		var record = Grommunio.core.data.RecordFactory.createRecordObjectByMessageClass(
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record The detached record
	 * @private
	 */
...
/**
 * @class Grommunio.core.data.UndoLocationCommand
 * @extends Grommunio.core.data.UndoCommand
 *
 ...
 */
Grommunio.core.data.UndoLocationCommand = Ext.extend(Grommunio.core.data.UndoCommand, {
	/**
...
/**
 * @class Grommunio.core.data.UndoPropsCommand
 * @extends Grommunio.core.data.UndoCommand
 *
 ...
 */
Grommunio.core.data.UndoPropsCommand = Ext.extend(Grommunio.core.data.UndoCommand, {
	/**
