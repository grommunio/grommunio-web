...
/**
 * @class Grommunio.core.Events
 * Utility class for handling special events.
 * @singleton
 */
Grommunio.core.Events = {
	/**
	...
	{
		var observables = Grommunio.core.Events.listeners[field.id];
		if (observables) {
			observables.push(observable);
		} else {
			Grommunio.core.Events.listeners[field.id] = [ observable ];
		}
	},
			...
			for (var i = 0, len = observables.length; i < len; i++) {
				...
				if (observable.el === el) {
					...
					if (observables.length === 0) {
						delete Grommunio.core.Events.listeners[field.id];
					}
				}
			}
		}
	...
	addPasteEventHandler: function(field, el, fn, scope, obj)
	{
		...
		var observable = Grommunio.core.Events.getListener(field, el);
		if (!Ext.isDefined(observable)) {
			...
			Grommunio.core.Events.registerListener(field, el, observable);
		}
	...
	removePasteEventHandler: function(field, el, fn, scope)
	{
		...
		if (Ext.isDefined(observable)) {
			...
			if (!observable.hasListener('paste')) {
				Grommunio.core.Events.unregisterListener(field, el);
			}
		}
	},
	...
	onPaste: function()
	{
		Grommunio.core.Events.onPastePoll.call(this, this.field, this.field.getValue(), 5);
	},
	...
	onPastePoll: function(field, oldValue, limit)
	{
		if (limit === 0) {
			...
			Grommunio.core.Events.onPastePoll.defer(1, this, [field, oldValue, --limit]);
			return;
		}
		this.fireEvent('paste');
