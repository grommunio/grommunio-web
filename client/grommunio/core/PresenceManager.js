...
/**
 * @class Grommunio.core.PresenceManager
 ...
 * @singleton
 */
Grommunio.core.PresenceManager = Ext.extend(Ext.util.Observable, {
	/**
	 * The stores that have been registered with the {@link Grommunio.core.PresenceManager PresenceManager}
	 * @property
	 * @private
	 */
	registeredStores: [],
	 ...
	 * {@link Grommunio.core.PresencePlugin PresencePlugins}
	 * @return {Grommunio.core.PresencePlugin[]}
	 */
	getPresencePlugins: function()
	{
		return container.getPlugins().filter(function(plugin){
			return plugin instanceof Grommunio.core.PresencePlugin;
		});
	},

	/**
	 * Returns the presence status for the users for whom the status has been requested
	 * @param {Grommunio.core.data.UserIdObject[]} users An array of userIdObjects
	 * for whom a presence status is requested.
	 * @return {Object[]} An array of objects that contain a
	 * {@link Grommunio.core.data.PresenceStatus presence status} for
	 ...
	 * @param {Grommunio.core.data.UserIdObject} user The user for whom a presence
	 * status is requested.
	 * @return {Grommunio.core.data.PresenceStatus} The squashed presence status
	 */
	...
	/**
	 * Add a {@link Grommunio.core.data.UserIdObject user} to the user queue for which
	 * a presence status will be requested. The queue will be resolved in a
	 * delayedTask of 500ms.
	 * @param {Grommunio.core.data.UserIdObject} user An object that identifies the user for which
	 ...
	 * {@link Grommunio.core.data.PresenceStatus presence status} for all
	 ...
	 * @param {Grommunio.core.data.UserIdObject[]} users An array with users
	 ...
	 * {@link Grommunio.core.data.UserIdObject user} and a status.
	 */
	...
	/**
	 * Registers a store with the {@link Grommunio.core.PresenceManager}. The
	 * {@link Grommunio.core.PresenceManager} will then make sure that presence statuses
	 * are fetched when the store loads or when the {@link Grommunio.core.PresenceManager}
	 * polls for updates.
	 ...
	 * users for which the {@link Grommunio.core.data.PresenceCache} does not have an
	 ...
	 * @param {Grommunio.core.data.MAPIRecord[]} records The records that are being loaded into
	 * this store.
	...
	onStoreLoad: function(store, records, options)
	{
		...
		var users = Grommunio.core.data.UserIdObjectFactory.createFromStore(store);
		...
		Ext.each(users, function(user){
			if (!Ext.isDefined(Grommunio.core.data.PresenceCache.getUser(user))){
	 ...
	 * in the {@link Grommunio.core.data.PresenceCache} and the UI will be updated.
	 ...
	 * {Grommunio.core.data.UserIdObject UserIdObject} and a
	 * {@link Grommunio.core.data.PresenceStatus} for that user
	 */
	updateStatuses: function(pluginName, updates) {
		...
		Ext.each(updates, function(update){
			var statusBeforeUpdate = Grommunio.core.data.PresenceCache.getStatusForUser(update.user);
			Grommunio.core.data.PresenceCache.addStatusForUser(pluginName, update.user, update.status);
			var statusAfterUpdate = Grommunio.core.data.PresenceCache.getStatusForUser(update.user);
		...
		Ext.each(this.registeredStores, function(registeredStore){
			...
			if ( modified.length ){
				...
				if ( registeredStore.store instanceof Grommunio.core.data.IPMRecipientStore ){
	...
	{
		var users = Grommunio.core.data.PresenceCache.getUserInfoList();
...
Grommunio.core.PresenceManager = new Grommunio.core.PresenceManager();
