 ...
 * {@link Grommunio.core.ResponseRouter ResponseRouter} on which events are provided
 ...
 * can be obtained from the globally available {@link Grommunio.core.Container container}
 * object.
 * <p>
 ...
 * </code></pre>
 */
Grommunio.core.Request = Ext.extend(Ext.util.Observable, (function() {
	/**
	...
	/**
	 * The flag to indicate if grommunioTag holds JSON data.
	 * This influences if the contents of grommunioTag will be encoded or not.
	 * @property
	 * @type Boolean
	 * @private
	 */
	...
	/**
	 * The flag to indicate if grommunioTag holds RAW data.
	 * This influences if the contents of grommunioTag will be encoded or not.
	 * @property
	 * @type Boolean
	 * @private
	...
	return {
		...
		constructor: function(config)
		{
			...
			this.addEvents(
				 ...
				 * @param {Grommunio.core.Request} request
				 * @param {Grommunio.core.data.ParalyzeReason} reason The reason to paralyze the grommunio Web
				 */
				'connectionparalyzed',
				/**
				 ...
				 * @param {Grommunio.core.Request} request
				 * @param {Grommunio.core.PingService} service The ping service which will
			...
			Grommunio.core.Request.superclass.constructor.call(this, config);
		...
		interrupt: function()
		{
			...
			var service = new Grommunio.core.PingService({
				url: this.defaultUrl,
		...
		restore: function(service, response)
		{
			...
			if (response && response.active === false) {
				this.paralyze(Grommunio.core.data.ParalyzeReason.SESSION_EXPIRED);
				return;
			} else {
				interrupted = false;
		...
		/**
		 * Sets request headers that will be sent with every request made through {@link Grommunio.core.Request} object.
		 ...
		 * responseHandler of this particular request with {@link Grommunio.core.data.DummyResponseHandler}, which will
		...
		cancelActiveRequest: function(requestId)
		{
			...
			if (queuedRequests.indexOf(requestId) >= 0) {
				if (queuedRequests.length === 1) {
					...
					responseRouter.addRequestResponseHandler(requestId, new Grommunio.core.data.DummyResponseHandler());
				}
				return;
			}
		 ...
		 * completed it calls {@link Grommunio.core.ResponseRouter#receive receive}} which handles the rest of the process.
		...
		stateChange: function(xmlHttpRequest)
		{
			...
			switch (xmlHttpRequest.status) {
				case 401: /* Unauthorized */
					...
					if (reason === 'MAPI_E_INVALID_WORKSTATION_ACCOUNT') {
						this.paralyze(Grommunio.core.data.ParalyzeReason.SESSION_INVALID);
					} else {
						this.paralyze(Grommunio.core.data.ParalyzeReason.SESSION_EXPIRED);
					}
					return;
			...
			if (Ext.isEmpty(response) || Ext.isEmpty(response.grommunio)) {
				this.receiveFailure(xmlHttpRequest);
				return;
			}
		 ...
		 * {@link Grommunio.core.ResponseRouter#receiveFailure receiveFailure} on
		 * the {@link Grommunio.core.ResponseRouter ResponseRouter}.
		...
		/**
		 * Resets the {@link Grommunio.core.Request Request} object and prepares it for
		 ...
		 * @param {Grommunio.core.data.AbstractResponseHandler} responseHandler The response handler which must be
		...
		addRequest: function(moduleName, actionType, actionData, responseHandler)
		{
			...
			if (!Ext.isDefined(grommunioTag.grommunio[moduleName])) {
				grommunioTag.grommunio[moduleName] = {};
			}

			// create new module id tag if not present
			if (!Ext.isDefined(grommunioTag.grommunio[moduleName][requestId])) {
				grommunioTag.grommunio[moduleName][requestId] = {};
			}

			// add action data
			grommunioTag.grommunio[moduleName][requestId][actionType] = actionData;
		...
		send: function(url, headers)
		{
			...
			if (!this.isParalyzed()) {
				var xmlHttpRequest = this.prepareHttpRequest(hasJson ? Ext.encode(grommunioTag) : grommunioTag, hasJson, url, headers);
			...
			// All data has been send, reset the grommunioTag to
