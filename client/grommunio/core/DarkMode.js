...
Grommunio.core.DarkMode = {
	...
	init: function()
	{
		...
		var savedMode = container.getSettingsModel().get('grommunio/v1/main/dark_mode');
	...
	setMode: function(mode, save)
	{
		...
		if (save !== false) {
			...
			container.getSettingsModel().set('grommunio/v1/main/dark_mode', mode);
	...
	syncToolbarButton: function(mode)
	{
		...
		if (btn && btn.getEl()) {
			var tabbar = btn.findParentByType('grommunio.maintabbar');
			if (tabbar) {
	...
	updateMailPreview: function()
	{
		...
		var docs = [document];
		if (Grommunio.core.BrowserWindowMgr && Grommunio.core.BrowserWindowMgr.browserWindows) {
			Grommunio.core.BrowserWindowMgr.browserWindows.each(function(win) {
				try {
