...
/**
 * @class Grommunio.common.data.FolderSuggestions
 * @extends Object
 * @singleton
 *
 ...
 */
Grommunio.common.data.FolderSuggestions = Ext.extend(Object, {
	/**
	 * @cfg {String} settingsPath The setting which holds the remembered folders
	 */
	settingsPath: 'grommunio/v1/state/folder_suggestions',
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The messages which were moved
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder they went to
	 */
	remember: function(records, folder)
	{
		...
		Ext.each(keys, function(key) {
			var folders = (stored[key] || []).filter(function(id) {
				return !Grommunio.core.EntryId.compareEntryIds(id, entryid);
			});
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The messages which are being moved
	 * @return {Grommunio.hierarchy.data.MAPIFolderRecord[]} the folders, most recent first
	 */
		...
		Ext.each(this.getKeys(records), function(key) {
			Ext.each(stored[key] || [], function(entryid) {
				...
				var known = folders.some(function(other) {
					return Grommunio.core.EntryId.compareEntryIds(other.get('entryid'), entryid);
				});
				if (folder && !known) {
					folders.push(folder);
				}
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The messages
...
Grommunio.common.data.FolderSuggestions = new Grommunio.common.data.FolderSuggestions();
