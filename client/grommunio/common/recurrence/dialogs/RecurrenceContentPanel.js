...
/**
 * @class Grommunio.common.recurrence.dialogs.RecurrenceContentPanel
 * @extends Grommunio.core.ui.RecordContentPanel
 * @xtype grommunio.recurrencecontentpanel
 */
Grommunio.common.recurrence.dialogs.RecurrenceContentPanel = Ext.extend(Grommunio.core.ui.RecordContentPanel, {
	/**
	...
	constructor: function(config)
	{
		...
		config = Ext.applyIf(config, {
			...
			height: 500,
			items: [{
				xtype: 'grommunio.recurrencepanel',
				ref: 'recurrencePanel',
				buttons: [{
		...
		Grommunio.common.recurrence.dialogs.RecurrenceContentPanel.superclass.constructor.call(this, config);
	...
	/**
	 * When the record is about to be set on the {@link Grommunio.core.ui.RecordContentPanel RecordContentPanel}
	 * we must enable recurrence in the record. By default, opening the RecurrenceContentPanel implies
	 * enabled it.
	 *
	 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The content panel to which the record was set.
	 * @param {Grommunio.core.data.IPMRecord} record The record which must be used for initialization.
	 * @param {Grommunio.core.data.IPMRecord} oldrecord The oldrecord which was previously set
	 * @private
	 */
	onBeforeSetRecord: function(contentpanel, record, oldrecord)
	{
		if (!record.get('recurring')) {
			...
			record.set('recurrence_type', Grommunio.common.recurrence.data.RecurrenceType.WEEKLY);
			record.set('recurrence_subtype', Grommunio.common.recurrence.data.RecurrenceSubtype.WEEKLY.type);
			record.set('recurrence_regen', Grommunio.common.recurrence.data.RecurrenceSubtype.WEEKLY.regen);
			record.set('recurrence_term', Grommunio.common.recurrence.data.RecurrenceEnd.NEVER);
	...
	onOk: function()
	{
		...
		if (this.record.phantom !== true && this.editRecurrence === true && this.record.hasRecurringExceptions()) {
			Ext.MessageBox.show({
				...
				fn: function(button) {
					if (button === 'yes') {
						Grommunio.common.recurrence.dialogs.RecurrenceContentPanel.superclass.onOk.apply(this, arguments);
					}
				},
				scope: this
			});
	...
	/**
	 * When the record is about to be saved on the {@link Grommunio.core.ui.RecordContentPanel RecordContentPanel}
	 * we must update the recurrence description, based on all the properties which have been filled in
	 * from the {@link Grommunio.common.recurrence.dialogs.RecurrencePanel RecurrencePanel}.
	 * @param {Grommunio.core.ui.RecordContentPanel} contentpanel The contentpanel which invoked the event
	 * @param {Grommunio.core.data.IPMRecord} record The record which is being saved.
	 * @private
	 */
	...
	/**
	 * Override the onSetRecord function from {@link Grommunio.core.ui.RecordContentPanel RecordContentPanel}
	...
	{
		Grommunio.common.recurrence.dialogs.RecurrenceContentPanel.superclass.onSetRecord.call(this, contentpanel, record, oldrecord);
		this.updateTitleFromRecord(record);
	},
	...
	onRemoveRecurrence: function()
	{
		...
		this.record.set('recurrence_type', Grommunio.common.recurrence.data.RecurrenceType.NONE);
		this.record.endEdit();
...
Ext.reg('grommunio.recurrencecontentpanel', Grommunio.common.recurrence.dialogs.RecurrenceContentPanel);
