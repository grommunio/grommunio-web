...
/**
 * @class Grommunio.common.recurrence.dialogs.RecurrencePanel
 * @extends Ext.Panel
 * @xtype grommunio.recurrencepanel
 *
 * Panel that is used to create Recurrences
 */
Grommunio.common.recurrence.dialogs.RecurrencePanel = Ext.extend(Ext.Panel, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.common.recurrence.dialogs.RecurrencePanel.superclass.constructor.call(this, config);
	},
		...
		return {
			...
			items: [{
				...
				items: [{
					...
					name: 'pattern',
					targetId: 'card-daily',
					patternValue: Grommunio.common.recurrence.data.RecurrenceType.DAILY,
					...
					patternValue: Grommunio.common.recurrence.data.RecurrenceType.WEEKLY,
					handler: this.onSwitchRecurrenceView,
					scope: this
				},{
					boxLabel: _('Monthly'),
					...
					patternValue: Grommunio.common.recurrence.data.RecurrenceType.MONTHLY,
					handler: this.onSwitchRecurrenceView,
					scope: this
				},{
					boxLabel: _('Yearly'),
					...
					patternValue: Grommunio.common.recurrence.data.RecurrenceType.YEARLY,
					handler: this.onSwitchRecurrenceView,
					scope: this
				}]
			},{
			...
			items: [{
				...
				items: [{
					...
					items: [{
						...
						endTerm: Grommunio.common.recurrence.data.RecurrenceEnd.NEVER,
						hideLabel: true,
						width: 25,
						listeners: {
					...
					items: [{
						xtype: 'radio',
						name: 'recurrence_term',
						endTerm: Grommunio.common.recurrence.data.RecurrenceEnd.N_OCCURRENCES,
						hideLabel: true,
						width: 25,
						listeners: {
							check: this.onSwitchRecurrenceTerm,
					...
					items: [{
						xtype: 'radio',
						name: 'recurrence_term',
						endTerm: Grommunio.common.recurrence.data.RecurrenceEnd.ON_DATE,
						hideLabel: true,
						width: 25,
						listeners: {
							check: this.onSwitchRecurrenceTerm,
	...
	onUTCFieldChange: function(field, newValue, oldValue)
	{
		...
		Ext.each(this.endPatternPanel.findByType('radio'), function(radio) {
			radio.setValue(radio.endTerm == Grommunio.common.recurrence.data.RecurrenceEnd.ON_DATE);
		});
	},
	...
	onToggleAllDay: function(checkbox, checked)
	{
		...
		var busyStatus = checked ? container.getSettingsModel().get('grommunio/v1/contexts/calendar/default_allday_busy_status') : Grommunio.core.mapi.BusyStatus.BUSY;
	 ...
	 * the data from the {@link Grommunio.core.data.IPMRecord record}.
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The record used to update the panel
	 * @param {Boolean} contentReset force the component to perform a full update of the data.
	 */
	update: function(record, contentReset)
	{
		...
		switch (endTerm) {
			case Grommunio.common.recurrence.data.RecurrenceEnd.NEVER:
				...
				break;
			case Grommunio.common.recurrence.data.RecurrenceEnd.N_OCCURRENCES:
				// Only apply default values at the first update
				...
				break;
			case Grommunio.common.recurrence.data.RecurrenceEnd.ON_DATE:
				// Only apply default values at the first update
				if (contentReset === true) {
					this.endOccurrencesSpinner.setValue(10);
				}
	...
	updateRecord: function(record)
	{
		...
		if (pattern) {
			...
			record.set('recurrence_type', Grommunio.common.recurrence.data.RecurrenceType.NONE);
		}
	...
	{
		Ext.each(this.endPatternPanel.findByType('radio'), function(radio) {
			radio.setValue(radio.endTerm == Grommunio.common.recurrence.data.RecurrenceEnd.N_OCCURRENCES);
		});
	}
});

Ext.reg('grommunio.recurrencepanel', Grommunio.common.recurrence.dialogs.RecurrencePanel);
