...
Ext.namespace('Grommunio.common.form');

/**
 * @class Grommunio.common.form.TextArea
 * @extends Ext.form.TextArea
 * @xtype grommunio.textarea
 *
 * The Textarea which extends{@link Ext.form.TextArea} and adds extra functionality
 * like inserting text at cursor position.
 */
Grommunio.common.form.TextArea = Ext.extend(Ext.form.TextArea, {
	 ...
	 * will apply the 'grommunio-contextmenu-enabled' CSS class on the {@link #el element}.
	...
	{
		Grommunio.common.form.TextArea.superclass.onRender.apply(this, arguments);

		if (this.enableSystemContextMenu) {
			this.el.addClass('grommunio-contextmenu-enabled');
		}
	...
	onFileDragOver: function(e)
	{
		...
		if (dt && (Array.prototype.indexOf.call(dt.types, 'Files') >= 0 || dt.files.length > 0 ||
			Grommunio.common.attachment.AttachmentDragDrop.hasPayload(dt))) {
			e.preventDefault();
		}
	},
	...
	onFileDrop: function(e)
	{
		...
		var ownerWindow = Grommunio.core.BrowserWindowMgr.activateOwnerWindow(this.el);
		...
		Ext.MessageBox.confirm(
			...
			function(btn) {
				...
				var record = Grommunio.core.Util.getDropTargetRecord(self);
				if (!record) {
					return;
				}
...
Ext.reg('grommunio.textarea', Grommunio.common.form.TextArea);
