/*
 * #dependsFile client/grommunio/core/mapi/RecipientType.js
 */
Ext.namespace('Grommunio.common.recipientfield.ui');

/**
 * @class Grommunio.common.recipientfield.ui.RecipientField
 ...
 * {@link Grommunio.core.data.IPMRecipientStore IPMRecipientStore} for storing
 * and resolving {@link Grommunio.core.data.IPMRecipientsRecord Recipients}.
 *
 ...
 */
Grommunio.common.recipientfield.ui.RecipientField = Ext.extend(Grommunio.common.ui.BoxField, {
	/**
	 * @cfg {Grommunio.core.mapi.RecipientType} defaultRecipientType The default recipient type value which should be used for the field
	...
	/**
	 * @cfg {Grommunio.core.mapi.RecipientType} filterRecipientType The recipient type value which is used for this Input field
	 ...
	 * @property
	 * @type Grommunio.common.recipientfield.ui.RecipientDragZone
	 */
	dragZone: undefined,
	 ...
	 * @property
	 * @type Grommunio.common.recipientfield.ui.RecipientDropZone
	 */
	dropZone: undefined,
	...
	constructor: function(config)
	{
		...
		if (!Ext.isDefined(config.store)) {
			config.store = new Grommunio.common.recipientfield.data.SuggestionListStore();
		}

		Ext.applyIf(config, {
			boxType: 'grommunio.recipientbox',
			itemSelector: 'div.x-grommunio-boxfield-suggestion-item',
			extraItemSelector: 'a.x-grommunio-boxfield-suggestion-cross',
			border: false,
			minChars: 1,
		...
		Grommunio.common.recipientfield.ui.RecipientField.superclass.constructor.call(this, config);
	...
	handleInput: function(value)
	{
		...
		var emailAddresses = value.match(Grommunio.reMultipleEmailAddresses);
		var splitAddresses = [];
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store to set on this field
	 */
	...
	update: function(record, contentReset)
	{
		if (record && record instanceof Grommunio.core.data.MAPIRecord) {
			...
			Grommunio.common.recipientfield.ui.RecipientField.superclass.update.apply(this, arguments);
		}
	},
	...
	setBoxStore: function(boxStore, initial)
	{
		...
		Grommunio.common.recipientfield.ui.RecipientField.superclass.setBoxStore.apply(this, arguments);
	 ...
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records The records that were sent in the resolve request.
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store which fired the event
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records The records that are pending resolving
	 */
	...
	initEvents: function()
	{
		Grommunio.common.recipientfield.ui.RecipientField.superclass.initEvents.apply(this, arguments);

		if ((this.enableDrag || this.enableDragDrop) && !this.dragZone) {
			this.dragZone = new Grommunio.common.ui.BoxFieldDragZone(this.container, {
				ddGroup: 'dd.mapiitem',
				field: this
			});
		}
		...
		if ((this.enableDrop || this.enableDragDrop) && !this.dropZone) {
			this.dropZone = new Grommunio.common.recipientfield.ui.RecipientDropZone(this.container, {
				ddGroup: 'dd.mapiitem',
				field: this
			});
		}
...
Ext.reg('grommunio.recipientfield', Grommunio.common.recipientfield.ui.RecipientField);
