...
/**
 * @class Grommunio.common.Actions
...
Grommunio.common.Actions = {
	...
	/**
	 * Open a {@link Grommunio.common.dialogs.CopyMoveContentPanel CopyMoveContentPanel} for
	 * copying or moving {@link Grommunio.core.data.IPMRecord records} to the
	 * preferred destination folder.
	 *
	 * @param {Grommunio.core.data.IPMRecord} records The record which must be copied or moved.
	...
	openCopyMoveContent: function(records, config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.copymoverecords'];
	 ...
	 *
	 * @param {Grommunio.core.data.MAPIRecord/Grommunio.core.data.MAPIRecord[]} records The record or records to normalize.
	 * @param {Grommunio.core.data.MAPIStore} store The store that should own the records.
	 * @return {Grommunio.core.data.MAPIRecord[]} Array containing the resolved records, excluding falsy entries.
	 */
	...
	/**
	 * Opens a {@link Grommunio.common.recurrence.dialogs.RecurrenceContentPanel RecurrenceContentPanel} for configuring
	 * the recurrence of the given {@link Grommunio.core.data.IPMRecord record}.
	 *
	 * @param {Grommunio.core.data.IPMRecord} records The record for which the recurrence must be configured.
	...
	openRecurrenceContent: function(records, config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.recurrence'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, records, config);
	},

	/**
	 * Opens the {@link Grommunio.common.categories.ui.CategoriesContextMenu CategoriesContextMenu} for
	...
	{
		Grommunio.core.data.UIFactory.openContextMenu(Grommunio.core.data.SharedComponentType['common.contextmenu.categories'], records, {
			position: position
		});
	},

	/**
	 * Opens the {@link Grommunio.common.flags.ui.FlagsMenu FlagsMenu} for
	 * the given {@link Grommunio.core.data.IPMRecord records}.
	 *
	 * @param {Grommunio.core.data.IPMRecord} records The record, or records for which the flags
	 * menu will be shown.
	 * @param {Array} position An array with the [x, y] position where the menu will be shown.
	...
	openFlagsMenu: function(records, position, shadowEdit)
	{
		...
		var component = Grommunio.core.data.SharedComponentType['common.contextmenu.flags'];
		Grommunio.core.data.UIFactory.openContextMenu(component, records, {
			position: position,
	...
	/**
	 * Opens a {@link Grommunio.common.flag.dialogs.CustomFlagContentPanel CustomFlagContentPanel} for set
	 * the custom flag and reminder to given {@link Grommunio.core.data.IPMRecord records}.
	 *
	 * @param {Grommunio.core.data.IPMRecord} records The record, or records for which the custom
	...
	openCustomFlagContent: function(records, config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.flags.dialogs.customflag'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, records, config);
	},

	/**
	 * Opens a {@link Grommunio.common.categories.dialogs.CategoriesContentPanel CategoriesContentPanel} for configuring
	 * the categories of the given {@link Grommunio.core.data.IPMRecord records}.
	 *
	 * @param {Grommunio.core.data.IPMRecord} records The record, or records for which the categories
	 * must be configured
	 * @param {Object} config (optional) Configuration object for creating the ContentPanel
	...
	openCategoriesContent: function(records, config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.categories'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, records, config);
	},

	/**
	 * Opens a {@link Grommunio.common.categories.dialogs.NewCategoryPanel NewCategoryPanel} for creating
	 * a new category.
	 *
	...
	openNewCategoryContent: function(config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.categories.dialogs.newcategory'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, undefined, config);
	},

	/**
	 * Opens a {@link Grommunio.common.categories.dialogs.RenameCategoryPanel RenameCategoryPanel} for renaming
	 * a standard category.
	 *
	 * @param {Object} config (optional) Configuration object for renaming the {@link Grommunio.common.categories.dialogs.RenameCategoryPanel RenameCategoryPanel}.
	 */
	openRenameCategoryContent: function(config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.categories.dialogs.renamecategory'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, undefined, config);
	},

	/**
	 * Opens a {@link Grommunio.common.attachment.dialogs.AttachItemContentPanel Attach Item Content Panel} which is used
	 * to attach an item as embedded attachment to a message.
	 *
	 * @param {Grommunio.core.data.MAPIRecord} record record that will be used to add embedded attachment
	 * @param {Object} config (optional) Configuration object for creating the ContentPanel
	 */
	openAttachItemSelectionContent: function(record, config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.attachment.dialog.attachitem'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, record, config);
	},

	/**
	 * Opens a {@link Grommunio.core.ui.widget.WidgetContentPanel}
	 * for inserting widgets into the {@link Grommunio.core.ui.widget.WidgetPanel}
	 * @param {Object} config (optional) Configuration object for creating the ContentPanel
	 */
	openWidgetsContent: function(config)
	{
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.widgets'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, undefined, config);
	},
	 ...
	 * @param {Grommunio.core.data.IPMRecipientRecord} recipient The recipient which must be opened
	 * @param {Object} config configuration object.
	 */
	openViewRecipientContent: function(recipient, config)
	{
		if (recipient.isResolved()) {
			...
			} else if (recipient.isPersonalDistList() || recipient.isSharedRecord(Grommunio.core.mapi.ObjectType.MAPI_DISTLIST)) {
			...
			Grommunio.core.data.UIFactory.openViewRecord(recipient, config);
		}
	},

	/**
	 * Opens a {@link Grommunio.common.delegates.dialogs.DelegatePermissionContentPanel DelegatePermissionContentPanel} for editing
	 * delegate permissions of a single delegate
	 * @param {Grommunio.common.delegates.data.DelegateRecord} delegateRecord record that should be opened in
	 * {@link Grommunio.common.delegates.dialogs.DelegatePermissionContentPanel DelegatePermissionContentPanel}.
	 * @param {Object} config configuration object that should be passed to {@link Grommunio.common.delegates.dialogs.DelegatePermissionContentPanel DelegatePermissionContentPanel}.
	 */
	...
	/**
	 * Opens a {@link @link Grommunio.common.sendas.dialogs.SendAsEditContentPanel SendAsEditContentPanel} for editing
	 * user name and email address of a sendAs.
	 * @param {Ext.data.Record} record record that should be opened in
	 * {@link Grommunio.common.sendas.dialogs.SendAsEditContentPanel SendAsEditContentPanel}.
	 * @param {Object} config configuration object that should be passed to {@link Grommunio.common.sendas.dialogs.SendAsEditContentPanel SendAsEditContentPanel}.
	 */
	...
	/**
	 * Opens a {@link @link Grommunio.common.sendas.dialogs.SendAsEditContentPanel SendAsEditContentPanel} Or
	 * {@link Grommunio.common.manageCc.dialogs.ManageCcEditContentPanel ManageCcEditContentPanel} Or
	 * {@link Grommunio.common.recipientfield.ui.EditRecipientContentPanel EditRecipientContentPanel} for editing
	...
	/**
	 * Opens a {@link Grommunio.common.rules.dialogs.RulesEditContentPanel RulesEditContentPanel} for editing
	 * a single {@link Grommunio.common.rules.data.RulesRecord RulesRecord}.
	 * @param {Grommunio.common.rules.data.RulesRecord} record record to edit in
	 * {@link Grommunio.common.rules.dialogs.RulesEditContentPanel RulesEditContentPanel}.
	 * @param {Object} config config object that will be passed to {@link Grommunio.core.data.UIFactoryLayer UIFactoryLayer}.
	 */
	...
	/**
	 * Will create an object of {@link Grommunio.common.attachment.ui.AttachmentDownloader AttachmentDownloader}
	 * and call {@link Grommunio.common.attachment.ui.AttachmentDownloader#downloadMessage} method to start download the message as file
	 * by setting the dialogFrame's location to the download URL of the given {@link Grommunio.core.data.IPMRecord records}.
	 * @param {Grommunio.core.data.IPMRecord} records The record, or records which user want to save as file.
	...
	openSaveEmlDialog: function(records, allAsZip)
	{
		...
		if(!allAsZip) {
			for (var i = 0; i < records.length; i++) {
				...
				downloadComponent = new Grommunio.common.attachment.ui.AttachmentDownloader();
	 ...
	 * @param {Grommunio.core.data.MAPIRecord|Grommunio.core.Context} objectToPrint The record(s)
	...
	openPrintDialog: function(objectToPrint)
	{
		...
		var openHandler = function (store, objectToPrint) {
			...
			var componentType = Grommunio.core.data.SharedComponentType['common.printer.renderer'];
		...
		if (objectToPrint instanceof Grommunio.core.data.MAPIRecord && !objectToPrint.isOpened()) {
	...
	/**
	 * Opens a {@link Grommunio.common.checknames.dialogs.CheckNamesContentPanel CheckNamesContentPanel}
	 *
	...
	{
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.checknames'];
		config = Ext.applyIf(config || {}, {
			checkNamesData: checkNamesData,
			modal: true
		});
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, recipientRecord, config);
	},

	/**
	 * Opens a {@link Grommunio.common.reminder.dialogs.ReminderContentPanel remindercontentpanel}
	 * @param {Grommunio.common.reminder.ReminderRecord} records Records for which the reminder content panel will be displayed.
	 * @param {Object} config (optional) Configuration object
	 */
	openReminderContent: function(records, config)
	{
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.reminder'];
		...
		var contentPanelInstances = Grommunio.core.data.ContentPanelMgr.getContentPanelInstances(component);
		...
		if(contentPanelInstances.getCount() === 0) {
			...
			if(records.length > 0) {
				...
				Grommunio.core.BrowserWindowMgr.getOwnerWindow(reminderDialog).focus();
	 ...
	 * @param {Grommunio.common.reminder.data.ReminderRecord|Grommunio.common.reminder.data.ReminderRecord[]} record
	...
	openReminderRecord: function(record, config)
	{
		...
		if (Grommunio.isDeskApp && window.global && window.global.DA && window.global.DA.winWebApp) {
			window.global.DA.winWebApp.focus();
		}
	},

	/**
	 * Opens a {@link Grommunio.common.dialogs.MessageBox.select MessageBox} for
	...
	openRecurringSelectionContent: function(record, handler, scope)
	{
		...
		Grommunio.common.dialogs.MessageBox.select(
	...
	deleteMeetingRequestConfirmationContent: function(record, handler, scope)
	{
		...
		var text;
		if(record.get('responsestatus') == Grommunio.core.mapi.ResponseStatus.RESPONSE_NOT_RESPONDED) {
	...
	/**
	 * Deletes all {@link Grommunio.core.data.IPMRecord records} from the {@link Grommunio.core.data.IPMStore store}.
	 * If the records are deleted from the To-do list the deleting is delegated to
	 * {@link Grommunio.task.Actions.deleteRecordsFromTodoList}; otherwise, it is delegated to {@link #doDeleteRecords}.
	 *
	...
	deleteRecords: function(records, askOcc, softDelete)
	{
		...
		if (folder && folder.isTodoListFolder()) {
			Grommunio.task.Actions.deleteRecordsFromTodoList(records);
			return;
		}
		if (Grommunio.core.EntryId.compareEntryIds(records[0].get('parent_entryid'), folderEntryid) === false) {
	...
	/**
	 * Deletes all {@link Grommunio.core.data.IPMRecord records} from the {@link Grommunio.core.data.IPMStore store}.
	 * If any of the given {@link Grommunio.core.data.IPMRecord records} is an recurring item, then
	 * a {@link Grommunio.common.dialogs.MessageBox.select MessageBox} will be prompted which lets the user
	 * select between the series or the single occurrence.
	 * All given {@link Grommunio.core.data.IPMRecord records} must be located in the same
	 * {@link Grommunio.core.data.IPMStore store}.
	 *
	 * @param {Array} records The array of records which must be deleted.
	 * @param {Boolean} askOcc (private) False to prevent a dialog to appear to ask if the occurrence or series must
	 * be deleted
	...
	doDeleteRecords: function(records, askOcc, softDelete)
	{
		...
		for (var i = 0, len = records.length; i < len; i++) {
			...
			if (Grommunio.core.MessageClass.isClass(messageClass, 'IPM.Schedule.Meeting', true) ||
				Grommunio.core.MessageClass.isClass(messageClass, 'IPM.TaskRequest', true)) {
			...
			if (deleteRecurring) {
				...
				switch (container.getSettingsModel().get("grommunio/v1/contexts/mail/readreceipt_handling")) {
				case 'never':
	 ...
	 * function was triggered in scope of the {@link Grommunio.task.TaskRecord TaskRecord}.
	 *
	...
	deleteRecurringItem: function(record) {
		Grommunio.common.Actions.deleteRecurringSelectionContent(record, function(button, radio) {
			if (button != 'ok') {
				return;
			}
			...
			Grommunio.common.Actions.deleteRecords(record, false);
		}, this);
	},
	...
	/**
	 * Opens a {@link Grommunio.common.restore.ui.RestoreContentPanel restoreContentPanel}
	 *
	...
	{
		var componentType = Grommunio.core.data.SharedComponentType['common.dialog.restoreitems'];
		config = Ext.applyIf(config || {}, {
			folder: folder
		});
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, undefined, config);
	...
	openImportContent: function(folder, config)
	{
		...
		var attachComponent = new Grommunio.common.attachment.ui.UploadAttachmentComponent(config);
		attachComponent.openAttachmentDialog();
	},

	/**
	 * Opens a {@link Grommunio.addressbook.dialogs.ABUserSelectionContentPanel ABUserSelectionContentPanel}
	 *
	...
	{
		var componentType = Grommunio.core.data.SharedComponentType['addressbook.dialog.abuserselection'];
		config = Ext.applyIf(config || {}, {
			modal: true
		});
	...
	/**
	 * Callback function for {@link Grommunio.addressbook.dialogs.ABUserSelectionContent AddressBook}
	 * This callback is used in {@link Grommunio.common.sendas.ui.SendAsPanel SendAsPanel}
	 * and {@link Grommunio.common.manageCc.ui.ManageCcPanel ManageCcPanel} to display a message
	...
	abCallBack: function(records)
	{
		...
		for (var i = 0; i < records.length; i++) {
			...
			var recipientType;
			if (store.customObjectType === Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_CC_RECIPIENT) {
	...
	/**
	 * Opens a {@link Grommunio.addressbook.dialog.ABMultiUserSelectionContentPanel ABMultiUserSelectionContentPanel}
	 *
	...
	openABUserMultiSelectionContent: function(config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['addressbook.dialog.abmultiuserselection'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, undefined, config);
	},
	 ...
	 *
	 * @param {Grommunio.core.data.IPMAttachmentRecord} attachment The attachment to remove
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord/Array} records The record or records which must
	 * be marked as read.
			...
			default: {
				...
				Ext.MessageBox.show({
					...
					// If the mailgrid has reloaded, retrieve the newly updated record.
						var resolved = Grommunio.common.Actions.resolveRecords([this], store);
	 ...
	 *
	 * @param {Grommunio.core.data.IPMAttachmentRecord} records The record of the file to be downloaded
	 * @param {Boolean} allAsZip (optional) True to downloading all the attachments as ZIP
	 */
	downloadAttachment: function(record, allAsZip)
	{
		...
		if (this.downloadFrame) {
			...
			if (!Ext.getBody().contains(this.downloadFrame.getEl().dom)) {
				this.downloadFrame = new Grommunio.common.attachment.ui.AttachmentDownloader();
			}
		} else {
			this.downloadFrame = new Grommunio.common.attachment.ui.AttachmentDownloader();
		}
	...
	/**
	 * Opens a {@link Grommunio.common.rules.dialogs.RulesWordsEditContentPanel}
	 *
	 * @param {Object} config Configuration object for the dialog
	 */
	openRulesWordsEditContent: function(config)
	{
		var componentType = Grommunio.core.data.SharedComponentType['common.rules.dialog.ruleswordsedit'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, undefined, config);
	},

	/**
	 * Function is used to download attachments, for embedded message attachments additionally it will
	 * convert the {@link Grommunio.core.data.IPMAttachmentRecord IPMAttachmentRecord} to {@link Grommunio.core.data.IPMRecord IPMRecord}
	 * and then will pass it to {@link Grommunio.core.ui.ContentPanel ContentPanel} to open it.
	 * @param {Grommunio.core.data.IPMAttachmentRecord} record The attachment record which should be opened.
	 * @param {Object} config configuration object.
	 */
	openAttachmentRecord: function(record, config)
	{
		...
		} else if (Grommunio.common.Actions.isSupportedDocument(record.get("name"))) {
			// 'modal' is accepted by the dialog layer alone, and passing it forces
			// that layer whatever the setting asks for.
			layerType = Grommunio.common.Actions.getFilePreviewerTarget();
	...
	importToFolder: function(record, config)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.attachment.dialog.importtofolder'];
		Grommunio.core.data.UIFactory.openLayerComponent(componentType, record, config);
	},
			...
			if ((record.isUnsent() || record.isScheduled()) && !record.isFaultyMessage()) {
				...
				if (!record.isRead() && !record.isMessageClass('IPM.TaskRequest', true)) {
					if (!record.needsReadReceipt()) {
						...
						var handling = container.getSettingsModel().get('grommunio/v1/contexts/mail/readreceipt_handling');
						if (handling === 'never') {
							record.addMessageAction('mark_read', true);
							record.addMessageAction('send_read_receipt', false);
				...
				if(record.isMessageClass('IPM.TaskRequest', true)) {
					record = Grommunio.core.data.RecordFactory.createRecordObjectByMessageClass('IPM.Task', {
	 ...
	 * {@link Grommunio.core.mapi.RecipientType.MAPI_TO To}, {@link Grommunio.core.mapi.RecipientType.MAPI_CC Cc} or
	 * {@link Grommunio.core.mapi.RecipientType.MAPI_BCC BCc}.
	 *
	 * @param {Grommunio.core.data.IPMRecipientStore} store The {@link Grommunio.core.data.IPMRecipientStore store} which contains the recipients.
	 * @param {Number} recipientType The recipientType it can be {@link Grommunio.core.mapi.RecipientType.MAPI_TO To},
	 * {@link Grommunio.core.mapi.RecipientType.MAPI_CC Cc} or {@link Grommunio.core.mapi.RecipientType.MAPI_BCC BCc}.
	 *
	 * @returns {Grommunio.core.dat.IPMRecipientRecord[]} same record type of records from given store.
	 */
	...
	{
		return Grommunio.common.previewer.data.Formats.isSupported(path);
	},
	...
	isFilePreviewerEnabled: function ()
	{
		...
		return container.getSettingsModel().getOneOf('grommunio/v1/plugins/filepreviewer/enable', 'grommunio/v1/main/file_previewer/enable');
	},

	/**
	 * The {@link Grommunio.core.data.UIFactory} layers a preview can open in, in the
	 * order they are offered. A browser window needs
	 * {@link Grommunio#supportsPopOut pop-out}, so it is absent where that is missing.
	 *
	...
	{
		if (Grommunio.common.Actions.getFilePreviewerTargets().indexOf(target) === -1) {
			return false;
		}

		// The tab panel belongs to the main window, so a tab opened from a popped-out window lands behind it.
		return target !== 'tabs' || Grommunio.core.BrowserWindowMgr.isMainWindowActive();
	},

	/**
	 * The layer a preview opens in, from grommunio/v1/main/file_previewer/target.
	...
	{
		var target = container.getSettingsModel().get('grommunio/v1/main/file_previewer/target');

		return Grommunio.common.Actions.canPreviewInTarget(target) ? target : 'dialogs';
	},

	/**
	 * Copy email address(es) of the given recipient or type of recipient.
	 *
	 * @param {Grommunio.core.dat.IPMRecipientRecord} record The record is resolved
	 * @param {Grommunio.core.data.IPMRecipientStore} store The {@link Grommunio.core.data.IPMRecipientStore store} which contains the recipients.
	...
	copyEmailAddress: function (record, store, copyAll)
	{
		...
		Grommunio.core.Util.copyToClipboard(email);
	},

	/**
	 * It will create a new {@link Grommunio.core.data.MAPIRecord mail record}
	 * and generate new phantom recipient for new mail. Then
	 * it will open a new MailCreate ContentPanel for the new mail
	 *
	 * @param {Grommunio.core.dat.IPMRecipientRecord} record The record is recipient record
	 * @private
	 */
	onEmailRecipient: function(recipient)
	{
		...
		var recipientRecord = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, recipient.data);
		...
		recipientRecord.set('recipient_type', Grommunio.core.mapi.RecipientType.MAPI_TO);
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder to which files needs to be imported.
	 * @param {Boolean} show Open the imported item
	 */
	importItemCallback: function(files, folder, show)
	{
		Grommunio.common.Actions.brokenFiles = [];
		Grommunio.common.Actions.totalFiles = files.length;
		Grommunio.common.Actions.showImported = show === true;
	...
	importDone: function(response, folder)
	{
		...
		if ( this.showImported === true && Ext.isDefined(response.items)) {
			...
			record.opened = false;
			Grommunio.core.data.UIFactory.openViewRecord(record);
		}
	},
	...
	/**
	 * Helper function to import selected files in given {@link Grommunio.hierarchy.data.MAPIFolderRecord}.
	...
	importFiles: function(files, folder)
	{
		if (this.brokenFiles.length > 0) {
			if (this.brokenFiles.length > 1) {
				...
				var componentType = Grommunio.core.data.SharedComponentType['hierarchy.dialog.brokenfiles'];
				Grommunio.core.data.UIFactory.openLayerComponent(componentType, this.brokenFiles, {'modal': true});
			} else {
		...
		url = Ext.urlAppend(url, 'dialog_attachments=' + Grommunio.generateId(32));
	...
	/**
	 * Helper function which show the {@link Grommunio.common.dialogs.MessageBox#addCustomButtons MessageBox} to ask user to
	 * copy instead of move records due to lack of record access or folder rights.
	 *
	 * @param {Grommunio.core.data.MAPIRecord[]} records The records which is going to copy in targeted folder.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} targetFolder The target folder on which records is going to copy.
	 * @param {Grommunio.core.data.ListModuleStore} store The which contains the records.
	...
	showMessageBox: function(records, targetFolder, store, message, scope, callBack, options)
	{
		...
		Grommunio.common.dialogs.MessageBox.addCustomButtons({
			...
			fn: function(button) {
				...
				var actionableRecords = Grommunio.common.Actions.resolveRecords(records, store);
	...
	notify: function (title, options, handlers)
	{
		...
		var pluginBasePath = 'grommunio/v1/plugins/desktopnotifications/';
		var isPlugInEnabled = settingsModel.get(pluginBasePath + 'enable') === true;
		var baseSettingPath = isPlugInEnabled ? pluginBasePath : 'grommunio/v1/main/desktop_notification/';
		var soundDisabled = !isPlugInEnabled || settingsModel.get('grommunio/v1/main/desktop_notification/disable_sound');
