Ext.namespace('Grommunio.common.flags');

/**
 * @class Grommunio.common.flags.Util
 * @extends Object
 *
 * Collection of common functions to handle flags
 */
Grommunio.common.flags.Util = {
	/**
	...
	{
		return {
			flag_icon: 		Grommunio.core.mapi.FlagIcon.red,
			flag_request: 		'Follow up',
			flag_status: 		Grommunio.core.mapi.FlagStatus.flagged,
			flag_complete_time:	null,
			reminder:		true,
			// Outlook reads a flagged mail as an unfinished task.
			complete:		false,
			task_status:		Grommunio.core.mapi.TaskStatus.NOT_STARTED,
			percent_complete:	0,
			date_completed:		null
	...
	getFlagPropertiesComplete: function()
	{
		...
		return {
			...
			task_status:		Grommunio.core.mapi.TaskStatus.COMPLETE,
			percent_complete:	1,
	...
	getFlagPropertiesRemoveFlag: function()
	{
		return {
			flag_icon: 			Grommunio.core.mapi.FlagIcon.clear,
			flag_request: 		'',
			flag_status: 		Grommunio.core.mapi.FlagStatus.cleared,
			reminder:		false,
			startdate:     null,
			duedate:      null,
			reminder_time:		null,
	...
	retrieveWorkingDays: function()
	{
		if ( !this.firstWorkingDay ) {
			...
			var workingDays = container.getSettingsModel().get('grommunio/v1/main/working_days') || [1, 5];
			workingDays.sort();
	...
	getReminderTimeForDate: function(date)
	{
		...
		var startWorkingHours = parseInt(container.getSettingsModel().get('grommunio/v1/main/start_working_hour'), 10);
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The record for which configured flag needs to be identified.
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The records which categories update with old-style flags
	 * to real categories.
	 */
	updateCategories: function(records)
	{
		...
		records.forEach(function(record){
			...
			if ( flagStatus===Grommunio.core.mapi.FlagStatus.flagged && flagIcon!==Grommunio.core.mapi.FlagIcon.red){
				const categories = Grommunio.common.categories.Util.getCategories(record);
				Grommunio.common.categories.Util.setCategories(record, categories);
			}
		});
	}
};
