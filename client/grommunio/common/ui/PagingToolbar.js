...
/**
 * @class Grommunio.common.ui.PagingToolbar
 * @extends Ext.PagingToolbar
 * @xtype grommunio.paging
 *
 * A simple paging tool bar for context.
 */
Grommunio.common.ui.PagingToolbar = Ext.extend(Ext.PagingToolbar, {
	/**
	...
	{
		this.cls = 'grommunio-paging-toolbar';
	...
	bindStore: function(store, initial){
		Grommunio.common.ui.PagingToolbar.superclass.bindStore.apply(this, arguments);
		...
		if(this.store) {
			this.mon(this.store, 'search', function(){
				...
				this.pageSize = container.getSettingsModel().get('grommunio/v1/main/page_size');
			}, this);
	 ...
	 * {@link Grommunio.core.data.IPMStore#load onLoad}
	 * @private
	 */
	updateInfo: function()
	{
	...
	/**
	 * Event handler which is fired after the {@link Grommunio.core.data.IPMStore store} has
	 * loaded data from the server. This will enable the "Refresh" button again, causing a "loading"
	 * icon to disappear.
	 * @param {Grommunio.core.data.IPMStore} store.
	 ...
	 * {@link Grommunio.core.data.IPMStore store} which is used for mapping Object for load calls.
	 * @private
	 */
	getParams: function()
	{
	 ...
	 * {@link Grommunio.core.data.IPMStore store} to get the data
	...
	doLoad: function(start)
	{
		...
		if(this.fireEvent('beforechange', this, o) !== false){
			if (this.store.hasFilterApplied) {
				o.restriction["filter"] = this.store.getFilterRestriction(Grommunio.common.data.Filters.UNREAD);
				this.cursor = 0;
			}
			this.store.load({
...
Ext.reg('grommunio.paging', Grommunio.common.ui.PagingToolbar);
