...
/**
 * @class Grommunio.common.ui.messagepanel.ExtraInfoLinks
 * @extends Ext.Container
 * @xtype grommunio.extrainfolinks
 */
Grommunio.common.ui.messagepanel.ExtraInfoLinks = Ext.extend(Ext.Container, {
	/**
	 ...
	 * if there is any priority set in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if there is any sensitivity set in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if the current {@link Grommunio.core.data.IPMRecord record} is faulty and we will not be able to
	 ...
	 * if there is any change in meeting time set in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if meeting has occurred in the past or expired {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if change in meeting request is been received by the organizer himself in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if proposed time for meeting request has been accepted by the organizer himself in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if meeting time set in {@link Grommunio.core.data.IPMRecord record} is already elapsed.
	 */
	 ...
	 * if meeting accepted time set in {@link Grommunio.core.data.IPMRecord record} is empty.
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} is an update of a previously sent meeting request.
	 */
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} is received as a delegate.
	 */
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} last_verb_executed property is 102 or 103. Which
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} last_verb_executed property is 104. Which
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} taskhistory property is {@link Grommunio.core.mapi.TaskHistory#ASSIGNED}
	 * and {@link Grommunio.task.TaskRecord#isTaskReceived task is received}. Which
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} taskhistory property is {@link Grommunio.core.mapi.TaskHistory#ASSIGNED}
	 * and {@link Grommunio.task.TaskRecord#isTaskReceived task is not received}. Which
	 ...
	 * {@link Grommunio.core.data.IPMRecord record} taskhistory property is {@link Grommunio.core.mapi.TaskHistory#ACCEPTED}
	 ...
	 * dialog if the {@link Grommunio.core.data.IPMRecord record} taskhistory property is
	 * {@link Grommunio.core.mapi.TaskHistory#DECLINED}, and taskstate property is not
	 * {@link Grommunio.core.mapi.TaskState#ACCEPT}. Which means task is assigner received decline
	 ...
	 * @cfg {String} declineTaskRequest string which must be displayed in the
	 * dialog if the {@link Grommunio.core.data.IPMRecord record} taskhistory property is
	 * {@link Grommunio.core.mapi.TaskHistory#UPDATED}, and
	 * {@link Grommunio.task.TaskRecord#isTaskRequest record} is not task request record.
	...
	constructor: function(config)
	{
		...
		Grommunio.common.ui.messagepanel.ExtraInfoLinks.superclass.constructor.call(this, config);
	},

	/**
	 * Set sensitivity, important/priority information from {@link Grommunio.core.data.IPMRecord record}.
	 * it will also handle showing/hiding of {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks}.
	 *
	 * @param {Grommunio.core.data.IPMRecord} record to update the {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks} with
	 */
	update: function(record)
	{
		...
		if (entryid) {
			var objectIdText = Grommunio.core.EntryId.formatObjectId(entryid);
			if (objectIdText) {
		...
		if(Ext.isDefined(sensitivity) && sensitivity != Grommunio.core.mapi.Sensitivity.NONE && this.isShowExtraInfo()){
			var text = String.format(this.sensitivityInfoString, Grommunio.core.mapi.Sensitivity.getDisplayName(sensitivity));
			el.createChild({tag: 'div', html: text, cls: this.itemCls});
			isVisible = true;
		}

		if(Ext.isDefined(importance) && importance != Grommunio.core.mapi.Importance.NORMAL && this.isShowExtraInfo()){
			var text = String.format(this.importantInfoString, Grommunio.core.mapi.Importance.getDisplayName(importance));
			el.createChild({tag: 'div', html: text, cls: this.itemCls});
			isVisible = true;
		}
		...
		if(this.record instanceof Grommunio.core.data.MessageRecord) {
			...
			if(this.record instanceof Grommunio.calendar.MeetingRequestRecord) {
				isVisible = (this.setMeetingInfo(el) === true ? true : isVisible);
			} else if(this.record instanceof Grommunio.task.TaskRecord) {
		...
		if (isVisible) {
			...
			var parent = this.findParentByType('grommunio.taskgeneraltab');
			if ( parent ){
				parent.doLayout();
			} else {
				this.doLayout();
	...
	/**
	 * Function will be used to determine that {@link Grommunio.core.mapi.Sensitivity Sensitivity} or
	 * {@link Grommunio.core.mapi.Importance importance} related extra info link on header of the record.
	 *
	 * @return {boolean}
	 */
	...
	/**
	 * Function will be used to display {@link Grommunio.common.ui.messagepanel.ExtraInfoContextMenu ExtraInfoContextMenu}
	...
	{
		Grommunio.core.data.UIFactory.openContextMenu(Grommunio.core.data.SharedComponentType['common.contextmenu.previewpanel.extrainfo'], this.record, { position: evtObject.getXY() });
	},
	...
	/**
	 * Set task request information from {@link Grommunio.core.data.IPMRecord record} data.
	 *
	 * @param {Ext.Element/HTMLElement} el The target element which is being laid out
	 * @return {Boolean} true if function added any info to {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks}
	...
	setTaskRequestInfo: function (el)
	{
		...
		if(delegatorEntryid && !Grommunio.core.EntryId.compareABEntryIds(delegateEntryid, delegatorEntryid)) {
		...
		switch (taskHistory) {
			case Grommunio.core.mapi.TaskHistory.ASSIGNED:
				...
				isVisible = true;
				break;
			case Grommunio.core.mapi.TaskHistory.ACCEPTED:
				if (delegateResponse ) {
				...
				isVisible = true;
				break;
			case Grommunio.core.mapi.TaskHistory.DECLINED:
				if (delegateResponse ) {
				...
				isVisible = true;
				break;
			case Grommunio.core.mapi.TaskHistory.UPDATED:
				if (delegateResponse ) {
	...
	/**
	 * Set meeting information from {@link Grommunio.core.data.IPMRecord record} data
	 * and if found will update the panel by loading data from the record data into the template
	 *
	 * @param {Ext.Element/HTMLElement} el The target element which is being laid out
	 * @return {Boolean} true if {@link Grommunio.core.data.IPMRecord IPMRecord} contains any meeting info
	 * @private
	 */
	 ...
	 * get info from {@link Grommunio.core.data.IPMRecord record} data.
	 * @param {Ext.Element/HTMLElement} el The target element which is being laid out
	 * @return {Boolean} true if function added any info to {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks}
	 * and it should be made visible else false.
	 * @private
	...
	/**
	 * Set meeting request information from {@link Grommunio.core.data.IPMRecord record} data.
	 * @param {Ext.Element/HTMLElement} el The target element which is being laid out
	 * @return {Boolean} true if function added any info to {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks}
	 * and it should be made visible else false.
	 * @private
	...
	/**
	 * Set meeting response information from {@link Grommunio.core.data.IPMRecord record} data
	 * and if found will update the panel by loading data from the record data into the template
	 * @param {Ext.Element/HTMLElement} el The target element which is being laid out
	 * @return {Boolean} true if function added any info to {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks}
	 * and it should be made visible else false.
	...
	/**
	 * Set recurring meeting request information from {@link Grommunio.core.data.IPMRecord record} data.
	 * @param {Ext.Element/HTMLElement} el The target element which is being laid out
	 * @return {Boolean} true if function added any info to {@link Grommunio.common.ui.messagepanel.ExtraInfoLinks ExtraInfoLinks}
	 * and it should be made visible else false.
	 * @private
...
Ext.reg('grommunio.extrainfolinks', Grommunio.common.ui.messagepanel.ExtraInfoLinks);
