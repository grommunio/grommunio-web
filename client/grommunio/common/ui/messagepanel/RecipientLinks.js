...
/**
 * @class Grommunio.common.ui.messagepanel.RecipientLinks
 * @extends Ext.DataView
 * @xtype grommunio.recipientlinks
 *
 ...
 */
Grommunio.common.ui.messagepanel.RecipientLinks = Ext.extend(Ext.DataView, {
	/**
	...
	/**
	 * @cfg {Grommunio.core.mapi.RecipientType} recipientType The recipientType
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config,{
			...
			multiSelect: false,
			overClass: 'grommunio-recipient-link-over',
			itemSelector: 'span.grommunio-recipient-link',
				...
				'<tpl if="values.length &gt; 0">' +
					'<div class="preview-header-recipientbox">' +
						...
						'<div class="preview-recipient-data" style="max-height: {this.maxHeight}px">' +
							'<tpl for=".">' +
								'<span viewIndex="{viewIndex}" class="grommunio-emailaddress-link grommunio-recipient-link">' +
									'<span class="grommunio-presence-status {[Grommunio.core.data.PresenceStatus.getCssClass(values.presence_status)]}">'+
		...
		Grommunio.common.ui.messagepanel.RecipientLinks.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.common.ui.messagepanel.RecipientLinks.superclass.initComponent.call(this);
	...
	/**
	 * Apply a record on the DataView. If the record is {@link Grommunio.core.data.IPMRecord#isOpened},
	 * then the {@link Grommunio.core.data.IPMRecord#getAttachmentStore attachment store} will be
	 * {@link #bindStore bound} to this view.
	 * @param {Grommunio.core.data.IPMRecord} record The record to apply
	 */
		...
		for (var i = 0, len = records.length; i < len; i++) {
			...
			if (!Ext.isDefined(this.recipientType) || this.recipientType === record.get('recipient_type')) {
				...
				if (organizerEntryId && record.get('entryid') &&
					Grommunio.core.EntryId.compareABEntryIds(record.get('entryid'), organizerEntryId)) {
					continue;
				}
	 ...
	 * @param {Grommunio.core.data.IPMRecipientRecord} data The recipient record to be prepared
	 * @param {Number} index
	...
	update: function(record, contentReset)
	{
		if (record && record instanceof Grommunio.core.data.MAPIRecord) {
			...
			Grommunio.common.ui.messagepanel.RecipientLinks.superclass.update.apply(this, arguments);
		}
	},

	/**
	 * Function that is called to refresh a single node of this DataView. Overwritten
	 * because all {@link Grommunio.common.ui.messagepanel.RecipientLinks RecipientLinks} (To,
	 ...
	 * keep that in mind.
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store that holds the updated record
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The record that was updated
	 * @private
	 */
	...
	refresh: function()
	{
		...
		if (records.length < 1) {
			...
			Ext.each(filteredRecords, function(filteredRecord, index) {
				var user = Grommunio.core.data.UserIdObjectFactory.createFromRecord(new Ext.data.Record(filteredRecord));
				filteredRecords[index].presence_status = Grommunio.core.PresenceManager.getPresenceStatusForUser(user);
			});
	...
	/**
	 * Called when user right-clicks on an item in {@link Grommunio.common.ui.messagepanel.RecipientLinks}
	...
	onRecipientRightClick: function (dataView, index, node, e)
	{
		...
		Grommunio.core.data.UIFactory.openHoverCard(recipientRecord, {
			position: e.getXY(),
	...
	/**
	 * Called when user double-clicks on an item in {@link Grommunio.common.ui.messagepanel.RecipientLinks}
	 * invokes {@link Grommunio.common.Actions#openViewRecipientContent} with the selected {@link Grommunio.core.data.IPMRecord}
	 * @param {Ext.DataView} dataView Reference to this object
	 * @param {Number} index The index of the target node
	 * @param {HTMLElement} node The target node
	 ...
	 * @return {Grommunio.core.data.IPMRecipientRecord} recipient record that will be used for sending the mail
	 * @private
	 */
	createRecipientFromNode: function(node)
	{
		var record = this.getRecord(node);
		record = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, record.data);
...
Ext.reg('grommunio.recipientlinks', Grommunio.common.ui.messagepanel.RecipientLinks);
