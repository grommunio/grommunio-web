Ext.namespace('Grommunio.common.ui.messagepanel');

/**
 * @class Grommunio.common.ui.messagepanel.ExtraInfoContextMenu
 * @extends Grommunio.core.ui.menu.ConditionalMenu
 * @xtype grommunio.extrainfocontextmenu
 */
Grommunio.common.ui.messagepanel.ExtraInfoContextMenu = Ext.extend(Grommunio.core.ui.menu.ConditionalMenu, {
	// Insertion points for this class
	/**
	 * @insert context.common.previewpanel.extrainfo.contextmenu.actions
	 * @param {Grommunio.common.ui.messagepanel.ExtraInfoContextMenu} contextmenu This contextmenu
	 */

	/**
	 * @constructor
	 * @param {Object} config Configuration object
	 */
	constructor: function(config)
	{
		config = config || {};

		if (config.records) {
			if (Array.isArray(config.records)) {
				config.records = config.records[0];
			}
		}

		Ext.applyIf(config, {
			items: [{
				xtype: 'grommunio.conditionalitem',
				text: _('Download Pictures'),
				handler: this.showPictures,
				scope: this
			}, {
				xtype: 'menuseparator'
			}, {
				xtype: 'grommunio.conditionalitem',
				text: _('Add Sender to Safe Senders List'),
				handler: this.addSenderToSafeList,
				scope: this
			}, {
				xtype: 'grommunio.conditionalitem',
				text: _('Add Domain to Safe Senders List'),
				handler: this.addDomainToSafeList,
				scope: this
			}]
		});

		Grommunio.common.ui.messagepanel.ExtraInfoContextMenu.superclass.constructor.call(this, config);
	},

	/**
	 * Function will calculate block_status property value of {@link Grommunio.core.data.MessageRecord MessageRecord}
	 * and set it and will save the record to send changes to server
	 * @private
	 */
	showPictures: function()
	{
		this.setBlockStatus();
	},

	/**
	 * Function will add smtp address of sender of this mail to {@link Grommunio.settings.SettingsModel #safe_senders_list}
	 * so the mails from this user will not be checked for blocking external content.
	 * @private
	 */
	addSenderToSafeList: function()
	{
		var smtpAddress = this.records.get('sent_representing_email_address') || this.records.get('sender_email_address');
		if (Ext.isEmpty(smtpAddress)) {
			return;
		}

		Grommunio.mail.data.JunkMailStore.addSafeSender(smtpAddress);

		this.setBlockStatus();
	},

	/**
	 * This function adds domain address of sender of this mail to the safe senders list,
	 * so mails from this domain will not have external content blocked.
	 * @private
	 */
	addDomainToSafeList: function()
	{
		var smtpAddress = this.records.get('sent_representing_email_address') || this.records.get('sender_email_address');
		var domainName = '@' + smtpAddress.substr(smtpAddress.indexOf('@') + 1);
		if (Ext.isEmpty(domainName) || domainName === '@') {
			return;
		}

		Grommunio.mail.data.JunkMailStore.addSafeSender(domainName);

		this.setBlockStatus();
	},

	setBlockStatus: function()
	{
		var blockStatusValue = this.records.calculateBlockStatus();

		if (blockStatusValue) {
			this.records.set('block_status', blockStatusValue);
			this.records.save();
		}
	}
});

Ext.reg('grommunio.extrainfocontextmenu', Grommunio.common.ui.messagepanel.ExtraInfoContextMenu);
