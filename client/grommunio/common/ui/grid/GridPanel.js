...
/**
 * @class Grommunio.common.ui.grid.GridPanel
 ...
 */
Grommunio.common.ui.grid.GridPanel = Ext.extend(Ext.grid.GridPanel, {
	/**
	 * @cfg {String} ddText
	 * @hide
	 */
	...
	/**
	 * The entryid of the {@link Grommunio.core.data.MAPIFolder folder} which is currently being displayed
	...
	/**
	 * The store entryid of the {@link Grommunio.core.data.MAPIFolder folder} which is currently being displayed
	 * inside this panel. This is used by {@link #getStateName} to obtain the correct {@link #getState state}.
	 *
	 * @property
	 * @type String
	 ...
	 * @property
	 * @type {Grommunio.common.ui.grid.DateTooltip}
	 */
	dateTooltip: null,
	...
	constructor: function(config)
	{
		...
		if(Ext.isEmpty(config.view)) {
			...
			Ext.applyIf(config, {
				view: new Grommunio.common.ui.grid.GridView(config.viewConfig)
			});
		}

		Grommunio.common.ui.grid.GridPanel.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.common.ui.grid.GridPanel.superclass.afterRender.apply(this, arguments);
		var xtypes = this.getXTypes();

		// The first part leading up to grommunio.gridpanel will be stripped
		xtypes = xtypes.replace('component/box/container/panel/grid/grommunio.gridpanel','');

		// Then the "grommunio." will be stripped off from all the xtypes like "grommunio.somegrid".
	  ...
	  this.dateTooltip = new Grommunio.common.ui.grid.DateTooltip({
	...
	onDestroy: function()
	{
		Grommunio.common.ui.grid.GridPanel.superclass.onDestroy.apply(this, arguments);
	...
	initEvents: function()
	{
		...
		Grommunio.common.ui.grid.GridPanel.superclass.initEvents.call(this);

		// use our custom load mask
		if(this.loadMask) {
			// destroy loadmask created by superclass
			this.loadMask.destroy();
			this.loadMask = new Grommunio.common.ui.LoadMask(this.bwrap,
	 ...
	 * synchronize {@link Grommunio.core.data.ListModuleStore store} with as many numbers of new {@link Grommunio.core.data.IPMRecords[] records} as deleted
			...
			if (!store.syncStore) {
				...
				if(Ext.isDefined(gridScroller) && !gridScroller.isScrollable()) {
					if (store.totalLoadedRecord < store.totalLength) {
						...
						var limit = container.getSettingsModel().get('grommunio/v1/main/page_size');
	...
	reconfigure: function(store, colModel)
	{
		...
		Grommunio.common.ui.grid.GridPanel.superclass.reconfigure.call(this, store, colModel);
	},
	 ...
	 * or more accurately the {@link Grommunio.common.ui.LoadMask} do not handle the case nicely when the store
	...
	{
		var show = this.store && (this.store.isExecuting(Grommunio.core.Actions['list']) === true ||
			this.store.isExecuting(Grommunio.core.Actions['search']) === true);
	...
	onStoreBeforeLoad: function(store, options)
	{
		 ...
		 */
		if(options && (options.actionType === Grommunio.core.Actions['updatelist'])) {
			if(!this.isLoading) {
	 ...
	 *
	 * @param {Grommunio.core.data.MAPIStore} store The store which being loaded.
	 ...
	 * {@link Grommunio.common.ui.grid.ColumnModel#setConfig}.
	 *
	 ...
	 * Will set the class 'grommunio-multiselection' on selected rows when more than
	...
	onGridSelectionChange: function(selectionModel)
	{
		...
		if ( el ){
			var rows = el.query('.grommunio-multiselection');
			for ( var i=0; i<rows.length; i++ ){
				Ext.fly(rows[i]).removeClass('grommunio-multiselection');
			}
		}
		...
		if ( selection.length > 1 ){
			...
			Ext.each(selection, function(record){
				...
				if ( idx >= 0 ) {
					view.addRowClass(idx, 'grommunio-multiselection');
				}
			}, this);
		}
	},
	...
	getState: function()
	{
		var state = Grommunio.common.ui.grid.GridPanel.superclass.getState.call(this);
	...
	applyState: function(state)
	{
		if (state) {
			...
			Grommunio.common.ui.grid.GridPanel.superclass.applyState.call(this, unwrap);
		}
	}
});

Ext.reg('grommunio.gridpanel', Grommunio.common.ui.grid.GridPanel);
