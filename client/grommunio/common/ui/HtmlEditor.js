...
/*
 * #dependsFile client/grommunio/mail/data/ActionTypes.js
 */
Ext.namespace("Grommunio.common.ui");

/**
 * @class Grommunio.common.ui.HtmlEditor
 * @extends Ext.ux.form.TinyMCETextArea
 * @xtype grommunio.htmleditor
 *
 * Our HtmlEditor is a simple override of the TinyMCE provided HtmlEditor.
 * It automatically enables all buttons which are needed within Grommunio, and adds
 ...
 */
Grommunio.common.ui.HtmlEditor = Ext.extend(Ext.ux.form.TinyMCETextArea, {
	/**
	...
	constructor: function(config)
	{
		...
		var themeIsDark = Grommunio.core.DarkMode.isDark();

		this.defaultFontFamily = container.getSettingsModel().get("grommunio/v1/main/default_font");
		this.defaultFontSize = Grommunio.common.ui.htmleditor.Fonts.getDefaultFontSize();
		...
		// Grommunio.mail.MailContextModel is used here.
		...
		config = Ext.applyIf(config, {
			xtype: "grommunio.tinymcetextarea",
			hideLabel: true,
			hideMode: "offsets",
			readOnly: false,
			tinyMCEConfig: {
				...
				setup: function(editor) {
					...
					editor.on("drop", function(e) {
						...
						var editorWindow = Grommunio.core.BrowserWindowMgr.activateOwnerWindow(self.getEl()) || window;
						...
						var dragDrop = Grommunio.common.attachment.AttachmentDragDrop;
						...
						// Grommunio.core.data.IPMAttachmentStore#uploadFiles() only treats its
		...
		var lang = container.getSettingsModel().get("grommunio/v1/main/language", true);
		var tinyLanguageCode = Grommunio.common.ui.htmleditor.LanguageMap.getTinyLanguageCode(lang);
		 ...
		 * @param {Grommunio.common.ui.HtmlEditor} this The component which fired the event
		...
		Grommunio.common.ui.HtmlEditor.superclass.constructor.call(this, config);
	 ...
	 * @param {Grommunio.common.ui.HtmlEditor} htmlEditor The editor which is initialized.
	...
	setValue: function(value)
	{
		...
		this.withEd((function() {
			...
			Grommunio.common.ui.HtmlEditor.superclass.setValue.call(this, setValue);
			if (self.rendered) {
	...
	*/
	...
	{
		if (this.disabled) {
			Grommunio.common.ui.HtmlEditor.superclass.enable.apply(this, arguments);
			var editor = this.getEditor();
	 ...
	 *
	 * @param {Grommunio.common.ui.htmleditor.HtmlEditor} editor The HTML editor
	...
	*/
	...
	{
		return new RegExp(Grommunio.common.ui.HtmlEditor.FILE_LINK_RE.source, "gi");
	},
	...
	{
		var s = String(raw).replace(Grommunio.common.ui.HtmlEditor.FILE_LINK_TRAILING_RE, "");
		if ((/^file:/i).test(s)) {
	...
	linkifyFileLinkTextNode: function(textNode)
	{
		...
		while ((match = re.exec(text)) !== null) {
			var raw = match[0];
			var trailing = raw.match(Grommunio.common.ui.HtmlEditor.FILE_LINK_TRAILING_RE);
	 ...
	 * See {@link Grommunio.common.ui.htmleditor.WordClipboard}. TinyMCE's own
	...
	onPasteWordImages: function(event)
	{
		...
		var wordClipboard = Grommunio.common.ui.htmleditor.WordClipboard;
	...
	findFileTokenBeforeCaret: function(editor, afterEnter)
	{
		...
		var match = considered.match(Grommunio.common.ui.HtmlEditor.FILE_LINK_TAIL_RE);
		if (!match) {
			return null;
		}
		var trailing = match[0].match(Grommunio.common.ui.HtmlEditor.FILE_LINK_TRAILING_RE);
	...
	/**
	 * bind an {@link Grommunio.core.data.IPMRecord} to this component
	 ...
	 * which owns it. See {@link Grommunio.core.Util#getDropTargetRecord}.
	 * @return {Grommunio.core.data.IPMRecord} The record being edited, or undefined
	 * when this editor is not editing a record at all.
	 */
	getAttachmentRecord: function()
	{
		return Grommunio.core.Util.getDropTargetRecord(this);
	},
 ...
 * @static
 */
Grommunio.common.ui.HtmlEditor.FILE_LINK_RE =
 ...
 * @static
 */
Grommunio.common.ui.HtmlEditor.FILE_LINK_TAIL_RE =
	new RegExp("(?:" + Grommunio.common.ui.HtmlEditor.FILE_LINK_RE.source + ")$", "i");
 ...
 * @static
 */
Grommunio.common.ui.HtmlEditor.FILE_LINK_TRAILING_RE = /[.,;:!?)\]}]+$/;

Ext.reg("grommunio.htmleditor", Grommunio.common.ui.HtmlEditor);
...
Grommunio.onUIReady(function()
{
	...
	if (!body.hasClass("grommunio-webclient")) {
		return;
	}
