...
/**
 * @class Grommunio.common.ui.PreviewPanelToolbarButtons
 * @extends Object
 *
 * Contains special toolbar buttons for the previewpanel.
 */
Grommunio.common.ui.PreviewPanelToolbarButtons = Ext.extend(Object, {
	/**
	 * @cfg {@link Grommunio.core.ContextModel}
	 */
	model: undefined,
	...
	getToolbarButtons: function(insertionPoint, options)
	{
		...
		if(Ext.isDefined(options.model) && options.model instanceof Grommunio.core.ContextModel) {
			model = options.model;
		}
	...
	onBeforeShowMoreMenu: function (menu)
	{
		...
		var defaultFolderType = Grommunio.core.MessageClass.getDefaultFolderTypeFromMessageClass(record.get('message_class'));
	 ...
	 * then the {@link #Grommunio.common.dialogs.MessageBox.select MessageBox} will be used
	 * to select between the recurring and single appointment.
	 * @private
	 */
	onDelete: function()
	{
		Grommunio.common.Actions.deleteRecords(this.model.getSelectedRecords());
	},

	/**
	 * Open the selected record in separate browser window by supplying layer type as "separateWindows".
	 * And if record is opened in {@link Grommunio.core.ui.MainContentTabPanel tab} then close the existing tab.
	 * @private
	 */
	onPopout: function()
	{
		...
		Grommunio.common.Actions.openMessageContent(record, {layerType: 'separateWindows'});
	},
	...
	{
		Grommunio.mail.Actions.openCreateMailResponseContent(this.model.getSelectedRecords(), this.model, Grommunio.mail.data.ActionTypes.EDIT_AS_NEW);
	},
	...
	{
		Grommunio.common.Actions.openSaveEmlDialog(this.model.getPreviewRecord());
	},
	...
	{
		Grommunio.common.Actions.openPrintDialog(this.model.getPreviewRecord());
	},
	...
	{
		var record = this.model.getPreviewRecord();
		Grommunio.common.Actions.markAsRead(record, !item.readState);
		if (!item.isXType('grommunio.conditionalitem')) {
