...
/**
 * @class Grommunio.common.ui.IconClass
 *
 ...
 * @singleton
 */
Grommunio.common.ui.IconClass = {
	/**
	 * Convenience method for getting the icon class for the record.
	 *
	 * It will check for {@link Grommunio.core.mapi.ObjectType ObjectType} of record.
	 * and on the basis of it, It will return CSS iconClass.
	 *
	 * @param {Grommunio.core.data.MAPIRecord} record The record for which we need to find iconClass.
	...
	getIconClass: function(record)
	{
		...
		if(!Ext.isEmpty(iconIndex)) {
			if ((iconIndex !== Grommunio.core.mapi.IconIndex['appt_recurring'] &&
				 iconIndex !== Grommunio.core.mapi.IconIndex['appt_appointment'] ) ||
				!Grommunio.core.MessageClass.isClass(record.get('message_class'), ['IPM.Schedule.Meeting'], true)) {
				iconCls = Grommunio.core.mapi.IconIndex.getClassName(iconIndex, 'icon');
			}
		}
		...
		if (!Ext.isEmpty(iconCls) &&
			objectType === Grommunio.core.mapi.ObjectType.MAPI_MESSAGE &&
		...
		if (Ext.isEmpty(iconCls)) {
			switch(objectType) {
				...
				case Grommunio.core.mapi.ObjectType.MAPI_ABCONT:
					...
					break;
				case Grommunio.core.mapi.ObjectType.MAPI_ATTACH:
		...
		if ( iconCls === 'icon_mail') {
			if (objectType === Grommunio.core.mapi.ObjectType.MAPI_MESSAGE) {
	 ...
	 * {@link Grommunio.core.data.IPFRecord IPFRecord}.
	 *
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.IPFRecord} folder The mapi folder/store record.
	...
	getIconClassFromContainerClass: function(folder, containerClass)
	{
		...
		if (Grommunio.core.ContainerClass.isClass(containerClass, 'IPF.Note.OutlookHomepage', true)) {
			return 'icon_folder_rss';
		} else if (Grommunio.core.ContainerClass.isClass(containerClass, 'IPF', true)) {
	 ...
	 * @param {Grommunio.core.data.IPMRecord} message The mapi message record.
	 ...
	 * @param {Grommunio.contact.DistlistMemberRecord} record The mapi message record.
	...
	/**
	 * Function returns icon_class from the distlist_type for showing icons for members
	 * @param {Grommunio.core.data.IPMRecord} record The mapi message record.
	...
	getIconClassFromDisplayType: function(record, displayType)
	{
		...
		switch(displayType) {
			...
			case Grommunio.core.mapi.DisplayType.DT_AGENT:
			case Grommunio.core.mapi.DisplayTypeEx.DT_SEC_DISTLIST:
				return 'icon_contact_distlist';
	 ...
	 * @param {Grommunio.core.data.IPMAttachmentRecord} record The mapi attachment record.
	 * @return {String} icon class.
	 */
	 ...
	 * @param {Number} counter The number of reminders in {@link Grommunio.common.reminder.data.ReminderStore ReminderStore}.
