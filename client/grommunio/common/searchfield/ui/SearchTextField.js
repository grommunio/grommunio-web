...
/**
 * @class Grommunio.common.searchfield.ui.SearchTextField
 ...
 *
 * #dependsFile client/grommunio/common/searchfield/ui/SearchDropdownPanel.js
 * #dependsFile client/grommunio/advancesearch/KQLParser.js
 */
Grommunio.common.searchfield.ui.SearchTextField = Ext.extend(Ext.form.TextField, {

	/**
	 * @cfg {String} searchIndicatorClass CSS class applied during active search.
	 */
	searchIndicatorClass: 'grommunio-tbar-loading',
	 ...
	 * @property
	 * @type Grommunio.common.searchfield.ui.SearchDropdownPanel
	 */
	dropdownPanel: null,
	 ...
	 * {@link Grommunio.advancesearch.KQLParser#kqlKeys} so the field list and the
	...
	constructor: function(config)
	{
		...
		this.tokens = [];
		this.kqlFields = Grommunio.advancesearch.KQLParser.kqlKeys.slice(0);
		this.filterLabels = Grommunio.advancesearch.KQLParser.getKeywordLabels();
		...
		Grommunio.common.searchfield.ui.SearchTextField.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.common.searchfield.ui.SearchTextField.superclass.initComponent.call(this);
	...
	onAfterRenderSearch: function()
	{
		...
		this.dropdownPanel = new Grommunio.common.searchfield.ui.SearchDropdownPanel({
	...
	parseQueryToTokens: function(query)
	{
		...
		var tokens = [];
		var fieldsPattern = Grommunio.advancesearch.KQLParser.getKeywordPattern(this.kqlFields);
		...
		while ((match = regex.exec(query)) !== null) {
			...
			var key = match[1] ? Grommunio.advancesearch.KQLParser.resolveKeyword(match[1], this.kqlFields) : false;
			if (key) {
				var val = match[2] || '';
				if (val.length >= 2 &&
	...
	parseInputText: function(text)
	{
		...
		if (colonIdx > 0) {
			var key = Grommunio.advancesearch.KQLParser.resolveKeyword(raw.substring(0, colonIdx), this.kqlFields);
			if (key) {
	...
	setWidth: function(w)
	{
		Grommunio.common.searchfield.ui.SearchTextField.superclass.setWidth.call(this, w);
		if (this.tokenWrapEl) {
	...
	onDestroy: function()
	{
		...
		Grommunio.common.searchfield.ui.SearchTextField.superclass.onDestroy.call(this);
	}
});

Ext.reg('grommunio.searchtextfield', Grommunio.common.searchfield.ui.SearchTextField);
