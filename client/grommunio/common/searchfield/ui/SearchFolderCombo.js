...
/**
 * @class Grommunio.common.searchfield.ui.SearchFolderCombo
 ...
 * in search folder combo box using {@link Grommunio.advancesearch.dialogs.SelectFolderContentPanel SelectFolderContentPanel}
 * respectively. while 'Other…' option used to open the {@link Grommunio.advancesearch.dialogs.SelectFolderContentPanel SelectFolderContentPanel}
 */
Grommunio.common.searchfield.ui.SearchFolderCombo = Ext.extend(Ext.form.ComboBox, {
	...
	constructor: function(config)
	{
		...
		Grommunio.common.searchfield.ui.SearchFolderCombo.superclass.constructor.call(this, config);
	},

	/**
	 * Event handler which is raised just before the {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	...
	onBeforeRender: function()
	{
		...
		var data = [{
			...
			'flag': Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.ALL_FOLDERS
		}, {
			'name': Ext.util.Format.htmlEncode(defaultFolder.getDisplayName()),
			'value': defaultFolder.get('entryid'),
			'include_subfolder': false,
			'flag': Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.CURRENT_SELECTED_FOLDER
		}, {
			'name': _('Other…'),
			'value': 'other'
		}];
	...
	initEvents: function()
	{
		Grommunio.common.searchfield.ui.SearchFolderCombo.superclass.initEvents.apply(this, arguments);
	...
	/**
	 * Event handler triggered when {@link Grommunio.calendar.ui.CalendarMultiView CalendarMultiView}
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder which will be current folder in search folder combo box.
	 */
	...
	/**
	 * Function is used to set the folder in {@link Grommunio.common.searchfield.ui.SearchFolderCombo SearchFolderCombo}
	...
	setFolder: function(folder, changeCurrentFolder)
	{
		...
		// not found folder.
		if (this.findParentByType("grommunio.searchpaneltoolbar") || !Ext.isDefined(folder)) {
			return;
		...
		if (folder.getDefaultFolderKey() === 'inbox' || folder.getDefaultFolderKey() === 'publicfolders') {
			var allFolderRecord = store.getAt(store.find('flag', Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.ALL_FOLDERS));
	...
	{
		var currentFolderRecord = store.getAt(store.find('flag', Grommunio.advancesearch.data.SearchComboBoxFieldsFlags.CURRENT_SELECTED_FOLDER));
		...
		if (!Grommunio.core.EntryId.compareEntryIds(allFolderRecord.get('value'), subTreeEntryid)) {
...
Ext.reg('grommunio.searchfoldercombo', Grommunio.common.searchfield.ui.SearchFolderCombo);
