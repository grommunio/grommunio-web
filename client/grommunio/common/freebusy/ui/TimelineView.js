...
/**
 * @class Grommunio.common.freebusy.ui.TimelineView
 * @extends Ext.BoxComponent
 * @xtype grommunio.freebusytimelineview
 */
Grommunio.common.freebusy.ui.TimelineView = Ext.extend(Ext.BoxComponent,
{
	/**
	 * @cfg {Grommunio.common.freebusy.data.FreebusyModel} model
	 * <b>This is a required setting</b>. The {@link Grommunio.common.freebusy.data.FreebusyModel} holds the data for the
	 * Freebusy component.
	 */
	model: null,
	/**
	 * @cfg {Grommunio.common.freebusy.data.TimelineSelector} selector
	 ...
	 * 'grommunio/v1/main/start_working_hour').
	 */
	workingHoursStart: 0,
	/**
	 ...
	 * the default settings value for 'grommunio/v1/main/working_days').
	 */
	workDays: undefined,
	/**
	...
	constructor: function(config)
	{
		...
		config = Ext.applyIf(config, {
			workingHoursStart: container.getSettingsModel().get('grommunio/v1/main/start_working_hour'),
			workingHoursEnd: container.getSettingsModel().get('grommunio/v1/main/end_working_hour'),
			workDays: container.getSettingsModel().get('grommunio/v1/main/working_days')
		});
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.common.freebusy.ui.TimelineView} Reference to the TimelineView
			 */
			'rendertimeline',
			/**
		...
		Grommunio.common.freebusy.ui.TimelineView.superclass.constructor.call(this, config);
	...
	initComponent: function(){
		...
		Grommunio.common.freebusy.ui.TimelineView.superclass.initComponent.call(this);
	...
	onRender: function(){
		...
		Grommunio.common.freebusy.ui.TimelineView.superclass.onRender.apply(this, arguments);
...
//TODO: The initial render should also contain a refresh call to directly put in all the blocks of the already loaded users.
	 ...
	 * <li>focusDateRange {@link Grommunio.core.DateRange} The period of time that is centered on in the
	 * viewport.</li>
	...
	restoreViewportSnapshot: function(snapshot){
		...
		if(snapshot.selectionInView){
			...
			focusDateRange = new Grommunio.core.DateRange({ startDate: new Date(centerTimestamp*1000), dueDate: new Date(centerTimestamp*1000) });
		} else {
	...
	/**
	 * Returns the {@link Grommunio.core.data.DateRange DateRange} that the focus is centered on. It
	 * looks for the hour slot that is in the center of the viewport. That slot will be returned in
	 * a DateRange.
	 * @return {Grommunio.core.DateRange} Range of the focus
	 */
	getFocusDateRange: function(){
		...
		return new Grommunio.core.DateRange({ startDate: new Date(focusStartTimestamp*1000), dueDate: new Date(focusEndTimestamp*1000) });
	},

	/**
	 * Returns the {@link Grommunio.core.data.DateRange DateRange} that the viewport is showing. The
	 ...
	 * @return {Grommunio.core.DateRange} Range of the viewed area
	 */
	getViewedDateRange: function(){
		...
		return new Grommunio.core.DateRange({ startDate: new Date(startTimestamp*1000), dueDate: new Date(endTimestamp*1000) });
	},
	...
	afterRender: function(){
		Grommunio.common.freebusy.ui.TimelineView.superclass.afterRender.apply(this, arguments);
	...
	{
		Grommunio.core.data.UIFactory.openContextMenu(Grommunio.core.data.SharedComponentType['common.contextmenu.freebusy.timelinebody'], undefined, { position: evt.getXY(), model: this.model });
	},
	...
	{
		Grommunio.core.data.UIFactory.openContextMenu(Grommunio.core.data.SharedComponentType['common.contextmenu.freebusy.timelineheader'], undefined, { position: evt.getXY(), model: this.model });
	},
	 ...
	 * @param date {Date|Number|Grommunio.core.DateRange.js} Date to be scrolled into view.
	 * @private
	 */
	scrollDateIntoView: function(date){
		...
		} else if(date instanceof Grommunio.core.DateRange){
	...
	prepareData: function(data, recordIndex, record, sumHeader){
		...
		if(data.status != Grommunio.core.mapi.BusyStatus.UNKNOWN){
			data.busyStatusName = Grommunio.core.mapi.BusyStatus.getName(data.status) || 'busy';
	...
	/**
	 * Called when the {@link Grommunio.common.freebusy.data.FreebusyModel model} fires the userstorechange event to indicate
	...
	/**
	 * Called when the {@link Grommunio.common.freebusy.data.FreebusyModel model} fires the blockstorechange event to indicate
	...
	/**
	 * Called when the {@link Grommunio.common.freebusy.data.FreebusyModel model} fires the sumblockstorechange event to indicate
...
Ext.reg('grommunio.freebusytimelineview', Grommunio.common.freebusy.ui.TimelineView);
