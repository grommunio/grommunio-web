...
/**
 * @class Grommunio.common.freebusy.data.FreebusyModel
 * @extends Ext.util.Observable
 */
Grommunio.common.freebusy.data.FreebusyModel = Ext.extend(Ext.util.Observable,
{
	/**
	 * @cfg {Boolean} nonWorkingHoursHidden
	 * The {@link Grommunio.common.freebusy.ui.TimelineView TimelineView} must only
	 * display the working hours for the user.
	 */
	nonWorkingHoursHidden: true,
	/**
	 * @cfg {Grommunio.core.DateRange} daterange
	 * The {@link Grommunio.core.DateRange} object that determines what time period will be shown
	 * (defaults to 7 days before today till 3 weeks after).
	 */
	daterange: null,
	/**
	 * @cfg {Grommunio.core.DateRange} selectorRange
	 * The {@link Grommunio.core.DateRange} object that determines what time period will be selected
	 * (defaults to next whole or half hour and will last by default for 30 minutes).
	 */
	selectorRange: null,
	/**
	 * The {@link Grommunio.core.DateRange} object that indicates the daterange for which the
	 * suggestions for the {@link #suggestionBlockStore} must be calculated.
	 * @property
	 * @type Grommunio.common.freebusy.data.FreebusyBlockStore
	 */
	suggestionRange: null,
	/**
	 * @cfg {Grommunio.core.data.IPMRecipientStore} userStore
	 * The {@link Grommunio.core.data.IPMRecipientStore store} object that handles the freebusy blocks displayed on the timeline.
	 */
	userStore: null,
	/**
	 * @cfg {Grommunio.common.freebusy.data.FreebusyBlockStore} blockStore
	 * The {@link Grommunio.common.freebusy.data.FreebusyBlockStore store} object that handles the freebusy blocks displayed on the timeline.
	 */
	blockStore: null,
	/**
	 * @cfg {Grommunio.common.freebusy.data.FreebusyBlockStore} sumBlockStore
	 * The {@link Grommunio.common.freebusy.data.FreebusyBlockStore} object that handles the freebusy sum blocks displayed in the header of the timeline.
	 */
	sumBlockStore: null,
	/**
	...
	freeBlockStore: null,
	/**
	 * @cfg {Grommunio.common.freebusy.data.FreebusyBlockStore} suggestionBlockStore
	 * The {@link Grommunio.common.freebusy.data.FreebusyBlockStore} object that handles the freebusy sum blocks which indicates which blocks are
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			userStore: config.userStore || new Grommunio.core.data.IPMRecipientStore(),
			daterange: config.daterange || new Grommunio.core.DateRange({ startDate: startDate, dueDate: endDate}),
			selectorRange: new Grommunio.core.DateRange({ startDate: selectStart, dueDate: selectEnd}),
			blockStore: config.blockStore || new Grommunio.common.freebusy.data.FreebusyBlockStore(),
			sumBlockStore: config.sumBlockStore || new Grommunio.common.freebusy.data.FreebusyBlockStore({ remoteSort: false}),
			suggestionBlockStore: config.suggestionBlockStore || new Grommunio.common.freebusy.data.FreebusyBlockStore({ remoteSort: false})
		});
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.core.data.IPMRecipientStore} newStore The new store
			 * @param {Grommunio.core.data.IPMRecipientStore} oldStore The old store
			 */
			'userstorechange',
			/**
			 * @event blockstorechange
			 * Fires when the block store is changed.
			 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} newStore The new store
			 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} oldStore The old store
			 */
			'blockstorechange',
			/**
			 ...
			 * Fires when the {@link Grommunio.core.DateRange DateRange} has been changed.
			 * @param {Grommunio.core.DateRange} newRange The new DateRange object.
			 * @param {Grommunio.core.DateRange} oldRange The old DateRange object.
			 */
			'daterangechange',
			/**
			 * @event selectorrangeupdate
			 * Fires when the selector {@link Grommunio.core.DateRange DateRange}
			 * has been changed.
			 * @param {Grommunio.core.DateRange} newRange The new DateRange object.
			 * @param {Grommunio.core.DateRange} oldRange The old DateRange object.
			 */
		...
		Grommunio.common.freebusy.data.FreebusyModel.superclass.constructor.call(this, config);
	...
	/**
	 * Initialize all {@link Grommunio.common.freebusy.data.FreebusyModel FreebusyModel} related events.
	 * @private
	 */
	initEvents: function()
	{
	...
	/**
	 * Create a {@link Grommunio.common.freebusy.data.FreebusyBlockStore FreebusyBlockStore} which
	 * is used internally to keep track of the TimeBlocks in which all users are available.
	 * @return {Grommunio.common.freebusy.data.FreebusyBlockStore} The created store
	 * @private
	 */
	createFreeBlockStore: function()
	{
		if (!this.freeBlockStore) {
			this.freeBlockStore = new Grommunio.common.freebusy.data.FreebusyBlockStore({ remoteSort: false});
		}
		return this.freeBlockStore;
	},

	/**
	 * Create a {@link Grommunio.core.DateRange DateRange} object which is used internally to keep
	 * track of the range for which the suggestions must be calculated.
	 * @return {Grommunio.core.DateRange} The created daterange
	 * @private
	 */
	createSuggestionRange: function()
	{
		if (!this.suggestionRange) {
			this.suggestionRange = new Grommunio.core.DateRange();
	 ...
	 * @return {Grommunio.core.data.IPMRecipientStore} store
	 */
	...
	/**
	 * Sets the store.
	 * @param {Grommunio.core.data.IPMRecipientStore} store store
	 ...
	 * @return {Grommunio.common.freebusy.data.FreebusyBlockStore} store
	 */
	...
	/**
	 * Sets the store.
	 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} store
	 * @param {Boolean} initial (optional) True if this function is called
	 * during initialization.
	 * @return {Grommunio.common.freebusy.data.FreebusyBlockStore} store
	 */
	 ...
	 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} store The sum block store
	 * @param {Boolean} initial (optional) True if this function is called
	 * during initialization.
	 * @return {Grommunio.common.freebusy.data.FreebusyBlockStore} The sum block store
	 * @private
	 */
	...
	/**
	 * Sets the block store.
	 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} store The free block store
	 * @param {Boolean} initial (optional) True if this function is called
	 * during initialization.
	 * @return {Grommunio.common.freebusy.data.FreebusyBlockStore} The free block store
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.mapi.RecipientType} type (optional) The recipientType for the user (defaults to MAPI_TO)
	 * @return {Grommunio.core.data.IPMRecipientRecord} the record representing the newly added user.
	 */
	addUser: function(name, type)
	{
		type = Ext.isNumber(type) ? type : Grommunio.core.mapi.RecipientType.MAPI_TO;

		var record = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, {
			display_name: name,
			recipient_type: type
		});
	 ...
	 * @return {Grommunio.util.DateRange} Daterange set in this model
	 */
	...
	/**
	 * Sets the date range.
	 * @param {Grommunio.util.DateRange} dateRange Daterange set for this model
	 * @param {Boolean} initial (optional) True if this function is called
	 * during initialization.
	 * @return {Grommunio.util.DateRange} Daterange set for this model
	 */
	 ...
	 * @return {Grommunio.util.DateRange} Daterange set for the selector in this model
	 */
	 ...
	 * @param {Grommunio.util.DateRange} selectorRange Daterange set for the selector in this model
	 * @param {Boolean} initial (optional) True if this function is called
	 * during initialization.
	 * @return {Grommunio.util.DateRange} Daterange set for the selector in this model
	 * @private
	...
	selectRange: function(startDate, dueDate){
		 ...
		 * update.
		 */
		if(!this.selectorRange.equals( new Grommunio.core.DateRange({ startDate: startDate, dueDate: dueDate}) )){
	 ...
	 * @return {Grommunio.util.DateRange} Daterange set for the suggestions in this model
	 */
	...
	setSuggestionDate: function(startDate, duration)
	{
		...
		if (this.nonWorkingHoursHidden) {
			start = start.add(Date.MINUTE, container.getSettingsModel().get('grommunio/v1/main/start_working_hour'));
			due = due.add(Date.MINUTE, container.getSettingsModel().get('grommunio/v1/main/end_working_hour'));
		} else {
	 ...
	 * @param {Grommunio.core.data.IPMRecipient[]} userRecords The users for which the
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} userStore
	 * @param {Grommunio.core.data.IPMRecipient} userRecords
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} userStore Store
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} userRecords Records of the added users
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} userStore Store
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} userRecords
	 * @param {Number} index Index
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} userStore Store
	 * @param {Grommunio.core.data.IPMRecipientStore[]} userRecords
	 * @private
	 */
	...
	{
		return new Grommunio.common.freebusy.data.FreebusyBlockRecord({
	...
	createSuggestionBlocks: function(start, end, duration, interval)
	{
		...
		for (var i = start; i <= end; i += interval) {
			blocks.push(new Grommunio.common.freebusy.data.FreebusyBlockRecord({
				start: i,
				end: i + duration
			}));
		}
	 ...
	 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} sumBlockStore The store containing the sum blocks
	...
	mergeBlocksToSumBlockStore: function(records, sumBlockStore, splitBusyStatus)
	{
		...
		Ext.each(records, function(record) {
			...
			if (busyStatus === Grommunio.core.mapi.BusyStatus.UNKNOWN || busyStatus === Grommunio.core.mapi.BusyStatus.FREE) {
				return true;
			}
	 ...
	 * @param {Grommunio.common.freebusy.data.FreebusyBlockStore} sumBlockStore The store containing the sumblocks
	 */
	 ...
	 * @param {Grommunio.core.mapi.BusyStatus} status The busy status to check
	 * @return {Boolean} True if the attendee cannot take a meeting
	 */
	occupiesAttendee: function(status)
	{
		var BusyStatus = Grommunio.core.mapi.BusyStatus;
	 ...
	 * @param {Grommunio.core.data.IPMRecipientStore} newStore
	 * @param {Grommunio.core.data.IPMRecipientStore} oldStore
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.DateRange} newRange The new selected range.
	 * @param {Grommunio.core.DateRange} oldRange The old selected range.
	 * @private
	 */
