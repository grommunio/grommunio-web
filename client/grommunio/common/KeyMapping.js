...
/**
 * @class Grommunio.common.KeyMapping
 * @extends Object
 *
 * The map of keys used for global functionalities.
 * @singleton
 */
Grommunio.common.KeyMapping = Ext.extend(Object, {
	...
	constructor: function()
	{
		...
		Grommunio.core.KeyMapMgr.register('global', mainTabBar);
		Grommunio.core.KeyMapMgr.register('global', mainToolbarKeys);
		Grommunio.core.KeyMapMgr.register('global', paletteKeys);
		Grommunio.core.KeyMapMgr.register('global', searchKeys);
		Grommunio.core.KeyMapMgr.register('global', undoRedoKeys);

		Grommunio.core.KeyMapMgr.register('grid', selectionKey);
		Grommunio.core.KeyMapMgr.register('view.mapimessage', selectionKey);

		Grommunio.core.KeyMapMgr.register('grid.mapimessage', mapiMessageKeys);
		Grommunio.core.KeyMapMgr.register('view.mapimessage', mapiMessageKeys);
		Grommunio.core.KeyMapMgr.register('grid.reminder', reminderKeys);

		Grommunio.core.KeyMapMgr.register('contentpanel', contentPanelActionsKeys);
		Grommunio.core.KeyMapMgr.register('contentpanel.record', defaultPrintMessageKey);
		Grommunio.core.KeyMapMgr.register('contentpanel.record', defaultRecordActionsKeys);
		Grommunio.core.KeyMapMgr.register('contentpanel.record.message', defaultMessageActionsKeys);

		Grommunio.core.KeyMapMgr.register('previewpanel.mail', defaultPrintMessageKey);
	},

	/**
	 * Event handler for the keydown event of the {@link Grommunio.core.KeyMap KeyMap}
	 ...
	 * {@link Grommunio.core.data.UndoManager#enabled}) and when it originates
	...
	{
		return container.getSettingsModel().get('grommunio/v1/main/undo_redo/enable') === true;
	},
	...
	/**
	 * Event handler for the keydown event of the {@link Grommunio.core.KeyMap KeyMap}
	 * when the user wants to switch between {@link Grommunio.core.Context Contexts}.
	 * @param {Number} key Key code
	 * @param {Ext.EventObject} event The event
	 * @param {Ext.Component} component The component on which key event is fired.
	 */
	 ...
	 * {@link Grommunio.core.ui.MainContentTabPanel MainContentTabPanel}.
	 * @param {Number} key Key code
	 * @param {Ext.EventObject} event The event
	 * @param {Ext.Component} component The component on which key event is fired.
	 */
	 ...
	 * {@link Grommunio.core.ui.MessageContentPanel MessageContentPanel}.
	 * @param {Number} key Key code
	 * @param {Ext.EventObject} event The event
	 * @param {Ext.Component} component The component on which key event is fired.
	 */
	 ...
	 * {@link Grommunio.core.ui.RecordContentPanel RecordContentPanel}.
	 * @param {Number} key Key code
	 * @param {Ext.EventObject} event The event
	 * @param {Ext.Component} component The component on which key event is fired.
	 */
	...
	{
		if(component.record){
			Grommunio.common.Actions.openPrintDialog(component.record);
		}
	},
	...
	onPrintList: function(key, event, component)
	{
		...
		if (component.context) {
			Grommunio.common.Actions.openPrintDialog(component.context);
		}
	},
	...
	onMoveItem: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			Grommunio.common.Actions.openCopyMoveContent(records);
		}
	},
	...
	onDeleteItem: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			Grommunio.common.Actions.deleteRecords(records);
		}
	},
	...
	onSoftDeleteItem: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			Grommunio.common.Actions.deleteRecords(records, false, true);
		}
	},
	...
	onCategorize: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			Grommunio.common.Actions.openCategoriesContent(records);
		}
	},
		...
		if(!Ext.isEmpty(records)) {
			for(var i = 0, len = records.length; i < len; i++){
				if(records[i].isUnsent() && !records[i].isFaultyMessage()) {
					Grommunio.core.data.UIFactory.openCreateRecord(records[i]);
				} else {
					Grommunio.core.data.UIFactory.openViewRecord(records[i]);
				}
			}
		}
	},
	...
	onOpenReminder: function(key, event, component)
	{
		...
		if(!Ext.isEmpty(records)) {
			for(var i = 0, len = records.length; i < len; i++){
				Grommunio.common.Actions.openReminderRecord(records[i]);
			}
		}
	},
	...
	/**
	 * Event handler for the keydown event of the {@link Ext.KeyMap KeyMap}
	 * when the user wants to select all items below or above the current selected item {@link Grommunio.common.ui.grid.GridPanel}.
	 * @param {Number} key Key code
	 * @param {Ext.EventObject} event The event
	 * @param {Ext.Component} component The component on which key event is fired.
	 */
	...
	/**
	 * Event handler for the keydown event of the {@link Ext.KeyMap KeyMap}
	 * when the user wants to select all items in the {@link Grommunio.common.ui.grid.GridPanel}.
	 * @param {Number} key Key code
	 * @param {Ext.EventObject} event The event
	 * @param {Ext.Component} component The component on which key event is fired.
	 */
	 ...
	 *
	 * @return {Grommunio.common.searchfield.ui.SearchTextField|Boolean} the field
	 * @private
	 */
	getVisibleSearchField: function()
	{
		...
		Ext.ComponentMgr.all.each(function(component) {
			if (field || !component.isXType || !component.isXType('grommunio.searchtextfield')) {
				return;
			}
	 ...
	 *
	 * @param {Grommunio.common.searchfield.ui.SearchTextField} field the search box
	 * @return {Boolean}
	 * @private
	 */
	...
	/**
	 * Opens or closes the {@link Grommunio.core.ui.CommandPalette command palette}.
	 * @private
	 */
	onCommandPalette: function(key, event)
	{
		...
		Grommunio.core.ui.CommandPalette.toggle();
	}
});

Grommunio.common.KeyMapping = new Grommunio.common.KeyMapping();
