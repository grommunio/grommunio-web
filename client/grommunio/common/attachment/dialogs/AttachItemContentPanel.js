...
/**
 * @class Grommunio.common.attachment.dialogs.AttachItemContentPanel
 * @extends Grommunio.core.ui.ContentPanel
 * @xtype grommunio.attachitemcontentpanel
 *
 ...
 */
Grommunio.common.attachment.dialogs.AttachItemContentPanel = Ext.extend(Grommunio.core.ui.ContentPanel, {
	/**
	...
	/**
	 * @cfg {Grommunio.core.data.MAPIRecord} record record in which we will add embedded attachment if user is adding it as attachment.
	 */
	record: undefined,

	/**
	 * @cfg {Grommunio.common.ui.EditorField} editor editor in which we will be adding message as text if user has selected to add text in body.
	 */
	editor: undefined,
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			items: [{
				xtype: 'grommunio.attachitempanel',
				record: config.record,
				editor: config.editor
			}]
		});

		Grommunio.common.attachment.dialogs.AttachItemContentPanel.superclass.constructor.call(this, config);
	},
	...
	getState: function()
	{
		var state = Grommunio.common.attachment.dialogs.AttachItemContentPanel.superclass.getState.call(this);
	...
	applyState: function(state)
	{
		...
		Grommunio.common.attachment.dialogs.AttachItemContentPanel.superclass.applyState.call(this, state);
	},
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder that should be used for bidding process.
	 * @return {Grommunio.core.data.ListModuleStore} store that will be used to load data in {@link Grommunio.common.attachment.dialogs.AttachItemGrid AttachItemGrid}.
	 * @private
	 */
	getStoreByFolder: function(folder)
	{
		...
		if(!store) {
			...
			store = new Grommunio.mail.MailStore();
		}
	...
	/**
	 * Function will be used to get instance of {@link Grommunio.common.ui.grid.ColumnModel ColumnModel}. Function will use
	 * bidding process to get instance of column model.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder that should be used for bidding process.
	 * @return {Grommunio.core.data.ListModuleStore} store that will be used to show columns in {@link Grommunio.common.attachment.dialogs.AttachItemGrid AttachItemGrid}.
	 * @private
	 */
	getColumnModelByFolder: function(folder)
	{
		// find the correct column model for showing in grid, this will use bidding process
		var componentType = Grommunio.core.data.SharedComponentType['common.attachment.dialog.attachitem.columnmodel'];
	...
	/**
	 * Function will be used to get instance of {@link Grommunio.common.attachment.dialogs.AttachItemBaseRenderer Renderer}. Function will use
	 * bidding process to get instance of renderer.
	 * @param {Grommunio.core.data.IPMRecord} record record that is selected and should use for bidding process.
	 * @private
	 */
	getRendererByMessage: function(record)
	{
		...
		var componentType = Grommunio.core.data.SharedComponentType['common.attachment.dialog.attachitem.textrenderer'];
...
Ext.reg('grommunio.attachitemcontentpanel', Grommunio.common.attachment.dialogs.AttachItemContentPanel);
