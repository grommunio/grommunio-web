...
/**
 * @class Grommunio.common.attachment.ui.AttachmentField
 * @extends Grommunio.common.ui.BoxField
 * @xtype grommunio.attachmentfield
 *
 * Extension to the default {@link Grommunio.common.ui.BoxField} which
 * handles attachments as for rendering. It uses a
 * {@link Grommunio.core.data.IPMAttachmentStore IPMAttachmentStore} for storing
 * and resolving {@link Grommunio.core.data.IPMAttachmentRecord Attachments}.
 *
 ...
 * load the {@link Grommunio.core.data.IPMAttachmentStore AttachmentStore} into the component.
 * Otherwise the user of this component needs to call {@link #setAttachmentStore}.
 */
Grommunio.common.attachment.ui.AttachmentField = Ext.extend(Grommunio.common.ui.BoxField, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			boxType: 'grommunio.attachmentbox',
			enableComboBox: false,
			height: 25,
			boxMinHeight: 25,
			boxMaxHeight: 75
		});
		config.wrapCls = config.wrapCls ? config.wrapCls + ' grommunio-attachmentfield' : 'grommunio-attachmentfield';

		Grommunio.common.attachment.ui.AttachmentField.superclass.constructor.call(this, config);
	...
	{
		Grommunio.common.attachment.ui.AttachmentField.superclass.onRender.apply(this, arguments);
	...
	onBrowserDrop: function(event)
	{
		...
		var ownerWindow = Grommunio.core.BrowserWindowMgr.activateOwnerWindow(this.wrap);
		...
		if (!files || files.length === 0) {
			files = Grommunio.common.attachment.AttachmentDragDrop.getFileList(dataTransfer, ownerWindow);
			if (!files || files.length === 0) {
				return;
			}
		}
	 ...
	 * @param {Grommunio.core.data.IPMAttachmentStore} store The store to set on this field
	 */
	 ...
	 * @return {Grommunio.core.data.IPMAttachmentStore} store
	 */
	...
	update: function(record, contentReset)
	{
		if (record && record instanceof Grommunio.core.data.MAPIRecord) {
			...
			Grommunio.common.attachment.ui.AttachmentField.superclass.update.apply(this, arguments);
		}
	},
	 ...
	 * @param {Grommunio.common.attachment.ui.AttachmentField} field This field to which the box belongs
	 * @param {Grommunio.common.attachment.ui.AttachmentBox} box The box for which was double-clicked
	 * @param {Grommunio.core.data.IPMAttachmentRecord} record The record which is attached to the box
	 * @private
	 */
	onBoxDblClick: function(field, box, record)
	{
		Grommunio.common.Actions.openAttachmentRecord(record);
	},

	/**
	 * Event handler when the contextmenu is requested for a Box.
	 * @param {Grommunio.common.attachment.ui.AttachmentField} field This field to which the box belongs
	 * @param {Grommunio.common.attachment.ui.AttachmentBox} box The box for which the contextmenu is requested
	 * @param {Grommunio.core.data.IPMAttachmentRecord} record The record which is attached to the box
	 * @private
	 */
	onBoxContextMenu: function(field, box, record)
	{
		Grommunio.core.data.UIFactory.openDefaultContextMenu(record, {
	...
	onBoxRemove: function(field, box, record)
	{
		...
		Grommunio.common.attachment.ui.AttachmentField.superclass.onBoxRemove.apply(this, arguments);
	},

	/**
	 * Callback function from {@link Grommunio.common.attachment.ui.AttachmentBox} which indicates that
	...
	doBoxRemove: function(box)
	{
		if (box.record.isUploaded() || !(Ext.isIE || Ext.isEdge)) {
			Grommunio.common.attachment.ui.AttachmentField.superclass.doBoxRemove.apply(this, arguments);
		}
	}
});

Ext.reg('grommunio.attachmentfield', Grommunio.common.attachment.ui.AttachmentField);
