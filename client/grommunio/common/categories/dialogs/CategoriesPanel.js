...
Ext.namespace('Grommunio.common.categories.dialogs');

/**
 * @class Grommunio.common.categories.dialogs.CategoriesPanel
 * @extends Ext.Panel
 * @xtype grommunio.categoriespanel
 *
 * Panel for users to edit the categories on a given {@link Grommunio.core.data.IPMRecord record}
 */
Grommunio.common.categories.dialogs.CategoriesPanel = Ext.extend(Ext.Panel, {
	/**
	 ...
	 * @param {Grommunio.core.data.IPMRecord[]} records The records that are edited with this panel
	 * @return {Object} The configuration object for the grid panel.
	 * @private
	 */
	createCategoriesGrid: function(records, storeEntryId)
	{
		var store = new Grommunio.common.categories.data.CategoriesStore(
	 ...
	 * {@link Grommunio.common.categories.data.CategoriesStore CategoriesStore} for
	...
	{
		var html = Grommunio.common.categories.Util.getCategoryIconSVG(color,'16');
	...
	{
		var cls = record.get('quickAccess') ? ' grommunio-pinned' : '';
		...
		return Ext.util.Format.htmlEncode(value) +
			'<div class="grommunio-grid-button-container' + cls + '">' +
				'<div class="grommunio-grid-button k-grid-button-edit" role="button" tabindex="0" aria-label="' + _('Rename category') + '"></div>' +
				'<div class="grommunio-grid-button k-grid-button-delete" role="button" tabindex="0" aria-label="' + _('Delete category') + '"></div>' +
				'<div class="grommunio-grid-button k-grid-button-pin" role="button" tabindex="0" aria-label="' + _('Quick access') + '"></div>' +
			'</div>';
	},
	...
	onAfterRender: function(grid)
	{
		...
		this.mon(grid.getEl(), 'keydown', function(e) {
			if (e.getKey() === e.ENTER || e.getKey() === e.SPACE) {
				...
				if (target && target.hasClass('grommunio-grid-button')) {
					e.stopEvent();
					target.dom.click();
				}
			}
	...
	/**
	 * Adds a {@link Grommunio.common.ui.ColorPicker} to every color cell.
	...
	addColorPickers: function(view)
	{
		this.categoriesGrid.store.each(function(record, index){
			...
			var colorPicker = new Grommunio.common.ui.ColorPicker({
				renderTo: el.dom,
				value: color.replace('#', '')
			});
	 ...
	 * {@link Grommunio.common.categories.data.CategoriesStore CategoriesStore}
	 ...
	 * @param {Grommunio.common.ui.ColorPicker} colorPicker the color picker that
	 * fired the event.
	 */
	 ...
	 * also it will open {@link Grommunio.common.categories.dialogs.RenameCategoryPanel RenameCategoryPanel}
	...
	onRowClick: function(grid, rowIndex, event)
	{
		...
		var targetEl = Ext.get(event.target);
		if ( targetEl.hasClass('k-grid-button-pin') && !targetEl.up('div').hasClass('grommunio-pin-fixed') ){
	...
	getAvailableCategories: function()
	{
		...
		var categories = Grommunio.common.categories.Util.getAllCategories(this.record);
...
Ext.reg('grommunio.categoriespanel', Grommunio.common.categories.dialogs.CategoriesPanel);
