...
/**
 * @class Grommunio.common.dialogs.CopyMovePanel
 * @extends Ext.Panel
 * @xtype grommunio.copymovepanel
 *
 * Panel for users to copy or move the given {@link Grommunio.core.data.IPMRecord[] records}
 * to a different {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
 */
Grommunio.common.dialogs.CopyMovePanel = Ext.extend(Ext.Panel, {
	/**
	 * @cfg {Grommunio.core.data.IPMRecord} record The record(s) which are being
	 * copied or moved through this panel
	 */
	record: undefined,
	/**
	 * @cfg {Grommunio.core.mapi.ObjectType} objectType The Objecttype of the
	...
	/**
	 * {Grommunio.mail.MailStore} store or {Grommunio.hierarchy.data.IPFSubStore} store
		...
		Ext.applyIf(config, {
			...
			listeners: {
				render: function() {
					...
					// Add the keymap
					Grommunio.core.KeyMapMgr.activate(this, 'Grommunio.common.dialogs.CopyMovePanel');
				}
			}
		});

		Grommunio.common.dialogs.CopyMovePanel.superclass.constructor.call(this, config);
	},
	 ...
	 * to which the {@link Grommunio.core.data.IPMRecord records} can be
	 * copied or moved to.
	 * @param {Object} config Configuration structure
	...
	{
		Grommunio.common.dialogs.CopyMovePanel.superclass.onRender.call(this, ct, position);

		this.showSuggestions();

		if (this.objectType == Grommunio.core.mapi.ObjectType.MAPI_MESSAGE) {
		...
		} else if (this.objectType == Grommunio.core.mapi.ObjectType.MAPI_FOLDER) {
	...
	showSuggestions: function()
	{
		...
		var folders = Grommunio.common.data.FolderSuggestions.get(this.record);
		...
		Ext.each(folders, function(folder) {
			this.suggestionBox.add({
				...
				iconCls: Grommunio.common.ui.IconClass.getIconClass(folder),
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The suggested folder
	 * @private
	 */
	...
	initEvents: function()
	{
		Grommunio.common.dialogs.CopyMovePanel.superclass.initEvents.apply(this, arguments);
	...
	/**
	 * Check whether the {@link Grommunio.core.data.IPMRecord records} being copied or moved
	 * are compatible with the {@link Grommunio.hierarchy.data.MAPIFolderRecord#container_class container_class}
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The target folder
	...
	isContainerClassCompatible: function(folder)
	{
		...
		for (var i = 0, len = records.length; i < len; i++) {
			...
			if (!Grommunio.core.MessageClass.isContainerClassCompatible(messageClass, containerClass)) {
				return false;
			}
		}
	 ...
	 * This will call {@link Grommunio.hierarchy.actions.openCreateFolderContent} with the selected {@link Grommunio.hierarchy.data.MAPIFolderRecord folder}.
	 * @private
	 */
	onCreateFolder: function()
	{
		var folder = this.hierarchyTree.getSelectionModel().getSelectedNode().getFolder();
		Grommunio.hierarchy.Actions.openCreateFolderContent(folder);
	 ...
	 * @param {Grommunio.hierarchy.ui.Tree} tree the folder tree
	 ...
	 * {@link Ext.Button button}. It will check necessary {@link Grommunio.core.mapi.Rights Rights} and
	 * {@link Grommunio.core.mapi.Access Access} of folder.
	 *
	...
	onClickHandler: function(item)
	{
		...
		records = Grommunio.common.Actions.resolveRecords(records, this.store);
		...
		if (records[0] instanceof Grommunio.core.data.IPFRecord) {
			var access = folder.get('access') & Grommunio.core.mapi.Access.ACCESS_CREATE_HIERARCHY;
			if (!access) {
	...
	/**
	 * Function will copy all {@link Grommunio.core.data.IPMRecord records}
	 * and will close the {@link Grommunio.common.dialogs.CopyMovePanel dialog} when it is done.
	 * @private
	 */
	onCopy: function(records, folder)
	{
		...
		if (showPrivateWarning) {
			...
			Grommunio.common.Actions.showMessageBox(records, folder, this.store, msg, this, undefined, {
	...
	/**
	 * Function will move all {@link Grommunio.core.data.IPMRecord records}
	 * and will close the {@link Grommunio.common.dialogs.CopyMovePanel dialog} when it is done.
	 *
	 * @param {Grommunio.core.data.IPMRecord[]} records The array of {@link Grommunio.core.data.IPMRecord records} which are
	 * going to move the selected {@link Grommunio.core.data.IPFRecord folder}.
	 * @param {Grommunio.core.data.IPFRecord} folder The {@link Grommunio.core.data.IPFRecord folder } in which selected
	 * {@link Grommunio.core.data.IPMRecord records} are going to move.
	 * @private
	 */
	onMove: function(records, folder)
	{
		...
		if (folder.hasCreateRights() && !sourceFolder.hasDeleteOwnRights()) {
			Grommunio.common.Actions.showMessageBox(records, folder, this.store,undefined, this);
			return false;
		}

		var requireDeleteCheck = this.objectType !== Grommunio.core.mapi.ObjectType.MAPI_MESSAGE && !sourceFolder.hasDeleteOwnRights();
		var noAccessRecord = [];
		var moveRecords = [];
		Ext.each(records, function(record, index) {
			...
			if (Ext.isFunction(record.get)) {
				...
				if (!Ext.isNumber(accessValue) && record.data) {
					record.data.access = (record.data.access || 0) | Grommunio.core.mapi.Access.ACCESS_DELETE;
				}
			} else if (!Ext.isNumber(record.access)) {
				record.access = Grommunio.core.mapi.Access.ACCESS_DELETE;
			}
			...
			if (!hasDeleteAccess && sourceFolder) {
				...
				if (parentEntryId && sourceEntryId && Grommunio.core.EntryId.compareEntryIds(parentEntryId, sourceEntryId)) {
			...
			if (!hasDeleteAccess) {
				if (sourceFolder && sourceFolder.hasDeleteOwnRights() && sourceStoreEntryId) {
					...
					if (recordStoreEntryId && Grommunio.core.EntryId.compareStoreEntryIds(sourceStoreEntryId, recordStoreEntryId)) {
						hasDeleteAccess = true;
					}
				}
				...
				if (defaultStore && recordStoreEntryId) {
					...
					if (defaultStoreEntryId && Grommunio.core.EntryId.compareStoreEntryIds(defaultStoreEntryId, recordStoreEntryId)) {
						hasDeleteAccess = true;
					}
				}
			}
			if (!hasDeleteAccess) {
				...
				if (!userOwnsStore) {
					...
					if (defaultStore) {
						var defaultStoreEntryId = defaultStore.get('store_entryid');
						var recordStoreEntryId = Ext.isFunction(record.get) ? record.get('store_entryid') : record.store_entryid;
						if (defaultStoreEntryId && recordStoreEntryId && Grommunio.core.EntryId.compareStoreEntryIds(defaultStoreEntryId, recordStoreEntryId)) {
							userOwnsStore = true;
						}
					}
				}
				var canDeleteAny = sourceFolder && (sourceFolder.get('rights') & Grommunio.core.mapi.Rights.RIGHTS_DELETE_ANY) > 0;
		...
		if (!Ext.isEmpty(moveRecords)) {
			Grommunio.common.Actions.ensureStoreReloadOnEmpty(this.store);
			Ext.each(moveRecords, function(record) {
				record.moveTo(folder);
			});
			Grommunio.common.data.FolderSuggestions.remember(moveRecords, folder);
		}
		...
		if (requireDeleteCheck && !Ext.isEmpty(noAccessRecord)) {
			...
			Grommunio.common.Actions.showMessageBox(noAccessRecords, folder, this.store, msg, this);
			return;
		}
		...
		if (showPrivateWaring) {
			...
			Grommunio.common.Actions.showMessageBox(moveRecords.length ? moveRecords : records, folder, this.store, msg, this, undefined, {
				title: _('Private item'),
				actionBtn : _('Move')
			});
			return;
	 ...
	 * {@link Ext.Button button}. This will close the {@link Grommunio.common.dialogs.CopyMovePanel dialog}
	 * without copying or moving any {@link Grommunio.core.data.IPMRecord records}.
	 * @private
	 */
	onCancel: function()
	{
...
Ext.reg('grommunio.copymovepanel', Grommunio.common.dialogs.CopyMovePanel);
