...
/**
 * @class Grommunio.common.rules.dialogs.SensitivityLink
 ...
 * condition for it.
 */
Grommunio.common.rules.dialogs.SensitivityLink = Ext.extend(Grommunio.common.rules.dialogs.BaseLink, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			items: [{
				...
				store: {
					xtype: 'jsonstore',
					fields: [ 'name', 'value' ],
					data: Grommunio.common.data.SensitivityFlags.flags
				},
				mode: 'local',
				triggerAction: 'all',
				displayField: 'name',
		...
		Grommunio.common.rules.dialogs.SensitivityLink.superclass.constructor.call(this, config);
	},
	...
	{
		var sensitivity = Grommunio.core.mapi.Sensitivity['NONE'];

		if (condition) {
			sensitivity = condition[1][Grommunio.core.mapi.Restrictions.VALUE]['PR_SENSITIVITY'];
		}

		this.sensitivityCombo.setValue(sensitivity);
		Grommunio.common.rules.dialogs.SensitivityLink.superclass.setCondition.apply(this, arguments);
	},
...
Ext.reg('grommunio.sensitivitylink', Grommunio.common.rules.dialogs.SensitivityLink);
