...
/**
 * @class Grommunio.common.rules.dialogs.RulesExceptionContainer
 * @extends Grommunio.common.rules.dialogs.BaseContainer
 ...
 * the rules_conditions property of a {@link Grommunio.common.rules.data.RulesRecord rule}.
 */
Grommunio.common.rules.dialogs.RulesExceptionContainer = Ext.extend(Grommunio.common.rules.dialogs.BaseContainer, {
  /**
  ...
  constructor: function(config)
  {
    ...
    Grommunio.common.rules.dialogs.RulesExceptionContainer.superclass.constructor.call(this, config);
  },
   ...
   * @override {@link Grommunio.common.rules.dialogs.BaseContainer#createComboBoxContainer}.
  ...
  createComboBoxContainer: function(index)
  {
    ...
    var profileStore = {
      ...
      data: Grommunio.common.rules.data.ExceptionProfiles
    };

    return Grommunio.common.rules.dialogs.RulesExceptionContainer.superclass.createComboBoxContainer.call(this, id, profileStore);
  },
  ...
  onComboSelect: function(combo, record, index)
  {
    ...
    Grommunio.common.rules.dialogs.RulesExceptionContainer.superclass.onComboSelect.call(this, combo, record, index);
  },

  /**
   * Updates the panel by loading data from the record into the form panel.
   * @param {Grommunio.common.rules.data.RulesRecord} record The record update the panel with.
  ...
  update: function(record, contentReset)
  {
    ...
    if (contentReset || isExceptionChanged) {
      ...
      for (var i = 0, len = exceptions.length; i < len; i++) {
        ...
        var isAtleastSizeException = exceptionFlag === Grommunio.common.rules.data.ConditionFlags.ATLEAST_SIZE;
        var isAtmostSizeException = exceptionFlag === Grommunio.common.rules.data.ConditionFlags.ATMOST_SIZE;
        if (isAtleastSizeException) {
          ...
          if (exceptions[i] && exceptions[i] !== Grommunio.common.rules.data.ExceptionsConstants.INVALID_EXCEPTION) {
  ...
  removeComboBoxContainer: function()
  {
    if (this.boxContainerCount >= 1) {
      ...
      if (exceptionFlag === Grommunio.common.rules.data.ConditionFlags.ATMOST_SIZE) {
        this.record.set('rule_exception_atmost_size_unit', '');
      } else if (exceptionFlag === Grommunio.common.rules.data.ConditionFlags.ATLEAST_SIZE) {
  ...
  /**
   * Read a exception object as located in the {@link Grommunio.common.rules.data.RulesRecord Rule}
   ...
   * @return {Grommunio.common.rules.data.ConditionFlags} The Exception Flag
   * @private
   */
  getExceptionFlagFromCondition: function(exception)
  {
    if (exception === Grommunio.common.rules.data.ExceptionsConstants.INVALID_EXCEPTION) {
      return Grommunio.common.rules.data.ExceptionsConstants.INVALID_EXCEPTION;
    }
  ...
  updateRecord: function(record)
  {
    ...
    var RestrictionFactory = Grommunio.core.data.RestrictionFactory;
    ...
    for (var i = 0; i < this.boxContainerCount; i++) {
      ...
      // and set value to {@Grommunio.common.rules.data.ExceptionsConstants.INVALID_EXCEPTION}.
      ...
      if (exception === false || (this.boxContainerCount > 1 && exception === null)) {
        ...
        exception = Grommunio.common.rules.data.ExceptionsConstants.INVALID_EXCEPTION;
      } else if (this.boxContainerCount === 1 && exception === null) {
        break;
      }

      // If a proper condition exist only then convert it into exception.
      if (exception && exception !== Grommunio.common.rules.data.ExceptionsConstants.INVALID_EXCEPTION) {
    ...
    // of 'rule_condition' property of {@link Grommunio.core.data.IPMRecord record}
  ...
  /**
   * Load a Exception from a {@Link Grommunio.common.rules.data.RulesRecord} and apply it
   ...
   * @param {Grommunio.common.data.SizeUnits} sizeUnit Selected size unit for the "at least" and "at most" size exception components.
   * @private
   */
  applyException: function(panel, exception, sizeUnit)
  {
    // Parsing original condition by removing NOT part of the condition.
    // And sending to {@link Grommunio.common.rules.dialogs.RuleConditionsContainer}
    Grommunio.common.rules.dialogs.RulesExceptionContainer.superclass.applyCondition.call(this, panel, exception[1], sizeUnit);
  }

});

Ext.reg('grommunio.rulesexceptionscontainer', Grommunio.common.rules.dialogs.RulesExceptionContainer);
