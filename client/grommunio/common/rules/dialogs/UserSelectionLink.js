...
/**
 * @class Grommunio.common.rules.dialogs.UserSelectionLink
 * @extends Ext.BoxComponent
 * @xtype grommunio.userselectionlink
 */
Grommunio.common.rules.dialogs.UserSelectionLink = Ext.extend(Ext.BoxComponent, {
	/**
	...
	/**
	 * @cfg {Grommunio.core.data.IPMRecipientStore} store The store in which
	 ...
	 * @property
	 * @type Grommunio.common.rules.data.ActionFlags
	 */
	...
	constructor: function(config)
	{
		...
		Grommunio.common.rules.dialogs.UserSelectionLink.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.common.rules.dialogs.UserSelectionLink.superclass.initComponent.apply(this, arguments);
	...
	/**
	 * Handler will be called when recipient in {@link Grommunio.core.data.IPMRecipientStore IPMRecipientStore}.
	 ...
	 * @param {Grommunio.core.data.IPMRecipientRecord} record The record which was updated
	...
	/**
	 * Handler will be called when recipient is added to {@link Grommunio.core.data.IPMRecipientStore IPMRecipientStore}.
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store which fired the event
	 * @param {Grommunio.core.data.IPMRecipientRecord} records The records which have been added
	 * @private
	 */
	onRecipientAdd: function(store, records)
	{
		...
		if (record.get('object_type') === Grommunio.core.mapi.ObjectType.MAPI_DISTLIST &&
	 ...
	 * @param {Grommunio.core.data.IPMRecipientRecord} recip The records which should been expanded
	 */
	...
	/**
	 * Called when user clicks on a {@link Grommunio.common.rules.dialogs.UserSelectionLink}
	...
	{
		var hideGroups = [];
		if(this.conditionFlag === Grommunio.common.rules.data.ConditionFlags.RECEIVED_FROM) {
	 ...
	 * the contents in a user-friendly way to the user.
	 * @param {Grommunio.common.rules.data.ActionFlags} actionFlag The action type
	...
	setAction: function(actionFlag, action)
	{
		...
		this.isValid = false;
		if (action) {
			var RecordFactory = Grommunio.core.data.RecordFactory;
			var CustomObjectType = Grommunio.core.data.RecordCustomObjectType;

			for (var i = 0, len = action.adrlist.length; i < len; i++) {
				var address = action.adrlist[i];
				this.isValid = true;

				var recipient = RecordFactory.createRecordObjectByCustomType(CustomObjectType.GROMMUNIO_RECIPIENT, {
	 ...
	 * the {@link #store}.
	 * @param {Grommunio.core.data.IPMRecipientStore} store The store to show
	 */
	update: function(store)
	{
		...
		Grommunio.common.rules.dialogs.UserSelectionLink.superclass.update.call(this, this.tpl.apply(data));
	}
});

Ext.reg('grommunio.userselectionlink', Grommunio.common.rules.dialogs.UserSelectionLink);
