...
/**
 * @class Grommunio.common.rules.dialogs.RulesActionsContainer
 ...
 * the rules_actions property of a {@link Grommunio.common.rules.data.RulesRecord rule}.
 */
Grommunio.common.rules.dialogs.RulesActionsContainer = Ext.extend(Ext.Container, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.common.rules.dialogs.RulesActionsContainer.superclass.constructor.call(this, config);
	},
	...
	createActionBox: function(index)
	{
		...
		var profileStore = {
			...
			data: Grommunio.common.rules.data.ActionProfiles
		};
	...
	createActionContentPanels: function(baseId)
	{
		return [{
			...
			xtype: 'grommunio.folderselectionlink',
			id: baseId + '-folder',
			storeEntryId: this.storeEntryId
		},{
			xtype: 'grommunio.deletelink',
			id: baseId + '-delete',
			storeEntryId: this.storeEntryId
		},{
			...
			xtype: 'grommunio.markasreadlink',
	...
	updateRecord: function(record)
	{
		...
		for (var i = 0; i < this.actionCount; i++) {
			...
			if (action.action === Grommunio.core.mapi.RuleActions.OP_MARK_AS_READ) {
	...
	/**
	 * Load an Action from a {@Link Grommunio.common.rules.data.RulesRecord} and apply it
	...
	applyAction: function(panel, action)
	{
		...
		if (index >= 0) {
			...
			actionFlag = Grommunio.common.rules.data.ActionFlags.UNKNOWN;
		...
		switch (actionFlag) {
			case Grommunio.common.rules.data.ActionFlags.UNKNOWN:
			/* falls through */
			default:
				break;
			case Grommunio.common.rules.data.ActionFlags.MOVE:
			case Grommunio.common.rules.data.ActionFlags.COPY:
			case Grommunio.common.rules.data.ActionFlags.DELETE:
			case Grommunio.common.rules.data.ActionFlags.REDIRECT:
			case Grommunio.common.rules.data.ActionFlags.FORWARD:
			case Grommunio.common.rules.data.ActionFlags.FORWARD_ATTACH:
			case Grommunio.common.rules.data.ActionFlags.MARK_AS_READ:
	...
	/**
	 * Read a Action object as located in the {@link Grommunio.common.rules.data.RulesRecord Rule}
	 ...
	 * @return {Grommunio.common.rules.data.ActionFlags} The Action Flag
	 * @private
	 */
	getActionFlagFromAction: function(action)
	{
		switch (action.action) {
			case Grommunio.core.mapi.RuleActions.OP_MOVE:
				...
				var RulesStates = Grommunio.core.mapi.RuleStates;
				if ((this.record.get('rule_state') & RulesStates.ST_EXIT_LEVEL) === RulesStates.ST_EXIT_LEVEL) {
					...
					if (deletedItems && Grommunio.core.EntryId.compareEntryIds(deletedItems.get('entryid'), action.folderentryid)) {
						return Grommunio.common.rules.data.ActionFlags.DELETE;
					}
				}
				return Grommunio.common.rules.data.ActionFlags.MOVE;
			case Grommunio.core.mapi.RuleActions.OP_COPY:
				// Normal copy, nothing fancy.
				return Grommunio.common.rules.data.ActionFlags.COPY;
			case Grommunio.core.mapi.RuleActions.OP_FORWARD:
				// The exact forward action depends on the 'flavor' property.
				var FlavorFlags = Grommunio.core.mapi.FlavorFlags;
				switch (action.flavor) {
					case 0: // Forward
						return Grommunio.common.rules.data.ActionFlags.FORWARD;
					case FlavorFlags.FWD_PRESERVE_SENDER | FlavorFlags.FWD_DO_NOT_MUNGE_MSG:
						return Grommunio.common.rules.data.ActionFlags.REDIRECT;
					case FlavorFlags.FWD_AS_ATTACHMENT:
						return Grommunio.common.rules.data.ActionFlags.FORWARD_ATTACH;
					default:
						return Grommunio.common.rules.data.ActionFlags.UNKNOWN;
				}
			case Grommunio.core.mapi.RuleActions.OP_MARK_AS_READ:
				return Grommunio.common.rules.data.ActionFlags.MARK_AS_READ;
				/* falls through */
			default:
	...
	onActionComboSelect: function(combo, record, index)
	{
		...
		switch (value) {
			...
			case Grommunio.common.rules.data.ActionFlags.DELETE:
				...
				this.record.set('rule_state', this.record.get('rule_state') | Grommunio.core.mapi.RuleStates.ST_EXIT_LEVEL);
				break;
			case Grommunio.common.rules.data.ActionFlags.REDIRECT:
			case Grommunio.common.rules.data.ActionFlags.FORWARD:
			case Grommunio.common.rules.data.ActionFlags.FORWARD_ATTACH:
...
Ext.reg('grommunio.rulesactionscontainer', Grommunio.common.rules.dialogs.RulesActionsContainer);
