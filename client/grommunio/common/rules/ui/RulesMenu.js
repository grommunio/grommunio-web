...
Ext.namespace('Grommunio.common.rules.ui');

/**
 * @class Grommunio.common.rules.ui.RulesContextMenu
 * @extends Grommunio.core.ui.menu.ConditionalMenu
 * @xtype grommunio.rulescontextmenu
 *
 ...
 */
Grommunio.common.rules.ui.RulesContextMenu = Ext.extend(Grommunio.core.ui.menu.ConditionalMenu, {

	/**
	 * @cfg {Grommunio.common.rules.data.RulesStore} store store to use for adding rules
	 */
	store: undefined,
	...
	constructor: function (config)
	{
		...
		Ext.applyIf(config || {}, {
			...
			mailStore: mailStore,
			store: new Grommunio.common.rules.data.RulesStore({
		...
		Grommunio.common.rules.ui.RulesContextMenu.superclass.constructor.call(this, config);
	...
	/**
	 * Show a {@link Grommunio.core.ui.notifier.Notifier Notification} to the user indicating that Rule
	...
	onCreateRule: function()
	{
		...
		Grommunio.common.Actions.openRulesEditContent(ruleRecord, { removeRecordOnCancel: true , autoSave: true});
	},

	/**
	 * Event handler which is called when the user selects the 'Always move messages from'
	 * item in the sub context menu. This will open the rule dialog for creating new rule
	 * with condition as of type {@link Grommunio.common.rules.data.ConditionFlags.RECEIVED_FROM RECEIVED_FROM}
	 * and action as of type {@link Grommunio.common.rules.data.ActionFlags.MOVE MOVE}.
   *
   * @param {Ext.menu.Item} button which is clicked.
	...
	onCreateRuleForSender: function(button)
	{
		...
		var conditionId = Grommunio.common.rules.data.ConditionFlags.RECEIVED_FROM;
		var actionId = Grommunio.common.rules.data.ActionFlags.MOVE;
	 ...
	 * item in the sub context menu. This will open the rule dialog for creating new rule
	 * with condition as of type {@link Grommunio.common.rules.data.ConditionFlags.SENT_TO SENT_TO}
	 * and action as of type {@link Grommunio.common.rules.data.ActionFlags.MOVE MOVE}.
	 *
	 * @param {Ext.menu.Item} button which is clicked.
	 * @private
	...
	onCreateRuleForRecipient: function(button)
	{
		...
		var userStore = new Grommunio.core.data.IPMRecipientStore();
		...
		recepientSubStore.each(function(recipient) {
			...
			if(!Grommunio.core.EntryId.compareEntryIds(defaultUserEntryId, recipientEntryId)) {
		...
		var ruleRecord = this.createRuleRecord();
		var conditionId = Grommunio.common.rules.data.ConditionFlags.SENT_TO;
		var actionId = Grommunio.common.rules.data.ActionFlags.MOVE;
		var definitions = this.getDefinitions(conditionId, actionId);
	 ...
	 * item in the sub context menu. This will open the rule dialog for creating new rule
	 * with condition as of type {@link Grommunio.common.rules.data.ConditionFlags.SUBJECT_WORDS SUBJECT_WORDS}
	 * and action as of type {@link Grommunio.common.rules.data.ActionFlags.MOVE MOVE}.
	 * @private
	 */
	onCreateRuleForSubject: function()
	{
		...
		var ruleRecord = this.createRuleRecord();
		var conditionId = Grommunio.common.rules.data.ConditionFlags.SUBJECT_WORDS;
		var actionId = Grommunio.common.rules.data.ActionFlags.MOVE;
		var definitions = this.getDefinitions(conditionId, actionId);
	 ...
	 *
	 * @param {Grommunio.common.rules.data.ConditionFlags} conditionId for which we need definition function.
	 * @param {Grommunio.common.rules.data.ActionFlags} actionId for which we need definition function.
	...
	getDefinitions: function(conditionId, actionId)
	{
		...
		var actionFactory = container.getRulesFactoryByType(Grommunio.common.data.RulesFactoryType.ACTION);
	...
	/**
	 * General Function to create and get new {@link Grommunio.common.rules.data.RulesRecord RulesRecord}.
	 *
	 * @return {Grommunio.common.rules.data.RulesRecord} returns newly created Rule's Record.
	 * @private
	 */
	createRuleRecord: function() {
		return Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RULE);
	},

	/**
	 * Function which is used to open the mail record item and attach given handler for 'open' event with record's store.
	 *
	 * @param {Grommunio.core.data.IPMRecord} mailRecord which needs to be opened.
	...
	openRecord: function(mailRecord, buttonName)
	{
		...
		var handler = function(store, record) {
			if (!Grommunio.core.EntryId.compareEntryIds(mailRecord.get('entryid'), record.get('entryid'))) {
				return;
			}
...
Ext.reg('grommunio.rulescontextmenu', Grommunio.common.rules.ui.RulesContextMenu);
