...
/**
 * @class Grommunio.common.rules.data.RulesData
 *
 * Static methods for getting condition definitions and actions definitions.
 * @singleton
 */
Grommunio.common.rules.data.RulesData = {

	/**
	 * Function will return key-value pair of Object which contains key as {@link Grommunio.common.rules.data.ConditionFlags}
	 * and value as Function. These function will be used to generate rule condition restrictions.
	 * This function will be called by {@link Grommunio.common.data.ConditionRuleFactory#constructor}
	...
	getConditionRestriction: function ()
	{
		...
		var conditionFlags = Grommunio.common.rules.data.ConditionFlags;
		return {
			...
			[conditionFlags.SENDER_WORDS]: function (options) {
				...
				options.store.each(function (word) {
					var value = Grommunio.core.Util.stringToHex(word.get('words'));
			...
			[conditionFlags.ATTACHMENT]: function () {
				return restrictionFactory.dataResBitmask('PR_MESSAGE_FLAGS',
					...
					Grommunio.core.mapi.MessageFlags.MSGFLAG_HASATTACH);
			},
			[conditionFlags.IMPORTANCE]: function (options) {
				// options.value should be type of {@link Grommunio.core.mapi.Importance}
				if (!options || Grommunio.core.mapi.Importance.getDisplayName(options.value) === '') {
					return false;
				}
			...
			[conditionFlags.SENSITIVITY]: function (options) {
				// options.value should be time value
				if (!options || Grommunio.core.mapi.Sensitivity.getDisplayName(options.value) === '') {
					return false;
				}
			...
			[conditionFlags.SENT_TO]: function (options) {
				// options.store should be type of {Grommunio.core.data.IPMRecipientStore}
					...
					if (recipient.isResolved()) {
						conditions.push(restrictionFactory.createResSubRestriction('PR_MESSAGE_RECIPIENTS',
							restrictionFactory.dataResComment(
								...
								{
									'0x60000003': Grommunio.core.mapi.RecipientType.MAPI_TO,
			...
			[conditionFlags.NAME_BCC]: function (options) {
				// options.value should be boolean according to gromox version.
	 ...
	 * This is common function to generate restriction for condition types such as {@link Grommunio.common.rules.data.ConditionFlags.SUBJECT_WORDS},
	 * {@link Grommunio.common.rules.data.ConditionFlags.BODY_WORDS} and {@link Grommunio.common.rules.data.ConditionFlags.TRANSPORTHEADER_WORDS}.
	 *
	...
	/**
	 * Function will return key-value pair of Object which contains key as {@link Grommunio.common.rules.data.actionFlags}
	 * and value as Function. These function will be used to generate rule action object.
	 * This function will be called by {@link Grommunio.common.data.ActionRulesFactory#constructor}
	...
	getAction: function ()
	{
		var actionFlags = Grommunio.common.rules.data.ActionFlags;
		var rulesData = this;
		...
		return {
			...
			[actionFlags.MARK_AS_READ]: function () {
				var action = {};
				action.action = Grommunio.core.mapi.RuleActions.OP_MARK_AS_READ;
				action.flags = 0;
				action.flavor = 0;
				return action;
			},
	 ...
	 * This is common function to get action for action types such as {@link Grommunio.common.rules.data.actionFlags.MOVE},
	 * {@link Grommunio.common.rules.data.actionFlags.DELETE} and {@link Grommunio.common.rules.data.actionFlags.COPY}.
	 *
	 * @param {Grommunio.common.rules.data.actionFlags} actionType which required to get action according to given action type.
	...
	getFolderSelectionAction: function (actionType, options)
	{
		var action = {};
		var actionFlags = Grommunio.common.rules.data.ActionFlags;
		var ruleActions = Grommunio.core.mapi.RuleActions;
	...
	/**
	 * Function will return restriction according to given actionType in parameter.
	 * This is common function to get action for action types such as {@link Grommunio.common.rules.data.actionFlags.FORWARD},
	 * {@link Grommunio.common.rules.data.actionFlags.FORWARD_ATTACH} and {@link Grommunio.common.rules.data.actionFlags.REDIRECT}.
	 *
	 * @param {Grommunio.common.rules.data.actionFlags} actionType which required to get action according to given action type.
