/*
 * #dependsFile client/grommunio/common/reminder/data/ReminderProxy.js
 */
Ext.namespace('Grommunio.common.reminder.data');

/**
 * @class Grommunio.common.reminder.data.ReminderStore
 * @extends Grommunio.core.data.MAPIStore
 * @xtype grommunio.reminderstore
 */
Grommunio.common.reminder.data.ReminderStore = Ext.extend(Grommunio.core.data.ListModuleStore, {
	/**
	...
	/**
	 * Flag is used to indicate that we should reload the data in {@link Grommunio.common.reminder.data.ReminderStore ReminderStore}
	 ...
	 * @property
	 * @type Grommunio.core.ui.notifier.NotifyPlugin
	 */
	...
	constructor: function(config)
	{
		...
		var recordType = Grommunio.core.data.RecordFactory.getRecordClassByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_REMINDER);

		Ext.applyIf(config, {
			proxy: new Grommunio.common.reminder.data.ReminderProxy(),
		...
		Grommunio.common.reminder.data.ReminderStore.superclass.constructor.call(this, config);
	},

	/**
	 * Initialize events which Grommunio.common.reminder.data.ReminderStore ReminderStore} will listen to.
	 * @protected
	 */
	initEvents: function()
	{
		this.on('exception', this.onError, this);
		Grommunio.common.reminder.data.ReminderStore.superclass.initEvents.call(this);
	},
	...
	load: function(options)
	{
		...
		return Grommunio.common.reminder.data.ReminderStore.superclass.load.call(this, options);
	},
	 ...
	 * @param {Grommunio.core.data.MAPIProxy} proxy object that received the error
	...
	onError: function(proxy, type, action, options, response, args)
	{
		...
		if(action === Ext.data.Api.actions.read && !this.originalErrorNotifier) {
			...
			this.originalErrorNotifier = container.getSettingsModel().get('grommunio/v1/main/notifier/error/value');
			container.getSettingsModel().set('grommunio/v1/main/notifier/error/value', 'console');

			// Restore the original notifier when everything works again
			this.on('load', function() {
				container.getSettingsModel().set('grommunio/v1/main/notifier/error/value', this.originalErrorNotifier);
	...
	{
		var interval = container.getSettingsModel().get('grommunio/v1/main/reminder/polling_interval');
		this.pollTask = {
	...
	{
		Grommunio.common.reminder.data.ReminderStore.superclass.loadRecords.apply(this, arguments);
		...
		if (success !== false) {
			...
			if(this.lastChecksum !== newChecksum) {
				...
				if (this.showReminderDialog) {
					...
					Grommunio.common.Actions.openReminderContent(records);
	...
	updateReminderIcon: function(reminderEl, recordsLength)
	{
		...
		reminderBtn.setStyle('backgroundImage', 'url(\'' + Grommunio.common.ui.IconClass.getReminderSvgIcon(recordsLength) + '\')');
	 ...
	 * @param {Grommunio.common.reminder.data.ReminderRecord[]} reminderRecords
	 ...
	 * {@link Grommunio.common.reminder.ReminderRecord ReminderRecord} to an
	 * {@link Grommunio.core.data.IPMRecord IPMRecord} based on its message_class property and then pass that
	 * {@link Grommunio.core.data.IPMRecord IPMRecord} to {@link Grommunio.core.ui.ContentPanel ContentPanel} to open it.
	...
	onWrite: function(store, action, result, res, records)
	{
		...
		Grommunio.common.reminder.data.ReminderStore.superclass.onWrite.apply(this, arguments);
	}
});

Ext.reg('grommunio.reminderstore', Grommunio.common.reminder.data.ReminderStore);
