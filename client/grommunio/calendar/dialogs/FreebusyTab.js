...
/**
 * @class Grommunio.calendar.dialogs.FreebusyTab
 * @extends Ext.form.FormPanel
 * @xtype grommunio.freebusytab
 *
 * Freebusy tab in the {@link Grommunio.calendar.dialogs.AppointmentPanel}
 * that is used to create Appointments and Meeting Requests.
 */
Grommunio.calendar.dialogs.FreebusyTab = Ext.extend(Ext.form.FormPanel, {
	/**
	 * @constructor
	 * @param {Object} config
	 */
	constructor: function(config)
	{
		...
		Grommunio.calendar.dialogs.FreebusyTab.superclass.constructor.call(this, config);

		this.on('activate', this.onActivate, this);
	},

	/**
	 * Create the {@link Grommunio.calendar.dialogs.CounterProposalGrid CounterProposalGrid} object.
	...
	/**
	 * Create the {@link Grommunio.common.freebusy.ui.FreebusyPanel FreebusyPanel} object.
	 * @return {Object} The configuration object for the freebusy panel
	 * @private
	 */
	...
	{
		return {
			xtype: 'grommunio.freebusypanel',
			cls: 'grommunio-freebusy-panel',
			ref: 'freebusyPanel',
			flex: 1,
			modelConfig: {
				listeners: {
	 ...
	 * to control the {@link Grommunio.common.freebusy.ui.FreebusyPanel FreebusyPanel}.
	...
	createOptionsPanel: function()
	{
		return {
			...
			region: 'north',
			layout: 'column',
			cls: 'grommunio-freebusy-datetime-toolbar',
			border: false,
			autoHeight: true,
			items: [{
	 ...
	 * re-layout {@link Grommunio.calendar.dialogs.FreebusyTab FreebusyTab} to show
	 * {@link Grommunio.calendar.dialogs.CounterProposalGrid CounterProposalGrid} at proper place.
	...
	{
		Grommunio.calendar.dialogs.FreebusyTab.superclass.onRender.call(this, container, position);
	...
	onActivate: function(panel)
	{
		...
		if(this.record && !this.record.isMeeting() && this.record.get('responsestatus') === Grommunio.core.mapi.ResponseStatus.RESPONSE_NONE) {
	...
	updateUI: function(record, contentReset)
	{
		if (contentReset === true || record.isModifiedSinceLastUpdate('alldayevent')) {
			if (record.get('alldayevent')) {
				...
				this.datetimePeriod.defaultPeriod = container.getSettingsModel().get('grommunio/v1/contexts/calendar/default_appointment_period');
	...
	/**
	 * Event handler for when the {@link Grommunio.common.freebusy.ui.FreebusyPanel}
	 * fires the {@link Grommunio.common.freebusy.ui.FreebusyPanel#selectorrangeupdate}
	 * event. This will update the startdate and duedate in the {@link #record}
	 * accordingly.
	 * @param {Grommunio.core.DateRange} newRange The new daterange which was selected
	 * @param {Grommunio.core.DateRange} oldRange The old daterange which was selected
	 * @private
	 */
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.common.ui.DateRangeField} has been rendered.
	 ...
	 * the {@link Grommunio.common.freebusy.data.FreebusyModel FreebusyModel} has been
...
Ext.reg('grommunio.freebusytab', Grommunio.calendar.dialogs.FreebusyTab);
