...
/**
 * @class Grommunio.calendar.dialogs.AppointmentToolbar
 * @extends Grommunio.core.ui.ContentPanelToolbar
 * @xtype grommunio.appointmenttoolbar
 */
Grommunio.calendar.dialogs.AppointmentToolbar = Ext.extend(Grommunio.core.ui.ContentPanelToolbar, {
	// Insertion points for this class, insertion points are registered in {Grommunio.core.ui.Toolbar}
	/**
	 ...
	 * @param {Grommunio.calendar.dialogs.AppointmentToolbar} toolbar This toolbar
	 */
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.calendar.dialogs.AppointmentToolbar.superclass.constructor.call(this, config);
	...
	createActionButtons: function()
	{
		return [{
			...
			cls: 'tb-calendar-btn-calendar grommunio-action',
			...
			cls: 'button_sendMail grommunio-action',
			ref: 'sendInvitation',
			...
			ref: 'addressbookBtn',
			handler: function() {
				Grommunio.calendar.Actions.openRecipientSelectionContent(this.record, {
	...
	onAttachmentButtonAdded: function(item)
	{
		if ( item.isXType('menuitem') ){
			...
			item.update = Grommunio.common.attachment.ui.AttachmentButton.prototype.update.createDelegate(this.normalAttachmentsButton);
		}
	},
	...
	{
		Grommunio.common.Actions.openRecurrenceContent(this.record, { autoSave: false });
	},
	...
	{
		Grommunio.calendar.Actions.openForwardMeetingRequestContent(this.record);
	},
	...
	onCancelMeetingRequest: function()
	{
		if (this.record.isMeetingSent()) {
			if (this.record.isAppointmentInPast()) {
				...
				Grommunio.calendar.Actions.openSendCancellationContent(this.record);
			}
		} else {
	 ...
	 * This will {@link Grommunio.core.data.MessageContentPanel#sendRecord send} the given record.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	 ...
	 * This will {@link Grommunio.core.data.RecordContentPanel#deleteRecord delete} the given record.
	 *
	 * @param {Ext.Button} button The button which has been pressed
	 * @private
	 */
	...
	*/
	onSaveButtonBeforeShow: function(button)
	{
		if(this.record.isMeetingReceived()) {
			button.setTooltip(_('Save') + Grommunio.core.KeyMapMgr.formatShortcutHint('Ctrl + S', true));
		}
	},
	 ...
	 * read-only (see {@link Grommunio.core.data.MAPIRecord#isReadOnlyRecord}).
	...
	applyReadOnlyState: function(record)
	{
		...
		Ext.each(this.findByType('grommunio.meetingrequestbutton'), function(button) {
...
Ext.reg('grommunio.appointmenttoolbar', Grommunio.calendar.dialogs.AppointmentToolbar);
