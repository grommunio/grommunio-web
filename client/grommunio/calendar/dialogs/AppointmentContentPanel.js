...
/**
 * @class Grommunio.calendar.dialogs.AppointmentContentPanel
 * @extends Grommunio.core.ui.MessageContentPanel
 * @xtype grommunio.appointmentcontentpanel
 */
Grommunio.calendar.dialogs.AppointmentContentPanel = Ext.extend(Grommunio.core.ui.MessageContentPanel, {
	/**
	...
	/**
	 * @cfg {Number} activeTab number, the tab which should be active in {Grommunio.calendar.dialogs.AppointmentPanel tabs}
	 * 0 opens Appointment tab{@link Grommunio.calendar.dialogs.AppointmentTab}, this is defaultValue, we also want this tab when we accept any proposed time from attendee,
	 * 1 opens Freebusy tab{@link Grommunio.calendar.dialogs.FreebusyTab}; we want this tab opened when viewing all proposed times from attendees
	 * 2 opens Tracking tab{@link Grommunio.calendar.dialogs.TrackingTab}
	 */
	activeTab: undefined,
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			...
			confirmClose: true,
			items: [{
				xtype: 'grommunio.appointmentpanel',
		...
		Grommunio.calendar.dialogs.AppointmentContentPanel.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.calendar.dialogs.AppointmentContentPanel.superclass.createSendValidationQueue.apply(this, arguments);
	 ...
	 * {@link Grommunio.core.mapi.RecipientType#MAPI_BCC} to mark it as a resource if it is a room or equipment.
	 * This can be checked by using display_type and display_type_ex properties. This function is called when
	 * {@link Grommunio.core.data.IPMRecipientStore IPMRecipientStore} fires 'resolved' event.
	 * @param {Grommunio.core.data.IPMRecipientStore} store store which fired the 'resolved' event.
	 * @param {Grommunio.core.data.IPMRecipientRecord[]} records records which are in the response of resolve request.
	 */
	setRecipientTypeOnResolve: function(store, records)
	{
		for(var index = 0; index < records.length; index++) {
			...
			switch (displayTypeEx) {
				case Grommunio.core.mapi.DisplayTypeEx.DT_EQUIPMENT:
				case Grommunio.core.mapi.DisplayTypeEx.DT_ROOM:
					record.set('recipient_type', Grommunio.core.mapi.RecipientType.MAPI_BCC);
					break;
			}
		}
	},
	...
	onUpdateRecord: function(contentpanel, action, record)
	{
		if (action === Ext.data.Record.COMMIT) {
			...
			if(record.isMeetingOrganized() && this.isSending === true) {
				if(record.getRecipientStore().findExact('recipient_type', Grommunio.core.mapi.RecipientType.MAPI_BCC) !== -1){
		...
		Grommunio.calendar.dialogs.AppointmentContentPanel.superclass.onUpdateRecord.apply(this, arguments);
	},
	...
	saveRecord: function(storeSave)
	{
		...
		return Grommunio.calendar.dialogs.AppointmentContentPanel.superclass.saveRecord.apply(this, arguments);
	},
	...
	sendRecord: function()
	{
		...
		return Grommunio.calendar.dialogs.AppointmentContentPanel.superclass.sendRecord.apply(this, arguments);
	},
	...
	deleteRecord: function()
	{
		...
		return Grommunio.calendar.dialogs.AppointmentContentPanel.superclass.deleteRecord.apply(this, arguments);
	},
	...
	{
		var freebusyPanel = this.findByType('grommunio.freebusypanel')[0];
		var record = this.record;
		var busy = false;
...
Ext.reg('grommunio.appointmentcontentpanel', Grommunio.calendar.dialogs.AppointmentContentPanel);
