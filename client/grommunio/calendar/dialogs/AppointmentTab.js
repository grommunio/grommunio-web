...
/**
 * @class Grommunio.calendar.dialogs.AppointmentTab
 * @extends Ext.form.FormPanel
 * @xtype grommunio.appointmenttab
 *
 * Main tab in the {@link Grommunio.calendar.dialogs.AppointmentPanel}
 * that is used to create Appointments and Meeting Requests.
 */
Grommunio.calendar.dialogs.AppointmentTab = Ext.extend(Ext.form.FormPanel, {
	/**
	 * @cfg {String} trackingInfoString string which must be displayed in the dialog
	 * if any attendee has responded to the meeting request in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if there is any counter proposal for change in meeting time set in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if meeting accepted time set in {@link Grommunio.core.data.IPMRecord record}
	 */
	 ...
	 * if meeting tentatively accepted time set in {@link Grommunio.core.data.IPMRecord record}
	 */
	...
	constructor: function(config)
	{
		...
		Grommunio.calendar.dialogs.AppointmentTab.superclass.constructor.call(this, config);
	},
	...
	createExtraInfoPanel: function()
	{
		return {
			xtype: 'container',
			cls: 'grommunio-calendar-appointment-extrainfo',
			ref: 'extraInfoPanel',
			autoHeight: true,
			hidden: true
		};
	...
	/**
	 * Create the {@link Grommunio.common.ui.RecipientField RecipientField}
	...
	{
		return {
			xtype: 'grommunio.resizablecompositefield',
			cls: 'k-field-to',
			ref: 'recipientPanel',
			anchor: '99%',
			autoHeight: false,
	 ...
	 * Updates the {@link Grommunio.core.data.IPMRecord record} like
	...
	createOptionsPanel: function()
	{
		var busyStore = {
			xtype: 'jsonstore',
			fields: ['name', 'value'],
			data: Grommunio.calendar.data.BusyStatus
		};
	...
	createinPanel: function()
	{
		...
		const tplString = '<tpl for=".">' +
			'<div class="x-combo-list-item">' +
				'{[Grommunio.calendar.ui.IconCache.getCalendarSvgStructure(values.iconColor)]}' +
				'{displayString}' +
			'</div>' +
		'</tpl>';
	...
	/**
	 * Create the {@link Ext.Panel Panel} containing the
	 * {@link Grommunio.common.ui.htmleditor.HtmlEditor HtmlEditor} form field.
	 ...
	 * @return {Grommunio.core.DateRange} object which contains {@link Grommunio.core.DateRange DateRange} of an appointment.
	 */
	generateDefaultDate: function()
	{
		...
		return new Grommunio.core.DateRange({ startDate: start, dueDate: end });
	},
	 ...
	 * using the given {@link Grommunio.core.data.IPMRecord IPMRecord}.
	...
	updateUI: function(record, contentReset)
	{
		...
		if (contentReset === true || record.isModifiedSinceLastUpdate('meeting')) {
			switch (record.get('meeting')) {
				...
				case Grommunio.core.mapi.MeetingStatus.MEETING_RECEIVED_AND_CANCELED:
					...
					if(record.get('responsestatus') != Grommunio.core.mapi.ResponseStatus.RESPONSE_NONE || record.get('responsestatus') != Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED) {
		...
		if (contentReset === true || record.isModifiedSinceLastUpdate('alldayevent')) {
			if (record.get('alldayevent')) {
				...
				this.datetimePeriod.defaultPeriod = settingsModel.get('grommunio/v1/contexts/calendar/default_appointment_period');
		...
		if (contentReset === true || record.isModifiedSinceLastUpdate('reminder')) {
			if (record.get('alldayevent')) {
				this.comboReminder.setValue(settingsModel.get('grommunio/v1/contexts/calendar/default_allday_reminder_time'));
				this.comboBusyStatus.setValue(settingsModel.get('grommunio/v1/contexts/calendar/default_allday_busy_status'));
			} else {
				this.comboReminder.setValue(settingsModel.get('grommunio/v1/contexts/calendar/default_reminder_time'));
				this.comboBusyStatus.setValue(Grommunio.core.mapi.BusyStatus.BUSY);
			}
		}
	...
	update: function(record, contentReset)
	{
		...
		var isOrganizer = record.get('responsestatus') === Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED;
		var isAttendee = Ext.isDefined(meetingStatus) && meetingStatus !== Grommunio.core.mapi.MeetingStatus.NONMEETING && !isOrganizer;
		...
		if(record.isSubStoreModifiedSincelastUpdate('recipients')) {
			...
			if(recipients && recipients.changes) {
				...
				for(var i = 0; i < changedRecipients.length; i++) {
					...
					if(Grommunio.core.mapi.DisplayTypeEx.DTE_LOCAL(changedRecipients[i].get('display_type_ex')) === Grommunio.core.mapi.DisplayTypeEx.DT_ROOM) {
	 ...
	 * is read-only (see {@link Grommunio.core.data.MAPIRecord#isReadOnlyRecord}).
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record The record to check
	 * @private
	 */
	...
	/**
	 * Function updates the location in the {@link Grommunio.core.data.IPMRecord record}
	 * It uses {@link Grommunio.core.mapi.DisplayTypeEx.DT_ROOM room} resources to
	...
	updateLocation: function()
	{
		...
		recipientStore.each(function(recipient) {
			...
			if (Grommunio.core.mapi.DisplayTypeEx.DTE_LOCAL(recipient.get('display_type_ex')) === Grommunio.core.mapi.DisplayTypeEx.DT_ROOM){
	...
	{
		Grommunio.core.data.MessageRecordPhantomHandler(this.record);
	 ...
	 * @return {String} The color as defined in {@link Grommunio.calendar.ui.ColorSchemes color-scheme}
	 * for given folder
	 */
	...
	getSortedCreateInData: function()
	{
		...
		calendarFolders.forEach(function(dataItem) {
			if (!(dataItem.get('rights') & Grommunio.core.mapi.Rights.RIGHTS_CREATE)) {
				return;
			}
	 ...
	 * @param {Object} groupings Current state of {@link Grommunio.core.MultiFolderContextModel#groupings}.
	 * @param {Grommunio.core.data.IPFRecord} dataItem Calendar folder to check.
	...
	onToggleAllDay: function(checkbox, checked)
	{
		if (this.record.get('alldayevent') !== checked) {
			...
			if (checked) {
				...
				this.record.set('reminder_minutes', settingsModel.get('grommunio/v1/contexts/calendar/default_allday_reminder_time'));
				this.record.set('busystatus', settingsModel.get('grommunio/v1/contexts/calendar/default_allday_busy_status'));
			} else {
				var zoomLevel = settingsModel.get('grommunio/v1/contexts/calendar/default_zoom_level');
				var defaultPeriod = settingsModel.get('grommunio/v1/contexts/calendar/default_appointment_period');
				...
				this.record.set('reminder_minutes', settingsModel.get('grommunio/v1/contexts/calendar/default_reminder_time'));
				this.record.set('busystatus', Grommunio.core.mapi.BusyStatus.BUSY);
	...
	{
		Grommunio.calendar.Actions.openRecipientSelectionContent(this.record, { defaultRecipientType: Grommunio.core.mapi.RecipientType.MAPI_TO });
	},
	...
	updateExtraInfoPanel: function()
	{
		...
		if(this.record.isMeeting() && !this.record.phantom) {
			if (this.record.isMeetingOrganized()) {
				...
				if(responseStatus !== Grommunio.core.mapi.ResponseStatus.RESPONSE_NONE) {
	...
	setMeetingResponseInfo: function(notices)
	{
		...
		recipientStore.each(
			function (recipient) {
				...
				{
					case Grommunio.core.mapi.RecipientTrackStatus.RECIPIENT_TRACKSTATUS_TENTATIVE: // tentative
						tentative++;
						break;
					case Grommunio.core.mapi.RecipientTrackStatus.RECIPIENT_TRACKSTATUS_ACCEPTED: //accepted
						accepted++;
						break;
					case Grommunio.core.mapi.RecipientTrackStatus.RECIPIENT_TRACKSTATUS_DECLINED: //decline
						declined++;
						break;
				}
	 ...
	 * @param {Grommunio.core.mapi.ResponseStatus} responseStatus for the meeting
	 * @param {Array} notices The list of notices to add to
	 * @private
	 */
	setReplyTimeInfo: function(responseStatus, notices)
	{
		...
		if(responseStatus !== Grommunio.core.mapi.ResponseStatus.RESPONSE_NOT_RESPONDED) {
...
Ext.reg('grommunio.appointmenttab', Grommunio.calendar.dialogs.AppointmentTab);
