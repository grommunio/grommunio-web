...
/**
 * @class Grommunio.calendar.CalendarContextModel
 * @extends Grommunio.core.MultiFolderContextModel
 *
 ...
 */
Grommunio.calendar.CalendarContextModel = Ext.extend(Grommunio.core.MultiFolderContextModel, {
	/**
	 ...
	 * if the user clicked on January 11 2011, and the current DataMode is {@link Grommunio.calendar.data.DataMode.WEEK week}
	 * then the DateRange will be January 10 to January 16 2011.
	 *
	 * @property
	 * @type Grommunio.core.DateRange
	 */
	dateRange: undefined,
	 ...
	 * {@link Grommunio.calendar.data.DataModes#SEARCH}.
	 * @property
	 * @type Mixed
	 * @private
	 */
	...
	constructor: function(config)
	{
		...
		if(!Ext.isDefined(config.store)) {
			config.store = new Grommunio.calendar.AppointmentStore();
		}

		Ext.applyIf(config, {
			statefulRecordSelection: true,
			colorScheme: Grommunio.core.ColorSchemes.getColorSchemes(),
			current_data_mode: Grommunio.calendar.data.DataModes.WORKWEEK
		});

		this.date = new Date().clearTime(true);
		this.dateRange = new Grommunio.core.DateRange();

		this.addEvents([
			/**
			 * @event datechange
			 * Fires when the selected date is changed.
			 * @param {Grommunio.calendar.CalandarContextModel} model this calendar context model.
			 ...
			 * @param {Grommunio.calendar.CalandarContextModel} model this calendar context model.
			 * @param {Grommunio.core.DateRange} newDateRange new date range.
			 * @param {Grommunio.core.DateRange} oldDateRange old date range.
			 */
			'daterangechange',
		...
		Grommunio.calendar.CalendarContextModel.superclass.constructor.call(this, config);
	...
	{
		this.default_merge_state = container.getSettingsModel().get('grommunio/v1/contexts/calendar/default_merge_state');

		// Enable the superclass with suspended enabled, so we can safely change the date
		// after the superclass has been enabled.
		Grommunio.calendar.CalendarContextModel.superclass.enable.call(this, folder, true);
	...
	disable: function()
	{
		...
		Grommunio.calendar.CalendarContextModel.superclass.disable.apply(this, arguments);
	},
	...
	{
		if (folder.get('rights') & Grommunio.core.mapi.Rights.RIGHTS_CREATE) {
			return true;
		} else {
	 ...
	 * @param {Grommunio.core.DateRange} dateRange (optional) The {@link Grommunio.core.DateRange DateRange} object
	 * that should be used to get start and due dates for the appointment.
	 * @return {Grommunio.core.data.IPMRecord} record  which is associated to this context.
	 */
	createRecord: function(callback, folder, dateRange)
	{
		...
		var zoomLevel = container.getSettingsModel().get('grommunio/v1/contexts/calendar/default_zoom_level');
		var defaultPeriod = container.getSettingsModel().get('grommunio/v1/contexts/calendar/default_appointment_period');
		...
		var allDay = false;
		var busyStatus = Grommunio.core.mapi.BusyStatus.BUSY;
		...
		if(dateRange) {
			...
			if(dateRange.isAllDay()) {
				allDay = true;
				busyStatus = container.getSettingsModel().get('grommunio/v1/contexts/calendar/default_allday_busy_status');
			}
		}
	...
	_loadInternal: function(data)
	{
		if (Ext.isDefined(data)) {
			return Grommunio.calendar.CalendarContextModel.superclass.load.call(this, data);
		}

		if (this.ignoreDateRange) {
			return Grommunio.calendar.CalendarContextModel.superclass.load.call(this);
		}
	...
	calculateDateRange: function(date)
	{
		...
		switch (this.current_data_mode)
		{
			...
			case Grommunio.calendar.data.DataModes.WORKWEEK:
				...
				var workingDays = container.getSettingsModel().get('grommunio/v1/main/working_days');
				workingDays.sort();
	...
	setActiveDateRange: function(start, due)
	{
		if(!Ext.isObject(this.activeDateRange)) {
			this.activeDateRange = new Grommunio.core.DateRange();
		}
	 ...
	 *
	 * @return {Grommunio.core.DateRange} will return if activeDateRange is set, otherwise will return visibleDateRange.
	 */
	 ...
	 * {@link #date}.
	 *
	 * @param {Grommunio.calendar.CalendarContextModel} model The model which fired the event.
	 * @param {Grommunio.calendar.data.DataModes} newMode The new selected DataMode.
	 * @param {Grommunio.calendar.data.DataModes} oldMode The previously selected DataMode.
	 * @private
	 */
	onDataModeChange: function(model, newMode, oldMode)
	{
		Grommunio.calendar.CalendarContextModel.superclass.onDataModeChange.call(this, model, newMode, oldMode);

		if (newMode !== oldMode && oldMode === Grommunio.calendar.data.DataModes.SEARCH) {
			this.stopSearch();
		}

		switch (newMode) {
			case Grommunio.calendar.data.DataModes.SEARCH:
	...
	/**
	 * Sets the current mode (one of {@link Grommunio.calendar.data.DataModes DataModes}) and date. This switches between view
	 ...
	 *
	 * @param {Grommunio.calendar.data.DataModes} mode view mode (days, week, month).
	 ...
	 * This will {@link #setDataMode change the datamode} to {@link Grommunio.calendar.data.DataModes#SEARCH search mode}.
	...
	{
		if(this.getCurrentDataMode() != Grommunio.calendar.data.DataModes.SEARCH){
			this.oldDataMode = this.getCurrentDataMode();
			this.setDataMode(Grommunio.calendar.data.DataModes.SEARCH);
		}
	},
	 ...
	 * And will mark the given folder as {@link Grommunio.hierarchy.ui.Tree#selectFolderInTree selected}.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord[]} folder which will mark as selected
	 */
