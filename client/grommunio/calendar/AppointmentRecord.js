...
/**
 * @class Grommunio.calendar.AppointmentRecordFields
 *
 ...
 */
Grommunio.calendar.AppointmentRecordFields = [
	...
	{name: 'busystatus', type: 'int', defaultValue: Grommunio.core.mapi.BusyStatus.BUSY},
	{name: 'label', type: 'int', defaultValue: Grommunio.core.mapi.AppointmentLabels.NONE},
	...
	{name: 'meeting', type: 'int', defaultValue: Grommunio.core.mapi.MeetingStatus.NONMEETING},
	{name: 'location', type: 'string'},
	{name: 'onlinemeetingurl', type: 'string'},
	{name: 'body_preview', type: 'string'},
	{name: 'duration', type: 'int'},
	{name: 'auxiliary_flags', type: 'int'},
	{name: 'responsestatus', type: 'int', defaultValue: Grommunio.core.mapi.ResponseStatus.RESPONSE_NONE},
 ...
 */
Grommunio.core.data.RecordFactory.addFieldToMessageClass('IPM.OLE.CLASS.{00061055-0000-0000-C000-000000000046}', Grommunio.calendar.AppointmentRecordFields);
Grommunio.core.data.RecordFactory.addFieldToMessageClass('IPM.OLE.CLASS.{00061055-0000-0000-C000-000000000046}', Grommunio.core.data.MessageRecordFields);

Grommunio.core.data.RecordFactory.addFieldToMessageClass('IPM.Appointment', Grommunio.calendar.AppointmentRecordFields);
Grommunio.core.data.RecordFactory.addFieldToMessageClass('IPM.Appointment', Grommunio.core.data.MessageRecordFields);
Grommunio.core.data.RecordFactory.addListenerToMessageClass('IPM.Appointment', 'createphantom', Grommunio.core.data.MessageRecordPhantomHandler);

Grommunio.core.data.RecordFactory.addListenerToMessageClass('IPM.Appointment', 'createphantom', function(record, data) {
	record.beginEdit();
	...
	if (!data || !Ext.isDefined(data.reminder)) {
		...
		if (!store || !store.isPublicStore()) {
			reminder = settings.get('grommunio/v1/contexts/calendar/default_reminder');
		}
	...
	if (!data || !Ext.isDefined(data.reminder_minutes)) {
		if (record.get('alldayevent')) {
			record.set('reminder_minutes', settings.get('grommunio/v1/contexts/calendar/default_allday_reminder_time'));
		} else {
			record.set('reminder_minutes', settings.get('grommunio/v1/contexts/calendar/default_reminder_time'));
		}
	}
...
/**
 * @class Grommunio.calendar.AppointmentRecord
 * @extends Grommunio.core.data.MessageRecord
 *
 * An extension to the {@link Grommunio.core.data.MessageRecord MessageRecord} specific to Appointment Request/Response Messages.
 */
Grommunio.calendar.AppointmentRecord = Ext.extend(Grommunio.core.data.MessageRecord, {
	...
	{
		if (Grommunio.calendar.AppointmentRecord.superclass.equals.apply(this, arguments) === true) {
	...
	/**
	 * Function will get the recurrence information from (@link Grommunio.core.data.IPMRecord IPMRecord) and
	 * will generate the recurring pattern string that can be saved in recurring_pattern property of {@link Grommunio.core.data.IPMRecord IPMRecord}.
	 * @return {String} Recurring pattern string that can be saved in {@link Grommunio.core.data.IPMRecord IPMRecord}.
	 */
	 ...
	 *
	 * @param {Grommunio.core.mapi.ResponseStatus} responseType accept/decline/propose new time
	...
	/**
	 * Respond to a meeting request with the correct {@link Grommunio.core.mapi.ResponseStatus}.
	 *
	 * @param {Grommunio.core.mapi.ResponseStatus} responseType accept/decline/tentative
	 ...
	 *
	 * @param {Grommunio.core.mapi.ResponseStatus} responseType tentative accept/decline
	 ...
	 * {@link Grommunio.calendar.AppointmentRecord AppointmentRecord} is in past or not.
	...
	/**
	 * @return {Boolean} Returns true if the {@link Grommunio.core.data.AppointmentRecord AppointmentRecord} has
	...
	{
		return this.get('meeting') === Grommunio.core.mapi.MeetingStatus.MEETING_RECEIVED &&
			   this.get('responsestatus') !== Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED;
	},
	...
	{
		return this.get('meeting') === Grommunio.core.mapi.MeetingStatus.MEETING &&
			   this.get('responsestatus') === Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED;
	},
	...
	isMeeting: function()
	{
		...
		return (meeting && meeting !== Grommunio.core.mapi.MeetingStatus.NONMEETING);
	},
	...
	{
		var meeting = this.get('meeting');
		return (meeting === Grommunio.core.mapi.MeetingStatus.MEETING_CANCELED || meeting === Grommunio.core.mapi.MeetingStatus.MEETING_RECEIVED_AND_CANCELED);
	},
	...
	{
		return this.get('meeting') === Grommunio.core.mapi.MeetingStatus.MEETING_RECEIVED_AND_CANCELED &&
			   this.get('responsestatus') !== Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED;
	},
	...
	{
		return this.isMeetingReceived() && this.get('responsestatus') !== Grommunio.core.mapi.ResponseStatus.RESPONSE_NONE;
	},

	/**
	 * @return {Boolean} Returns true if the {@link Grommunio.core.data.AppointmentRecord AppointmentRecord} is
	...
	isSubMessage: function()
	{
		...
		return Grommunio.calendar.AppointmentRecord.superclass.isSubMessage.apply(this, arguments);
	},
	...
	afterOpen: function()
	{
		...
		Grommunio.calendar.AppointmentRecord.superclass.afterOpen.call(this);
	...
	isCopied: function ()
	{
		return (this.get('auxiliary_flags') & Grommunio.core.mapi.AppointmentAuxiliaryFlags.auxApptFlagCopied) > 0;
	},
	...
	{
		Grommunio.calendar.AppointmentRecord.superclass.addMessageAction.apply(this, arguments);
		if (name === 'send') {
	...
	afterEdit: function()
	{
		...
		Grommunio.calendar.AppointmentRecord.superclass.afterEdit.apply(this, arguments);
	},
	...
	updateMeetingRecipients: function()
	{
		...
		if (this.isMeeting()) {
			...
			if (orgIndex < 0) {
				var props = {
					recipient_type: Grommunio.core.mapi.RecipientType.MAPI_ORIG,
					recipient_flags: Grommunio.core.mapi.RecipientFlags.recipSendable | Grommunio.core.mapi.RecipientFlags.recipOrganizer,
					recipient_trackstatus: Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED
				};
				...
				var organizer = Grommunio.core.data.RecordFactory.createRecordObjectByCustomType(Grommunio.core.data.RecordCustomObjectType.GROMMUNIO_RECIPIENT, props, -1);
	...
	{
		var isRepresentingId = Grommunio.core.EntryId.compareEntryIds(organizerId, this.get('sent_representing_entryid'));
		var isSenderId = Grommunio.core.EntryId.compareEntryIds(organizerId, this.get('sender_entryid'));
	 ...
	 * @param {Grommunio.core.data.ShadowStore} store The store from which any record(s) gets updated
	...
	{
		this.beginEdit();
		this.set('meeting', Grommunio.core.mapi.MeetingStatus.MEETING);
		this.set('responsestatus', Grommunio.core.mapi.ResponseStatus.RESPONSE_ORGANIZED);
		this.updateMeetingRecipients();
		this.endEdit();
	},
	...
	{
		this.beginEdit();
		this.set('meeting', Grommunio.core.mapi.MeetingStatus.NONMEETING);
		this.set('responsestatus', Grommunio.core.mapi.ResponseStatus.RESPONSE_NONE);
		this.updateMeetingRecipients();
		this.endEdit();
	},
	 ...
	 * @return {Grommunio.core.data.IPMRecord} record that can be used to open occurrence of a series.
	 */
	 ...
	 * as a series record.
	 * @return {Grommunio.core.data.IPMRecord} record that can be used to open whole series.
	 */
...
Grommunio.core.data.RecordFactory.setBaseClassToMessageClass('IPM.Appointment', Grommunio.calendar.AppointmentRecord);
Grommunio.core.data.RecordFactory.setBaseClassToMessageClass('IPM.OLE.CLASS.{00061055-0000-0000-C000-000000000046}', Grommunio.calendar.AppointmentRecord);
