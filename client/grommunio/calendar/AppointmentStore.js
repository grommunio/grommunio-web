...
/**
 * @class Grommunio.calendar.AppointmentStore
 * @extends Grommunio.core.data.ListModuleStore
 * @xtype grommunio.appointmentstore
 *
 ...
 */
Grommunio.calendar.AppointmentStore = Ext.extend(Grommunio.core.data.ListModuleStore, {
	/**
	 * @constructor
	...
	constructor: function(config)
	{
		...
		Grommunio.calendar.AppointmentStore.superclass.constructor.call(this, config);
	...
	/**
	 * Obtain the {@link Grommunio.calendar.AppointmentRecord record} which a notification
	 ...
	 * @return {Grommunio.calendar.AppointmentRecord} The record, undefined when the store
	...
	getRecordFromNotification: function(item)
	{
		...
		this.each(function(record) {
			if (Grommunio.core.EntryId.compareEntryIds(record.get('entryid'), item.entryid)) {
				matches.push(record);
			}
		});
	...
	destroyRecord: function(store, record, index)
	{
		...
		Grommunio.calendar.AppointmentStore.superclass.destroyRecord.call(this, store, record, index);
	},
	...
	getRecordsForUpdateData: function(records, action)
	{
		if (!Ext.isDefined(records) || action === Ext.data.Api.actions.create) {
			return Grommunio.calendar.AppointmentStore.superclass.getRecordsForUpdateData.apply(this, arguments);
		}
		...
		for (var i = 0, len = records.length; i < len; i++) {
			...
			if (!record.isMessageClass('IPM.Appointment') && !record.isMessageClass('IPM.OLE.CLASS.{00061055-0000-0000-C000-000000000046}')) {
				...
				var index = this.findBy(function(rec) {
					return Grommunio.core.EntryId.compareEntryIds(record.get('entryid'), rec.get('entryid')) &&
				...
				do {
					index = this.findBy(function(rec) {
						return ((action === Ext.data.Api.actions.destroy) || !rec.isRecurringException()) &&
							   Grommunio.core.EntryId.compareEntryIds(record.get('entryid'), rec.get('entryid'));
					}, undefined, index + 1);
	 ...
	 * {@link Grommunio.calendar.AppointmentRecord appointment} Or  {@link Grommunio.calendar.MeetingRequestRecord meeting}from the
	 * {@link Grommunio.calendar.AppointmentStore store} and after removing store gets reload.
	 *
	...
	{
		Grommunio.calendar.AppointmentStore.superclass.onNotifyObjectdeleted.apply(this, arguments);
	},
	 ...
	 *
	 * @param {Grommunio.calendar.AppointmentStore} store which gets updated.
	 * @param {Grommunio.calendar.AppointmentRecord} record which is created in store.
...
Ext.reg('grommunio.appointmentstore', Grommunio.calendar.AppointmentStore);
