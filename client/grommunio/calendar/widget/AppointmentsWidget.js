...
/**
 * @class Grommunio.calendar.widget.AppointmentsWidget
 ...
 */
Grommunio.calendar.widget.AppointmentsWidget = Ext.extend(Grommunio.core.ui.widget.AbstractFolderWidget, {
	/**
	...
	constructor: function (config)
	{
		...
		var store = new Grommunio.calendar.AppointmentStore();
		...
		Grommunio.calendar.widget.AppointmentsWidget.superclass.constructor.call(this, config);
	...
	getConfigurationItems: function()
	{
		return [{
			...
			xtype: 'grommunio.calendarfolderselectionlink',
			folder: this.folder,
	...
	/**
	 * Initialize the event handlers for the {@link #store} and {@link Grommunio.hierarchy.data.HierarchyStore HierarchyStore}.
	 * @protected
	 */
	initEvents: function () {
		Grommunio.calendar.widget.AppointmentsWidget.superclass.initEvents.apply(this, arguments);
	 ...
	 * @param {Grommunio.hierarchy.data.HierarchyStore} hierarchyStore The store which fired the event
	 * @private
	 */
	onHierarchyLoad: function (hierarchyStore)
	{
		...
		var folderReadRight = Grommunio.core.mapi.Rights.RIGHTS_READ_ANY;
	 ...
	 * This will be called from {@link Grommunio.widgets.folderwidgets.AbstractFolderWidget#onHierarchyLoad onHierarchyLoad}
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder which is selected to show appointments from.
	 * @private
	 */
	...
	/**
	 * Event handler for 'remove' and 'removeFolder' events of {@link Grommunio.hierarchy.data.HierarchyStore Hierarchy}
	 * This will reset widget if state folder of this widget gets deleted.
	 *
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store hierarchy store after removing record.
	...
	*/
	...
	onDestroy: function ()
	{
		...
		Grommunio.calendar.widget.AppointmentsWidget.superclass.onDestroy.apply(this, arguments);
	},
	...
	timeRenderer: function (value, metaData, record)
	{
			...
			if (record.get('exception') === true) {
				recurringIcon =
					'&nbsp;<span ext:qwidth="300" ext:qtip="' + recurringPattern + '">' +
						'<img src="' + Grommunio.calendar.ui.IconCache.getExceptionIcon().src + '" alt="' + Ext.util.Format.htmlEncode(_('Exception')) + '"/>' +
					'</span>';
			} else {
				recurringIcon =
					'&nbsp;<span ext:qwidth="300" ext:qtip="' + recurringPattern + '">' +
						'<img src="' + Grommunio.calendar.ui.IconCache.getRecurringIcon().src + '" alt="' + Ext.util.Format.htmlEncode(_('Recurring')) + '"/>' +
					'</span>';
			}
		}
		...
		if (record.get('private') === true) {
			privateIcon = '&nbsp;<img src="' + Grommunio.calendar.ui.IconCache.getPrivateIcon().src + '" alt="' + Ext.util.Format.htmlEncode(_('Private')) + '"/>';
		}
...
Grommunio.onReady(function () {
	container.registerWidget(new Grommunio.core.ui.widget.WidgetMetaData({
		...
		widgetConstructor: Grommunio.calendar.widget.AppointmentsWidget
	}));
});
