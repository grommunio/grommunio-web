...
/**
 * @class Grommunio.calendar.ui.CalendarPanel
 * @extends Ext.Panel
 * @xtype grommunio.calendarpanel
 */
Grommunio.calendar.ui.CalendarPanel = Ext.extend(Ext.Panel, {
	/**
	 ...
	 *
	 * @property
	 * @type Grommunio.core.data.IPMStore
	 */
	store: undefined,
	 ...
	 *
	 * @property
	 * @type Grommunio.calendar.AppointmentRecord
	 */
	 ...
	 *
	 * @property
	 * @type Grommunio.calendar.ui.AppointmentSelectionModel
	 */
	selectionModel: undefined,

	/**
	 * The selection model which manages the selection of a {@link Grommunio.core.DateRange daterange}
	 * inside the calendar.
	 *
	 * @property
	 * @type Grommunio.calendar.ui.DateRangeSelectionModel
	 */
	rangeSelectionModel: undefined,

	/**
	 * @cfg {Object} viewConfig The configuration which must be applied to the {@link Grommunio.calendar.ui.CalendarMultiView}
	 * which belongs to this calendar panel.
	 */
	viewConfig: undefined,

	/**
	 * The {@link Grommunio.calendar.ui.CalendarMultiView calendarMultiView} which is attached to this panel, the creation
	 * of this view can be configured through {@link #viewConfig}.
	 *
	 * @property
	 * @type Grommunio.calendar.ui.CalendarMultiView
	 */
	view: undefined,
	...
	constructor: function(config)
	{
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.calendar.ui.CalendarPanel} calendar The calendar which fired the event
			 ...
			 * @param {Grommunio.calendar.ui.CalendarPanel} calendar The calendar which fired the event
			 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder MAPI folder the appointment should be created in.
			 * @param {Grommunio.core.data.IPMRecord} record Created appointment record.
			 */
			'appointmentcreate',
			/**
			 * @event contextmenu
		...
		Grommunio.calendar.ui.CalendarPanel.superclass.constructor.call(this, config);
	},
	...
	{
		Grommunio.calendar.ui.CalendarPanel.superclass.initComponent.apply(this, arguments);
		...
		if (!Ext.isDefined(this.selectionModel)) {
			...
			this.selectionModel = new Grommunio.calendar.ui.AppointmentSelectionModel();
		}

		// If no range selection model was provided, create a new one.
		if (!Ext.isDefined(this.rangeSelectionModel)) {
			this.rangeSelectionModel = new Grommunio.calendar.ui.DateRangeSelectionModel();
		}
		...
		this.view = new Grommunio.calendar.ui.CalendarMultiView(this.viewConfig);
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record The record on which the context menu was requested
	 * @private
	 */
	 ...
	 * @param {Object} event The event object
	 * @param {Grommunio.core.data.IPMRecord} record The record on which was double clicked
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.IPMFolder} folder The folder which was closed
	 * @private
	 */
		...
		if (this.fireEvent('beforeappointmentcalendardrop', this, appointment, sourceFolder, targetFolder, dateRange) !== false) {
			...
			if (isAppointmentFromPublicStore && !appointment.isOpened()) {
				const openHandler = function (store, record) {
					if (!Grommunio.core.EntryId.compareEntryIds(appointment.get('entryid'), record.get('entryid'))) {
						return;
					}
	 ...
	 *
	 * @param {Grommunio.calendar.AppointmentRecord} appointment the appointment record that was dragged
	 * @param {Grommunio.calendar.data.AppointmentStore} store the store in which appointment exists.
	 * @param {Grommunio.calendar.core.MAPIFolder} sourceFolder source folder
	 * @param {Grommunio.calendar.core.MAPIFolder} targetFolder target folder
	 * @param {Grommunio.core.DateRange} dateRange The new daterange for the appointment
	 * @param {Ext.EventObject} event The original event object
	...
	onDropCopyOrMoveAppointment: function(appointment, store, sourceFolder, targetFolder, dateRange, event)
	{
		...
		if (event.ctrlKey) {
			...
			if (appointment.get('private') && (mapiStore.isPublicStore() || mapiStore.isSharedStore())){
				...
				Grommunio.common.Actions.showMessageBox(appointment, targetFolder, store, msg, this, callBack, {
			...
			if (!sourceFolder.hasDeleteOwnRights() || !appointment.hasDeleteAccess()) {
				...
				Grommunio.common.Actions.showMessageBox(appointment, targetFolder, store, undefined, this, callBack);
				return;
			}
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.calendar.ui.CalendarMultiView#appointmentmove appointmentmove}
	 * event has been fired on the {@link #view}. This will update the start/due date of the given {@link Grommunio.core.data.IPMRecord appointment},
	 ...
	 * @param {Grommunio.calendar.AppointmentRecord} appointment The appointment which was changed
	 * @param {Grommunio.core.DateRange} dateRange The new daterange for the appointment
	 * @private
	 */
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.calendar.ui.CalendarMultiView#appointmentresize appointmentresize}
	 * event has been fired on the {@link #view}. This will update the start/due date of the given {@link Grommunio.core.data.IPMRecord appointment},
	 * and fire the {@link #beforeappointmentresize} and {@link #beforeappointmentresize} events.
	 *
	 * @param {Grommunio.calendar.ui.CalendarMultiView} multiview The Calendar MultiView which fired the event
	 * @param {Grommunio.calendar.AppointmentRecord} appointment the appointment record that was resized
	 * @param {Grommunio.core.DateRange} dateRange The dateRange which must be applied to the appointment
	 * @param {Ext.EventObject} event The original event object
	 * @private
	 */
	...
	/**
	 * This will update the given record with the new {@link Grommunio.core.DateRange dateRange} object.
	 * This is called by {@link #onAppointmentResize}.
	 * @param {Grommunio.calendar.AppointmentRecord} appointment The appointment which was changed
	 * @param {Grommunio.core.DateRange} dateRange The DateRange used to update the record
	 * @private
	 */
	 ...
	 * in a new {@link Grommunio.core.ui.ContentPanel ContentPanel} where he can modify his changes,
	 * or if he wants to directly send an update the attendees.
	 * @param {Grommunio.calendar.AppointmentRecord} appointment The appointment which was changed
	 * @param {Grommunio.core.DateRange} dateRange The daterange indicating the new time/duration
			...
			function(button, select) {
				if (button == 'ok') {
					if (select.id == 'review_changes') {
						...
						Grommunio.calendar.Actions.openMeetingRequestContent(appointment, config);
					} else {
	...
	/**
	 * This will update the given record with the new {@link Grommunio.core.DateRange dateRange} object.
	 * @param {Grommunio.core.data.IPMRecord} record The record which must be updated
	 * @param {Grommunio.core.DateRange} dateRange The DateRange used to update the record
	 * @private
	 */
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.calendar.ui.CalendarMultiView#appointmentcreate appointmentcreate}
	 * event has been fired on the {@link #view}. This will update the start/due date of the given {@link Grommunio.core.data.IPMRecord appointment},
	 * and fire the {@link #beforeappointmentcreate} and {@link #beforeappointmentcreate} events.
	 *
	 * @param {Grommunio.calendar.ui.CalendarMultiView} multiview The Calendar MultiView which fired the event
	 * @param {Grommunio.core.IPMFolder} folder The folder in which the appointment must be created
	 * @param {Grommunio.core.DateRange} dateRange The dateRange which must be applied to the appointment
	...
	/**
	 * Event handler for the {@link Grommunio.calendar.ui.CalendarMultiView#appointmentinitdrag} event
	 * This is initiated from {@link Grommunio.calendar.ui.CalendarViewDragZone} and passed through
	 * the {@link Grommunio.calendar.ui.AbstractCalendarDaysView individual calendar} and the {@link Grommunio.calendar.ui.CalendarMultiView mutliview}
	 *
	 * @param {Grommunio.calendar.ui.CalendarMultiView} multiview The Calendar MultiView which fired the event
	...
	/**
	 * Event handler for the {@link Grommunio.calendar.ui.CalendarMultiView#appointmentenddrag} event
	 * This is initiated from {@link Grommunio.calendar.ui.CalendarViewDragZone} and passed through
	 * the {@link Grommunio.calendar.ui.AbstractCalendarDaysView individual calendar} and the {@link Grommunio.calendar.ui.CalendarMultiView mutliview}
	 *
	 * @param {Grommunio.calendar.ui.CalendarMultiView} multiview The Calendar MultiView which fired the event
	...
	{
		// Parent render.
		Grommunio.calendar.ui.CalendarPanel.superclass.onRender.apply(this, arguments);
	...
	onResize: function()
	{
		Grommunio.calendar.ui.CalendarPanel.superclass.onResize.apply(this, arguments);
	...
	{
		Grommunio.calendar.ui.CalendarPanel.superclass.afterRender.apply(this, arguments);
		this.loadMask = new Grommunio.common.ui.LoadMask(this.body, {msg: this.loadMaskText});
		this.view.layout();

		Grommunio.core.KeyMapMgr.activate(this, 'view.mapimessage');
	},

	/**
	 * Returns the calendar's CalendarView object.
	 * @return {Grommunio.calendar.ui.CalendarMultiView} The calendar view
	 */
	getView: function()
	{
		return this.view;
	 ...
	 * @return {Grommunio.calendar.ui.AppointmentSelectionModel} the calendar's selection model.
	 */
	 ...
	 * @return {Grommunio.calendar.ui.DateRangeSelectionModel} date range selection model.
	 */
	 ...
	 * @param {Grommunio.core.data.IPMStore} store The store to bind
	 */
	 ...
	 * @param {Grommunio.core.data.IPMStore} store The store to release
	 */
	 ...
	 * {@link Grommunio.core.data.IPMStore#notify 'notify'} event. If this
	 * is a {@link Grommunio.core.data.Notifications#objectCreated objectCreated}
	 * notification, this will {@link Grommunio.calendar.ui.DateRangeSelectionModel#clearSelections clear}
	 * the {@link #rangeSelectionModel}.
	 * @param {Grommunio.core.data.IPMStore} store The store which fired the event
	...
	onStoreNotify: function(store, action, records, data, timestamp, success)
	{
		...
		if (!Ext.isEmpty(records) && selectionModel.isActive() && action === Grommunio.core.data.Notifications.objectCreated) {
	 ...
	 * show {@link Grommunio.common.dialogs.MessageBox#addCustomButtons message box} to intimate that
	 * new copy will no more private item.
	 *
	 * @param {Grommunio.core.data.IPMRecord} clipBoardRecord copied calendar item which will paste in calendar view.
	 * @private
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} clipBoardRecord copied calendar item which will paste in calendar view.
	 * @param {Grommunio.core.data.IPMRecord} record new calendar item which will paste in calendar view.
	 * @private
	 */
	paste : function(clipBoardRecord, record)
	{
		...
		if(record.get('recurring')) {
			Grommunio.common.Actions.openRecurrenceContent(record, {
				store: store,
				pasteItem: true
			});
		} else {
	 ...
	 * calendar view.
	 *
	 * @param {Grommunio.core.data.IPMRecord} copiedRecord copied calendar item.
	 * @return {Grommunio.core.DateRange} dateRange new date range.
	 * @private
	 */
	getNewDateRange: function(copiedRecord)
	{
		...
		if(dateModelType === Grommunio.calendar.data.DataModes.MONTH) {
			...
			if (!Ext.isDefined(dateRange)) {
				dateRange = new Grommunio.core.DateRange();
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} copiedRecord copied calendar item.
	 * @return {Grommunio.calendar.AppointmentRecord} record which is going to paste in calendar.
	 * @private
	 */
	createRecordCopy: function (copiedRecord)
	{
		...
		var auxiliaryFlags = Grommunio.core.mapi.AppointmentAuxiliaryFlags.auxApptFlagCopied | 0x00000008 | 0x00000080;
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record A new Meeting record
	 * @param {Grommunio.core.data.IPMRecord} copiedRecord Copy of original recurring appointment/meeting record
	 * @private
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPMRecord} record New meeting record
	 * @param {Grommunio.core.data.IPMRecord} copiedRecord Copy of original recurring appointment/meeting record
	 * @private
	 */
	...
	destroy: function()
	{
		this.view.destroy();
		// super class destroy
		Grommunio.calendar.ui.CalendarPanel.superclass.destroy.apply(this, arguments);
	}
});

Ext.reg('grommunio.calendarpanel', Grommunio.calendar.ui.CalendarPanel);
