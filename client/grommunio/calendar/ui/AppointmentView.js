...
/**
 * @class Grommunio.calendar.ui.AppointmentView
 * @extends Grommunio.calendar.ui.AbstractDateRangeView
 *
 * This view represents a {@link Grommunio.core.data.IPMRecord record} within the
 * {@link Grommunio.calendar.ui.AbstractCalendarView view}.
 */
Grommunio.calendar.ui.AppointmentView = Ext.extend(Grommunio.calendar.ui.AbstractDateRangeView, {
	/**
	...
	/**
	 * @cfg {Grommunio.core.data.IPMRecord} The record which is being displayed by this {@link Grommunio.core.ui.View view}.
	 */
	record: undefined,
	...
	/**
	 * The color scheme set by the {@link Grommunio.calendar.ui.AbstractCalendarView parent calendar View}.
	 * @property
	 * @type Object
	 */
	...
	init: function()
	{
		Grommunio.calendar.ui.AppointmentView.superclass.init.call(this);

		this.timeFormat = container.settingsModel.get('grommunio/v1/main/datetime_time_format');
		if (this.record) {
	...
	{
		Grommunio.calendar.ui.AppointmentView.superclass.render.apply(this, arguments);
	 ...
	 * {@link Grommunio.calendar.ui.AbstractDateRangeView view}.
	 * This must be called whenever the {@link #record} has been updated.
	 * @param {Grommunio.core.data.IPMRecord} record The record which must be used to update the {@link #dateRange}.
	 */
	updateDateRange: function(record)
	{
		...
		if (range) {
			...
			range = new Grommunio.core.DateRange({ startDate: start, dueDate: end });
		...
		if (adjustedRange) {
			...
			adjustedRange = new Grommunio.core.DateRange({ startDate: start, dueDate: end });
	 ...
	 * @param {Grommunio.core.DateRange} dateRange
	 */
	 ...
	 * @return {Grommunio.core.DateRange} dateRange
	 */
	...
	iconRenderer: function()
	{
		...
		if (record.get('importance') === Grommunio.core.mapi.Importance.URGENT) {
			icons.push('importance');
		}

		if (record.get('importance') === Grommunio.core.mapi.Importance.NONURGENT) {
	...
	/**
	 * Gets the busy status of the appointment. Is a value from the {@link Grommunio.core.mapi.BusyStatus BusyStatus}
	 * enumeration to indicate the status as free, tentative, busy, and out of office respectively.
	 * @return {Grommunio.core.mapi.BusyStatus} the busy status of the appointment.
	 */
	...
	{
		return Grommunio.core.mapi.BusyStatus.getName(this.getBusyStatus()).toLowerCase();
	},
	...
	{
		var categories = Grommunio.common.categories.Util.getCategories(this.record);
		...
		if ( this.isActive() &&  !Ext.isEmpty(categories) ){
			return Grommunio.common.categories.Util.getCategoryColor(categories.pop(), this.record.get('store_entryid'));
		}
