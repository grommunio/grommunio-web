...
/**
 * @class Grommunio.calendar.ui.ToolTip
 * @extends Ext.ToolTip
 *
 ...
 * be created by the {@link Grommunio.calendar.ui.CalendarMultiView calendar view} and that
 ...
 */
Grommunio.calendar.ui.ToolTip = Ext.extend(Ext.ToolTip, {
	/**
	 * @cfg {Grommunio.calendar.ui.CalendarMultiView} view The view for which
	...
	constructor: function(config)
	{
		...
		Ext.applyIf(config, {
			cls: 'grommunio-appointment-tooltip',
			width: 322,
		...
		Grommunio.calendar.ui.ToolTip.superclass.constructor.call(this, config);
	},
	 ...
	 * ({@link Grommunio.calendar.AppointmentRecord}) or a 'title' and/or 'text', and optionally
	...
	show: function(id, config, event)
	{
		...
		Grommunio.calendar.ui.ToolTip.superclass.show.call(this);
	...
	buildHtml: function(config)
	{
		...
		var tone = '';
		if ( color ) {
			tone = Grommunio.core.ColorSchemes.isDark(color) ? ' k-dark' : ' k-light';
		}
		var head = '';
		var body = '';
		...
		if ( config.record ) {
			...
			if ( !Ext.isEmpty(config.categories) ){
				body += '<div class="k-appt-tip-cats">' + Grommunio.common.categories.Util.getCategoriesHtml(config.categories, config.storeEntryId) + '</div>';
			}
			if ( body ) {
	 ...
	 * @param {Grommunio.calendar.AppointmentRecord} record The appointment
	 * @return {String} html
	 * @private
	 */
	...
	renderRows: function(record)
	{
		...
		if ( busy !== Grommunio.core.mapi.BusyStatus.BUSY ) {
			rows += this.renderRow(_('Show as'), '<span class="k-appt-tip-chip">' +
				Ext.util.Format.htmlEncode(Grommunio.core.mapi.BusyStatus.getDisplayName(busy)) + '</span>');
		}
		...
		if ( !Ext.isEmpty(categories) ) {
			rows += '<div class="k-appt-tip-cats">' + Grommunio.common.categories.Util.getCategoriesHtml(categories, record.get('store_entryid')) + '</div>';
		}
	...
	hide: function(delay)
	{
		if ( !Ext.isDefined(this.hideTask) ){
			this.hideTask = new Ext.util.DelayedTask(function(){
				Grommunio.calendar.ui.ToolTip.superclass.hide.call(this);
			}, this);
		}
