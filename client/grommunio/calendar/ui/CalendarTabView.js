...
/**
 * @class Grommunio.calendar.ui.CalendarTabView
 ...
 */
Grommunio.calendar.ui.CalendarTabView = Ext.extend(Grommunio.core.ui.View, {
	/**
	 * @cfg {String} title The title which must be displayed as tab title.
	 * If not provided, then the {@link #folder} {@link Grommunio.hierarchy.data.MAPIFolderRecord#getDisplayname name}
	 * will be used.
	 */
	title: undefined,

	/**
	 * @cfg {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder which is being loaded in the {@link Grommunio.calendar.ui.AbstractCalenderView Calendar}
	 * to which this {@link Grommunio.calendar.ui.CalendarTabView tab} is attached.
	 */
	folder: undefined,
	 ...
	 * handler which can merge two {@link Grommunio.calendar.ui.AbstractCalendarView calendars}
	 ...
	 * an event handler which can separate two {@link Grommunio.calendar.ui.AbstractCalendarView calendars}
	 ...
	 * an event handler which can close the selected {@link Grommunio.calendar.ui.AbstractCalendarView calendar}.
	 * @property
	 * @type Ext.Element
	 */
	closeIcon: undefined,
	...
	constructor: function(config)
	{
		...
		Grommunio.calendar.ui.CalendarTabView.superclass.constructor.call(this, config);
	},
	 ...
	 * Used by {@link Grommunio.calendar.ui.AbstractCalendarView AbstractCalendarView} to lay out and size the tabs.
	...
	getDesiredWidth: function()
	{
		...
		var addClass = !this.tabContents.hasClass('grommunio-calendar-tab-selected');
		if ( addClass ){
			this.tabContents.addClass('grommunio-calendar-tab-selected');
		}
		var textMetrics = Ext.util.TextMetrics.createInstance(this.tabText);
		if ( addClass ){
			this.tabContents.removeClass('grommunio-calendar-tab-selected');
		}
	 ...
	 * {@link Grommunio.calendar.ui.AbstractCalendarView AbstractCalendarView} before layout.
	...
	applyCSSClassNames: function()
	{
		...
		if (contextModel instanceof Grommunio.calendar.CalendarContextModel) {
	...
	onLayout: function()
	{
		...
		if (contextModel instanceof Grommunio.calendar.CalendarContextModel) {
			...
			var isDarkColor = Grommunio.core.ColorSchemes.isDark(colorScheme.base);
			if ( !isDarkColor ){
		...
		if ( this.showMergeIcon ){
			this.tabContents.addClass('grommunio-with-merge-icon');
		} else {
			this.tabContents.removeClass('grommunio-with-merge-icon');
		}
		if ( this.showSeparateIcon ){
			this.tabContents.addClass('grommunio-with-separate-icon');
		} else {
			this.tabContents.removeClass('grommunio-with-separate-icon');
		}
		if ( this.showCloseIcon ){
			this.tabContents.addClass('grommunio-with-close-icon');
		} else {
			this.tabContents.removeClass('grommunio-with-close-icon');
		}
		...
		Grommunio.calendar.ui.CalendarTabView.superclass.onLayout.call(this);
	},
	...
	render: function(container)
	{
		...
		Grommunio.calendar.ui.CalendarTabView.superclass.render.call(this, container);
	},
