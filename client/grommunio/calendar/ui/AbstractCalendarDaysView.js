...
/**
 * @class Grommunio.calendar.ui.AbstractCalendarDaysView
 * @extends Grommunio.calendar.ui.AbstractCalendarView
 *
 * The DaysView is used to display one or more days in columns next to each other. This is usually limited to 7 days,
 * although more days are supported. For an overview of a large number of days, the {@link Grommunio.calendar.ui.AbstractCalendarBoxView BoxView}
 * is however more recommended
 */
Grommunio.calendar.ui.AbstractCalendarDaysView = Ext.extend(Grommunio.calendar.ui.AbstractCalendarView, {
	/**
	 * Array of configuration {@link Grommunio.calendar.data.DayLayoutPosition DayLayoutPositions} within this view.
	 * @property
	 * @type Array
	 */
	dayLayoutPositions: [],
	 ...
	 * {@link Grommunio.calendar.ui.CalendarMultiView#minHeaderDayTextWidth minHeaderDayTextWidth}.
	...
	constructor: function(config)
	{
		...
		Grommunio.calendar.ui.AbstractCalendarDaysView.superclass.constructor.call(this, config);
	},
	 ...
	 * @return {Array} The array of {@link Grommunio.calendar.data.DayLayoutPosition dayLayoutPositions}
	 * @private
	 */
	calculateDayLayoutPositions: function()
	{
		...
		for (var i = 0; i < numDays; i++) {
			...
			this.dayLayoutPositions.push(new Grommunio.calendar.data.DayLayoutPosition({
	...
	screenLocationToDate: function(x, y)
	{
		...
		if (Grommunio.core.Util.inside(this.header.parent().getBox(), x, y)) {
	...
	screenLocationToDateRange: function(x, y)
	{
		...
		if (Grommunio.core.Util.inside(this.header.parent().getBox(), x, y)) {
			...
			return new Grommunio.core.DateRange({ startDate: date, dueDate: dueDate });
		} else {
			...
			return new Grommunio.core.DateRange({ startDate: new Date(snap), dueDate: new Date(snap + snapSize) });
		}
	},
	 ...
	 * @return {Array} Array of {@link Grommunio.calendar.data.AppointmentBounds Bounds} which are used
	...
	dateRangeToBodyBounds: function(dateRange, column, columnCount, useMargin)
	{
		...
		// during the construction of the Grommunio.calendar.data.AppointmentBounds objects.
		...
		if (startDay == dueDay)	{
			...
			// is left to be done is creating a single Grommunio.calendar.data.AppointmentBounds
			...
			ret.push(new Grommunio.calendar.data.AppointmentBounds({
	...
	dateRangeToHeaderBounds: function(dateRange, row, rowCount, useMargin)
	{
		...
		if (startDay < 0 && dueDay >= numDays) {
			...
			return new Grommunio.calendar.data.AppointmentBounds({
	...
	/**
	 * Splits a set of {@link Grommunio.calendar.ui.Appointment Appointment} objects into overlapping clusters.
	 * @param {Grommunio.calendar.ui.Appointment|Array} appointment The appointment list
	 * @return {Grommunio.calendar.ui.Appointment|Array} clusters The matrix of appointments
	 */
	...
	{
		Grommunio.calendar.ui.AbstractCalendarDaysView.superclass.onAppointmentsLoad.apply(this, arguments);
	...
	{
		var added = Grommunio.calendar.ui.AbstractCalendarDaysView.superclass.addAppointment.apply(this, arguments);
		if ( !added ){
			return false;
		}
	...
	{
		Grommunio.calendar.ui.AbstractCalendarDaysView.superclass.removeAppointment.apply(this, arguments);
		if (record.get('alldayevent')) {
			this.calculateHeaderOverlaps();
		} else {
			this.calculateBodyOverlaps();
	...
	{
		Grommunio.calendar.ui.AbstractCalendarDaysView.superclass.onBeforeLayout.apply(this, arguments);
	}
});
