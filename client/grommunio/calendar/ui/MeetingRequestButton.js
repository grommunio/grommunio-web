...
/**
 * @class Grommunio.calendar.ui.MeetingRequestButton
 * @extends Ext.Button
 ...
 */
Grommunio.calendar.ui.MeetingRequestButton = Ext.extend(Ext.Button, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.calendar.ui.MeetingRequestButton.superclass.constructor.call(this, config);
	...
	{
		if (!(record instanceof Grommunio.calendar.AppointmentRecord)) {
		...
		switch(this.name) {
			case Grommunio.calendar.data.MeetingRequestButtonNames.REMOVEFROMCALENDAR:
				...
				break;
			case Grommunio.calendar.data.MeetingRequestButtonNames.NORESPONSE:
				...
				break;
			case Grommunio.calendar.data.MeetingRequestButtonNames.ACCEPT:
			case Grommunio.calendar.data.MeetingRequestButtonNames.TENTATIVE:
			case Grommunio.calendar.data.MeetingRequestButtonNames.DECLINE:
				...
				if (this.name === Grommunio.calendar.data.MeetingRequestButtonNames.ACCEPT) {
					var calendarButton = this.parentScope ? this.parentScope.calendarButton : false;
					if (this.visible && calendarButton) {
						calendarButton.getEl().removeClass('grommunio-action');
					} else if (calendarButton && calendarButton.isVisible()) {
						calendarButton.getEl().addClass('grommunio-action');
					}
				}
				break;
			case Grommunio.calendar.data.MeetingRequestButtonNames.PROPOSENEWTIME:
				...
				break;
			case Grommunio.calendar.data.MeetingRequestButtonNames.VIEWPROPOSALS:
			case Grommunio.calendar.data.MeetingRequestButtonNames.ACCEPTPROPOSAL:
				this.visible = isMeetingProposal && !isSubMessage && !isMeetingUpdated && !senderIsStoreOwner;
				break;
			case Grommunio.calendar.data.MeetingRequestButtonNames.CALENDAR:
				this.visible = (isMeetingRequest || isMeetingResponse) && !isSubMessage && !apptNotFound;
				break;
			case Grommunio.calendar.data.MeetingRequestButtonNames.FORWARD:
	 ...
	 * of type {@link Grommunio.calendar.ui.MeetingRequestButton MeetingRequestButton}.
	...
	openSendConfirmationContent: function(button, eventObject)
	{
		if (this.record.get('appointment_not_found')) {
			...
			Grommunio.calendar.Actions.openSendConfirmationContent(this.record, { responseType: button.responseStatus });
		}
	},
	...
	{
		if (button === 'yes') {
			Grommunio.calendar.Actions.openSendConfirmationContent(this.record, { responseType: responseType });
		}
	},

	/**
	 * Opens a {@link Grommunio.calendar.dialogs.openAppointmentContentPanel}
	 * @param {Ext.Button} button button object.
	 * @param {EventObject} eventObject The click event object.
	 * @private
	 */
	acceptProposal: function(button, eventObject)
	{
		...
		Grommunio.calendar.Actions.openAppointmentContentToAcceptProposal(this.record);
	},

	/**
	 * Loads {@link Grommunio.calendar.CalendarContext CalendarContext} in container
	 * @param {Ext.Button} button button object.
	 * @param {EventObject} eventObject The click event object.
	 * @private
	 */
	showMeetingInCalendar: function(button, eventObject)
	{
		...
		Grommunio.calendar.Actions.showMeetingInCalendar(this.record);
	},
	...
	openProposeNewTimeContent: function(button, eventObject)
	{
		if (this.record.get('appointment_not_found')) {
			...
			Grommunio.calendar.Actions.openProposeNewTimeContent(this.record);
		}
    },
	...
	viewAllProposals: function(button, eventObject)
	{
		...
		Grommunio.calendar.Actions.openAppointmentContentToViewAllProposals(this.record);
	}

});

Ext.reg('grommunio.meetingrequestbutton', Grommunio.calendar.ui.MeetingRequestButton);
