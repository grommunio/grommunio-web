...
/**
* @class Grommunio.calendar.ui.DatePicker
* @extends Ext.DatePicker
* @xtype grommunio.datepicker
...
*/
Grommunio.calendar.ui.DatePicker = Ext.extend(Ext.DatePicker, {
	/**
	 * A {@link Grommunio.calendar.data.busytime.BusyTimeStore} store used for keeping appointments (containing only time and busy status)
	 * @property
	 * @type Grommunio.calendar.data.busytime.BusyTimeStore
	 * @private
	 */
	store: undefined,
	...
	constructor: function(config)
	{
		config = config || {};
		if(!config.store){
			config.store = new Grommunio.calendar.data.busytime.BusyTimeStore();
		}
		Grommunio.calendar.ui.DatePicker.superclass.constructor.call(this, config);
	...
	{
		Grommunio.calendar.ui.DatePicker.superclass.afterRender.call(this);
	 ...
	 * @param {Grommunio.calendar.data.busytime.BusyTimeStore} store The store bound to this datepicker
	...
	onLoad: function(store, records, options)
	{
		...
		for (var i = 0, len = this.cells.elements.length; i < len; i++) {
			...
			for(var j = 0, len2 = records.length; j < len2; j++){
				...
				if(pickerDate == storeDate && record.get('busystatus') != Grommunio.core.mapi.BusyStatus['FREE']){
					foundAppt=true;
					break;
				}
			}
	...
	onShow: function()
	{
		...
		Grommunio.calendar.ui.DatePicker.superclass.onShow.apply(this, arguments);
	},
	...
	onHide: function()
	{
		...
		Grommunio.calendar.ui.DatePicker.superclass.onHide.apply(this, arguments);
	},
	...
	{
		Grommunio.calendar.ui.DatePicker.superclass.update.call(this, date, forceRefresh);
	...
	{
		return container.getSettingsModel().get('grommunio/v1/contexts/calendar/datepicker_show_busy');
	},
...
Ext.reg('grommunio.datepicker', Grommunio.calendar.ui.DatePicker);
