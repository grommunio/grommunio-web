...
/**
 * @class Grommunio.calendar.ui.CalendarMultiView
 * @extends Grommunio.core.ui.View
 *
 * The CalendarMultiView implements the visual presentation for the {@link Grommunio.calendar.ui.CalendarPanel CalendarPanel}
 * component.
 * <p>
 * A single instance of this view manages several {@link Grommunio.calendar.ui.AbstractCalendarView AbstractCalendarView} views, each showing one
 ...
 * The individual {@link Grommunio.calendar.ui.AbstractCalendarView AbstractCalendarView} instances are laid out on these areas.
 * <p>
 ...
 * <p>
 * Child views are expected to be subclasses of {@link Grommunio.calendar.ui.AbstractCalendarView AbstractCalendarView}.
 */
Grommunio.calendar.ui.CalendarMultiView = Ext.extend(Grommunio.core.ui.View, {
	/**
	 * @cfg {Grommunio.core.Context} context The context which this view is associated with
	 */
	context: undefined,
	/**
	...
	zoomLevel: 30,
	/**
	 * The {@link Grommunio.core.ContextModel contextmodel} obtained from the {@link #context}.
	 * @property
	 ...
	 *
	 * @property
	 * @type Grommunio.calendar.ui.ToolTip
	 * @private
	 */
	tooltip: undefined,
	...
	constructor: function(config)
	{
		...
		if (!Ext.isDefined(config.firstWorkingHour)) {
			config.firstWorkingHour = container.getSettingsModel().get('grommunio/v1/main/start_working_hour');
		}
		if (!Ext.isDefined(config.lastWorkingHour)) {
			config.lastWorkingHour = container.getSettingsModel().get('grommunio/v1/main/end_working_hour');
		}
		if (!Ext.isDefined(config.workingDays)) {
			config.workingDays = container.getSettingsModel().get('grommunio/v1/main/working_days');
		}
		...
		this.addEvents(
			 ...
			 * @param {Grommunio.calendar.AppointmentRecord} appointment the appointment record that was dragged
			 * @param {Grommunio.calendar.core.MAPIFolder} sourceFolder source folder
			 * @param {Grommunio.calendar.core.MAPIFolder} targetFolder target folder
			 ...
			 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder MAPI folder the appointment should be created in.
			 * @param {Grommunio.core.DateRange} dateRange Appointment date range.
			 ...
			 * @param {Grommunio.calendar.ui.AppointmentView} appointment The appointment that is being dragged
			 */
			'appointmentinitdrag',
			/**
			 ...
			 * @param {Ext.EventObject} event The mouse event
			 * @param {Grommunio.calendar.ui.AppointmentView} appointment The appointment that was dragged
			 */
			'appointmentenddrag',
			/**
			 * @event contextmenu
			 ...
			 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder MAPI folder.
			 */
			'calendarclose'
		);

		Grommunio.calendar.ui.CalendarMultiView.superclass.constructor.call(this, config);
	},
	...
	{
		// parent init
		Grommunio.calendar.ui.CalendarMultiView.superclass.init.call(this);
	...
	/**
	 * @return {Grommunio.calendar.ui.AppointmentSelectionModel} selection model.
	 */
	...
	/**
	 * @return {Grommunio.calendar.ui.DateRangeSelectionModel} range selection model.
	 */
	 ...
	 * @return {Grommunio.core.DateRange} Current date range.
	 */
	 ...
	 * @param {Number} time difference in hours. See {Grommunio.calendar.ui.TimeStripView TimeStripView}.
	 * @param {String} name of the time zone. Will be shown above the time strip.
	 */
	addTimeStrip: function(timeDifference, name)
	{
		// add a view
		var timeStrip = new Grommunio.calendar.ui.TimeStripView({
	...
	/**
	 * Obtain the singleton instance of the {@link Grommunio.calendar.ui.ToolTip Tooltip}.
	 * @return {Grommunio.calendar.ui.ToolTip} The tooltip
	 */
	getTooltipInstance: function()
	{
		if (!this.tooltip) {
			this.tooltip = new Grommunio.calendar.ui.ToolTip({
				target: this.el,
				view: this
			});
		}
	...
	/**
	 * Event handler which is fired when the {@link Grommunio.core.Context#viewmodechange viewmodechange}
	 ...
	 * @param {Grommunio.calendar.data.ViewModes} viewMode The new viewmode
	 * @param {Grommunio.calendar.data.ViewModes} oldViewMode The previous active viewmode
	 * @private
	 */
	onViewModeChanged: function(context, viewMode, oldViewMode)
	{
		...
		switch (viewMode) {
			case Grommunio.calendar.data.ViewModes.DAYS:
				this.showTimeStrips = true;
				this.calendarViewConstructor = Grommunio.calendar.ui.html.CalendarDaysView;
				this.container.addClass('grommunio-calendar-daysview');
				this.container.removeClass('grommunio-calendar-boxview');
				break;
			case Grommunio.calendar.data.ViewModes.BOX:
				this.showTimeStrips = false;
				this.calendarViewConstructor = Grommunio.calendar.ui.html.CalendarBoxView;
				this.container.removeClass('grommunio-calendar-daysview');
				this.container.addClass('grommunio-calendar-boxview');
				break;
			default:
				this.showTimeStrips = false;
				this.container.removeClass('grommunio-calendar-daysview');
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarDaysView} calendar The calendar which fired the event
	 * @param {Grommunio.calendar.AppointmentRecord} appointment The appointment over which the mouse is moving
	 * @param {Ext.EventObject} event The original event object
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarDaysView} calendar The calendar which fired the event
	 * @param {Grommunio.calendar.AppointmentRecord} appointment The appointment over which the mouse has moved out
	 * @param {Ext.EventObject} event The original event object
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendarView Calendar view the appointment belongs to.
	 * @param {Grommunio.core.DateRange} dateRange Date range of the appointment.
	 * @param {String} text Appointment text.
	 * @private
	 */
	...
	/**
	 * Handler for the appointmentinitdrag event, forwarded by the {@link Grommunio.calendar.ui.AbstractCalendarDaysView},
	 * originally coming from {@link Grommunio.calendar.ui.CalendarViewDragZone}
	 * initiated from {@link Grommunio.calendar.ui.CalendarViewDragZone}
	 *
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendarView Calendar view the appointment was dragged from
	 * @param {Ext.EventObject} event The mouse event
	...
	/**
	 * Handler for the appointmentenddrag event, forwarded by the {@link Grommunio.calendar.ui.AbstractCalendarDaysView},
	 * initiated from {@link Grommunio.calendar.ui.CalendarViewDragZone}
	 *
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendarView Calendar view the appointment was dragged to
	 * @param {Ext.EventObject} event The mouse event
	 * @param {Grommunio.calendar.ui.AppointmentView} appointment The appointment on which the event occurred
	 * @private
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record If the user right-clicked on an appointment,
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarDaysView} calendar Calendar view
	 * @param {Ext.EventObject} event Mouse event object
	 * @param {Grommunio.core.data.IPMRecord} record If the user double-clicked on an appointment,
	 * this will contain the IPMRecord of the appointment in question. Otherwise undefined.
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} source Source calendar view.
	...
	render: function(container)
	{
		...
		Grommunio.calendar.ui.CalendarMultiView.superclass.render.call(this, container);
	 ...
	 * be destroyed.
	 * <p>
	 * @param {Grommunio.core.data.IMPStore} store Store to bind to this view.
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPMStore} store Store to release from this view.
	 */
	...
	onLayout: function()
	{
		...
		if (this.context.getCurrentViewMode() === Grommunio.calendar.data.ViewModes.DAYS) {
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder Folder to find.
	 * @return {Grommunio.calendar.ui.AbstractCalendarView} the calendar view that contains the given folder or undefined if not found.
	 * @private
	 */
	 ...
	 * @param {String} groupId The group from {@link Grommunio.core.MultiFolderContextModel#getGroupings} for which this calendar is created.
	 * @param {Mixed} folders MAPI folder(s) to show in the newly created calendar view. Can either a single {@link Grommunio.hierarchy.data.MAPIFolderRecord folder} or an array.
	 * @return {Grommunio.calendar.ui.AbstractCalendarView} calendarView new created calendarView.
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar calendar to remove
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord[]} folders list of folders.
	 * @private
	 */
	 ...
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord[]} folders list of folders. The order of the calendar views will be according to the order of the folders in this list.
	 * @private
	 */
	 ...
	 *
	 * @param {Grommunio.core.data.IPFRecord[]} folders Array of the folders that are shown in the view. Normally
	...
	/**
	 * Handles the {@link Grommunio.core.data.IPMStore#beforeload beforeload} event from the appointment {@link #store store}.
	...
	/**
	 * Called in response to a {@link Grommunio.core.data.IPMStore#load load} event from the appointment {@link #store store}.
	 * @param {Grommunio.core.data.IPMStore} store store that fired the event.
	...
	/**
	 * Called in response to an {@link Grommunio.core.data.IPMStore#add add} event from the appointment {@link #store store}.
	...
	/**
	 * Called in response to a {@link Grommunio.core.data.IPMStore#remove remove} event from the appointment {@link #store store}.
	 * @param {Ext.data.Store} store data store
	 * @param {Ext.data.Record} record that was added
	...
	/**
	 * Called in response to an {@link Grommunio.core.data.IPMStore#update update} event from the appointment {@link #store store}.
	 * @param {Ext.data.Store} store data store
	 * @param {Ext.data.Record} record that was added
	...
	* @param {Grommunio.calendar.ui.AbstractCalendarView} calendar The calendar to remove.
	* @private
	*/
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} sourceCalendar Source calendar view.
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} targetCalendar Target calendar view.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder MAPI folder to move.
	 * @private
	 */
	moveFolder: function(sourceCalendar, targetCalendar, folder)
	{
		...
		for (var i=0, record; (record=records[i]); i++) {
			if (Grommunio.core.EntryId.compareEntryIds(record.get('parent_entryid'), folder.get('entryid'))) {
	 ...
	 * {@link Grommunio.calendar.CalendarContextModel#mergeFolderToGroup register}
	 ...
	 * If the source calendar view ends up empty, it is destroyed.
	 * This method does not force a layout, but does
	 * {@link Grommunio.calendar.CalendarContextModel#separateFolderFromGroup register}
	 * the change to the {@link #model}.
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} sourceCalendar Source calendar view.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder MAPI folder to move.
	 * @private
	...
	/**
	 * Check the {@link #model} {@link Grommunio.core.MultiFolderContextModel#getGroupings groupings}
	 * to {@link Grommunio.calendar.ui.AbstractCalendarView#setSelectedFolder apply} the new active
	 * folder to the calendar.
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar The calendar to update
	 * @private
	 */
	...
	/**
	 * Handles the {@link Grommunio.calendar.ui.AbstractCalendarView#activate} event
	 * from one of the {@link #calendars calendar views}.
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar event source.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder The folder which was activated
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar event source.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder to merge.
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar event source.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder to separate.
	 * @private
	 */
	 ...
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar event source.
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} folder folder to close.
	 * @private
	 */
	...
	onDestroy: function()
	{
		Grommunio.calendar.ui.CalendarMultiView.superclass.onDestroy.apply(this, arguments);
