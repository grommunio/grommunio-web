...
/*
 * #dependsFile client/grommunio/calendar/data/SnapModes.js
 * #dependsFile client/grommunio/calendar/data/DragStates.js
 */
Ext.namespace('Grommunio.calendar.ui');

/**
 * @class Grommunio.calendar.ui.CalendarViewDropZone
 * @extends Ext.dd.DropZone
 *
 * A special DropZone which supports dragging appointments over the calendar.
 * The calendar is represented using a special {@link Grommunio.calendar.ui.AbstractDateRangeView}
 * which can visualize the daterange which is selected or will be occupied by the selected
 * appointment.
 */
Grommunio.calendar.ui.CalendarViewDropZone = Ext.extend(Ext.dd.DropZone, {
	/**
	 ...
	 * the calendar, or in the body. This determines if the {@link Grommunio.calendar.ui.AbstractCalendarView#header}
	 * or {@link Grommunio.calendar.ui.AbstractCalendarView#body} will be used to connect the event handlers.
	 */
	headerMode: false,

	/**
	 * @cfg {Grommunio.calendar.data.SnapModes} selectingSnapMode The snapmode for selections and resizing.
	 * If this is {@link Grommunio.calendar.data.SnapModes#ZOOMLEVEL} then appointments or selections that
	 * are resized are snapped to the time which matches the
	 * {@link Grommunio.calendar.ui.AbstractCalendarView#getZoomLevel zoomlevel}. When this option is
	 * {@link Grommunio.calendar.data.SnapModes#DAY} the time is snapped to the entire day.
	 */
	selectingSnapMode: Grommunio.calendar.data.SnapModes.ZOOMLEVEL,

	/**
	 * @cfg {Grommunio.calendar.data.SnapModes} draggingSnapMode The snapmode for appointments when dragging.
	 * If this is {@link Grommunio.calendar.data.SnapModes#ZOOMLEVEL} then appointments are snapped to the
	 * time which matches the {@link Grommunio.calendar.ui.AbstractCalendarView#getZoomLevel zoomlevel}.
	 * When this option is {@link Grommunio.calendar.data.SnapModes#DAY} the appointments are snapped
	 * to the entire day.
	 */
	 ...
	 * @property
	 * @type Grommunio.calendar.ui.AbstractDateRangeView
	 * @private
	 */
	proxy: undefined,
	 ...
	 * @property
	 * @type Grommunio.calendar.data.DragStates
	 * @private
	 */
	state: Grommunio.calendar.data.DragStates.NONE,
	...
	/**
	 * The {@link Grommunio.core.DateRange DateRange} of the selection area on which the
	...
	/**
	 * @constructor
	 * @param {Grommunio.calendar.ui.AbstractCalendarView} calendar The calendar on which this Dropzone is installed
	...
	constructor: function(calendar, config)
	{
		...
		Grommunio.calendar.ui.CalendarViewDropZone.superclass.constructor.call(this, element, config);
	},
	...
	onNodeEnter: function(target, dd, e, data)
	{
		...
		var DragStates = Grommunio.calendar.data.DragStates;
		...
		if (appointment) {
			...
			this.initDateRange = new Grommunio.core.DateRange({ startDate: appointment.get('startdate'), dueDate: appointment.get('duedate') });
		} else {
			...
			if (this.snapMode === Grommunio.calendar.data.SnapModes.DAY) {
			...
			this.initDateRange = new Grommunio.core.DateRange({ startDate: startDate, dueDate: dueDate });
		}
		...
		this.onDragKeyDown = Grommunio.calendar.ui.CalendarViewDropZone.prototype.onDragKeyDown.createDelegate({ dd: dd, dz: this });
		this.onDragKeyUp = Grommunio.calendar.ui.CalendarViewDropZone.prototype.onDragKeyUp.createDelegate({ dd: dd, dz: this });
		...
		// Update the proxy
		this.proxy.setShowTime(this.selectingSnapMode === Grommunio.calendar.data.SnapModes.ZOOMLEVEL);
	...
	/**
	 * Helper function which show {@link Grommunio.common.dialogs.MessageBox MessageBox}
	...
	/**
	 * Recalculate the {@link Grommunio.core.DateRange date values} for the {@link #proxy}. This
	 ...
	 * @param {Grommunio.calendar.AppointmentRecord|Array} The array of currently selected appointments
	 * @private
	 */
	updateProxy: function(xy, selections)
	{
		...
		if(Ext.isDate(overDate)){
			...
			if (snapMode === Grommunio.calendar.data.SnapModes.DAY ) {
			...
			} else if (snapMode === Grommunio.calendar.data.SnapModes.ZOOMLEVEL) {
		...
		switch (this.state) {
			...
			case DragStates.SELECTING:
				if (startDate > this.initDate) {
					...
					this.state = DragStates.RESIZING_DUE;
				} else if (dueDate < this.initDate || snapMode === Grommunio.calendar.data.SnapModes.DAY) {
					this.state = DragStates.RESIZING_START;
