...
/**
 * @class Grommunio.calendar.ui.AbstractCalendarBoxView
 * @extends Grommunio.calendar.ui.AbstractCalendarView
 *
 * The main calendar showing appointments as boxed (see {@link Grommunio.calendar.ui.AbstractCalendarBoxView BoxView}.
 * A {@link Grommunio.calendar.ui.CalendarPanel CalendarPanel} may show several individual calendars,
 * each represented by a subclass of {@link Grommunio.calendar.ui.AbstractCalendarBoxView AbstractCalendarBoxView}.
 *
 * The BoxView is used to display multiple weeks in a single view (e.g. a month). For displaying only a few days,
 * it is recommended to use the {@link Grommunio.calendar.ui.AbstractCalendarDaysView DaysView} instead.
 ...
 */
Grommunio.calendar.ui.AbstractCalendarBoxView = Ext.extend(Grommunio.calendar.ui.AbstractCalendarView, {
	/**
	 * Array of {@link Grommunio.calendar.data.DayBoxConfiguration configuration} objects for the
	...
	constructor: function(config)
	{
		...
		if (!Ext.isDefined(config.firstDayOfWeek)) {
			config.firstDayOfWeek = container.getSettingsModel().get('grommunio/v1/main/week_start');
		}
		...
		Grommunio.calendar.ui.AbstractCalendarBoxView.superclass.constructor.call(this, config);
	},
	...
	screenLocationToDateRange: function(x, y)
	{
		...
		if (Ext.isDate(date)) {
			...
			return new Grommunio.core.DateRange({ startDate: date.clearTime(), dueDate: dueDate.clearTime() });
		}
	...
	/**
	 * Find all {@link Grommunio.calendar.ui.AppointmentView appointments} which
	 * fall within the given {@link Grommunio.core.DateRange dateRange}.
	 * @param {Grommunio.core.DateRange} The daterange for which the appointments are sought.
	 * @return {Array} The array of {@link Grommunio.calendar.ui.AppointmentView appointments}.
	 */
	...
	/**
	 * Find the {@link Grommunio.calendar.ui.AppointmentView appointment} with the highest
	 * {@link Grommunio.calendar.ui.AppointmentView#slot slot} value. within the given
	 * {@link Grommunio.core.DateRange dateRange}.
	 * @param {Grommunio.core.DateRange} The daterange for which the appointments are sought.
	...
	/**
	 * Return the {@link Grommunio.core.DateRange dateRange} of the period
	 ...
	 * @return {Grommunio.core.DateRange} The visible daterange
	 */
	getVisibleDateRange: function()
	{
		...
		return new Grommunio.core.DateRange({ startDate: visibleStart, dueDate: visibleDue });
	},

	/**
	 * Return the number of visible weeks (See {@link #getVisibleDateRange}.
	 * @param {Grommunio.core.DateRange} range (optional) The daterange for which the number of weeks must be calculated,
	 ...
	 * @return {Array} The {@link Grommunio.calendar.data.DayBoxConfiguration DayBox} Configuration objects
	 */
	calculateDayBoxConfigurations: function(height)
	{
		...
		for (var i=0; i<visibleWeeks; i++) {
			for (var j=0; j<this.numDaysInWeek; j++)
			{
				...
				var dayRange = new Grommunio.core.DateRange({ startDate: date, dueDate: dueDate });
				...
				this.dayBoxConfigurations.push(new Grommunio.calendar.data.DayBoxConfiguration({
					left: j * dayWidth,
