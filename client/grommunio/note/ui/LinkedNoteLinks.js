/*
 * #dependsFile client/grommunio/note/NoteStore.js
 * #dependsFile client/grommunio/note/Actions.js
 * #dependsFile client/grommunio/core/data/RestrictionFactory.js
 * #dependsFile client/grommunio/core/mapi/Restrictions.js
 * #dependsFile client/grommunio/core/mapi/NoteColor.js
 */
Ext.namespace('Grommunio.note.ui');

/**
 * @class Grommunio.note.ui.LinkedNoteLinks
 ...
 */
Grommunio.note.ui.LinkedNoteLinks = Ext.extend(Ext.Container, {
	/**
	...
	constructor: function(config)
	{
		...
		Grommunio.note.ui.LinkedNoteLinks.superclass.constructor.call(this, config);
		...
		if (Ext.isString(this.linkedNotesTemplate)) {
			this.linkedNotesTemplate = new Ext.XTemplate(this.linkedNotesTemplate, {
				 ...
				 * @param {Number} color The {@link Grommunio.core.mapi.NoteColor} value
				 * @return {String} the CSS class for that color
				 */
				colorName: function(color)
				{
					return Grommunio.core.mapi.NoteColor.getName(color) || 'note_yellow';
				},
				...
				noteBody: function(body)
				{
					...
					if (body.length >= Grommunio.note.ui.LinkedNoteLinks.bodyPreviewLimit) {
						text += '…';
					}
		...
		this.noteStore = new Grommunio.note.NoteStore();
	...
	{
		Grommunio.note.ui.LinkedNoteLinks.superclass.afterRender.apply(this, arguments);
	...
	/**
	 * Update the panel for the given {@link Grommunio.core.data.IPMRecord record}.
	 * Only a mail with an 'internet_message_id' can be annotated; anything
	 * else clears the panel.
	 *
	 * @param {Grommunio.core.data.IPMRecord} record The record to show the notes of
	...
	update: function(record, contentReset)
	{
		...
		if (!folder || (folder.get('rights') & Grommunio.core.mapi.Rights.RIGHTS_READ_ANY) === 0) {
			this.reset();
			return;
		}
	...
	onNoteClick: function(event, target)
	{
		...
		if (record) {
			Grommunio.note.Actions.openNoteContent([record]);
		}
	},
	...
	onDestroy: function()
	{
		...
		Grommunio.note.ui.LinkedNoteLinks.superclass.onDestroy.apply(this, arguments);
	}
});
 ...
 * @static
 */
Grommunio.note.ui.LinkedNoteLinks.bodyPreviewLimit = 255;

Ext.reg('grommunio.linkednotelinks', Grommunio.note.ui.LinkedNoteLinks);
