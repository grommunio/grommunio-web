...
/**
 * @class Grommunio.addressbook.ui.AddressBookContextMenu
 * @extends Grommunio.core.ui.menu.ConditionalMenu
 * @xtype grommunio.addressbookcontextmenu
 *
 * The context menu for the {@link Grommunio.addressbook.ui.AddressBookPanel AddressBookPanel}.
 */
Grommunio.addressbook.ui.AddressBookContextMenu = Ext.extend(Grommunio.core.ui.menu.ConditionalMenu, {
	 ...
	 * @param {Grommunio.addressbook.ui.AddressBookContextMenu} contextmeny This contextmenu
	 */
	...
	/**
	 * @cfg {Grommunio.core.ui.ContentPanel} contentpanel The content panel from where the contextmenu is requested
	 */
	dialog: undefined,
	...
	constructor: function(config)
	{
		...
		Grommunio.addressbook.ui.AddressBookContextMenu.superclass.constructor.call(this, config);
	},
	 ...
	 * {@link Grommunio.addressbook.Actions#openDetailsContent details dialog}.
	 * @private
	 */
	onOpenDetails: function()
	{
		Grommunio.addressbook.Actions.openDetailsContent(this.records);
	},
	...
	onBeforeShowItem: function (item, records) {
		...
		if (isGABContact && records.length > 1) {
			...
			var hasGABContact = records.some(function(record){
				return record.get('object_type') === Grommunio.core.mapi.ObjectType.MAPI_MAILUSER;
			});
		...
		var hasDistlist = records.some(function(record){
			return record.get('object_type') === Grommunio.core.mapi.ObjectType.MAPI_DISTLIST;
		});
	 ...
	 * It will open the {@link Grommunio.hierarchy.dialogs.FolderSelectionContentPanel FolderSelectionContentPanel} to select
	 * the folder to copy GAB contact.
	 */
	onCopyContact: function ()
	{
		Grommunio.hierarchy.Actions.openFolderSelectionContent({
	...
	/**
	 * Callback function called when ok button pressed of {@link Grommunio.hierarchy.dialogs.FolderSelectionPanel FolderSelectionPanel}.
	 * It will create the contact record and set necessary properties from the GAB contact.
	 *
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord} The selected folder
	 */
...
Ext.reg('grommunio.addressbookcontextmenu', Grommunio.addressbook.ui.AddressBookContextMenu);
