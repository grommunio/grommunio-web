...
/**
 * @class Grommunio.addressbook.ui.AddressBookMainPanel
 * @extends Ext.Panel
 * @xtype grommunio.addressbookmainpanel
 */
Grommunio.addressbook.ui.AddressBookMainPanel = Ext.extend(Ext.Panel, {
	/**
	 * @cfg {Grommunio.addressbook.AddressBookStore} addressBookStore
	...
	constructor: function(config)
	{
		...
		var stateUpdated = sm.get('grommunio/v1/contexts/addressbook/stateUpdated');
		if ( Ext.isEmpty(stateUpdated) ) {
			sm.remove('grommunio/v1/state/dialogs/addressbookcontentpanel');
			sm.remove('grommunio/v1/state/dialogs/abuserselectioncontentpanel');
			sm.remove('grommunio/v1/state/gab/contacts');
			sm.remove('grommunio/v1/state/gab/globaladdressbook');
			sm.set('grommunio/v1/contexts/addressbook/stateUpdated', 1);
		}

		config = config || {};

		if (!Ext.isDefined(config.addressBookStore)) {
			config.addressBookStore = new Grommunio.addressbook.AddressBookStore();
		}
		...
		Grommunio.addressbook.ui.AddressBookMainPanel.superclass.constructor.call(this, config);
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.HierarchyStore} store The hierarchy store
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} record The removed store record
	...
	onHierarchyStoreRemove: function(store, record)
	{
		...
		var toRemove = [];
		Grommunio.addressbook.AddressBookHierarchyStore.each(function(r) {
		...
		if (toRemove.length > 0) {
			Grommunio.addressbook.AddressBookHierarchyStore.remove(toRemove);

			// Clean up group headers that no longer have any children
			var emptyHeaders = [];
			var count = Grommunio.addressbook.AddressBookHierarchyStore.getCount();
			Grommunio.addressbook.AddressBookHierarchyStore.each(function(r, index) {
			...
			if (emptyHeaders.length > 0) {
				Grommunio.addressbook.AddressBookHierarchyStore.remove(emptyHeaders);
			}
		}
	},
	 ...
	 *
	 * @param {Grommunio.hierarchy.data.HierarchyStore} hierarchyStore The hierarchy store
	 * @param {Grommunio.hierarchy.data.MAPIStoreRecord} storeRecord The store containing the folder
	 * @param {Grommunio.hierarchy.data.MAPIFolderRecord/Array} records The added/removed folder(s)
	 * @private
	 */
		...
		return {
			...
			items: [{
				...
				items: [{
					xtype: 'combo',
					width: 200,
					plugins: [ 'grommunio.fieldlabeler', 'grommunio.comboautowidth' ],
	 ...
	 * @param {Grommunio.addressbook.AddressBookStore} addressBookStore The store which must
	...
	createViewPanel: function(addressBookStore, viewConfig)
	{
		return Ext.apply(viewConfig, {
			xtype: 'grommunio.addressbookgrid',
			hideLabel: true,
			name: 'viewpanel',
	 ...
	 * @return {Grommunio.addressbook.ui.AddressBookGrid} The GridPanel for this panel
	 */
	 ...
	 * @param {Grommunio.core.data.IPMRecord} record The selected Record
	...
	onAddressBookComboLoad: function(hideContactsFolders) {
		if ( hideContactsFolders === true ) {
			Grommunio.addressbook.AddressBookHierarchyStore.filter('type', 'gab');
		} else {
			Grommunio.addressbook.AddressBookHierarchyStore.clearFilter();
		}
	...
	initDialog: function()
	{
		...
		Grommunio.addressbook.AddressBookHierarchyStore.on('load', this.selectDefaultAddressBook, this, {single: true});
		Grommunio.addressbook.AddressBookHierarchyStore.loadAddressBookHierarchy();
	},
	...
	selectDefaultAddressBook: function()
	{
		...
		var folderEntryId = container.getSettingsModel().get('grommunio/v1/main/default_addressbook');
		...
		if (!Ext.isEmpty(folderEntryId)) {
			record = Grommunio.addressbook.AddressBookHierarchyStore.getById(folderEntryId);
		}
	...
	onSearchButtonClick: function()
	{
		...
		var params = {
			subActionType: Grommunio.core.Actions['globaladdressbook'],
		...
		if (folderType === 'contacts' || folderType === 'sharedcontacts') {
			...
			if (folderType === 'contacts' && Grommunio.core.EntryId.hasContactProviderGUID(folderId)) {
				lookupEntryId = Grommunio.core.EntryId.unwrapContactProviderEntryId(folderId);
			} else {
	 ...
	 *
	 * @return {Grommunio.core.data.IPMRecord IPMRecord} The selected record from the
	 * {@link #hierarchyStore}.
	 */
	getSelectedFolderRecord: function()
	{
		var entryid = this.addressBookSelectionCB.getValue();
		var index = Grommunio.addressbook.AddressBookHierarchyStore.find('entryid', entryid);
		return Grommunio.addressbook.AddressBookHierarchyStore.getAt(index);
	},
...
Ext.reg('grommunio.addressbookmainpanel', Grommunio.addressbook.ui.AddressBookMainPanel);
