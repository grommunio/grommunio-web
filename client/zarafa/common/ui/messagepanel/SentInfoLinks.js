...
Zarafa.common.ui.messagepanel.SentInfoLinks = Ext.extend(Ext.Container, {
	...
	update: function(record)
	{
		if (this.senderElem){
			...
			if (!Ext.isDefined(record)) {
				...
				this.mon(senderElem.select('.zarafa-sentinfo-link'), 'mouseenter', this.onMouseEnter, this, {buffer : 100});
				this.mon(senderElem.select('.zarafa-sentinfo-link'), 'mouseleave', this.onMouseLeave, this, {buffer : 100});
				this.mon(senderElem.select('.zarafa-sentinfo-on-behalf'), 'mouseenter', this.onMouseEnter, this, {buffer : 100});
				this.mon(senderElem.select('.zarafa-sentinfo-on-behalf'), 'mouseleave', this.onMouseLeave, this, {buffer : 100});
	...
	onMouseLeave: function (e, node)
	{
		...
		var win = Ext.WindowMgr.getBy(function (win) {
			if (win.records) {
				return win.records.get('entryid') === recipient.get('entryid');
			}
		}, this);
